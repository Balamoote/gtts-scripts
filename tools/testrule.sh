...
IN="$@"
 ...
 time echo $IN| sed -r "s/[\xcc\x81\xcc\xa0\xcc\xa3\xcc\xa4\xcc\xad\xcc\xb0]+//g" |\
     awk -vindb=scriptdb/ -vinax=scriptaux/ -vlocdic=scriptdb/ -ddebug.txt -vdbg=1 -f scriptdb/main.awk 
#    awk -vindb=scriptdb/ -vinax=scriptaux/ -vlocdic=scriptdb/ -ddebug.txt -vdbg=1 -f scriptdb/deomo.awk 
