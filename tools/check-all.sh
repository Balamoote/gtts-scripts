...
if [[ ! -z "$1" ]]; then
case "$1" in
	r  | f |--redo ) # Принудительно заново создать все вспомогательные файлы для скриптов
	...
	*) # Проверить, существует ли файл книги
		if [[ -s "$1" ]]; then book="$1"
			...
			"Использование:" "./check-lexx.sh" "или" "./check-lexx.sh [r|f|fg|fy|fm]"; fi ;;
esac; fi
...
pack="scriptdb/automo.gz scriptdb/awx/beautify.awk scriptdb/class.list.gz scriptdb/classes.awk scriptdb/cstauto.awk scriptdb/cstring.awk scriptdb/defunct.awk \
      ...
      scriptdb/vsevso.awk scriptdb/stress_uni.gz scriptdb/yodef.awk scriptdb/yodef0.txt.gz scriptdb/yodef1.txt.gz scriptdb/yolc.txt.gz scriptdb/yomo-lc.txt.gz \
...
if [[ $jofik -eq 1 ]]; then
  ...
  printf '\e[36m%s \e[93m%s ' "YOF:" "новые"
	md5sum scriptdb/yodef0.txt.gz scriptaux/yodef0.pat.gz scriptdb/yodef1.txt.gz scriptaux/yodef1.pat.gz scriptdb/yomo-lc.txt.gz scriptaux/yomo-lc.pat.gz \
         scriptdb/yodef.awk scriptdb/yomo-uc.txt.gz scriptaux/yomo-uc.pat.gz scriptdb/yolc.txt.gz scriptaux/yolc.pat.gz scriptaux/yomo-cc.pat.gz > scriptaux/zjofik.md5
fi
