...
if [[ -n $names ]]; then
	readarray -t narr < <(grep -Po "(?<![$RUCl$unxs$unxc])[$RUUC][$RUCl$unxs$unxc]+" trip-"$book"/text-book.txt | sed -r "s/[^$RUCl]//g; s/^.+$/0\L\0=/g" | \
		grep -Ff <(zcat scriptaux/namebase0.pat.gz scriptaux/override.pat.gz) | sort -u | sed -r "s/[0=]//g; s/^./\u\0/g")
