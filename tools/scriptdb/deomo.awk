...
BEGIN {
    ...
    patword = "[АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя\xcc\x81\xcc\xa0\xcc\xa3\xcc\xa4\xcc\xad\xcc\xb0A-Za-z0-9]+"
 ...
 if ( q(1,mest_it) &&
       q(2,pre_ro) &&
        q(3,mest_ro) && s(0,2) && p(3) )
 { sub(/е/, "ё", l[i]); if(dbg){r[37]++; print "R37"}; continue;};
 if ( q(1,mest_it) &&
       q(2,qast) &&
        q(3,pre_ro) &&
         q(4,mest_ro) && s(0,3) && p(4) )
 { sub(/е/, "ё", l[i]); if(dbg){r[37]++; print "R37"}; continue;};
 if ( q(-1,mest_it) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[42]++; print "R42"}; continue;};
 # всё, ввод, гл.ед
 if ( sc(0,",") &&
       q(1,vvod) && sc(1,",") &&
       (q(2,gl_ed)||q(2,gl_in)) )
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[44]++; print "R44"}; continue;};
 # все + это + ... + нар[….,:;!?—]
 if ( q(1,mest_it) &&
       q(2,mest_da) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[60]++; print "R60"}; continue;};
 ...
 if ( w(1,"до") &&
       w(2,"единого") && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[64]++; print "R64"}; continue;};
 ...
 if ( sc(0,",") &&
       q(1,pre_vi) &&
        w(2,"кого") && s(1,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[66]++; print "R66"}; continue;};
 if ( sc(0,",") &&
       q(1,pre_pr) &&
        w(2,"чём|чем") && s(1,1) )
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[84]++; print "R84"}; continue;};
 # все + ... + прл.
 if ( (q(1,prl_mniv)||q(1,prq_mniv)||q(1,prl_mnvr)||q(1,prq_mnvr)||q(1,prl_krmn)) &&
 ...
 if ( q(1,qast) &&
      (q(2,prl_mniv)||q(2,prq_mniv)||q(2,prl_mnvr)||q(2,prq_mnvr)||q(2,prl_krmn)) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[86]++; print "R86"}; continue;};
 # все + прл.
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[119]++; print "R119"}; continue;};
 # все + сущ.мн.
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[131]++; print "R131"}; continue;};
 # все  + нар[….,:;!?—]
 ...
 if ( q(1,nar_obme) &&
       q(2,gl_pnmn) &&
        q(3,qast) &&
         q(4,gl_in) && s(0,3) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[188]++; print "R188"}; continue;};
 # все + гл.мн + гл.инф
 if ( (q(1,gl_pnmn)||q(1,gl_pemn)) &&
       q(2,gl_in) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[189]++; print "R189"}; continue;};
 # все + гл.мн + гл.инф
 if ( (q(1,gl_pnmn)||q(1,gl_pemn)) &&
       q(2,qast) &&
        q(3,gl_in) && s(0,2) )
 ...
 #dbg = 1
 #dbgstat = 1
 if (dbgstat==1) {for (i=1; i<=231; i++) { printf ("%s%s %s %s\n", "R", i, "=", r[i]) }; };
