...
{makebookvars();for(i in wpos){i=strtonum(i);if(tolower(l[i])!=iwrd)continue; # header2
 #
 if ( sw_ez_d_f() )
 { l[i]=omo2; r[655]++; if(dbg){print "R655"}; continue};
 #
 if ( sw_ez_p_f() )
 { l[i]=omo2; r[655]++; if(dbg){print "R655"}; continue};
 #
 ...
 #
 if ( sw_mn_iv_f() )
 { l[i]=omo2; r[656]++; if(dbg){print "R656"}; continue};
...
{makebookvars();for(i in wpos){i=strtonum(i);if(tolower(l[i])!=iwrd)continue; # header2
 #
 if ( gl_mn2e_f() )
 { l[i]=omo1; r[643]++; if(dbg){print "R643"}; continue};
 #
 if ( gl_pomn_f() )
...
### x2112 !_#_!
 #  x2112	m3_1sw_ez_dpr_2sw_mn_iv_3gl_pamn_		sw_ez_l_
for(wrd in omap["x2112"]){omakevars(x2112,"x2112");for(y=1;y<=wln;y++)        # header1
{makebookvars();for(i in wpos){i=strtonum(i);if(tolower(l[i])!=iwrd)continue; # header2
 #
 if ( sw_ez_l_f() )
 { l[i]=omo2; r[658]++; if(dbg){print "R658"}; continue};
 #
 if ( sw_mn_iv_f() )
 { l[i]=omo1; r[656]++; if(dbg){print "R656"}; continue};
 #
...
{makebookvars();for(i in wpos){i=strtonum(i);if(tolower(l[i])!=iwrd)continue; # header2
 #
 if ( sw_mn_iv_f() )
 { l[i]=omo3; r[656]++; if(dbg){print "R656"}; continue};
...
{makebookvars();for(i in wpos){i=strtonum(i);if(tolower(l[i])!=iwrd)continue; # header2
 #
 if ( sw_mn_r_f() )
 { l[i]=omo3; r[649]++; if(dbg){print "R649"}; continue};
...
#if (dbgstat==1) {system(cmd); for (i=1; i<=667; i++) { printf ("%s%s %s %s\n", "R", i, "=", r[i]) >> "_stat.txt"};
