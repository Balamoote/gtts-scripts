...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 #
 cst="баронский белоснежный блистающий вампирский величественный вражеский высокий высоченный герцогский гордый городской гостеприимный готический графский грозный древний живописный жуткий загадочный заоблачный императорский исполинский каменный карточный княжеский королевский легендарный ледяной мрачный недостроенный неприступный облачный песочный песчаный приземистый разбоничий родовой роскошный рыцарский сказочный средневековый старинный фамильный виндзорский меблированный настоящий мёртвый близкий царский воздушный";
 if ( bb(-3,-1,cst) && s(bbn,-1) )
 { l[i]=omo1; r[1572]++; if(dbg){print "R1572"}; continue};
 if ( w(1,cst) && s(0) )
 { l[i]=omo1; r[1573]++; if(dbg){print "R1573"}; continue};

 cst="блистать брать вздыматься взять возвышаться выкупить вылепить выситься гореть заложить занимать занять заполонить запустить запылать затихать затихнуть захватить защитить защищать лепить летать маячить наполнить наполнять облететь обогнуть обойти обшарить ограбить окружать окружить осадить осаждать основать основывать отвоевать отвоёвывать отстраивать отстроить охранять оцепить оцеплять перебудить пограбить покидать покинуть полуразорить полуразрушить попинуть построить править пылать разграбить разрушить сгореть сжечь снести сносить соорудить сооружать срыть строить укрепить усладить услаждать штурмовать наводнить создать создавать ограбить ограждать обнести обносить объявиться меблировать стереть завещать ощущать скрыться скрываться подниматься подняться возникнуть возникать задрожать возноситься вознестись вздрогнуть агонизировать наполниться наполняться охраняться приближаться приближаться раскатать раскатывать порушить";
 ...
 cst="попасть попадать";
 if ( bb(-7,-2,cst) &&
        wb(bbn+1,-1,"в") && s(bbn,-1) && qb(bbn-3,-1,"suw_edtv suw_mntv") )
 { l[i]=omo2; r[1576]++; if(dbg){print "R1576"}; continue};

 cst="получить передать передавать";
 if ( bb(-7,-3,cst) &&
      ( qxs(-1,"в","дар подарок наследство собственность")||
        qxs(-1,"во","владение") ) && s(bbn,-3) )
 { l[i]=omo1; r[1576]++; if(dbg){print "R1576"}; continue};

 cst="бежать брать бросать бросить везти вернуться вести взять вломиться возвратить возвратиться возвращать возвращаться войти ворваться впускать впустить врываться входить въезжать въехать вырваться гнать двигаться двинуться доставить доставлять ехать забирать забрать забраться забредать забрести заезжать заехать зайти закрывать закрыть залезть заманивать заманить заходить зачастить заявиться идти лезть наезжать направить направиться направляться отвести отводить отправить отправиться отправлять пойти поместить посадить поспешить поторопиться потянуть прибежать прибывать прибыть приволочь пригласить приглашать приехать прийти притащить приходить пробраться провести проводить пройти прокрасться пролезть проникать проникнуть пропускать пропустить прорваться прорываться проходить пускать пустить развалить сбежать спешить торопиться уезжать уехать уйти ходить явиться отпускать повести завести попасть смотаться попадать тянуть скакать ускакать вливаться привести поехать вернуться возвратиться обратно";
 if ( bb(-7,-2,cst) &&
        wb(bbn+1,-1,"в к") && s(bbn,-1) && Qb(bbn+1,wbn-1,"pre_any") && !(bb(bbn-3,bbn-1,"ключ") && s(bbn,-1)) )
 { l[i]=omo1; r[1576]++; if(dbg){print "R1576"}; continue};
 if ( wb(-3,-1,"в к") &&
 ...
 cst="бегство визит ворота вход дорога заключение заточение поездка прибытие приезд проникновение путь тропа возвращение дверь";
 if ( bb(-5,-2,cst) &&
       wb(bbn+1,-1,"в к") && s(bbn,-1) )
 { l[i]=omo1; r[1578]++; if(dbg){print "R1578"}; continue};

 cst="выступить нападать напасть ориентироваться ровняться нападение атака";
 if ( bb(-7,-2,cst) &&
 ...
 cst="бандитов барона боса брата герцога графа князя колдуна колдуньи королевы короля людоеда магната матери некроманта отца принца скэгов тролля царицы царя чародея штурмом эрцгерцога надежды надежд лорда феодала родственников владетеля грааля заклинателей магистра хана волщебникаш сфорца ужасов хранителей принцев дракулы зла семьи семейства отчаяния";
 if ( wf(1,4,cst) && s(0,wfn-1) )
 { l[i]=omo1; r[1581]++; if(dbg){print "R1581"}; continue};

 cst="берегу горе горизонте море озере острове реке скале холме туче небе севере юге востоке западе вершине границе земле гравюре";
 if ( w(1,"на") &&
       w(2,cst) && s(0,1) )
 ...
 cst="берегу горе горизонту морю озеру острову реке скале холму туче небу северу югу востоку западу вершине границе";
 if ( w(1,"к") &&
       w(2,cst) && s(0,1) )
 { l[i]=omo1; r[1582]++; if(dbg){print "R1582"}; continue};
 if ( wb(-6,-1,"к") &&
 ...
 cst="башня окно стена оборона осада штурм нападение";
 if ( bf(1,6,cst) && s(0,bfn-1) )
 { l[i]=omo1; r[1581]++; if(dbg){print "R1581"}; continue};
 if ( bb(-6,-1,cst) && s(bfn,-1) )
 { l[i]=omo1; r[1581]++; if(dbg){print "R1581"}; continue};

 cst="выглядеть казаться представлять оказаться";
 ...
 #
 cst="автоматический амбарный вакуумный висячий врезной входной дверной инфракрасный капсюльный кодовый колесцовый кремнёвый лазерный лёгонький магнитный механический навесный незатейливый никелированный орудийный папиллярный потайной пудовый пушечный ржавый сейфовый сенсорный тяжёлый фитильный хилый хитрый хлипкий центральный цифровой электрический электромагнитный электронный неподатливый капризный открытый одноразовый противоугонный особый сверхнадёжный";
 if ( bb(-3,-1,cst) && s(bbn,-1) )
 { l[i]=omo2; r[1583]++; if(dbg){print "R1583"}; continue};
 if ( base(1,cst) && s(0) )
 { l[i]=omo2; r[1584]++; if(dbg){print "R1584"}; continue};


 cst="активировать взламывать взломать включать включить врезать вскрывать вскрыть вспарывать вспороть выбивать выбить выключать выключить вырвать вырезать вырывать вышибить деактивировать загреметь заклинить закодировать закрывать закрыть заменить заменять замкнуть замыкать запереть запирать запрограммировать заржавить заскрежетать застегнуть застёгивать захлопнуть захлопывать защёлкивался защёлкнуть защёлкнуться звякнуть изготовить клацать клацнуть ключ кодировать лязгать лязгнуть менять навесить навешивать настроить нащупать нащупывать откидывать откинуть открыть отмыкать отмычка отомкнуть отпереть отпирать перекосить провернуть провернуться проворачивать проворачиваться программировать проржаветь разблокировать раскодировать расстегнуть расстёгивать резать сбивать сбить скрежетать сломать смазать смазывать сорвать сработать срезать установить щёлкать щёлкнуть вставить врезать поменять скрипнуть скрипеть ощупать ощупывать размыкать разомкнуть разрезать открыться открывать разработать поддаться поддаваться вывалиться вываливаться податься подаваться заблокировать хрустнуть приносить принести сломаться испортить отщёлкиваться отстрелить отлететь захлопнуться заскрипеть закрываться раскрываться открываеться открыться заржаветь заедать мяукнуть маукать вылететь вылетать вешать повесить";
 ...
 cst="автомата багажника винтовки ворот двери забрала зажигания камеры капота клетки контейнера орудия пушки ружья цепочки чемодана ящика домофона люка гермолюка казённика сейфа входа хода квартиры застёжки";
 ...
 cst="вставить вставлять выстрелить захватить захватывать ключ нащупать нащупывать отмычка сжать сжимать сложить складывать сплести сплестись сплетать стискивать стиснуть стрелять сцепить сцеплять тыкать ткнуть сжаться сжиматься выставить выставлять постучать постукивать";
 if ( bb(-7,-2,cst) &&
 ...
 cst="ворота дверь дверца калитка люк цепочка чемодан комната камера ангар ящик защёлка сундук рундук шкаф шкафчик тумбочка контейнер гараж";
 if ( w(1,"на") &&
 ...
 cst="дверь дверца калитка люк цепочка чемодан комната камера ангар ящик";
 if ( bb(-5,-1,cst) &&
       wb(bbn-3,bbn-1,"на") && s(wbn,-1) )
 { l[i]=omo2; r[1592]++; if(dbg){print "R1592"}; continue};

 cst="код скважина шифр секрет ключ фиксатор пистон";
 if ( w(1,"с") &&
...
#if (dbgstat==1) {system(cmd); for (i=1; i<=1594; i++) { printf ("%s%s %s %s\n", "R", i, "=", r[i]) >> "_stat.txt"};
