...
BEGIN {
    ...
    cmd = "zcat basedic.gz quv.gz"; while((cmd|getline)>0) {
   if (substr($2,1,3) == "гл_") {
      if (substr($2,1,7) == "гл_чув_") {
        if($2~  /^гл_чув_/                               ){  gl_quv     [$1];                  continue }; #gl_sonebued   [$1];
      };
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[21]++; print "R21"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[24]++; print "R24"}; continue;};
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[28]++; print "R28"}; continue;};
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[55]++; print "R55"}; continue;};
 # все + это + ... + нар[….,:;!?—]
 if ( q(1,mest_it) &&
       q(2,mest_da) &&
 ...
 if ( phs(-2,"как и") && s(-3,-1) &&
       q(-1,mest_mn) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[70]++; print "R70"}; continue;};
 ...
 if ( q(1,mest_mnim) && sc(0,",") &&
      (q(2,pre_ro)||q(2,pre_vi)) &&
        w(3,"кого") && s(1,2) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[76]++; print "R76"}; continue;};
 ...
 # Все + ... + глагол восприятия =====================================
 if ( q(-2,gl_quv) &&
       q(-1,pre_vi) && s(-2,-1) && p(0) )
 { sub(/е/, "ё", l[i]); if(dbg){r[89]++; print "R89"}; continue;};
 if ( q(-2,nar_obvr) &&
 ...
 if ( (q(1,prl_mniv)||q(1,prq_mniv)||q(1,prl_mnvr)||q(1,prq_mnvr)||q(1,prl_krmn)) &&
        ...
        (q(3,prl_mniv)||q(3,prq_mniv)||q(3,prl_mnvr)||q(3,prq_mnvr)||q(3,prl_krmn)) &&
         (q(4,suw_odmnim)||q(4,suw_nomniv)) &&
           q(5,gl_nemn) && s(0,4)  )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[105]++; print "R105"}; continue;};
 if ( (q(1,prl_mniv)||q(1,prq_mniv)||q(1,prl_mnvr)||q(1,prq_mnvr)||q(1,prl_krmn)) &&
        w(2,"и|или") &&
        (q(3,prl_mniv)||q(3,prq_mniv)||q(3,prl_mnvr)||q(3,prq_mnvr)||q(3,prl_krmn)) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[107]++; print "R107"}; continue;};
 # все + ... + прл.
 if ( w(1,"и") &&
      (q(2,prl_mniv)||q(2,prq_mniv)||q(2,prl_mnvr)||q(2,prq_mnvr)||q(2,prl_krmn)) && s(0,1) )
 ...
 if ( (qb(-5,prl_krmn)||qb(-5,mest_mnim)) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[132]++; print "R132"}; continue;};
 ...
 if ( (q(1,mest_ed)||q(1,mest_mn)) &&
        q(2,suw_edim) &&
         q(3,qisl) && s(0,2) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[146]++; print "R146"}; continue;};
 # всё + мест + част+ сущ.ед.
 if ( q(1,mest_ed) &&
       q(2,qast) &&
        q(3,suw_edim) && s(0,2) )
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[162]++; print "R162"}; continue;};
 # все + более + 
 if ( q(1,nar_opst) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[176]++; print "R176"}; continue;};
 if ( q(1,mest_im) &&
       (q(2,gl_vzmn)||q(2,gl_nemn)||q(2,prl_krmn)) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[177]++; print "R177"}; continue;};
 # все + мест.мн + [….,;:!?]
 if ( q(1,mest_ed) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[245]++; print "R245"}; continue;};
 # все посмотрели в
 if (q(1,qast) &&
      q(2,gl_pnmn) &&
       q(3,pre_vi) &&
        q(4,suw_edvi) && s(0,3) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[246]++; print "R246"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[261]++; print "R261"}; continue;};
 # все те, кто
 if ( w(1,"те") && sc(1,",") &&
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[273]++; print "R273"}; continue;};
 ...
 #dbg = 1
 #dbgstat = 1
 if (dbgstat==1) {for (i=1; i<=275; i++) { printf ("%s%s %s %s\n", "R", i, "=", r[i]) }; };
