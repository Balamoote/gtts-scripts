...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 ...
 cst="берегу вершине востоке горе горизонте гравюре границе западе земле край море небе озере острове пригорке реке севере скале туче холме холмике юге";
 if ( w(1,"на") &&
       wf(2,4,cst) && s(0,wfn-1) )
 { l[i]=omo1; r[1605]++; if(dbg){print "R1605"}; continue };
 if ( wb(-6,-1,"на") &&
       wb(wbn+1,-1,cst) && s(wbn,-1) && !(bf(1,5,"повесить") && s(0,bfn-1)) )
 { l[i]=omo1; r[1606]++; if(dbg){print "R1606"}; continue };
 ...
 { l[i]=omo1; r[1608]++; if(dbg){print "R1608"}; continue };
   

 if ( wc(-1,"этажн[оы]") && s(-1) )
 { l[i]=omo1; r[1609]++; if(dbg){print "R1609"}; continue };

 if ( w(1,"веками") && s(0) )
 { l[i]=omo1; r[1610]++; if(dbg){print "R1610"}; continue };

 ### lock_
 cst="автоматический амбарный битумный вакуумный висячий врезной входной глиняный дверной инфракрасный кандальный капризный капсюльный коварный кодовый колесцовый кремнёвый лазерный логический лёгонький магнитный механический навесный накидной незатейливый неподатливый никелированный нутряной одноразовый орудийный особый открытый папиллярный потайной противоугонный пудовый пушечный ржавый сверхнадёжный сейфовый сенсорный сундучный тугой тяжёлый фитильный хилый хитрый хлипкий центральный цифровой электрический электромагнитный электронный";
 if ( bb(-3,-1,cst) && s(bbn,-1) )
 ...
 { l[i]=omo2; r[1618]++; if(dbg){print "R1618"}; continue };
 if ( bb(-5,-1,cst) &&
       wb(bbn-3,bbn-1,"на") && s(bbn-3,-1) )
 { l[i]=omo2; r[1619]++; if(dbg){print "R1619"}; continue };
 ...
 { l[i]=omo2; r[1622]++; if(dbg){print "R1622"}; continue };

 cst="защёлка молния";
 if ( sc(0,"-") && base(1,cst) )
 { l[i]=omo2; r[1623]++; if(dbg){print "R1623"}; continue };
 ...
 #
    ...
    { l[i]=omo2; r[1624]++; if(dbg){print "R1624"}; continue };
    ...
    if ( wb(-3,-1,"на") &&
          bf(1,3,cst) && s(wbn,bbf-1) )
    { l[i]=omo1; r[1630]++; if(dbg){print "R1630"}; continue };
   
    cst="подкоп";
    if ( bb(-7,-2,cst) &&
          wb(bbn+1,-1,"под") && s(bbn,-1) && Qb(bbn+1,wbn-1,"pre_any") )
    { l[i]=omo1; r[1631]++; if(dbg){print "R1631"}; continue };
#   if ( wb(-3,-1,"под") &&
#         bf(1,3,cst) && s(wbn,bbf-1) )
#   { l[i]=omo1; r[1632]++; if(dbg){print "R1632"}; continue };
    ...
    if ( wb(-3,-1,"с со") &&
          bf(1,3,cst) && s(wbn,bbf-1) )
    { l[i]=omo2; r[1642]++; if(dbg){print "R1642"}; continue };
   
    if ( w(-3,"в") &&
          q(-2,"suw_edvi suw_mnvi") &&
           w(-1,"под") && s(-3,-1) )
    { l[i]=omo2; r[1643]++; if(dbg){print "R1643"}; continue };
    if ( w(-1,"под") &&
          w(1,"в") &&
           q(2,"suw_edvi suw_mnvi") && s(-1,1) )
    { l[i]=omo2; r[1644]++; if(dbg){print "R1644"}; continue };
 ...
 if ( q(1,"pre_vi pre_ro pre_pr") &&
       q(2,"isname") && s(0,1) )
 { l[i]=omo1; r[1646]++; if(dbg){print "R1646"}; continue };
 if ( q(-1,"isname") && ( s(-2,-1) || ( sc(-2,",") && s(-1) ) ) )
 { l[i]=omo1; r[1647]++; if(dbg){print "R1647"}; continue };
 if ( q(0,"isname") && s(-1) )
 { l[i]=omo1; r[1648]++; if(dbg){print "R1648"}; continue };
...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 #
#if ( base(-1,"главный") && s(-1) )
#{ l[i]=omo2; r[1649]++; if(dbg){print "R1649"}; continue };
...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 #
 l[i]=omo1; r[1650]++; if(dbg){print "R1650"}; continue;
