...
function s(m, n,    k, rett)                      # разделители в диапазоне НЕ содержат препинаний? = пробел
                { rett = 1; for (k=m; k<=n; k++) { if (sep[i+k] ~ "[…,.:;!?—]" ) {rett = 0; break} else {rett = 1}; }; return rett }
function pv(m, n, pu,    k, rett)                 # разделители в диапазоне НЕ содержат препинания pu
                { rett = 1; for (k=m; k<=n; k++) { if (sep[i+k] ~ pu) {rett = 0; break} else {rett = 1}; }; return rett }
...
BEGIN {
 ...
 if ( w(1,"и") &&
       w(2,"вся") && s(0,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[9]++; print "R9"}; continue;};
 if ( w(1,"и") &&
       w(2,"всяческие") && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[10]++; print "R10"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[61]++; print "R61"}; continue;};
 if ( q(-1,mest_it) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[63]++; print "R63"}; continue;};
 if ( q(-1,mest_it) &&
       q(1,mest_mn) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[91]++; print "R91"}; continue;};
 if ( w(1,"как") &&
       w(2,"будто") &&
        q(3,gl_edsr) && s(0,2) )
 ...
 # Все то же
 if ( q(1,prl_kred_sr) &&
      (q(2,mest_it)||w(2,"эти|те|той|того|тех|тем")) &&
        w(3,"же|ж") && s(0,2) )
 { sub(/е/, "ё", l[i]); if(dbg){r[95]++; print "R95"}; continue;};
 if ( (q(1,mest_it)||w(1,"эти|те|той|того|тех|тем|так|такой|таком|такого|таким|такими|им|ей")) &&
        w(2,"же|ж") && s(0,1) )
 ...
 if ( (q(1,pre_ro)||q(1,pre_vi)) && sc(0,",") &&
       w(2,"кого" ) && s(1,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[102]++; print "R102"}; continue;};
 if ( q(-1,pre_ro) && !(q(-1,pre_im)||q(-1,pre_vi)||q(-1,pre_da)||q(-1,pre_tv)||q(-1,pre_pr)) && s(-1,-1) )
 ...
 if ( sc(0,",") &&
       q(1,pre_da) &&
        w(2,"чему") && s(1,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[111]++; print "R111"}; continue;};
 if ( sc(0,",") &&
       q(1,pre_da) &&
        w(2,"кому") && s(1,1) )
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[130]++; print "R130"}; continue;};
 if ( (q(-1,mest_ed)||q(-1,mest_mn)) &&
        phf(1,"не так") &&
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[134]++; print "R134"}; continue;};
 if ( (q(-1,gl_quv)||q(-1,gl_gov)) &&
        w(1,"что") && s(-1,-1) && sc(0,",") )
 ...
 if ( (q(-2,suw_noed)||q(-2,suw_nomn)) &&
       (q(-1,gl_quvmn)||q(-1,gl_govmn)) && s(-2,-1) && p(0) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[145]++; print "R145"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[155]++; print "R155"}; continue;};
 # все + прч.
 ...
 if ( (q(1,prl_mniv)||q(1,prq_mniv)||q(1,prl_mnvr)||q(1,prq_mnvr)||q(1,prl_krmn)) &&
       (q(2,suw_odmnim)||q(2,suw_nomniv)) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[168]++; print "R168"}; continue;};
 # всё + прл.
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[182]++; print "R182"}; continue;};
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[187]++; print "R187"}; continue;};
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[191]++; print "R191"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[206]++; print "R206"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[211]++; print "R211"}; continue;};
...
# все + более + 
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[247]++; print "R247"}; continue;};
 # все + гл.мн + сущ
 if ( q(1,mest_mn) &&
       q(2,mest_nar) &&
        q(3,gl_vzmn) && s(0,2) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[248]++; print "R248"}; continue;};
 # все + гл.мн + сущ
 if ( q(1,mest_mn) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[256]++; print "R256"}; continue;};
 ...
 if ( q(-1,gl_nemn) && 
      (q(1,prl_mniv)||q(1,prq_mniv)||q(1,prl_mnvr)||q(1,prq_mnvr)||q(1,prl_krmn)) &&
        q(2,deep) && s(-1,0) && sc(1,",") )
 { sub(/е/, "ё", l[i]); if(dbg){r[264]++; print "R264"}; continue;};
 ...
 if ( q(1,gl_nemn) &&
       q(2,nar_opka) &&
        q(3,prq_krmn) && s(0,2) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[265]++; print "R265"}; continue;};
 if ( q(1,gl_nemn) &&
       q(2,prl_krmn) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[266]++; print "R266"}; continue;};
 if ( (q(1,gl_pemn)||q(1,gl_pnmn)) &&
       (q(2,suw_edro)||q(2,suw_mnro)||q(2,suw_odmnvr)) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[267]++; print "R267"}; continue;};

 if ( (q(1,gl_pemn)||q(1,gl_pnmn)) &&
       (q(2,suw_edtv)||q(2,suw_mntv)||q(2,suw_edda)||q(2,suw_mnda)) && s(0,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[268]++; print "R268"}; continue;};
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[269]++; print "R269"}; continue;};
 if ( (q(1,gl_pemn)||q(1,gl_nemn)||q(1,gl_pnmn)||q(1,gl_vzmn)) &&
        w(2,"и") &&
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[276]++; print "R276"}; continue;};
 # всё + гл.ед
 ...
 if ( q(1,nar_obme) &&
       q(2,mest_da) &&
        q(3,qast) &&
         q(4,gl_ed) && s(0,3) )
 { sub(/е/, "ё", l[i]); if(dbg){r[290]++; print "R290"}; continue;};
 # все + гл.мн + гл.инф
 if ( q(1,nar_obme) &&
       q(2,gl_pnmn) &&
        q(3,gl_in) && s(0,2) )
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[295]++; print "R295"}; continue;};
 # всё + гл.ед
 if ( q(-2,pre_ro) &&
       q(-1,mest_ro) &&
       (q(1,gl_ed)||q(1,gl_in)) && s(-2,0) )
 { sub(/е/, "ё", l[i]); if(dbg){r[296]++; print "R296"}; continue;};
 # всё + гл.ед
 if ( q(1,qast) &&
      (q(2,gl_ed)||q(2,gl_in)) && s(0,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[297]++; print "R297"}; continue;};
 # их все + гл.ед
 if ( q(-3,gl_ed) &&
       w(-2,"бы") &&
        q(-1,mest_mnvi) &&
 ...
 if (  (q(1,nar_obvr)||q(1,nar_obme)||q(1,nar_opsp)||q(1,nar_opst)||q(1,nar_srav)||q(1,nar_opka)) &&
        (q(2,gl_ed)||q(2,gl_in)) && s(0,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[302]++; print "R302"}; continue;};
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[304]++; print "R304"}; continue;};
 # все вас знают
 if ( q(1,mest_vi) &&
      (q(2,gl_pemn)||q(2,gl_pnmn)) && s(0,1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[305]++; print "R305"}; continue;};
 # все вас знают
 if ( l[i+1]~/ы$/ &&
 ...
 if ( w(1,"то") && sc(1,",") &&
       q(2,mest_edsr) )
 { sub(/е/, "ё", l[i]); if(dbg){r[329]++; print "R329"}; continue;};
 # всё, что
 if ( sc(0,",") &&
       q(1,mest_edsr) && W(2,"есть") )
 { sub(/е/, "ё", l[i]); if(dbg){r[330]++; print "R330"}; continue;};
 # все те, кто
 if ( w(1,"те") && sc(1,",") &&
 ...
 if ( sc(0,",") &&
       (q(1,mest_edmu)||q(1,mest_edze)) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[332]++; print "R332"}; continue;};
 # учитывало всё, кроме
 if ( q(-1,gl_ed) && s(-1,-1) &&
       sc(0,",") &&
        w(1,"кроме") )
 ...
 { sub(/е/, "ё", l[i]); if(dbg){r[334]++; print "R334"}; continue;};
 if ( sc(0,",") &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[337]++; print "R337"}; continue;};
 if ( (q(1,gl_pemn)) && sc(1,",") &&
        w(2,"что") && s(0,0) )
 ...
 if ( w(1,"были") && s(0,0) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[343]++; print "R343"}; continue;};

 # поиск вперёд
 if ( qf(7,gl_edsr) &&
       q(qfn+1,prl_kred_sr) && s(0,qfn) )
 { sub(/е/, "ё", l[i]); if(dbg){r[344]++; print "R344"}; continue;};
 if ( q(1,mest_it) &&
       q(2,mest_da) &&
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[346]++; print "R346"}; continue;};

 # поиск назад
 if ( q(1,pre_ro) && qb(-4,mest_mn) &&
      (q(2,suw_edro)||q(2,suw_mnro)||q(2,suw_odmnvr)) && s(0,1) )
 { sub(/е/, "ё", l[i]); if(dbg){r[347]++; print "R347"}; continue;};
 if ( qb(-7,mest_im) &&
       phs(-1,"как и") && s(-2,-1) )
 ...
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[349]++; print "R349"}; continue;};
 if ( (qb(-5,prl_krmn)||qb(-5,mest_mnim)) && pv(-5,0,".") &&
       (q(1,prl_kred_sr)||q(1,prl_edsrim)) && s(0,0) && p(1) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[350]++; print "R350"}; continue;};

 # Конечное подбирание хвостов
 if ( (q(1,suw_odmnim)||q(1,suw_nomniv)) && (q(1,prl_mniv)||q(1,prl_mnvr)) && s(0,0) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[351]++; print "R351"}; continue;};
 if ( q(1,gl_vzmn) && s(0,0) )
 { sub(/е/, "<_je_>", l[i]); if(dbg){r[352]++; print "R352"}; continue;};
 ...
 #dbg = 1
 #dbgstat = 1
 if (dbgstat==1) {for (i=1; i<=352; i++) { printf ("%s%s %s %s\n", "R", i, "=", r[i]) }; };
