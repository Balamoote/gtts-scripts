...
# ОТКЛЮЧЕНО!
function sw_ex_r_cy(ret, stopp) { while (stopp == 0) {
 #
 if ( w(-2,"не") &&
...
function sw_em_r_cy(ret, stopp) { while (stopp == 0) {
 #
...
function sw_ez_r_cy(ret, stopp) { while (stopp == 0) {
 ...
 { ret=stopp=1; d[259]++; if(dbg){print "D259"}; continue };
 #
 if ( (w(-1,"нет")||qi_duoz(-1)) && s(-1) )
 { ret=stopp=1; d[260]++; if(dbg){print "D260"}; continue };
 ...
 if ( prex_ro(-1) && s(-1) )
 { ret=stopp=1; d[263]++; if(dbg){print "D263"}; continue };
 } else {#
 if ( q(-3,"pre_ro preph_ro") &&
       q(-2,"prl_edzero prq_edzero mest_edzero") &&
        q(-1,"prl_edzero prq_edzero mest_edzero") && s(-3,-1) )
 { ret=stopp=1; d[264]++; if(dbg){print "D264"}; continue };
 if ( q(-2,"pre_ro preph_ro") &&
      (q(-1,"prl_edzero prq_edzero mest_edzero")||w(-1,"его их")) && s(-2,-1) )
 { ret=stopp=1; d[265]++; if(dbg){print "D265"}; continue };
 if ( pre_ro(-1) && s(-1) )
 { ret=stopp=1; d[266]++; if(dbg){print "D266"}; continue };
 };#_#_#
 if ( q(-1,"preph_ro prl_edzero prq_edzero mest_edzero") && W(-1,"её") && s(-1) )
 { ret=stopp=1; d[267]++; if(dbg){print "D267"}; continue };

break}; return ret }

function sw_mn_i_cy(ret, stopp) { while (stopp == 0) {
 #
...
function sw_mn_v_cy(ret, stopp) { while (stopp == 0) {
 #
 if ( W(-3,"не") &&
