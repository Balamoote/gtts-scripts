...
sz_iili(-3) &&
 ...
 if ( sz_iili(1) &&
            q(2,"suw_edro suw_mnro") && Q(2,"suw_mnim gl_ed sz") && s(0,1) )
...
sz_iili(-1) &&
 ...
 if ( q(1,"mest_im suw_edim suw_mnim") && s(0,1) &&
          narph_any(2) &&
      w(xwn+1,"не") &&
...
sz_iili(-3) &&
 ...
 if ( q(-5,"mest_im suw_edim suw_mnim sz") &&
 pre_ro(-4) &&
 ...
 if ( q(-4,"mest_im suw_edim suw_mnim sz") &&
 pre_ro(-3) &&
      q(-2,"prl_edro prl_mnro mest_ro") &&
      q(-1,"suw_edro suw_mnro") && s(-4,-1) && Q(-1,"narph_any") )
 { rett=stopp=1; D[165]++; if(dbg){print "D165"}; continue };
 if ( q(-3,"mest_im suw_edim suw_mnim sz") && prex!=1 &&
 pre_ro(-2) &&
...
sz_iili(-1) &&
      q(1,"suw_edim suw_mnim") && s(-2,0) && Q(0,"gl_paedze")  )
...
sz_iili(-2) &&
 ...
 if ( q(-2,"gl_pnin gl_pnin gl_pned gl_pnmn deep_pn") && Q(-2,"gl_poed gl_pomn") &&
      q(-1,"suw_edda suw_mnda mest_da") && suw_mnim(0) && Q(0,"gl_in") && s(-2,-1) && (p(0)||((sz_iili(1)||pre_any(1)) && s(0))) )
 ...
 if ( pre_vi(-2) &&
           q(-1,"suw_edvi suw_mnvi") && q(1,"gl_nein gl_need gl_nemn gl_vzmn deep_ne") && s(-2,0) && Q(-1,"mest_vi prl_edvi prl_mnvi") )
...
#{ rett=stopp=1; D[409]++; if(dbg){print "D409"}; continue };
 ...
 if ( sw_im_f() )
 { rett=stopp=1; D[412]++; if(dbg){print "D412", "sw_im_f"}; continue };

break}; return rett }

function sw_em_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; D[413]++; if(dbg){print "D413", "sw_vi_f"}; continue };
...
function sw_em_d_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_da_f() )
 { rett=stopp=1; D[423]++; if(dbg){print "D423", "sw_da_f"}; continue };
...
mest_3e(-1) && s(-2,-1) )
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; D[446]++; if(dbg){print "D446", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; D[447]++; if(dbg){print "D447", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; D[448]++; if(dbg){print "D448"}; continue };

break}; return rett }

function sw_em_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; D[449]++; if(dbg){print "D449", "sw_tv_f"}; continue };
...
function sw_em_p_f(rett, stopp) { while (stopp == 0) {
 #
 if ( ismark(-1,"Npre") )
 { rett=0; stopp=1; D[454]++; if(dbg){print "D454"}; continue };
...
function sw_msz_z_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( suw_any(-2) && type(-2,"unit") &&
            q(-1,"prl_edro prq_edro mest_ro") && s(-2,-1) )
 { rett=stopp=1; stopp=1; D[467]++; if(dbg){print "D467"}; continue };
...
function sw_em_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; D[475]++; if(dbg){print "D475", "sw_msz_z_f"}; continue };
...
function sw_es_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; D[479]++; if(dbg){print "D479", "sw_msz_z_f"}; continue };
...
function sw_es_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; D[487]++; if(dbg){print "D487", "sw_im_f"}; continue };
...
function sw_es_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; D[503]++; if(dbg){print "D503", "sw_vi_f"}; continue };
...
function sw_es_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; D[517]++; if(dbg){print "D517", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; D[518]++; if(dbg){print "D518", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; D[519]++; if(dbg){print "D519"}; continue };
...
function sw_es_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; D[528]++; if(dbg){print "D528", "sw_da_f"}; continue };
 #
...
function sw_es_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; D[535]++; if(dbg){print "D535", "sw_tv_f"}; continue };
 #_#_#
 if ( q(-3,"pre_tv preph_tv") &&
...
function sw_ez_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; D[543]++; if(dbg){print "D543", "sw_im_f"}; continue };
 ...
 { rett=0; stopp=1; D[546]++; if(dbg){print "D546"}; continue };
 if ( q(-2,"mest_ro suw_edro suw_mnro") &&
      q(-1,"gl_pnin gl_pned gl_pnmn deep_pn") && regn(-1,"Ur") && s(-2,-1) )
...
function sw_ez_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; D[560]++; if(dbg){print "D560", "sw_vi_f"}; continue };
...
function sw_ez_d_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_da_f() )
 { rett=stopp=1; D[569]++; if(dbg){print "D569", "sw_da_f"}; continue };
 if ( muk_edzeda(-2) &&
...
function sw_em_l_f(rett, stopp) { while (stopp == 0) {
 ...
 { rett=0; stopp=1; loc2emd=1; D[576]++; if(dbg){print "D576"}; continue };
 if ( id(-1,"hw4loc") && s(-1) )
...
function sw_ez_l_f(rett, stopp) { while (stopp == 0) {
 ...
 !( (wb(-5,-1,"в во на") && !(sq(wbn,-1,"[….:;!?]"))) || (wf(1,3,"в во на") && !(sq(0,wfn-1,"[….:;!?]")))) )
 { rett=0; stopp=1; loc2emd=1; D[586]++; if(dbg){print "D586"}; continue };
 if ( (regn(-2,"Nme")||type(-2,"dvi del")) &&
          w(-1,"в во на") && s(-2,-1) )
 { rett=0; stopp=1; D[587]++; if(dbg){print "D587"}; continue };
 if ( type(-2,"vos") &&
         w(-1,"на") && s(-2,-1) )
 { rett=0; stopp=1; D[588]++; if(dbg){print "D588"}; continue };
 #
 if ( !(regn(-4,"Nme") && s(-4) ) &&
      pre_pr(-3) &&
    prq_edpr(-2) &&
...
function sw_ez_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; D[605]++; if(dbg){print "D605", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; D[606]++; if(dbg){print "D606", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; D[607]++; if(dbg){print "D607"}; continue };
...
function sw_ez_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; D[619]++; if(dbg){print "D619", "sw_tv_f"}; continue };
...
function sw_mn_i_f(rett, stopp) { while (stopp == 0) {
 if ( edro2mnim==1 )
 { edro2mnim==""; rett=stopp=1; D[630]++; if(dbg){print "D630"}; continue };
 ...
 if ( p(-1) && pre_vi(1) &&
      q(2,"suw_edvi suw_mnvi suw_edne") && s(0,1) && p(2) && Q(0,"gl_paedze") )
...
sz_iili(-2) &&
 ...
 if ( qB(-5,-1,"suw_edim suw_mnim","suw_edro suw_mnro") && sq(qbn,0,",") && Q(0,"gl_paedze") )
...
sz_iili(3) &&
 ...
 if ( sw_im_f() )
 { rett=stopp=1; D[765]++; if(dbg){print "D765", "sw_im_f"}; continue };
break}; return rett }
...
function sw_mn_v_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( q(1,"suw_edro suw_mnro name_ro_sy isname") && s(0,1) &&
          narph_any(2) &&
      q(xwn+1,"gl_pein gl_peed gl_pemn gl_popeed gl_popemn deep_pe") &&
 pre_tv(xwn+2) &&
      q(xwn+3,"suw_edtv suw_mntv") && s(xwn,xwn+2) )
...
sz_iili(-1) && s(-3,-1) )
 ...
 pre_pr(-3) &&
      q(-2,"prl_edpr prq_edpr mest_pr") &&
      q(-1,"suw_edpr suw_mnpr suw_edme") && s(-4,-1) )
 ...
 if ( q(1,"mest_ro suw_edro suw_mnro name_ro_sy isname") && s(0,1) &&
      q(2,"narph_any") &&
...
sz_iili(-1) && s(-2,-1) && Q(0,"gl_paedze") )
 ...
 if ( qB(-5,-1,"suw_edvi suw_mnvi","suw_edro suw_mnro") && sq(qbn,0,",") && Q(0,"gl_paedze") )
...
function sw_mn_r_f(rett, stopp) { while (stopp == 0) {
 ...
 { rett=0; stopp=1; D[852]++; if(dbg){print "D852"}; continue };
 ...
 { rett=0; stopp=1; D[854]++; if(dbg){print "D854"}; continue };
 ...
 #
 if ( sw_ro_f() )
 { rett=stopp=1; D[880]++; if(dbg){print "D880", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; D[881]++; if(dbg){print "D881"}; continue };
...
function sw_mn_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; D[882]++; if(dbg){print "D882", "sw_da_f"}; continue };
 #_#_#
 if (prex == 1) { pre_string = "prex_da preph_da" } else { pre_string ="pre_da preph_da" };
 if ( q(-2,pre_string) &&
...
function gl_mn2e_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( w(-2,"а да") &&
      ...
      W(0,"шутите гоните"))
 { rett=0; stopp=1; mn2e2pomn=1; D[1007]++; if(dbg){print "D1007"}; continue };
 if ( w(-2,"вы") &&
      w(-1,"только лучше") && s(-2,-1) )
 { rett=0; stopp=1; mn2e2pomn=1; D[1008]++; if(dbg){print "D1008"}; continue };
 if ( w(-3,"вы") &&
      q(-2,"mest_vi suw_edvi suw_mnvi") &&
      w(-1,"не") && s(-3,-1) && p(0,"?") )
 { rett=0; stopp=1; mn2e2pomn=1; D[1009]++; if(dbg){print "D1009"}; continue };
