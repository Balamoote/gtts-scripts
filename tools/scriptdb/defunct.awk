...
#{ rett=stopp=1; d[109]++; if(dbg){print "D109"}; continue };
 ...
 { rett=0; stopp=1; edro2mnim=1; d[118]++; if(dbg){print "D118"}; continue };
...
function sw_edro_f(rett,   stopp) { while (stopp == 0) {
 ...
 { rett=stopp=1; stopp=1; d[219]++; if(dbg){print "D219"}; continue };
 ...
 { rett=0; stopp=1; edro2mnim=1; d[243]++; if(dbg){print "D243"}; continue };
 ...
 if ( prq_mnim(-3) &&
       pre_ro(-2) &&
        q(-1,"suw_edro suw_mnro") &&
	 suw_mnim(1) && s(-3,0) )
 { rett=stopp=1; stopp=1; d[248]++; if(dbg){print "D248"}; continue };
 ...
 if ( q(-3,"gl_pnin gl_pned gl_pnmn deep_pn") && regn(-3,"Pt-v") &&
       pre_tv(-2) &&
        q(-1,"suw_edtv suw_mntv mest_tv") && s(-2,-1) )
 { rett=0; edro2mnvi=1; stopp=1; d[252]++; if(dbg){print "D252"}; continue };
 ...
 { rett=0; edro2mnim=1; stopp=1; d[254]++; if(dbg){print "D254"}; continue };
 ...
 { rett=0; edro2mnim=1; stopp=1; d[258]++; if(dbg){print "D258"}; continue };
 ...
 if ( Ww_(-2,"не") &&
         deep_na(-1) && s(-1) )
 { rett=0; stopp=1; d[262]++; if(dbg){print "D262"}; continue };
 ...
 { rett=0; stopp=1; edro2mnim=1; d[280]++; if(dbg){print "D280"}; continue };
 ...
 { rett=stopp=1; stopp=1; d[284]++; if(dbg){print "D284"}; continue };
...
#{ rett=stopp=1; d[407]++; if(dbg){print "D407"}; continue };
 ...
 if ( sw_im_f() )
 { rett=stopp=1; d[410]++; if(dbg){print "D410", "sw_im_f"}; continue };

break}; return rett }

function sw_em_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[411]++; if(dbg){print "D411", "sw_vi_f"}; continue };
...
function sw_em_d_f(rett, stopp) { while (stopp == 0) {
 if ( loc2emd==1 )
 { rett=stopp=1; d[420]++; if(dbg){print "D420", "sw_da_f"}; continue };
...
function sw_em_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[444]++; if(dbg){print "D444", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[445]++; if(dbg){print "D445", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[446]++; if(dbg){print "D446"}; continue };

break}; return rett }

function sw_em_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[447]++; if(dbg){print "D447", "sw_tv_f"}; continue };
...
function sw_em_p_f(rett, stopp) { while (stopp == 0) {
 #
 if ( ismark(-1,"Npre") )
 { rett=0; stopp=1; d[452]++; if(dbg){print "D452"}; continue };
...
function sw_msz_z_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( suw_any(-2) && type(-2,"unit") &&
       q(-1,"prl_edro prq_edro mest_ro") && s(-2,-1) )
 { rett=stopp=1; stopp=1; d[465]++; if(dbg){print "D465"}; continue };
...
function sw_em_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[473]++; if(dbg){print "D473", "sw_msz_z_f"}; continue };
...
function sw_es_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[477]++; if(dbg){print "D477", "sw_msz_z_f"}; continue };
...
function sw_ez_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[481]++; if(dbg){print "D481", "sw_msz_z_f"}; continue };
...
function sw_es_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; d[485]++; if(dbg){print "D485", "sw_im_f"}; continue };
...
function sw_es_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[501]++; if(dbg){print "D501", "sw_vi_f"}; continue };
...
function sw_es_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[515]++; if(dbg){print "D515", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[516]++; if(dbg){print "D516", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[517]++; if(dbg){print "D517"}; continue };
...
function sw_es_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; d[526]++; if(dbg){print "D526", "sw_da_f"}; continue };
 #
...
function sw_es_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[533]++; if(dbg){print "D533", "sw_tv_f"}; continue };
 #_#_#
 if ( q(-3,"pre_tv preph_tv") &&
...
function sw_ez_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; d[541]++; if(dbg){print "D541", "sw_im_f"}; continue };
 ...
 { rett=0; stopp=1; d[544]++; if(dbg){print "D544"}; continue };
 if ( q(-2,"mest_ro suw_edro suw_mnro") &&
       q(-1,"gl_pnin gl_pned gl_pnmn deep_pn") && regn(-1,"Ur") && s(-2,-1) )
...
function sw_ez_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[558]++; if(dbg){print "D558", "sw_vi_f"}; continue };
...
function sw_ez_d_f(rett, stopp) { while (stopp == 0) {
 if ( loc2ezd==1 )
 { rett=stopp=1; d[566]++; if(dbg){print "D566", "sw_da_f"}; continue };
...
function sw_em_l_f(rett, stopp) { while (stopp == 0) {
 ...
 { rett=0; stopp=1; loc2emd=1; d[574]++; if(dbg){print "D574"}; continue};
 if ( id(-1,"hw4loc") && s(-1) )
...
function sw_ez_l_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( (regn(-2,"Nme")||type(-2,"dvi del")) &&
       w(-1,"в во на") && s(-2,-1) )
 { rett=0; stopp=1; d[585]++; if(dbg){print "D585"}; continue };
 if ( type(-2,"vos") &&
       w(-1,"на") && s(-2,-1) )
...
function sw_ez_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[603]++; if(dbg){print "D603", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[604]++; if(dbg){print "D604", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[605]++; if(dbg){print "D605"}; continue };
...
function sw_ez_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[617]++; if(dbg){print "D617", "sw_tv_f"}; continue };
...
function sw_mn_i_f(rett, stopp) { while (stopp == 0) {
 if ( edro2mnim==1 )
 { edro2mnim==""; rett=stopp=1; d[628]++; if(dbg){print "D628"}; continue };
...
#{ rett=stopp=1; d[743]++; if(dbg){print "D743"}; continue };
 ...
 if ( sw_im_f() )
 { rett=stopp=1; d[763]++; if(dbg){print "D763", "sw_im_f"}; continue };
break}; return rett }
...
function sw_mn_r_f(rett, stopp) { while (stopp == 0) {
 ...
 #
 if ( sw_ro_f() )
 { rett=stopp=1; d[876]++; if(dbg){print "D876", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[877]++; if(dbg){print "D877"}; continue };
...
function sw_mn_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; d[878]++; if(dbg){print "D878", "sw_da_f"}; continue };
 #_#_#
 if (prex == 1) { pre_string = "prex_da preph_da" } else { pre_string ="pre_da preph_da" };
 if ( q(-2,pre_string) &&
...
function gl_mn2e_f(rett, stopp) { while (stopp == 0) {
 ...
 { rett=0; stopp=1; mn2e2pomn=1; d[998]++; if(dbg){print "D998"}; continue };
 if ( w(-2,"а да") &&
      w(-1,"вы") && s(-2,-1) && Q_w(1,"mest_da suw_edda suw_mnda") &&
      W(0,"шутите гоните"))
 { rett=0; stopp=1; mn2e2pomn=1; d[999]++; if(dbg){print "D999"}; continue };
 if ( w(-2,"вы") &&
      w(-1,"только лучше") && s(-2,-1) )
 { rett=0; stopp=1; mn2e2pomn=1; d[1000]++; if(dbg){print "D1000"}; continue };
 if ( w(-3,"вы") &&
       q(-2,"mest_vi suw_edvi suw_mnvi") &&
        w(-1,"не") && s(-3,-1) && p(0,"?") )
 { rett=0; stopp=1; mn2e2pomn=1; d[1001]++; if(dbg){print "D1001"}; continue };
