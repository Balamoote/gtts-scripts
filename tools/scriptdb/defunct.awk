...
#{ rett=stopp=1; d[101]++; if(dbg){print "D101"}; continue };
 ...
 { rett=0; stopp=1; edro2mnvi=1; d[109]++; if(dbg){print "D109"}; continue };

 if ( pre_tv(-3) && p(-4) &&
      q(-2,"mest_tv suw_edtv suw_mntv") &&
       q(-1,"gl_in gl_ed gl_mn deep") && s(-3,-1) && p(0) )
 { rett=0; stopp=1; edro2mnim=1; d[110]++; if(dbg){print "D110"}; continue };
 if ( q(-2,"mest_ro suw_edro suw_mnro") &&
       q(-1,"gl_pnin gl_pned gl_pnmn deep_pn") && regn(-1,"Ur") && s(-2,-1) )
 { rett=0; stopp=1; edro2mnvi=1; d[111]++; if(dbg){print "D111"}; continue };
...
function sw_edro_f(rett,   stopp) { while (stopp == 0) {
 ...
 if ( suw_any(-2) && type(-2,"unit") &&
       q(-1,"prl_edro prq_edro mest_ro") && s(-2,-1) )
 { rett=stopp=1; stopp=1; d[211]++; if(dbg){print "D211"}; continue };
 ...
 { rett=0; stopp=1; edro2mnim=1; d[235]++; if(dbg){print "D235"}; continue };
 ...
 if ( q(-2,"prl_mnim prq_mnim") &&
	...
	q(1,"gl_mn gl_vzmn") && s(-2,0) )
 { rett=stopp=1; stopp=1; d[241]++; if(dbg){print "D241"}; continue };
 #
 if ( q(-3,"pre_vi") &&
       q(-2,"suw_edvi suw_mnvi") &&
        q(-1,"suw_edro") &&
 ...
 if ( Ww_(-4,"не") && regn(-3,"Pd-v") &&
         q(-3,"gl_pnin gl_pned gl_pnmn deep_pn") &&
          pre_da(-2) &&
           q(-1,"suw_edda suw_mnda mest_da") && s(-3,-1) )
 { rett=0; edro2mnvi=1; stopp=1; d[245]++; if(dbg){print "D245"}; continue };
 ...
 if ( q(-3,"mest_mnim") &&
       suw_mnim(-2) &&
        souz_iili(-1) && s(-3,-1) )
 { rett=0; edro2mnim=1; stopp=1; d[248]++; if(dbg){print "D248"}; continue };

 if ( narph_any(-1) &&
       gl_nemn(xsn-1) && s(xsn-1,-1) )
 { rett=0; edro2mnvi=1; stopp=1; d[249]++; if(dbg){print "D249"}; continue };
 ...
 if ( p(-1) &&
       ...
       (q(2,"suw_edro suw_mnro")||name_ro_sy(2)||isname(2)) && sc(2,",") &&
         prq_mnim(3) && s(0,1) )
 { rett=0; edro2mnim=1; stopp=1; d[253]++; if(dbg){print "D253"}; continue };
 ...
 { rett=0; stopp=1; d[257]++; if(dbg){print "D257"}; continue };
 ...
 { rett=0; stopp=1; edro2mnim=1; d[272]++; if(dbg){print "D272"}; continue };
 ...
 { rett=stopp=1; stopp=1; d[277]++; if(dbg){print "D277"}; continue };
...
#{ rett=stopp=1; d[399]++; if(dbg){print "D399"}; continue };
 ...
 if ( sw_im_f() )
 { rett=stopp=1; d[402]++; if(dbg){print "D402", "sw_im_f"}; continue };

break}; return rett }

function sw_em_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[403]++; if(dbg){print "D403", "sw_vi_f"}; continue };
...
function sw_em_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[434]++; if(dbg){print "D434", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[435]++; if(dbg){print "D435", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[436]++; if(dbg){print "D436"}; continue };

break}; return rett }

function sw_em_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[437]++; if(dbg){print "D437", "sw_tv_f"}; continue };
...
function sw_em_p_f(rett, stopp) { while (stopp == 0) {
 #
 if ( ismark(-1,"Npre") )
 { rett=0; stopp=1; d[442]++; if(dbg){print "D442"}; continue };
...
function sw_msz_z_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( suw_any(-2) && type(-2,"unit") &&
       q(-1,"prl_edro prq_edro mest_ro") && s(-2,-1) )
 { rett=stopp=1; stopp=1; d[453]++; if(dbg){print "D453"}; continue };
 if ( suw_any(-1) && type(-1,"unit") && s(-1) )
...
function sw_em_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[461]++; if(dbg){print "D461", "sw_msz_z_f"}; continue };
...
function sw_es_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[465]++; if(dbg){print "D465", "sw_msz_z_f"}; continue };
...
function sw_ez_z_f(rett, stopp) { if (wmark("TY","part")) { while (stopp == 0) {
 if ( sw_msz_z_f() )
 { rett=stopp=1; d[469]++; if(dbg){print "D469", "sw_msz_z_f"}; continue };
...
function sw_es_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; d[473]++; if(dbg){print "D473", "sw_im_f"}; continue };
...
function sw_es_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[487]++; if(dbg){print "D487", "sw_vi_f"}; continue };
...
function sw_es_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[501]++; if(dbg){print "D501", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[502]++; if(dbg){print "D502", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[503]++; if(dbg){print "D503"}; continue };
...
function sw_es_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; d[512]++; if(dbg){print "D512", "sw_da_f"}; continue };
 #
...
function sw_es_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[519]++; if(dbg){print "D519", "sw_tv_f"}; continue };
 #_#_#
 if ( q(-3,"pre_tv preph_tv") &&
...
function sw_ez_i_f(rett, stopp) { while (stopp == 0) {
 if ( sw_im_f() )
 { rett=stopp=1; d[527]++; if(dbg){print "D527", "sw_im_f"}; continue };
...
function sw_ez_v_f(rett, stopp) { while (stopp == 0) {
 if ( sw_vi_f() )
 { rett=stopp=1; d[544]++; if(dbg){print "D544", "sw_vi_f"}; continue };
...
function sw_ez_r_f(rett, stopp) { while (stopp == 0) {
 ...
 if ( sw_edro_f() )
 { rett=stopp=1; d[581]++; if(dbg){print "D581", "sw_edro_f"}; continue };
 if ( sw_ro_f() )
 { rett=stopp=1; d[582]++; if(dbg){print "D582", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[583]++; if(dbg){print "D583"}; continue };
...
function sw_ez_t_f(rett, stopp) { while (stopp == 0) {
 if ( sw_tv_f() )
 { rett=stopp=1; d[595]++; if(dbg){print "D595", "sw_tv_f"}; continue };
...
function sw_mn_i_f(rett, stopp) { while (stopp == 0) {
 if ( edro2mnim==1 )
 { edro2mnim==""; rett=stopp=1; d[605]++; if(dbg){print "D605"}; continue };
...
#{ rett=stopp=1; d[720]++; if(dbg){print "D720"}; continue };
 ...
 if ( sw_im_f() )
 { rett=stopp=1; d[740]++; if(dbg){print "D740", "sw_im_f"}; continue };
break}; return rett }
...
function sw_mn_r_f(rett, stopp) { while (stopp == 0) {
 ...
 { rett=0; stopp=1; d[824]++; if(dbg){print "D824"}; continue };
 if ( q(-1,"gl_pein gl_peed gl_pemn deep_pe") && regn(-1,"Nnero") && s(-1) )
 { rett=0; stopp=1; d[825]++; if(dbg){print "D825"}; continue };
 if ( q(-1,"gl_nein gl_need gl_nemn gl_vzmn gl_vzed deep_ne") && regn(-1,"Nnero") && s(-1) )
 { rett=0; stopp=1; d[826]++; if(dbg){print "D826"}; continue };
 ...
 #
 if ( sw_ro_f() )
 { rett=stopp=1; d[852]++; if(dbg){print "D852", "sw_ro_f"}; continue };
 if ( edro2mnvi )
 { rett=0; stopp=1; d[853]++; if(dbg){print "D853"}; continue };
...
function sw_mn_d_f(rett, stopp) { while (stopp == 0) {
 if ( sw_da_f() )
 { rett=stopp=1; d[854]++; if(dbg){print "D854", "sw_da_f"}; continue };
 #_#_#
 if (prex == 1) { pre_string = "prex_da preph_da" } else { pre_string ="pre_da preph_da" };
 if ( q(-2,pre_string) &&
...
function gl_mn2e_f(rett, stopp) { while (stopp == 0) {
 #
 if ( qxs(-1,"так","что") )
 { rett=0; stopp=1; mn2e2pomn=1; d[957]++; if(dbg){print "D957"}; continue };
 if ( w(-2,"а да") &&
      w(-1,"вы") && s(-2,-1) && Q_w(1,"mest_da suw_edda suw_mnda") &&
      W(0,"шутите гоните"))
 { rett=0; stopp=1; mn2e2pomn=1; d[958]++; if(dbg){print "D958"}; continue };
 if ( w(-2,"вы") &&
      w(-1,"только лучше") && s(-2,-1) )
 { rett=0; stopp=1; mn2e2pomn=1; d[959]++; if(dbg){print "D959"}; continue };
 if ( w(-3,"вы") &&
       q(-2,"mest_vi suw_edvi suw_mnvi") &&
        w(-1,"не") && s(-3,-1) && p(0,"?") )
 { rett=0; stopp=1; mn2e2pomn=1; d[960]++; if(dbg){print "D960"}; continue };
