...
hyphback(book[b]);hyphbscy(bscy[b]);getwpos(wrd);for(i in wpos){i=strtonum(i);if(tolower(l[i])!=tolower(wrd))continue;is_vsyo=vsyo[l[i]];is_vsje=vsje[l[i]];          # header2
 ...
 if ( w(1,"до") &&
       w(2,"единого") && s(0,1) )
 { l[i]=is_vsje; V[143]++; if(dbg){print "V143"}; continue };
 if ( pre_pr(-3) &&
 ...
 if ( sc(0,",") &&
       pre_vi(1) &&
        w(2,"кого") && s(1) )
 { l[i]=is_vsje; V[149]++; if(dbg){print "V149"}; continue };
 if ( pre_vi(-1) &&
       gl_vzmn(1) &&
 ...
 if ( mest_it(1) && s(0) && sc(1,",") &&
       vvod(2) && sc(2,",") &&
        q(3,"gl_nemn gl_pnmn") )
 { l[i]=is_vsje; V[246]++; if(dbg){print "V246"}; continue };
 if ( gl_ed(-1) &&
 ...
 if ( sc(0,",") &&
       vvod(1) && sc(1,",") &&
        q(2,"gl_nemn gl_pnmn") )
 { l[i]=is_vsje; V[253]++; if(dbg){print "V253"}; continue };
 if ( mest_it(1) &&
       qast(2) &&
 ...
 if ( mest_it(-1) &&
       w(1,"кто") && s(-1) && sc(0,",") )
 { l[i]=is_vsje; V[262]++; if(dbg){print "V262"}; continue };
 if ( mest_it(-1) &&
       pre_ro(1) &&
        w(2,"того") &&
 ...
 { l[i]=is_vsje; V[282]++; if(dbg){print "V282"}; continue };
 if ( prq_kred_sr(-2) &&
 ...
 { l[i]=is_vsyo; V[307]++; if(dbg){print "V307"}; continue };
 ...
 { l[i]=is_vsje; V[352]++; if(dbg){print "V352"}; continue };
 ...
 if ( q(1,"mest_ed mest_mn") &&
       suw_edim(2) &&
        qik_edim(3) && s(0,2) )
 { l[i]=is_vsje; V[384]++; if(dbg){print "V384"}; continue };
 if ( mest_ed(1) &&
       qast(2) &&
        suw_edim(3) && s(0,2) )
 ...
 { l[i]=is_vsje; V[401]++; if(dbg){print "V401"}; continue };
 ...
 { l[i]=is_vsje; V[412]++; if(dbg){print "V412"}; continue };
 if ( w(1,"как") &&
       w(2,"раз будто") &&
 ...
 { l[i]=is_vsyo; V[433]++; if(dbg){print "V433"}; continue };
 if ( mest_im(-2) &&
       mest_mnim(-1) &&
        mest_mnim(1) && s(-2,0) )
 ...
 if ( mest_da(1) &&
       gl_ed(2) && s(0,1) && p(2) )
 { l[i]=is_vsyo; V[439]++; if(dbg){print "V439"}; continue };
 if ( mest_mnim(1) &&
 ...
 if ( w(-2,"а") &&
       mest_mnim(-1) &&
        q(1,"gl_pnmn gl_nemn") &&
         prl_kred_sr(2) && s(-2,1) )
 { l[i]=is_vsyo; V[457]++; if(dbg){print "V457"}; continue };
 if ( mest_mn(1) &&
 ...
 { l[i]=is_vsje; V[523]++; if(dbg){print "V523"}; continue };
 if ( deep(-1) &&
       nar_mest(1) && s(-1,0) )
...
if ( gl_nemn(1) &&
 ...
 { l[i]=is_vsje; V[573]++; if(dbg){print "V573"}; continue };
 ...
 if ( gl_ed(-2) &&
       mest_mnvi(-1) &&
        q(1,"gl_ed gl_in") && s(-2,0) )
 { l[i]=is_vsje; V[588]++; if(dbg){print "V588"}; continue };
 if ( mest_mnvi(-1) &&
 ...
 { l[i]=is_vsyo; V[609]++; if(dbg){print "V609"}; continue };
 if ( q(1,"gl_nemn gl_pnmn gl_pemn") &&
       pre_pr(2) &&
 ...
 if (pre_pr(1) &&
      q(2,"prl_edpr prl_mnpr") &&
       q(3,"suw_edpr suw_mnpr") &&
        gl_ed(4) && s(0,3) )
 { l[i]=is_vsyo; V[619]++; if(dbg){print "V619"}; continue };
 if (pre_pr(1) &&
      q(2,"prl_edpr prl_mnpr") &&
       q(3,"suw_edpr suw_mnpr") &&
 ...
 if ( q(1,"gl_pemn gl_pnmn") && cap(0) &&
       q(2,"suw_edvi suw_mnvi mest_vi") &&
        gl_in(3) && s(0,2) )
 { l[i]=is_vsyo; V[623]++; if(dbg){print "V623"}; continue };
 if ( q(1,"gl_pemn gl_pnmn") &&
 ...
 { l[i]=is_vsyo; V[628]++; if(dbg){print "V628"}; continue };
 ...
 { l[i]=is_vsyo; V[640]++; if(dbg){print "V640"}; continue };
 ...
 # SpaCy
 if ( vsyo_sy(0) )
 { l[i]=is_vsyo; V[694]++; if(dbg){print "V694"}; continue };
 if ( vsje_sy(0) )
 { l[i]=is_vsje; V[695]++; if(dbg){print "V695"}; continue };
