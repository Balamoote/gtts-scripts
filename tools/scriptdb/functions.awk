...
function tuptoar(string, arrto,    al, k) { # добавить в массив arrto элементы строки string типа 'слово=слово' с вариантами слово, Слово, СЛОВО
...
function sto2xw(string, arrto,   j,k) {             # добавить в массив arrto элементы строки для 2-х связанных слов w1:w2 arrto[w1][w2]
                split(string, artmp, " "); for (k in artmp) { if(artmp[k]) { split(artmp[k], arr1, "_"); arrto[arr1[1]][arr1[2]] }; }; }
function sto3xw(string, arrto,   j,k) {             # добавить в массив arrto элементы строки для 3-х связанных слов w1:w2,w3 arrto[w1][w2][w3]
...
function hyphback(hystring,     l0,l1,l2,sw,hyw) {    # Склеить слова с дефисом, присутствующие в словаре
                hystring = gensub(/unxyp/,"","g",tolower(hystring)); if (hystring ~ hysnip) { for (i=1; i<=nf-1; i++) {sw=""; if( sc(0,"-") ){ sw++; if( sc(1,"-") ) sw++ };
                  switch (sw) {
                  ...
                  case "2": l0=lc(0); l1=lc(1); l2=lc(2); if (l0 != iwrd && l1 != iwrdi && l2 != iwrd) { hyw = l0 "-" l1 "-" l2; if (hyw in dichyph) {
                            ...
                            delete sep[i]; delete l[i+1]; nf=arrpack(i+1,l); arrpack(i, sep) };}; break
                  default: break }; }; }; }
function hyphbphy(hystring,    l0,l1,l2,sw,hyw) {      # Склеить слова с дефисом, присутствующие в словаре
...
function bf_(n,m,wl,   itmz,wls,j,k, ret) {                   # нахождение ВПЕРЁД базовых форм wl в диапазоне n-m из массивов BASE
...
function bb_(n,m,wl,   itmz,wls,j,k, ret) {                   # нахождение НАЗАД базовых форм wl в диапазоне n-m из массивов BASE
...
function esca(    j,k,f, ret) {   # точная фраза как строка из файла pointesque.gz -- выставляет омограф сама
                if ( length(eOMO[wrd]) ) { for (j in eOMO[wrd]) {ret=1; for (k in eWRD[j]) { if (!(gensub(unxy,"","g",l[i+k]) == eWRD[j][k]) ) { ret=0; break};};
                if (ret) { for (k in eSEP[j]) {if ( !(gensub(unxy,"","g",sep[i+k]) == eSEP[j][k]) ) { ret=0; break };};};
                if (ret) { switch (eOMO[wrd][j] ) { case "omo1": l[i]=omo1; break; case "omo2": l[i]=omo2; break; case "omo3": l[i]=omo3; break;
                default: ret=0; break }; break };};}; return ret}
