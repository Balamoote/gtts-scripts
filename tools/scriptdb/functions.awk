...
function q(n, alist,    k,wd, rett)               # обертка для нескольких функций
                { rett=0; wd=lc(n); split(alist,wrds," "); for(k in wrds) { afun=wrds[k]; if(@afun(n,wd)) {rett=1; break} }; return rett}
function qv(n, alist,    k,wd, rett)              # обертка для нескольких функций - НЕ нахождение
                { rett=1; wd=lc(n); split(alist,wrds," "); for(k in wrds) { afun=wrds[k]; if(@afun(n,wd)) {rett=0; break} }; return rett}
...
function Qb(n, isclass,    k, rett)               # поиск на n шагов назад отсутствия слова в слассе
                { rett=1; for (k=-1; k>=n; k--)   { if ( @isclass(k) && s(k,k) ) {rett=0; break}; }; return rett }
...
function wmark(mrk,wl,    k, el, vmrk, rett)      # нахождение в подстроке #xxx (mrk) метки wl
...
function mest_pedmuim(n,  wd,rett) { if(!(wd))wd=lc(n); if (wd in mstc_edmupr && sep[i+n-1]!="-")                                              {rett=1} else {rett=0}; return rett}
...
# причастия
function prq_kred_mu(n,   wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_kred_mu)                                                                  {rett=1} else {rett=0}; return rett}
function prq_kred_ze(n,   wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_kred_ze)                                                                  {rett=1} else {rett=0}; return rett}
function prq_kred_sr(n,   wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_kred_sr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_kred(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_kred_mu||wd in pq_kred_ze||wd in pq_kred_sr)                              {rett=1} else {rett=0}; return rett}
function prq_krmn(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_krmn)                                                                     {rett=1} else {rett=0}; return rett}
function prq_edmuim(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_im)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edsrim(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_im)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzeim(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_im)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edmuvi(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_im||wd in pq_edmu_ro)                                                {rett=1} else {rett=0}; return rett}
function prq_edsrvi(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_im)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzevi(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_vi)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edmuro(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_ro)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edsrro(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_ro)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzero(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_dr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edmuda(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_da)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edsrda(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_da)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzeda(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_dr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edmutv(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_tv)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edsrtv(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_tv)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzetv(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_tv||wd in pq_edze_dr)                                                {rett=1} else {rett=0}; return rett}
function prq_edmupr(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_pr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edsrpr(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edsr_pr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edzepr(n,    wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edze_dr)                                                                  {rett=1} else {rett=0}; return rett}
function prq_edim(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_im||wd in pq_edsr_im||wd in pq_edze_im)                              {rett=1} else {rett=0}; return rett}
function prq_edvi(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_im||wd in pq_edmu_ro||wd in pq_edsr_im||wd in pq_edze_vi)            {rett=1} else {rett=0}; return rett}
function prq_edro(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_ro||wd in pq_edsr_ro||wd in pq_edze_dr)                              {rett=1} else {rett=0}; return rett}
function prq_edda(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_da||wd in pq_edsr_da||wd in pq_edze_dr)                              {rett=1} else {rett=0}; return rett}
function prq_edtv(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_tv||wd in pq_edsr_tv||wd in pq_edze_tv||wd in pq_edze_dr)            {rett=1} else {rett=0}; return rett}
function prq_edpr(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_edmu_pr||wd in pq_edsr_pr||wd in pq_edze_dr)                              {rett=1} else {rett=0}; return rett}
function prq_mnim(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_im)                                                                    {rett=1} else {rett=0}; return rett}
function prq_mnvi(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_im||wd in pq_mn_ro)                                                    {rett=1} else {rett=0}; return rett}
function prq_mnro(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_ro)                                                                    {rett=1} else {rett=0}; return rett}
function prq_mnda(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_da)                                                                    {rett=1} else {rett=0}; return rett}
function prq_mntv(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_tv)                                                                    {rett=1} else {rett=0}; return rett}
function prq_mnpr(n,      wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_ro)                                                                    {rett=1} else {rett=0}; return rett}
...
function prq_mn(n,        wd,rett) { if(!(wd))wd=lc(n); if (wd in pq_mn_im||wd in pq_mn_ro||wd in pq_mn_da||wd in pq_mn_tv)                    {rett=1} else {rett=0}; return rett}
...
function suw_odedim(n,                                                                                                                          wd,rett) { if(!(wd))wd=lc(n);
...
function suw_noedim(n,                                                                                                                          wd,rett) { if(!(wd))wd=lc(n);
...
function mest_mn(n,       wd,rett) { if(!(wd))wd=lc(n); if (wd in mst_mn_im||wd in mst_mn_ro||wd in mst_mn_vi||wd in mst_mn_da||
	                  wd in mst_mn_pr||wd in mst_mn_tv||wd in mst_mn)                                                                       {rett=1} else {rett=0}; return rett}
...
function zvat(n,        wd,rett) { if(!(wd))wd=lc(n); if (wd in zvt)                                                                            {rett=1} else {rett=0}; return rett}
...
function wordmark(n,   el, stopp, rett) { el=lc(n);
   ...
   if(el in pq_edmu_da        ) { rett = rett "#" pq_edmu_da        [el]; };
   if(el in pq_edmu_im        ) { rett = rett "#" pq_edmu_im        [el]; };
   if(el in pq_edmu_pr        ) { rett = rett "#" pq_edmu_pr        [el]; };
   if(el in pq_edmu_ro        ) { rett = rett "#" pq_edmu_ro        [el]; };
   if(el in pq_edmu_tv        ) { rett = rett "#" pq_edmu_tv        [el]; };
   if(el in pq_edsr_da        ) { rett = rett "#" pq_edsr_da        [el]; };
   if(el in pq_edsr_im        ) { rett = rett "#" pq_edsr_im        [el]; };
   if(el in pq_edsr_pr        ) { rett = rett "#" pq_edsr_pr        [el]; };
   if(el in pq_edsr_ro        ) { rett = rett "#" pq_edsr_ro        [el]; };
   if(el in pq_edsr_tv        ) { rett = rett "#" pq_edsr_tv        [el]; };
   if(el in pq_edze_dr        ) { rett = rett "#" pq_edze_dr        [el]; };
   if(el in pq_edze_im        ) { rett = rett "#" pq_edze_im        [el]; };
   if(el in pq_edze_vi        ) { rett = rett "#" pq_edze_vi        [el]; };
   if(el in pq_kred_mu        ) { rett = rett "#" pq_kred_mu        [el]; };
   if(el in pq_kred_sr        ) { rett = rett "#" pq_kred_sr        [el]; };
   if(el in pq_kred_ze        ) { rett = rett "#" pq_kred_ze        [el]; };
   if(el in pq_krmn           ) { rett = rett "#" pq_krmn           [el]; };
   if(el in pq_mn_da          ) { rett = rett "#" pq_mn_da          [el]; };
   if(el in pq_mn_im          ) { rett = rett "#" pq_mn_im          [el]; };
   if(el in pq_mn_ro          ) { rett = rett "#" pq_mn_ro          [el]; };
   if(el in pq_mn_tv          ) { rett = rett "#" pq_mn_tv          [el]; };
