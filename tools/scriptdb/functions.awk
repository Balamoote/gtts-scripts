...
function wordmark(n,   el, stoppe, rett) { el=lc(n); while (stoppe == 0) {
   if(dpn_ne_na         [el]) { rett = dpn_ne_na         [el]; stoppe=1; continue };
   if(dpn_ne_pa         [el]) { rett = dpn_ne_pa         [el]; stoppe=1; continue };
   if(dpn_pe_na         [el]) { rett = dpn_pe_na         [el]; stoppe=1; continue };
   if(dpn_pe_pa         [el]) { rett = dpn_pe_pa         [el]; stoppe=1; continue };
   if(dpn_pn_na         [el]) { rett = dpn_pn_na         [el]; stoppe=1; continue };
   if(dpn_pn_pa         [el]) { rett = dpn_pn_pa         [el]; stoppe=1; continue };
   if(dpn_vz_ne_na      [el]) { rett = dpn_vz_ne_na      [el]; stoppe=1; continue };
   if(dps_ne_pa         [el]) { rett = dps_ne_pa         [el]; stoppe=1; continue };
   if(dps_pe_pa         [el]) { rett = dps_pe_pa         [el]; stoppe=1; continue };
   if(dps_pn_pa         [el]) { rett = dps_pn_pa         [el]; stoppe=1; continue };
   if(dps_vz_ne_pa      [el]) { rett = dps_vz_ne_pa      [el]; stoppe=1; continue };
   if(g2_nebu_e1        [el]) { rett = g2_nebu_e1        [el]; stoppe=1; continue };
   if(g2_nebu_e2        [el]) { rett = g2_nebu_e2        [el]; stoppe=1; continue };
   if(g2_nebu_e3        [el]) { rett = g2_nebu_e3        [el]; stoppe=1; continue };
   if(g2_nebu_m1        [el]) { rett = g2_nebu_m1        [el]; stoppe=1; continue };
   if(g2_nebu_m2        [el]) { rett = g2_nebu_m2        [el]; stoppe=1; continue };
   if(g2_nebu_m3        [el]) { rett = g2_nebu_m3        [el]; stoppe=1; continue };
   if(g2_nein           [el]) { rett = g2_nein           [el]; stoppe=1; continue };
   if(g2_nena_e1        [el]) { rett = g2_nena_e1        [el]; stoppe=1; continue };
   if(g2_nena_e2        [el]) { rett = g2_nena_e2        [el]; stoppe=1; continue };
   if(g2_nena_e3        [el]) { rett = g2_nena_e3        [el]; stoppe=1; continue };
   if(g2_nena_m1        [el]) { rett = g2_nena_m1        [el]; stoppe=1; continue };
   if(g2_nena_m2        [el]) { rett = g2_nena_m2        [el]; stoppe=1; continue };
   if(g2_nena_m3        [el]) { rett = g2_nena_m3        [el]; stoppe=1; continue };
   if(g2_nepa_edmu      [el]) { rett = g2_nepa_edmu      [el]; stoppe=1; continue };
   if(g2_nepa_edsr      [el]) { rett = g2_nepa_edsr      [el]; stoppe=1; continue };
   if(g2_nepa_edze      [el]) { rett = g2_nepa_edze      [el]; stoppe=1; continue };
   if(g2_nepa_mn        [el]) { rett = g2_nepa_mn        [el]; stoppe=1; continue };
   if(g2_nepo_ed        [el]) { rett = g2_nepo_ed        [el]; stoppe=1; continue };
   if(g2_nepo_mn        [el]) { rett = g2_nepo_mn        [el]; stoppe=1; continue };
   if(g2_pebu_e1        [el]) { rett = g2_pebu_e1        [el]; stoppe=1; continue };
   if(g2_pebu_e2        [el]) { rett = g2_pebu_e2        [el]; stoppe=1; continue };
   if(g2_pebu_e3        [el]) { rett = g2_pebu_e3        [el]; stoppe=1; continue };
   if(g2_pebu_m1        [el]) { rett = g2_pebu_m1        [el]; stoppe=1; continue };
   if(g2_pebu_m2        [el]) { rett = g2_pebu_m2        [el]; stoppe=1; continue };
   if(g2_pebu_m3        [el]) { rett = g2_pebu_m3        [el]; stoppe=1; continue };
   if(g2_pein           [el]) { rett = g2_pein           [el]; stoppe=1; continue };
   if(g2_pena_e1        [el]) { rett = g2_pena_e1        [el]; stoppe=1; continue };
   if(g2_pena_e2        [el]) { rett = g2_pena_e2        [el]; stoppe=1; continue };
   if(g2_pena_e3        [el]) { rett = g2_pena_e3        [el]; stoppe=1; continue };
   if(g2_pena_m1        [el]) { rett = g2_pena_m1        [el]; stoppe=1; continue };
   if(g2_pena_m2        [el]) { rett = g2_pena_m2        [el]; stoppe=1; continue };
   if(g2_pena_m3        [el]) { rett = g2_pena_m3        [el]; stoppe=1; continue };
   if(g2_pepa_edmu      [el]) { rett = g2_pepa_edmu      [el]; stoppe=1; continue };
   if(g2_pepa_edsr      [el]) { rett = g2_pepa_edsr      [el]; stoppe=1; continue };
   if(g2_pepa_edze      [el]) { rett = g2_pepa_edze      [el]; stoppe=1; continue };
   if(g2_pepa_mn        [el]) { rett = g2_pepa_mn        [el]; stoppe=1; continue };
   if(g2_pepo_ed        [el]) { rett = g2_pepo_ed        [el]; stoppe=1; continue };
   if(g2_pepo_mn        [el]) { rett = g2_pepo_mn        [el]; stoppe=1; continue };
   if(g2_pnbu_e1        [el]) { rett = g2_pnbu_e1        [el]; stoppe=1; continue };
   if(g2_pnbu_e2        [el]) { rett = g2_pnbu_e2        [el]; stoppe=1; continue };
   if(g2_pnbu_e3        [el]) { rett = g2_pnbu_e3        [el]; stoppe=1; continue };
   if(g2_pnbu_m1        [el]) { rett = g2_pnbu_m1        [el]; stoppe=1; continue };
   if(g2_pnbu_m2        [el]) { rett = g2_pnbu_m2        [el]; stoppe=1; continue };
   if(g2_pnbu_m3        [el]) { rett = g2_pnbu_m3        [el]; stoppe=1; continue };
   if(g2_pnin           [el]) { rett = g2_pnin           [el]; stoppe=1; continue };
   if(g2_pnna_e1        [el]) { rett = g2_pnna_e1        [el]; stoppe=1; continue };
   if(g2_pnna_e2        [el]) { rett = g2_pnna_e2        [el]; stoppe=1; continue };
   if(g2_pnna_e3        [el]) { rett = g2_pnna_e3        [el]; stoppe=1; continue };
   if(g2_pnna_m1        [el]) { rett = g2_pnna_m1        [el]; stoppe=1; continue };
   if(g2_pnna_m2        [el]) { rett = g2_pnna_m2        [el]; stoppe=1; continue };
   if(g2_pnna_m3        [el]) { rett = g2_pnna_m3        [el]; stoppe=1; continue };
   if(g2_pnpa_edmu      [el]) { rett = g2_pnpa_edmu      [el]; stoppe=1; continue };
   if(g2_pnpa_edsr      [el]) { rett = g2_pnpa_edsr      [el]; stoppe=1; continue };
   if(g2_pnpa_edze      [el]) { rett = g2_pnpa_edze      [el]; stoppe=1; continue };
   if(g2_pnpa_mn        [el]) { rett = g2_pnpa_mn        [el]; stoppe=1; continue };
   if(g2_pnpo_ed        [el]) { rett = g2_pnpo_ed        [el]; stoppe=1; continue };
   if(g2_pnpo_mn        [el]) { rett = g2_pnpo_mn        [el]; stoppe=1; continue };
   if(g2_vz_nebu_e1     [el]) { rett = g2_vz_nebu_e1     [el]; stoppe=1; continue };
   if(g2_vz_nebu_e2     [el]) { rett = g2_vz_nebu_e2     [el]; stoppe=1; continue };
   if(g2_vz_nebu_e3     [el]) { rett = g2_vz_nebu_e3     [el]; stoppe=1; continue };
   if(g2_vz_nebu_m1     [el]) { rett = g2_vz_nebu_m1     [el]; stoppe=1; continue };
   if(g2_vz_nebu_m2     [el]) { rett = g2_vz_nebu_m2     [el]; stoppe=1; continue };
   if(g2_vz_nebu_m3     [el]) { rett = g2_vz_nebu_m3     [el]; stoppe=1; continue };
   if(g2_vz_nein        [el]) { rett = g2_vz_nein        [el]; stoppe=1; continue };
   if(g2_vz_nena_e1     [el]) { rett = g2_vz_nena_e1     [el]; stoppe=1; continue };
   if(g2_vz_nena_e2     [el]) { rett = g2_vz_nena_e2     [el]; stoppe=1; continue };
   if(g2_vz_nena_e3     [el]) { rett = g2_vz_nena_e3     [el]; stoppe=1; continue };
   if(g2_vz_nena_m1     [el]) { rett = g2_vz_nena_m1     [el]; stoppe=1; continue };
   if(g2_vz_nena_m2     [el]) { rett = g2_vz_nena_m2     [el]; stoppe=1; continue };
   if(g2_vz_nena_m3     [el]) { rett = g2_vz_nena_m3     [el]; stoppe=1; continue };
   if(g2_vz_nepa_edmu   [el]) { rett = g2_vz_nepa_edmu   [el]; stoppe=1; continue };
   if(g2_vz_nepa_edsr   [el]) { rett = g2_vz_nepa_edsr   [el]; stoppe=1; continue };
   if(g2_vz_nepa_edze   [el]) { rett = g2_vz_nepa_edze   [el]; stoppe=1; continue };
   ...
   if(gn_nein_bz        [el]) { rett = gn_nein_bz        [el]; stoppe=1; continue };
   if(gn_nena_e1        [el]) { rett = gn_nena_e1        [el]; stoppe=1; continue };
   if(gn_nena_e2        [el]) { rett = gn_nena_e2        [el]; stoppe=1; continue };
   if(gn_nena_e3        [el]) { rett = gn_nena_e3        [el]; stoppe=1; continue };
   if(gn_nena_m1        [el]) { rett = gn_nena_m1        [el]; stoppe=1; continue };
   if(gn_nena_m2        [el]) { rett = gn_nena_m2        [el]; stoppe=1; continue };
   if(gn_nena_m3        [el]) { rett = gn_nena_m3        [el]; stoppe=1; continue };
   if(gn_nepa_edmu      [el]) { rett = gn_nepa_edmu      [el]; stoppe=1; continue };
   if(gn_nepa_edsr      [el]) { rett = gn_nepa_edsr      [el]; stoppe=1; continue };
   if(gn_nepa_edze      [el]) { rett = gn_nepa_edze      [el]; stoppe=1; continue };
   if(gn_nepa_mn        [el]) { rett = gn_nepa_mn        [el]; stoppe=1; continue };
   if(gn_nepo_ed        [el]) { rett = gn_nepo_ed        [el]; stoppe=1; continue };
   if(gn_nepo_mn        [el]) { rett = gn_nepo_mn        [el]; stoppe=1; continue };
   if(gn_pein           [el]) { rett = gn_pein           [el]; stoppe=1; continue };
   if(gn_pein_bz        [el]) { rett = gn_pein_bz        [el]; stoppe=1; continue };
   if(gn_pena_e1        [el]) { rett = gn_pena_e1        [el]; stoppe=1; continue };
   if(gn_pena_e2        [el]) { rett = gn_pena_e2        [el]; stoppe=1; continue };
   if(gn_pena_e3        [el]) { rett = gn_pena_e3        [el]; stoppe=1; continue };
   if(gn_pena_m1        [el]) { rett = gn_pena_m1        [el]; stoppe=1; continue };
   if(gn_pena_m2        [el]) { rett = gn_pena_m2        [el]; stoppe=1; continue };
   if(gn_pena_m3        [el]) { rett = gn_pena_m3        [el]; stoppe=1; continue };
   if(gn_pepa_edmu      [el]) { rett = gn_pepa_edmu      [el]; stoppe=1; continue };
   if(gn_pepa_edsr      [el]) { rett = gn_pepa_edsr      [el]; stoppe=1; continue };
   if(gn_pepa_edze      [el]) { rett = gn_pepa_edze      [el]; stoppe=1; continue };
   if(gn_pepa_mn        [el]) { rett = gn_pepa_mn        [el]; stoppe=1; continue };
   if(gn_pepo_ed        [el]) { rett = gn_pepo_ed        [el]; stoppe=1; continue };
   if(gn_pepo_mn        [el]) { rett = gn_pepo_mn        [el]; stoppe=1; continue };
   if(gn_pnin           [el]) { rett = gn_pnin           [el]; stoppe=1; continue };
   if(gn_pnna_e1        [el]) { rett = gn_pnna_e1        [el]; stoppe=1; continue };
   if(gn_pnna_e2        [el]) { rett = gn_pnna_e2        [el]; stoppe=1; continue };
   if(gn_pnna_e3        [el]) { rett = gn_pnna_e3        [el]; stoppe=1; continue };
   if(gn_pnna_m1        [el]) { rett = gn_pnna_m1        [el]; stoppe=1; continue };
   if(gn_pnna_m2        [el]) { rett = gn_pnna_m2        [el]; stoppe=1; continue };
   if(gn_pnna_m3        [el]) { rett = gn_pnna_m3        [el]; stoppe=1; continue };
   if(gn_pnpa_edmu      [el]) { rett = gn_pnpa_edmu      [el]; stoppe=1; continue };
   if(gn_pnpa_edsr      [el]) { rett = gn_pnpa_edsr      [el]; stoppe=1; continue };
   if(gn_pnpa_edze      [el]) { rett = gn_pnpa_edze      [el]; stoppe=1; continue };
   if(gn_pnpa_mn        [el]) { rett = gn_pnpa_mn        [el]; stoppe=1; continue };
   if(gn_pnpo_ed        [el]) { rett = gn_pnpo_ed        [el]; stoppe=1; continue };
   if(gn_pnpo_mn        [el]) { rett = gn_pnpo_mn        [el]; stoppe=1; continue };
   if(gn_vz_ne_in       [el]) { rett = gn_vz_ne_in       [el]; stoppe=1; continue };
   if(gn_vz_nein_bz     [el]) { rett = gn_vz_nein_bz     [el]; stoppe=1; continue };
   if(gn_vz_nena_e1     [el]) { rett = gn_vz_nena_e1     [el]; stoppe=1; continue };
   if(gn_vz_nena_e2     [el]) { rett = gn_vz_nena_e2     [el]; stoppe=1; continue };
   if(gn_vz_nena_e3     [el]) { rett = gn_vz_nena_e3     [el]; stoppe=1; continue };
   if(gn_vz_nena_m1     [el]) { rett = gn_vz_nena_m1     [el]; stoppe=1; continue };
   if(gn_vz_nena_m2     [el]) { rett = gn_vz_nena_m2     [el]; stoppe=1; continue };
   if(gn_vz_nena_m3     [el]) { rett = gn_vz_nena_m3     [el]; stoppe=1; continue };
   if(gn_vz_nepa_edmu   [el]) { rett = gn_vz_nepa_edmu   [el]; stoppe=1; continue };
   if(gn_vz_nepa_edsr   [el]) { rett = gn_vz_nepa_edsr   [el]; stoppe=1; continue };
   if(gn_vz_nepa_edze   [el]) { rett = gn_vz_nepa_edze   [el]; stoppe=1; continue };
   if(gn_vz_nepa_mn     [el]) { rett = gn_vz_nepa_mn     [el]; stoppe=1; continue };
   if(gn_vz_nepo_ed     [el]) { rett = gn_vz_nepo_ed     [el]; stoppe=1; continue };
   if(gn_vz_nepo_mn     [el]) { rett = gn_vz_nepo_mn     [el]; stoppe=1; continue };
   if(gs_nebu_e1        [el]) { rett = gs_nebu_e1        [el]; stoppe=1; continue };
   if(gs_nebu_e2        [el]) { rett = gs_nebu_e2        [el]; stoppe=1; continue };
   if(gs_nebu_e3        [el]) { rett = gs_nebu_e3        [el]; stoppe=1; continue };
   if(gs_nebu_m1        [el]) { rett = gs_nebu_m1        [el]; stoppe=1; continue };
   if(gs_nebu_m2        [el]) { rett = gs_nebu_m2        [el]; stoppe=1; continue };
   if(gs_nebu_m3        [el]) { rett = gs_nebu_m3        [el]; stoppe=1; continue };
   if(gs_nein           [el]) { rett = gs_nein           [el]; stoppe=1; continue };
   if(gs_nein_bz        [el]) { rett = gs_nein_bz        [el]; stoppe=1; continue };
   if(gs_nepa_edmu      [el]) { rett = gs_nepa_edmu      [el]; stoppe=1; continue };
   if(gs_nepa_edsr      [el]) { rett = gs_nepa_edsr      [el]; stoppe=1; continue };
   if(gs_nepa_edze      [el]) { rett = gs_nepa_edze      [el]; stoppe=1; continue };
   if(gs_nepa_mn        [el]) { rett = gs_nepa_mn        [el]; stoppe=1; continue };
   if(gs_nepo_ed        [el]) { rett = gs_nepo_ed        [el]; stoppe=1; continue };
   if(gs_nepo_mn        [el]) { rett = gs_nepo_mn        [el]; stoppe=1; continue };
   if(gs_pebu_e1        [el]) { rett = gs_pebu_e1        [el]; stoppe=1; continue };
   if(gs_pebu_e2        [el]) { rett = gs_pebu_e2        [el]; stoppe=1; continue };
   if(gs_pebu_e3        [el]) { rett = gs_pebu_e3        [el]; stoppe=1; continue };
   if(gs_pebu_m1        [el]) { rett = gs_pebu_m1        [el]; stoppe=1; continue };
   if(gs_pebu_m2        [el]) { rett = gs_pebu_m2        [el]; stoppe=1; continue };
   if(gs_pebu_m3        [el]) { rett = gs_pebu_m3        [el]; stoppe=1; continue };
   if(gs_pein           [el]) { rett = gs_pein           [el]; stoppe=1; continue };
   if(gs_pein_bz        [el]) { rett = gs_pein_bz        [el]; stoppe=1; continue };
   if(gs_pepa_edmu      [el]) { rett = gs_pepa_edmu      [el]; stoppe=1; continue };
   if(gs_pepa_edsr      [el]) { rett = gs_pepa_edsr      [el]; stoppe=1; continue };
   if(gs_pepa_edze      [el]) { rett = gs_pepa_edze      [el]; stoppe=1; continue };
   if(gs_pepa_mn        [el]) { rett = gs_pepa_mn        [el]; stoppe=1; continue };
   if(gs_pepo_ed        [el]) { rett = gs_pepo_ed        [el]; stoppe=1; continue };
   if(gs_pepo_mn        [el]) { rett = gs_pepo_mn        [el]; stoppe=1; continue };
   if(gs_pnbu_e1        [el]) { rett = gs_pnbu_e1        [el]; stoppe=1; continue };
   if(gs_pnbu_e2        [el]) { rett = gs_pnbu_e2        [el]; stoppe=1; continue };
   if(gs_pnbu_e3        [el]) { rett = gs_pnbu_e3        [el]; stoppe=1; continue };
   if(gs_pnbu_m1        [el]) { rett = gs_pnbu_m1        [el]; stoppe=1; continue };
   if(gs_pnbu_m2        [el]) { rett = gs_pnbu_m2        [el]; stoppe=1; continue };
   if(gs_pnbu_m3        [el]) { rett = gs_pnbu_m3        [el]; stoppe=1; continue };
   if(gs_pnin           [el]) { rett = gs_pnin           [el]; stoppe=1; continue };
   if(gs_pnpa_edmu      [el]) { rett = gs_pnpa_edmu      [el]; stoppe=1; continue };
   if(gs_pnpa_edsr      [el]) { rett = gs_pnpa_edsr      [el]; stoppe=1; continue };
   if(gs_pnpa_edze      [el]) { rett = gs_pnpa_edze      [el]; stoppe=1; continue };
   if(gs_pnpa_mn        [el]) { rett = gs_pnpa_mn        [el]; stoppe=1; continue };
   if(gs_pnpo_ed        [el]) { rett = gs_pnpo_ed        [el]; stoppe=1; continue };
   if(gs_pnpo_mn        [el]) { rett = gs_pnpo_mn        [el]; stoppe=1; continue };
   if(gs_vz_nebu_e1     [el]) { rett = gs_vz_nebu_e1     [el]; stoppe=1; continue };
   if(gs_vz_nebu_e2     [el]) { rett = gs_vz_nebu_e2     [el]; stoppe=1; continue };
   if(gs_vz_nebu_e3     [el]) { rett = gs_vz_nebu_e3     [el]; stoppe=1; continue };
   if(gs_vz_nebu_m1     [el]) { rett = gs_vz_nebu_m1     [el]; stoppe=1; continue };
   if(gs_vz_nebu_m2     [el]) { rett = gs_vz_nebu_m2     [el]; stoppe=1; continue };
   if(gs_vz_nebu_m3     [el]) { rett = gs_vz_nebu_m3     [el]; stoppe=1; continue };
   if(gs_vz_nein        [el]) { rett = gs_vz_nein        [el]; stoppe=1; continue };
   if(gs_vz_nein_bz     [el]) { rett = gs_vz_nein_bz     [el]; stoppe=1; continue };
   if(gs_vz_nepa_edmu   [el]) { rett = gs_vz_nepa_edmu   [el]; stoppe=1; continue };
   if(gs_vz_nepa_edsr   [el]) { rett = gs_vz_nepa_edsr   [el]; stoppe=1; continue };
   if(gs_vz_nepa_edze   [el]) { rett = gs_vz_nepa_edze   [el]; stoppe=1; continue };
   if(gs_vz_nepa_mn     [el]) { rett = gs_vz_nepa_mn     [el]; stoppe=1; continue };
   if(gs_vz_nepo_ed     [el]) { rett = gs_vz_nepo_ed     [el]; stoppe=1; continue };
   if(gs_vz_nepo_mn     [el]) { rett = gs_vz_nepo_mn     [el]; stoppe=1; continue };
   if(md_bz             [el]) { rett = md_bz             [el]; stoppe=1; continue };
   if(md_ed             [el]) { rett = md_ed             [el]; stoppe=1; continue };
   if(md_mn             [el]) { rett = md_mn             [el]; stoppe=1; continue };
   if(mst_da            [el]) { rett = mst_da            [el]; stoppe=1; continue };
   if(mst_ed_da         [el]) { rett = mst_ed_da         [el]; stoppe=1; continue };
   if(mst_ed_im         [el]) { rett = mst_ed_im         [el]; stoppe=1; continue };
   if(mst_ed_mu         [el]) { rett = mst_ed_mu         [el]; stoppe=1; continue };
   if(mst_ed_mu_da      [el]) { rett = mst_ed_mu_da      [el]; stoppe=1; continue };
   if(mst_ed_mu_im      [el]) { rett = mst_ed_mu_im      [el]; stoppe=1; continue };
   if(mst_ed_mu_pr      [el]) { rett = mst_ed_mu_pr      [el]; stoppe=1; continue };
   if(mst_ed_mu_ro      [el]) { rett = mst_ed_mu_ro      [el]; stoppe=1; continue };
   if(mst_ed_mu_tv      [el]) { rett = mst_ed_mu_tv      [el]; stoppe=1; continue };
   if(mst_ed_mu_vi      [el]) { rett = mst_ed_mu_vi      [el]; stoppe=1; continue };
   if(mst_ed_pr         [el]) { rett = mst_ed_pr         [el]; stoppe=1; continue };
   if(mst_ed_ro         [el]) { rett = mst_ed_ro         [el]; stoppe=1; continue };
   if(mst_ed_sr         [el]) { rett = mst_ed_sr         [el]; stoppe=1; continue };
   if(mst_ed_sr_da      [el]) { rett = mst_ed_sr_da      [el]; stoppe=1; continue };
   if(mst_ed_sr_im      [el]) { rett = mst_ed_sr_im      [el]; stoppe=1; continue };
   if(mst_ed_sr_pr      [el]) { rett = mst_ed_sr_pr      [el]; stoppe=1; continue };
   if(mst_ed_sr_ro      [el]) { rett = mst_ed_sr_ro      [el]; stoppe=1; continue };
   if(mst_ed_sr_tv      [el]) { rett = mst_ed_sr_tv      [el]; stoppe=1; continue };
   if(mst_ed_sr_vi      [el]) { rett = mst_ed_sr_vi      [el]; stoppe=1; continue };
   if(mst_ed_tv         [el]) { rett = mst_ed_tv         [el]; stoppe=1; continue };
   if(mst_ed_vi         [el]) { rett = mst_ed_vi         [el]; stoppe=1; continue };
   if(mst_ed_ze         [el]) { rett = mst_ed_ze         [el]; stoppe=1; continue };
   if(mst_ed_ze_da      [el]) { rett = mst_ed_ze_da      [el]; stoppe=1; continue };
   if(mst_ed_ze_im      [el]) { rett = mst_ed_ze_im      [el]; stoppe=1; continue };
   if(mst_ed_ze_pr      [el]) { rett = mst_ed_ze_pr      [el]; stoppe=1; continue };
   if(mst_ed_ze_ro      [el]) { rett = mst_ed_ze_ro      [el]; stoppe=1; continue };
   if(mst_ed_ze_tv      [el]) { rett = mst_ed_ze_tv      [el]; stoppe=1; continue };
   if(mst_ed_ze_vi      [el]) { rett = mst_ed_ze_vi      [el]; stoppe=1; continue };
   if(mst_im            [el]) { rett = mst_im            [el]; stoppe=1; continue };
   if(mst_mn            [el]) { rett = mst_mn            [el]; stoppe=1; continue };
   if(mst_mn_da         [el]) { rett = mst_mn_da         [el]; stoppe=1; continue };
   if(mst_mn_im         [el]) { rett = mst_mn_im         [el]; stoppe=1; continue };
   if(mst_mn_pr         [el]) { rett = mst_mn_pr         [el]; stoppe=1; continue };
   if(mst_mn_ro         [el]) { rett = mst_mn_ro         [el]; stoppe=1; continue };
   if(mst_mn_tv         [el]) { rett = mst_mn_tv         [el]; stoppe=1; continue };
   if(mst_mn_vi         [el]) { rett = mst_mn_vi         [el]; stoppe=1; continue };
   if(mst_pr            [el]) { rett = mst_pr            [el]; stoppe=1; continue };
   if(mst_ro            [el]) { rett = mst_ro            [el]; stoppe=1; continue };
   if(mst_suw           [el]) { rett = mst_suw           [el]; stoppe=1; continue };
   if(mst_tv            [el]) { rett = mst_tv            [el]; stoppe=1; continue };
   if(mst_vi            [el]) { rett = mst_vi            [el]; stoppe=1; continue };
   if(muc_edmuda        [el]) { rett = muc_edmuda        [el]; stoppe=1; continue };
   if(muc_edmuim        [el]) { rett = muc_edmuim        [el]; stoppe=1; continue };
   if(muc_edmupr        [el]) { rett = muc_edmupr        [el]; stoppe=1; continue };
   if(muc_edmuro        [el]) { rett = muc_edmuro        [el]; stoppe=1; continue };
   if(muc_edmutv        [el]) { rett = muc_edmutv        [el]; stoppe=1; continue };
   if(muc_edmuvi        [el]) { rett = muc_edmuvi        [el]; stoppe=1; continue };
   if(muc_edsrda        [el]) { rett = muc_edsrda        [el]; stoppe=1; continue };
   if(muc_edsrim        [el]) { rett = muc_edsrim        [el]; stoppe=1; continue };
   if(muc_edsrpr        [el]) { rett = muc_edsrpr        [el]; stoppe=1; continue };
   if(muc_edsrro        [el]) { rett = muc_edsrro        [el]; stoppe=1; continue };
   if(muc_edsrtv        [el]) { rett = muc_edsrtv        [el]; stoppe=1; continue };
   if(muc_edsrvi        [el]) { rett = muc_edsrvi        [el]; stoppe=1; continue };
   if(muc_edzeda        [el]) { rett = muc_edzeda        [el]; stoppe=1; continue };
   if(muc_edzeim        [el]) { rett = muc_edzeim        [el]; stoppe=1; continue };
   if(muc_edzepr        [el]) { rett = muc_edzepr        [el]; stoppe=1; continue };
   if(muc_edzero        [el]) { rett = muc_edzero        [el]; stoppe=1; continue };
   if(muc_edzetv        [el]) { rett = muc_edzetv        [el]; stoppe=1; continue };
   if(muc_edzevi        [el]) { rett = muc_edzevi        [el]; stoppe=1; continue };
   if(muc_mnda          [el]) { rett = muc_mnda          [el]; stoppe=1; continue };
   if(muc_mnim          [el]) { rett = muc_mnim          [el]; stoppe=1; continue };
   if(muc_mnpr          [el]) { rett = muc_mnpr          [el]; stoppe=1; continue };
   if(muc_mnro          [el]) { rett = muc_mnro          [el]; stoppe=1; continue };
   if(muc_mntv          [el]) { rett = muc_mntv          [el]; stoppe=1; continue };
   if(muc_mnvi          [el]) { rett = muc_mnvi          [el]; stoppe=1; continue };
   if(mzd               [el]) { rett = mzd               [el]; stoppe=1; continue };
   if(nr_mest           [el]) { rett = nr_mest           [el]; stoppe=1; continue };
   if(nr_nar            [el]) { rett = nr_nar            [el]; stoppe=1; continue };
   if(nr_obce           [el]) { rett = nr_obce           [el]; stoppe=1; continue };
   if(nr_obme           [el]) { rett = nr_obme           [el]; stoppe=1; continue };
   if(nr_obna           [el]) { rett = nr_obna           [el]; stoppe=1; continue };
   if(nr_obpr           [el]) { rett = nr_obpr           [el]; stoppe=1; continue };
   if(nr_obvr           [el]) { rett = nr_obvr           [el]; stoppe=1; continue };
   if(nr_opka           [el]) { rett = nr_opka           [el]; stoppe=1; continue };
   if(nr_opsp           [el]) { rett = nr_opsp           [el]; stoppe=1; continue };
   if(nr_opst           [el]) { rett = nr_opst           [el]; stoppe=1; continue };
   if(nr_pv             [el]) { rett = nr_pv             [el]; stoppe=1; continue };
   if(nr_srv            [el]) { rett = nr_srv            [el]; stoppe=1; continue };
   if(nr_vopr           [el]) { rett = nr_vopr           [el]; stoppe=1; continue };
   if(nrc_mest          [el]) { rett = nrc_mest          [el]; stoppe=1; continue };
   if(nrc_nar           [el]) { rett = nrc_nar           [el]; stoppe=1; continue };
   if(nrc_obce          [el]) { rett = nrc_obce          [el]; stoppe=1; continue };
   if(nrc_obme          [el]) { rett = nrc_obme          [el]; stoppe=1; continue };
   if(nrc_obna          [el]) { rett = nrc_obna          [el]; stoppe=1; continue };
   if(nrc_obpr          [el]) { rett = nrc_obpr          [el]; stoppe=1; continue };
   if(nrc_obvr          [el]) { rett = nrc_obvr          [el]; stoppe=1; continue };
   if(nrc_opka          [el]) { rett = nrc_opka          [el]; stoppe=1; continue };
   if(nrc_opsp          [el]) { rett = nrc_opsp          [el]; stoppe=1; continue };
   if(nrc_opst          [el]) { rett = nrc_opst          [el]; stoppe=1; continue };
   if(nrc_pv            [el]) { rett = nrc_pv            [el]; stoppe=1; continue };
   if(nrc_srv           [el]) { rett = nrc_srv           [el]; stoppe=1; continue };
   if(nrc_vopr          [el]) { rett = nrc_vopr          [el]; stoppe=1; continue };
   if(pl_edmu_da        [el]) { rett = pl_edmu_da        [el]; stoppe=1; continue };
   if(pl_edmu_im        [el]) { rett = pl_edmu_im        [el]; stoppe=1; continue };
   if(pl_edmu_pr        [el]) { rett = pl_edmu_pr        [el]; stoppe=1; continue };
   if(pl_edmu_ro        [el]) { rett = pl_edmu_ro        [el]; stoppe=1; continue };
   if(pl_edmu_tv        [el]) { rett = pl_edmu_tv        [el]; stoppe=1; continue };
   if(pl_edsr_da        [el]) { rett = pl_edsr_da        [el]; stoppe=1; continue };
   if(pl_edsr_im        [el]) { rett = pl_edsr_im        [el]; stoppe=1; continue };
   if(pl_edsr_pr        [el]) { rett = pl_edsr_pr        [el]; stoppe=1; continue };
   if(pl_edsr_ro        [el]) { rett = pl_edsr_ro        [el]; stoppe=1; continue };
   if(pl_edsr_tv        [el]) { rett = pl_edsr_tv        [el]; stoppe=1; continue };
   if(pl_edze_dr        [el]) { rett = pl_edze_dr        [el]; stoppe=1; continue };
   if(pl_edze_im        [el]) { rett = pl_edze_im        [el]; stoppe=1; continue };
   if(pl_edze_tv        [el]) { rett = pl_edze_tv        [el]; stoppe=1; continue };
   if(pl_edze_vi        [el]) { rett = pl_edze_vi        [el]; stoppe=1; continue };
   if(pl_kred_mu        [el]) { rett = pl_kred_mu        [el]; stoppe=1; continue };
   if(pl_kred_sr        [el]) { rett = pl_kred_sr        [el]; stoppe=1; continue };
   if(pl_kred_ze        [el]) { rett = pl_kred_ze        [el]; stoppe=1; continue };
   if(pl_krmn           [el]) { rett = pl_krmn           [el]; stoppe=1; continue };
   if(pl_mn_da          [el]) { rett = pl_mn_da          [el]; stoppe=1; continue };
   if(pl_mn_im          [el]) { rett = pl_mn_im          [el]; stoppe=1; continue };
   if(pl_mn_ro          [el]) { rett = pl_mn_ro          [el]; stoppe=1; continue };
   if(pl_mn_tv          [el]) { rett = pl_mn_tv          [el]; stoppe=1; continue };
   if(pl_neiz           [el]) { rett = pl_neiz           [el]; stoppe=1; continue };
   if(pl_pv_edmu_da     [el]) { rett = pl_pv_edmu_da     [el]; stoppe=1; continue };
   if(pl_pv_edmu_im     [el]) { rett = pl_pv_edmu_im     [el]; stoppe=1; continue };
   if(pl_pv_edmu_pr     [el]) { rett = pl_pv_edmu_pr     [el]; stoppe=1; continue };
   if(pl_pv_edmu_ro     [el]) { rett = pl_pv_edmu_ro     [el]; stoppe=1; continue };
   if(pl_pv_edmu_tv     [el]) { rett = pl_pv_edmu_tv     [el]; stoppe=1; continue };
   if(pl_pv_edsr_da     [el]) { rett = pl_pv_edsr_da     [el]; stoppe=1; continue };
   if(pl_pv_edsr_im     [el]) { rett = pl_pv_edsr_im     [el]; stoppe=1; continue };
   if(pl_pv_edsr_pr     [el]) { rett = pl_pv_edsr_pr     [el]; stoppe=1; continue };
   if(pl_pv_edsr_ro     [el]) { rett = pl_pv_edsr_ro     [el]; stoppe=1; continue };
   if(pl_pv_edsr_tv     [el]) { rett = pl_pv_edsr_tv     [el]; stoppe=1; continue };
   if(pl_pv_edze_dr     [el]) { rett = pl_pv_edze_dr     [el]; stoppe=1; continue };
   if(pl_pv_edze_im     [el]) { rett = pl_pv_edze_im     [el]; stoppe=1; continue };
   if(pl_pv_edze_tv     [el]) { rett = pl_pv_edze_tv     [el]; stoppe=1; continue };
   if(pl_pv_edze_vi     [el]) { rett = pl_pv_edze_vi     [el]; stoppe=1; continue };
   if(pl_pv_mn_da       [el]) { rett = pl_pv_mn_da       [el]; stoppe=1; continue };
   if(pl_pv_mn_im       [el]) { rett = pl_pv_mn_im       [el]; stoppe=1; continue };
   if(pl_pv_mn_pr       [el]) { rett = pl_pv_mn_pr       [el]; stoppe=1; continue };
   if(pl_pv_mn_ro       [el]) { rett = pl_pv_mn_ro       [el]; stoppe=1; continue };
   if(pl_pv_mn_tv       [el]) { rett = pl_pv_mn_tv       [el]; stoppe=1; continue };
   if(pl_srv            [el]) { rett = pl_srv            [el]; stoppe=1; continue };
   if(pq_edmu_da        [el]) { rett = pq_edmu_da        [el]; stoppe=1; continue };
   if(pq_edmu_im        [el]) { rett = pq_edmu_im        [el]; stoppe=1; continue };
   if(pq_edmu_pr        [el]) { rett = pq_edmu_pr        [el]; stoppe=1; continue };
   if(pq_edmu_ro        [el]) { rett = pq_edmu_ro        [el]; stoppe=1; continue };
   if(pq_edmu_tv        [el]) { rett = pq_edmu_tv        [el]; stoppe=1; continue };
   if(pq_edsr_da        [el]) { rett = pq_edsr_da        [el]; stoppe=1; continue };
   if(pq_edsr_im        [el]) { rett = pq_edsr_im        [el]; stoppe=1; continue };
   if(pq_edsr_pr        [el]) { rett = pq_edsr_pr        [el]; stoppe=1; continue };
   if(pq_edsr_ro        [el]) { rett = pq_edsr_ro        [el]; stoppe=1; continue };
   if(pq_edsr_tv        [el]) { rett = pq_edsr_tv        [el]; stoppe=1; continue };
   if(pq_edze_dr        [el]) { rett = pq_edze_dr        [el]; stoppe=1; continue };
   if(pq_edze_im        [el]) { rett = pq_edze_im        [el]; stoppe=1; continue };
   if(pq_edze_vi        [el]) { rett = pq_edze_vi        [el]; stoppe=1; continue };
   if(pq_kred_mu        [el]) { rett = pq_kred_mu        [el]; stoppe=1; continue };
   if(pq_kred_sr        [el]) { rett = pq_kred_sr        [el]; stoppe=1; continue };
   if(pq_kred_ze        [el]) { rett = pq_kred_ze        [el]; stoppe=1; continue };
   if(pq_krmn           [el]) { rett = pq_krmn           [el]; stoppe=1; continue };
   if(pq_mn_da          [el]) { rett = pq_mn_da          [el]; stoppe=1; continue };
   if(pq_mn_im          [el]) { rett = pq_mn_im          [el]; stoppe=1; continue };
   if(pq_mn_ro          [el]) { rett = pq_mn_ro          [el]; stoppe=1; continue };
   if(pq_mn_tv          [el]) { rett = pq_mn_tv          [el]; stoppe=1; continue };
   if(pred_da           [el]) { rett = pred_da           [el]; stoppe=1; continue };
   if(pred_im           [el]) { rett = pred_im           [el]; stoppe=1; continue };
   if(pred_pr           [el]) { rett = pred_pr           [el]; stoppe=1; continue };
   if(pred_ro           [el]) { rett = pred_ro           [el]; stoppe=1; continue };
   if(pred_tv           [el]) { rett = pred_tv           [el]; stoppe=1; continue };
   if(pred_vi           [el]) { rett = pred_vi           [el]; stoppe=1; continue };
   if(predk             [el]) { rett = predk             [el]; stoppe=1; continue };
   if(qall              [el]) { rett = qall              [el]; stoppe=1; continue };
   if(qko_da            [el]) { rett = qko_da            [el]; stoppe=1; continue };
   if(qko_ed_mu_da      [el]) { rett = qko_ed_mu_da      [el]; stoppe=1; continue };
   if(qko_ed_mu_im      [el]) { rett = qko_ed_mu_im      [el]; stoppe=1; continue };
   if(qko_ed_mu_pr      [el]) { rett = qko_ed_mu_pr      [el]; stoppe=1; continue };
   if(qko_ed_mu_ro      [el]) { rett = qko_ed_mu_ro      [el]; stoppe=1; continue };
   if(qko_ed_mu_tv      [el]) { rett = qko_ed_mu_tv      [el]; stoppe=1; continue };
   if(qko_ed_mu_vi      [el]) { rett = qko_ed_mu_vi      [el]; stoppe=1; continue };
   if(qko_ed_sr_da      [el]) { rett = qko_ed_sr_da      [el]; stoppe=1; continue };
   if(qko_ed_sr_im      [el]) { rett = qko_ed_sr_im      [el]; stoppe=1; continue };
   if(qko_ed_sr_pr      [el]) { rett = qko_ed_sr_pr      [el]; stoppe=1; continue };
   if(qko_ed_sr_ro      [el]) { rett = qko_ed_sr_ro      [el]; stoppe=1; continue };
   if(qko_ed_sr_tv      [el]) { rett = qko_ed_sr_tv      [el]; stoppe=1; continue };
   if(qko_ed_sr_vi      [el]) { rett = qko_ed_sr_vi      [el]; stoppe=1; continue };
   if(qko_ed_ze_da      [el]) { rett = qko_ed_ze_da      [el]; stoppe=1; continue };
   if(qko_ed_ze_im      [el]) { rett = qko_ed_ze_im      [el]; stoppe=1; continue };
   if(qko_ed_ze_pr      [el]) { rett = qko_ed_ze_pr      [el]; stoppe=1; continue };
   if(qko_ed_ze_ro      [el]) { rett = qko_ed_ze_ro      [el]; stoppe=1; continue };
   if(qko_ed_ze_tv      [el]) { rett = qko_ed_ze_tv      [el]; stoppe=1; continue };
   if(qko_ed_ze_vi      [el]) { rett = qko_ed_ze_vi      [el]; stoppe=1; continue };
   if(qko_im            [el]) { rett = qko_im            [el]; stoppe=1; continue };
   if(qko_mn_da         [el]) { rett = qko_mn_da         [el]; stoppe=1; continue };
   if(qko_mn_im         [el]) { rett = qko_mn_im         [el]; stoppe=1; continue };
   if(qko_mn_pr         [el]) { rett = qko_mn_pr         [el]; stoppe=1; continue };
   if(qko_mn_ro         [el]) { rett = qko_mn_ro         [el]; stoppe=1; continue };
   if(qko_mn_tv         [el]) { rett = qko_mn_tv         [el]; stoppe=1; continue };
   if(qko_mn_vi         [el]) { rett = qko_mn_vi         [el]; stoppe=1; continue };
   if(qko_pr            [el]) { rett = qko_pr            [el]; stoppe=1; continue };
   if(qko_ro            [el]) { rett = qko_ro            [el]; stoppe=1; continue };
   if(qko_tv            [el]) { rett = qko_tv            [el]; stoppe=1; continue };
   if(qko_vi            [el]) { rett = qko_vi            [el]; stoppe=1; continue };
   if(qko_ze_im         [el]) { rett = qko_ze_im         [el]; stoppe=1; continue };
   if(qko_ze_vi         [el]) { rett = qko_ze_vi         [el]; stoppe=1; continue };
   if(qne_da            [el]) { rett = qne_da            [el]; stoppe=1; continue };
   if(qne_im            [el]) { rett = qne_im            [el]; stoppe=1; continue };
   if(qne_pr            [el]) { rett = qne_pr            [el]; stoppe=1; continue };
   if(qne_ro            [el]) { rett = qne_ro            [el]; stoppe=1; continue };
   if(qne_tv            [el]) { rett = qne_tv            [el]; stoppe=1; continue };
   if(qne_vi            [el]) { rett = qne_vi            [el]; stoppe=1; continue };
   if(qpo_ed_mu_da      [el]) { rett = qpo_ed_mu_da      [el]; stoppe=1; continue };
   if(qpo_ed_mu_im      [el]) { rett = qpo_ed_mu_im      [el]; stoppe=1; continue };
   if(qpo_ed_mu_pr      [el]) { rett = qpo_ed_mu_pr      [el]; stoppe=1; continue };
   if(qpo_ed_mu_ro      [el]) { rett = qpo_ed_mu_ro      [el]; stoppe=1; continue };
   if(qpo_ed_mu_tv      [el]) { rett = qpo_ed_mu_tv      [el]; stoppe=1; continue };
   if(qpo_ed_sr_da      [el]) { rett = qpo_ed_sr_da      [el]; stoppe=1; continue };
   if(qpo_ed_sr_im      [el]) { rett = qpo_ed_sr_im      [el]; stoppe=1; continue };
   if(qpo_ed_sr_pr      [el]) { rett = qpo_ed_sr_pr      [el]; stoppe=1; continue };
   if(qpo_ed_sr_ro      [el]) { rett = qpo_ed_sr_ro      [el]; stoppe=1; continue };
   if(qpo_ed_sr_tv      [el]) { rett = qpo_ed_sr_tv      [el]; stoppe=1; continue };
   if(qpo_ed_sr_vi      [el]) { rett = qpo_ed_sr_vi      [el]; stoppe=1; continue };
   if(qpo_ed_ze_da      [el]) { rett = qpo_ed_ze_da      [el]; stoppe=1; continue };
   if(qpo_ed_ze_im      [el]) { rett = qpo_ed_ze_im      [el]; stoppe=1; continue };
   if(qpo_ed_ze_pr      [el]) { rett = qpo_ed_ze_pr      [el]; stoppe=1; continue };
   if(qpo_ed_ze_ro      [el]) { rett = qpo_ed_ze_ro      [el]; stoppe=1; continue };
   if(qpo_ed_ze_tv      [el]) { rett = qpo_ed_ze_tv      [el]; stoppe=1; continue };
   if(qpo_ed_ze_vi      [el]) { rett = qpo_ed_ze_vi      [el]; stoppe=1; continue };
   if(qpo_mn_da         [el]) { rett = qpo_mn_da         [el]; stoppe=1; continue };
   if(qpo_mn_im         [el]) { rett = qpo_mn_im         [el]; stoppe=1; continue };
   if(qpo_mn_pr         [el]) { rett = qpo_mn_pr         [el]; stoppe=1; continue };
   if(qpo_mn_ro         [el]) { rett = qpo_mn_ro         [el]; stoppe=1; continue };
   if(qpo_mn_tv         [el]) { rett = qpo_mn_tv         [el]; stoppe=1; continue };
   if(qso_da            [el]) { rett = qso_da            [el]; stoppe=1; continue };
   if(qso_im            [el]) { rett = qso_im            [el]; stoppe=1; continue };
   if(qso_mu_da         [el]) { rett = qso_mu_da         [el]; stoppe=1; continue };
   if(qso_mu_im         [el]) { rett = qso_mu_im         [el]; stoppe=1; continue };
   if(qso_mu_pr         [el]) { rett = qso_mu_pr         [el]; stoppe=1; continue };
   if(qso_mu_ro         [el]) { rett = qso_mu_ro         [el]; stoppe=1; continue };
   if(qso_mu_tv         [el]) { rett = qso_mu_tv         [el]; stoppe=1; continue };
   if(qso_mu_vi         [el]) { rett = qso_mu_vi         [el]; stoppe=1; continue };
   if(qso_pr            [el]) { rett = qso_pr            [el]; stoppe=1; continue };
   if(qso_ro            [el]) { rett = qso_ro            [el]; stoppe=1; continue };
   if(qso_sr_da         [el]) { rett = qso_sr_da         [el]; stoppe=1; continue };
   if(qso_sr_im         [el]) { rett = qso_sr_im         [el]; stoppe=1; continue };
   if(qso_sr_pr         [el]) { rett = qso_sr_pr         [el]; stoppe=1; continue };
   if(qso_sr_ro         [el]) { rett = qso_sr_ro         [el]; stoppe=1; continue };
   if(qso_sr_tv         [el]) { rett = qso_sr_tv         [el]; stoppe=1; continue };
   if(qso_sr_vi         [el]) { rett = qso_sr_vi         [el]; stoppe=1; continue };
   if(qso_tv            [el]) { rett = qso_tv            [el]; stoppe=1; continue };
   if(qso_vi            [el]) { rett = qso_vi            [el]; stoppe=1; continue };
   if(qso_ze_da         [el]) { rett = qso_ze_da         [el]; stoppe=1; continue };
   if(qso_ze_im         [el]) { rett = qso_ze_im         [el]; stoppe=1; continue };
   if(qso_ze_pr         [el]) { rett = qso_ze_pr         [el]; stoppe=1; continue };
   if(qso_ze_ro         [el]) { rett = qso_ze_ro         [el]; stoppe=1; continue };
   if(qso_ze_tv         [el]) { rett = qso_ze_tv         [el]; stoppe=1; continue };
   if(qso_ze_vi         [el]) { rett = qso_ze_vi         [el]; stoppe=1; continue };
   if(qst               [el]) { rett = qst               [el]; stoppe=1; continue };
   if(suyz              [el]) { rett = suyz              [el]; stoppe=1; continue };
   if(sw_edmu_da        [el]) { rett = sw_edmu_da        [el]; stoppe=1; continue };
   if(sw_edmu_im        [el]) { rett = sw_edmu_im        [el]; stoppe=1; continue };
   if(sw_edmu_ne        [el]) { rett = sw_edmu_ne        [el]; stoppe=1; continue };
   if(sw_edmu_pr        [el]) { rett = sw_edmu_pr        [el]; stoppe=1; continue };
   if(sw_edmu_ro        [el]) { rett = sw_edmu_ro        [el]; stoppe=1; continue };
   if(sw_edmu_tv        [el]) { rett = sw_edmu_tv        [el]; stoppe=1; continue };
   if(sw_edmu_vi        [el]) { rett = sw_edmu_vi        [el]; stoppe=1; continue };
   if(sw_edob_da        [el]) { rett = sw_edob_da        [el]; stoppe=1; continue };
   if(sw_edob_im        [el]) { rett = sw_edob_im        [el]; stoppe=1; continue };
   if(sw_edob_ne        [el]) { rett = sw_edob_ne        [el]; stoppe=1; continue };
   if(sw_edob_pr        [el]) { rett = sw_edob_pr        [el]; stoppe=1; continue };
   if(sw_edob_ro        [el]) { rett = sw_edob_ro        [el]; stoppe=1; continue };
   if(sw_edob_tv        [el]) { rett = sw_edob_tv        [el]; stoppe=1; continue };
   if(sw_edob_vi        [el]) { rett = sw_edob_vi        [el]; stoppe=1; continue };
   if(sw_edsr_da        [el]) { rett = sw_edsr_da        [el]; stoppe=1; continue };
   if(sw_edsr_im        [el]) { rett = sw_edsr_im        [el]; stoppe=1; continue };
   if(sw_edsr_pr        [el]) { rett = sw_edsr_pr        [el]; stoppe=1; continue };
   if(sw_edsr_ro        [el]) { rett = sw_edsr_ro        [el]; stoppe=1; continue };
   if(sw_edsr_tv        [el]) { rett = sw_edsr_tv        [el]; stoppe=1; continue };
   if(sw_edsr_vi        [el]) { rett = sw_edsr_vi        [el]; stoppe=1; continue };
   if(sw_edze_da        [el]) { rett = sw_edze_da        [el]; stoppe=1; continue };
   if(sw_edze_im        [el]) { rett = sw_edze_im        [el]; stoppe=1; continue };
   if(sw_edze_pr        [el]) { rett = sw_edze_pr        [el]; stoppe=1; continue };
   if(sw_edze_ro        [el]) { rett = sw_edze_ro        [el]; stoppe=1; continue };
   if(sw_edze_tv        [el]) { rett = sw_edze_tv        [el]; stoppe=1; continue };
   if(sw_edze_vi        [el]) { rett = sw_edze_vi        [el]; stoppe=1; continue };
   if(sw_mn_da          [el]) { rett = sw_mn_da          [el]; stoppe=1; continue };
   if(sw_mn_im          [el]) { rett = sw_mn_im          [el]; stoppe=1; continue };
   if(sw_mn_ne          [el]) { rett = sw_mn_ne          [el]; stoppe=1; continue };
   if(sw_mn_pr          [el]) { rett = sw_mn_pr          [el]; stoppe=1; continue };
   if(sw_mn_ro          [el]) { rett = sw_mn_ro          [el]; stoppe=1; continue };
   if(sw_mn_tv          [el]) { rett = sw_mn_tv          [el]; stoppe=1; continue };
   if(sw_mn_vi          [el]) { rett = sw_mn_vi          [el]; stoppe=1; continue };
   if(swn_edmu_da       [el]) { rett = swn_edmu_da       [el]; stoppe=1; continue };
   if(swn_edmu_im       [el]) { rett = swn_edmu_im       [el]; stoppe=1; continue };
   if(swn_edmu_me       [el]) { rett = swn_edmu_me       [el]; stoppe=1; continue };
   if(swn_edmu_ne       [el]) { rett = swn_edmu_ne       [el]; stoppe=1; continue };
   if(swn_edmu_pr       [el]) { rett = swn_edmu_pr       [el]; stoppe=1; continue };
   if(swn_edmu_pt       [el]) { rett = swn_edmu_pt       [el]; stoppe=1; continue };
   if(swn_edmu_ro       [el]) { rett = swn_edmu_ro       [el]; stoppe=1; continue };
   if(swn_edmu_sq       [el]) { rett = swn_edmu_sq       [el]; stoppe=1; continue };
   if(swn_edmu_tv       [el]) { rett = swn_edmu_tv       [el]; stoppe=1; continue };
   if(swn_edob_da       [el]) { rett = swn_edob_da       [el]; stoppe=1; continue };
   if(swn_edob_im       [el]) { rett = swn_edob_im       [el]; stoppe=1; continue };
   if(swn_edob_ne       [el]) { rett = swn_edob_ne       [el]; stoppe=1; continue };
   if(swn_edob_pr       [el]) { rett = swn_edob_pr       [el]; stoppe=1; continue };
   if(swn_edob_ro       [el]) { rett = swn_edob_ro       [el]; stoppe=1; continue };
   if(swn_edob_tv       [el]) { rett = swn_edob_tv       [el]; stoppe=1; continue };
   if(swn_edob_vi       [el]) { rett = swn_edob_vi       [el]; stoppe=1; continue };
   if(swn_edsr_da       [el]) { rett = swn_edsr_da       [el]; stoppe=1; continue };
   if(swn_edsr_im       [el]) { rett = swn_edsr_im       [el]; stoppe=1; continue };
   if(swn_edsr_ne       [el]) { rett = swn_edsr_ne       [el]; stoppe=1; continue };
   if(swn_edsr_pr       [el]) { rett = swn_edsr_pr       [el]; stoppe=1; continue };
   if(swn_edsr_ro       [el]) { rett = swn_edsr_ro       [el]; stoppe=1; continue };
   if(swn_edsr_tv       [el]) { rett = swn_edsr_tv       [el]; stoppe=1; continue };
   if(swn_edsr_vi       [el]) { rett = swn_edsr_vi       [el]; stoppe=1; continue };
   if(swn_edze_da       [el]) { rett = swn_edze_da       [el]; stoppe=1; continue };
   if(swn_edze_im       [el]) { rett = swn_edze_im       [el]; stoppe=1; continue };
   if(swn_edze_me       [el]) { rett = swn_edze_me       [el]; stoppe=1; continue };
   if(swn_edze_ne       [el]) { rett = swn_edze_ne       [el]; stoppe=1; continue };
   if(swn_edze_pr       [el]) { rett = swn_edze_pr       [el]; stoppe=1; continue };
   if(swn_edze_ro       [el]) { rett = swn_edze_ro       [el]; stoppe=1; continue };
   if(swn_edze_tv       [el]) { rett = swn_edze_tv       [el]; stoppe=1; continue };
   if(swn_edze_vi       [el]) { rett = swn_edze_vi       [el]; stoppe=1; continue };
   if(swn_mn_da         [el]) { rett = swn_mn_da         [el]; stoppe=1; continue };
   if(swn_mn_im         [el]) { rett = swn_mn_im         [el]; stoppe=1; continue };
   if(swn_mn_ne         [el]) { rett = swn_mn_ne         [el]; stoppe=1; continue };
   if(swn_mn_pr         [el]) { rett = swn_mn_pr         [el]; stoppe=1; continue };
   if(swn_mn_ro         [el]) { rett = swn_mn_ro         [el]; stoppe=1; continue };
   if(swn_mn_sq         [el]) { rett = swn_mn_sq         [el]; stoppe=1; continue };
   if(swn_mn_tv         [el]) { rett = swn_mn_tv         [el]; stoppe=1; continue };
   if(swo_edmu_da       [el]) { rett = swo_edmu_da       [el]; stoppe=1; continue };
   if(swo_edmu_im       [el]) { rett = swo_edmu_im       [el]; stoppe=1; continue };
   if(swo_edmu_ne       [el]) { rett = swo_edmu_ne       [el]; stoppe=1; continue };
   if(swo_edmu_pr       [el]) { rett = swo_edmu_pr       [el]; stoppe=1; continue };
   if(swo_edmu_ro       [el]) { rett = swo_edmu_ro       [el]; stoppe=1; continue };
   if(swo_edmu_tv       [el]) { rett = swo_edmu_tv       [el]; stoppe=1; continue };
   if(swo_edmu_zv       [el]) { rett = swo_edmu_zv       [el]; stoppe=1; continue };
   if(swo_edob_da       [el]) { rett = swo_edob_da       [el]; stoppe=1; continue };
   if(swo_edob_im       [el]) { rett = swo_edob_im       [el]; stoppe=1; continue };
   if(swo_edob_ne       [el]) { rett = swo_edob_ne       [el]; stoppe=1; continue };
   if(swo_edob_pr       [el]) { rett = swo_edob_pr       [el]; stoppe=1; continue };
   if(swo_edob_ro       [el]) { rett = swo_edob_ro       [el]; stoppe=1; continue };
   if(swo_edob_tv       [el]) { rett = swo_edob_tv       [el]; stoppe=1; continue };
   if(swo_edob_vi       [el]) { rett = swo_edob_vi       [el]; stoppe=1; continue };
   if(swo_edsr_da       [el]) { rett = swo_edsr_da       [el]; stoppe=1; continue };
   if(swo_edsr_im       [el]) { rett = swo_edsr_im       [el]; stoppe=1; continue };
   if(swo_edsr_pr       [el]) { rett = swo_edsr_pr       [el]; stoppe=1; continue };
   if(swo_edsr_ro       [el]) { rett = swo_edsr_ro       [el]; stoppe=1; continue };
   if(swo_edsr_tv       [el]) { rett = swo_edsr_tv       [el]; stoppe=1; continue };
   if(swo_edsr_vi       [el]) { rett = swo_edsr_vi       [el]; stoppe=1; continue };
   if(swo_edze_da       [el]) { rett = swo_edze_da       [el]; stoppe=1; continue };
   if(swo_edze_im       [el]) { rett = swo_edze_im       [el]; stoppe=1; continue };
   if(swo_edze_ne       [el]) { rett = swo_edze_ne       [el]; stoppe=1; continue };
   if(swo_edze_pr       [el]) { rett = swo_edze_pr       [el]; stoppe=1; continue };
   if(swo_edze_ro       [el]) { rett = swo_edze_ro       [el]; stoppe=1; continue };
   if(swo_edze_tv       [el]) { rett = swo_edze_tv       [el]; stoppe=1; continue };
   if(swo_edze_vi       [el]) { rett = swo_edze_vi       [el]; stoppe=1; continue };
   if(swo_edze_zv       [el]) { rett = swo_edze_zv       [el]; stoppe=1; continue };
   if(swo_mn_da         [el]) { rett = swo_mn_da         [el]; stoppe=1; continue };
   if(swo_mn_im         [el]) { rett = swo_mn_im         [el]; stoppe=1; continue };
   if(swo_mn_ne         [el]) { rett = swo_mn_ne         [el]; stoppe=1; continue };
   if(swo_mn_pr         [el]) { rett = swo_mn_pr         [el]; stoppe=1; continue };
   if(swo_mn_ro         [el]) { rett = swo_mn_ro         [el]; stoppe=1; continue };
   if(swo_mn_sq         [el]) { rett = swo_mn_sq         [el]; stoppe=1; continue };
   if(swo_mn_tv         [el]) { rett = swo_mn_tv         [el]; stoppe=1; continue };
   if(vvodn             [el]) { rett = vvodn             [el]; stoppe=1; continue };
   if(zvt               [el]) { rett = zvt               [el]; stoppe=1; continue };
 break }; return rett }
