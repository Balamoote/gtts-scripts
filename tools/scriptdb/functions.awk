...
function getmark(n,mrk,wl,    k, el, marka, rett)    # нахождение в подстроке #xxx (mrk) метки wl
                { marka= "^" mrk;el="_" wl "_";mk3=wordmark(lc(n)); split(mk3,wrds,"#");for(k in wrds){if(wrds[k]~marka&&wrds[k]~el){rett=1;break}else{rett=0};}; return rett }
...
function preph_da(n,                                                                                                                           rett) {
                     if ( qxs(n,"в","дополнение направлении","к ко") ||
                          qxs(n,"в","замену противность противовес противоположность угоду ущерб") ||
                          qxs(n,"в","память","о об") ||
                          qxs(n,"боком вдобавок вплотную плюс применительно","к ко") ||
                          qxs(n,"вдоль вниз вверх смотря судя","по") ||
...
function preph_tv(n,                                                                                                                           rett) {
                     if ( qxs(n,"в","ногу компании мире связи связи согласии сообществе соответствии сопоставлении сравнении унисон уровень","с со") ||
                          qxs(n,"в","бытность") ||
                          qxs(n,"вкупе","с со") ||
                          qxs(n,"во","главе","с со") ||
...
function wordmark(wrd,    stopp, rett) {while (stopp=="") {
   if(dpn_ne_na         [wrd]) { rett = dpn_ne_na         [wrd]; stopp=1; continue };
   if(dpn_ne_pa         [wrd]) { rett = dpn_ne_pa         [wrd]; stopp=1; continue };
   if(dpn_pe_na         [wrd]) { rett = dpn_pe_na         [wrd]; stopp=1; continue };
   if(dpn_pe_pa         [wrd]) { rett = dpn_pe_pa         [wrd]; stopp=1; continue };
   if(dpn_pn_na         [wrd]) { rett = dpn_pn_na         [wrd]; stopp=1; continue };
   if(dpn_pn_pa         [wrd]) { rett = dpn_pn_pa         [wrd]; stopp=1; continue };
   if(dpn_vz_ne_na      [wrd]) { rett = dpn_vz_ne_na      [wrd]; stopp=1; continue };
   if(dps_ne_pa         [wrd]) { rett = dps_ne_pa         [wrd]; stopp=1; continue };
   if(dps_pe_pa         [wrd]) { rett = dps_pe_pa         [wrd]; stopp=1; continue };
   if(dps_pn_pa         [wrd]) { rett = dps_pn_pa         [wrd]; stopp=1; continue };
   if(dps_vz_ne_pa      [wrd]) { rett = dps_vz_ne_pa      [wrd]; stopp=1; continue };
   if(g2_nebu_e1        [wrd]) { rett = g2_nebu_e1        [wrd]; stopp=1; continue };
   if(g2_nebu_e2        [wrd]) { rett = g2_nebu_e2        [wrd]; stopp=1; continue };
   if(g2_nebu_e3        [wrd]) { rett = g2_nebu_e3        [wrd]; stopp=1; continue };
   if(g2_nebu_m1        [wrd]) { rett = g2_nebu_m1        [wrd]; stopp=1; continue };
   if(g2_nebu_m2        [wrd]) { rett = g2_nebu_m2        [wrd]; stopp=1; continue };
   if(g2_nebu_m3        [wrd]) { rett = g2_nebu_m3        [wrd]; stopp=1; continue };
   if(g2_nein           [wrd]) { rett = g2_nein           [wrd]; stopp=1; continue };
   if(g2_nena_e1        [wrd]) { rett = g2_nena_e1        [wrd]; stopp=1; continue };
   if(g2_nena_e2        [wrd]) { rett = g2_nena_e2        [wrd]; stopp=1; continue };
   if(g2_nena_e3        [wrd]) { rett = g2_nena_e3        [wrd]; stopp=1; continue };
   if(g2_nena_m1        [wrd]) { rett = g2_nena_m1        [wrd]; stopp=1; continue };
   if(g2_nena_m2        [wrd]) { rett = g2_nena_m2        [wrd]; stopp=1; continue };
   if(g2_nena_m3        [wrd]) { rett = g2_nena_m3        [wrd]; stopp=1; continue };
   if(g2_nepa_edmu      [wrd]) { rett = g2_nepa_edmu      [wrd]; stopp=1; continue };
   if(g2_nepa_edsr      [wrd]) { rett = g2_nepa_edsr      [wrd]; stopp=1; continue };
   if(g2_nepa_edze      [wrd]) { rett = g2_nepa_edze      [wrd]; stopp=1; continue };
   if(g2_nepa_mn        [wrd]) { rett = g2_nepa_mn        [wrd]; stopp=1; continue };
   if(g2_nepo_ed        [wrd]) { rett = g2_nepo_ed        [wrd]; stopp=1; continue };
   if(g2_nepo_mn        [wrd]) { rett = g2_nepo_mn        [wrd]; stopp=1; continue };
   if(g2_pebu_e1        [wrd]) { rett = g2_pebu_e1        [wrd]; stopp=1; continue };
   if(g2_pebu_e2        [wrd]) { rett = g2_pebu_e2        [wrd]; stopp=1; continue };
   if(g2_pebu_e3        [wrd]) { rett = g2_pebu_e3        [wrd]; stopp=1; continue };
   if(g2_pebu_m1        [wrd]) { rett = g2_pebu_m1        [wrd]; stopp=1; continue };
   if(g2_pebu_m2        [wrd]) { rett = g2_pebu_m2        [wrd]; stopp=1; continue };
   if(g2_pebu_m3        [wrd]) { rett = g2_pebu_m3        [wrd]; stopp=1; continue };
   if(g2_pein           [wrd]) { rett = g2_pein           [wrd]; stopp=1; continue };
   if(g2_pena_e1        [wrd]) { rett = g2_pena_e1        [wrd]; stopp=1; continue };
   if(g2_pena_e2        [wrd]) { rett = g2_pena_e2        [wrd]; stopp=1; continue };
   if(g2_pena_e3        [wrd]) { rett = g2_pena_e3        [wrd]; stopp=1; continue };
   if(g2_pena_m1        [wrd]) { rett = g2_pena_m1        [wrd]; stopp=1; continue };
   if(g2_pena_m2        [wrd]) { rett = g2_pena_m2        [wrd]; stopp=1; continue };
   if(g2_pena_m3        [wrd]) { rett = g2_pena_m3        [wrd]; stopp=1; continue };
   if(g2_pepa_edmu      [wrd]) { rett = g2_pepa_edmu      [wrd]; stopp=1; continue };
   if(g2_pepa_edsr      [wrd]) { rett = g2_pepa_edsr      [wrd]; stopp=1; continue };
   if(g2_pepa_edze      [wrd]) { rett = g2_pepa_edze      [wrd]; stopp=1; continue };
   if(g2_pepa_mn        [wrd]) { rett = g2_pepa_mn        [wrd]; stopp=1; continue };
   if(g2_pepo_ed        [wrd]) { rett = g2_pepo_ed        [wrd]; stopp=1; continue };
   if(g2_pepo_mn        [wrd]) { rett = g2_pepo_mn        [wrd]; stopp=1; continue };
   if(g2_pnbu_e1        [wrd]) { rett = g2_pnbu_e1        [wrd]; stopp=1; continue };
   if(g2_pnbu_e2        [wrd]) { rett = g2_pnbu_e2        [wrd]; stopp=1; continue };
   if(g2_pnbu_e3        [wrd]) { rett = g2_pnbu_e3        [wrd]; stopp=1; continue };
   if(g2_pnbu_m1        [wrd]) { rett = g2_pnbu_m1        [wrd]; stopp=1; continue };
   if(g2_pnbu_m2        [wrd]) { rett = g2_pnbu_m2        [wrd]; stopp=1; continue };
   if(g2_pnbu_m3        [wrd]) { rett = g2_pnbu_m3        [wrd]; stopp=1; continue };
   if(g2_pnin           [wrd]) { rett = g2_pnin           [wrd]; stopp=1; continue };
   if(g2_pnna_e1        [wrd]) { rett = g2_pnna_e1        [wrd]; stopp=1; continue };
   if(g2_pnna_e2        [wrd]) { rett = g2_pnna_e2        [wrd]; stopp=1; continue };
   if(g2_pnna_e3        [wrd]) { rett = g2_pnna_e3        [wrd]; stopp=1; continue };
   if(g2_pnna_m1        [wrd]) { rett = g2_pnna_m1        [wrd]; stopp=1; continue };
   if(g2_pnna_m2        [wrd]) { rett = g2_pnna_m2        [wrd]; stopp=1; continue };
   if(g2_pnna_m3        [wrd]) { rett = g2_pnna_m3        [wrd]; stopp=1; continue };
   if(g2_pnpa_edmu      [wrd]) { rett = g2_pnpa_edmu      [wrd]; stopp=1; continue };
   if(g2_pnpa_edsr      [wrd]) { rett = g2_pnpa_edsr      [wrd]; stopp=1; continue };
   if(g2_pnpa_edze      [wrd]) { rett = g2_pnpa_edze      [wrd]; stopp=1; continue };
   if(g2_pnpa_mn        [wrd]) { rett = g2_pnpa_mn        [wrd]; stopp=1; continue };
   if(g2_pnpo_ed        [wrd]) { rett = g2_pnpo_ed        [wrd]; stopp=1; continue };
   if(g2_pnpo_mn        [wrd]) { rett = g2_pnpo_mn        [wrd]; stopp=1; continue };
   if(g2_vz_nebu_e1     [wrd]) { rett = g2_vz_nebu_e1     [wrd]; stopp=1; continue };
   if(g2_vz_nebu_e2     [wrd]) { rett = g2_vz_nebu_e2     [wrd]; stopp=1; continue };
   if(g2_vz_nebu_e3     [wrd]) { rett = g2_vz_nebu_e3     [wrd]; stopp=1; continue };
   if(g2_vz_nebu_m1     [wrd]) { rett = g2_vz_nebu_m1     [wrd]; stopp=1; continue };
   if(g2_vz_nebu_m2     [wrd]) { rett = g2_vz_nebu_m2     [wrd]; stopp=1; continue };
   if(g2_vz_nebu_m3     [wrd]) { rett = g2_vz_nebu_m3     [wrd]; stopp=1; continue };
   if(g2_vz_nein        [wrd]) { rett = g2_vz_nein        [wrd]; stopp=1; continue };
   if(g2_vz_nena_e1     [wrd]) { rett = g2_vz_nena_e1     [wrd]; stopp=1; continue };
   if(g2_vz_nena_e2     [wrd]) { rett = g2_vz_nena_e2     [wrd]; stopp=1; continue };
   if(g2_vz_nena_e3     [wrd]) { rett = g2_vz_nena_e3     [wrd]; stopp=1; continue };
   if(g2_vz_nena_m1     [wrd]) { rett = g2_vz_nena_m1     [wrd]; stopp=1; continue };
   if(g2_vz_nena_m2     [wrd]) { rett = g2_vz_nena_m2     [wrd]; stopp=1; continue };
   if(g2_vz_nena_m3     [wrd]) { rett = g2_vz_nena_m3     [wrd]; stopp=1; continue };
   if(g2_vz_nepa_edmu   [wrd]) { rett = g2_vz_nepa_edmu   [wrd]; stopp=1; continue };
   if(g2_vz_nepa_edsr   [wrd]) { rett = g2_vz_nepa_edsr   [wrd]; stopp=1; continue };
   if(g2_vz_nepa_edze   [wrd]) { rett = g2_vz_nepa_edze   [wrd]; stopp=1; continue };
   if(g2_vz_nepa_mn     [wrd]) { rett = g2_vz_nepa_mn     [wrd]; stopp=1; continue };
   if(g2_vz_nepo_ed     [wrd]) { rett = g2_vz_nepo_ed     [wrd]; stopp=1; continue };
   if(g2_vz_nepo_mn     [wrd]) { rett = g2_vz_nepo_mn     [wrd]; stopp=1; continue };
   if(gc2_nebu_m1       [wrd]) { rett = gc2_nebu_m1       [wrd]; stopp=1; continue };
   if(gc2_nebu_m2       [wrd]) { rett = gc2_nebu_m2       [wrd]; stopp=1; continue };
   if(gc2_nebu_m3       [wrd]) { rett = gc2_nebu_m3       [wrd]; stopp=1; continue };
   if(gc2_nena_m1       [wrd]) { rett = gc2_nena_m1       [wrd]; stopp=1; continue };
   if(gc2_nena_m2       [wrd]) { rett = gc2_nena_m2       [wrd]; stopp=1; continue };
   if(gc2_nena_m3       [wrd]) { rett = gc2_nena_m3       [wrd]; stopp=1; continue };
   if(gc2_nepa_mn       [wrd]) { rett = gc2_nepa_mn       [wrd]; stopp=1; continue };
   if(gc2_nepo_mn       [wrd]) { rett = gc2_nepo_mn       [wrd]; stopp=1; continue };
   if(gc2_pebu_m1       [wrd]) { rett = gc2_pebu_m1       [wrd]; stopp=1; continue };
   if(gc2_pebu_m2       [wrd]) { rett = gc2_pebu_m2       [wrd]; stopp=1; continue };
   if(gc2_pebu_m3       [wrd]) { rett = gc2_pebu_m3       [wrd]; stopp=1; continue };
   if(gc2_pena_m1       [wrd]) { rett = gc2_pena_m1       [wrd]; stopp=1; continue };
   if(gc2_pena_m2       [wrd]) { rett = gc2_pena_m2       [wrd]; stopp=1; continue };
   if(gc2_pena_m3       [wrd]) { rett = gc2_pena_m3       [wrd]; stopp=1; continue };
   if(gc2_pepa_mn       [wrd]) { rett = gc2_pepa_mn       [wrd]; stopp=1; continue };
   if(gc2_pepo_mn       [wrd]) { rett = gc2_pepo_mn       [wrd]; stopp=1; continue };
   if(gc2_pnbu_m1       [wrd]) { rett = gc2_pnbu_m1       [wrd]; stopp=1; continue };
   if(gc2_pnbu_m2       [wrd]) { rett = gc2_pnbu_m2       [wrd]; stopp=1; continue };
   if(gc2_pnbu_m3       [wrd]) { rett = gc2_pnbu_m3       [wrd]; stopp=1; continue };
   if(gc2_pnna_m1       [wrd]) { rett = gc2_pnna_m1       [wrd]; stopp=1; continue };
   if(gc2_pnna_m2       [wrd]) { rett = gc2_pnna_m2       [wrd]; stopp=1; continue };
   if(gc2_pnna_m3       [wrd]) { rett = gc2_pnna_m3       [wrd]; stopp=1; continue };
   if(gc2_pnpa_mn       [wrd]) { rett = gc2_pnpa_mn       [wrd]; stopp=1; continue };
   if(gc2_pnpo_mn       [wrd]) { rett = gc2_pnpo_mn       [wrd]; stopp=1; continue };
   if(gc2_vz_nebu_m1    [wrd]) { rett = gc2_vz_nebu_m1    [wrd]; stopp=1; continue };
   if(gc2_vz_nebu_m2    [wrd]) { rett = gc2_vz_nebu_m2    [wrd]; stopp=1; continue };
   if(gc2_vz_nebu_m3    [wrd]) { rett = gc2_vz_nebu_m3    [wrd]; stopp=1; continue };
   if(gc2_vz_nena_m1    [wrd]) { rett = gc2_vz_nena_m1    [wrd]; stopp=1; continue };
   if(gc2_vz_nena_m2    [wrd]) { rett = gc2_vz_nena_m2    [wrd]; stopp=1; continue };
   if(gc2_vz_nena_m3    [wrd]) { rett = gc2_vz_nena_m3    [wrd]; stopp=1; continue };
   if(gc2_vz_nepa_mn    [wrd]) { rett = gc2_vz_nepa_mn    [wrd]; stopp=1; continue };
   if(gc2_vz_nepo_mn    [wrd]) { rett = gc2_vz_nepo_mn    [wrd]; stopp=1; continue };
   if(gcn_nena_m1       [wrd]) { rett = gcn_nena_m1       [wrd]; stopp=1; continue };
   if(gcn_nena_m2       [wrd]) { rett = gcn_nena_m2       [wrd]; stopp=1; continue };
   if(gcn_nena_m3       [wrd]) { rett = gcn_nena_m3       [wrd]; stopp=1; continue };
   if(gcn_nepa_mn       [wrd]) { rett = gcn_nepa_mn       [wrd]; stopp=1; continue };
   if(gcn_nepo_mn       [wrd]) { rett = gcn_nepo_mn       [wrd]; stopp=1; continue };
   if(gcn_pena_m1       [wrd]) { rett = gcn_pena_m1       [wrd]; stopp=1; continue };
   if(gcn_pena_m2       [wrd]) { rett = gcn_pena_m2       [wrd]; stopp=1; continue };
   if(gcn_pena_m3       [wrd]) { rett = gcn_pena_m3       [wrd]; stopp=1; continue };
   if(gcn_pepa_mn       [wrd]) { rett = gcn_pepa_mn       [wrd]; stopp=1; continue };
   if(gcn_pepo_mn       [wrd]) { rett = gcn_pepo_mn       [wrd]; stopp=1; continue };
   if(gcn_pnna_m1       [wrd]) { rett = gcn_pnna_m1       [wrd]; stopp=1; continue };
   if(gcn_pnna_m2       [wrd]) { rett = gcn_pnna_m2       [wrd]; stopp=1; continue };
   if(gcn_pnna_m3       [wrd]) { rett = gcn_pnna_m3       [wrd]; stopp=1; continue };
   if(gcn_pnpa_mn       [wrd]) { rett = gcn_pnpa_mn       [wrd]; stopp=1; continue };
   if(gcn_pnpo_mn       [wrd]) { rett = gcn_pnpo_mn       [wrd]; stopp=1; continue };
   if(gcn_vz_nena_m1    [wrd]) { rett = gcn_vz_nena_m1    [wrd]; stopp=1; continue };
   if(gcn_vz_nena_m2    [wrd]) { rett = gcn_vz_nena_m2    [wrd]; stopp=1; continue };
   if(gcn_vz_nena_m3    [wrd]) { rett = gcn_vz_nena_m3    [wrd]; stopp=1; continue };
   if(gcn_vz_nepa_mn    [wrd]) { rett = gcn_vz_nepa_mn    [wrd]; stopp=1; continue };
   if(gcn_vz_nepo_mn    [wrd]) { rett = gcn_vz_nepo_mn    [wrd]; stopp=1; continue };
   if(gcs_nebu_m1       [wrd]) { rett = gcs_nebu_m1       [wrd]; stopp=1; continue };
   if(gcs_nebu_m2       [wrd]) { rett = gcs_nebu_m2       [wrd]; stopp=1; continue };
   if(gcs_nebu_m3       [wrd]) { rett = gcs_nebu_m3       [wrd]; stopp=1; continue };
   if(gcs_nepa_mn       [wrd]) { rett = gcs_nepa_mn       [wrd]; stopp=1; continue };
   if(gcs_nepo_mn       [wrd]) { rett = gcs_nepo_mn       [wrd]; stopp=1; continue };
   if(gcs_pebu_m1       [wrd]) { rett = gcs_pebu_m1       [wrd]; stopp=1; continue };
   if(gcs_pebu_m2       [wrd]) { rett = gcs_pebu_m2       [wrd]; stopp=1; continue };
   if(gcs_pebu_m3       [wrd]) { rett = gcs_pebu_m3       [wrd]; stopp=1; continue };
   if(gcs_pepa_mn       [wrd]) { rett = gcs_pepa_mn       [wrd]; stopp=1; continue };
   if(gcs_pepo_mn       [wrd]) { rett = gcs_pepo_mn       [wrd]; stopp=1; continue };
   if(gcs_pnbu_m1       [wrd]) { rett = gcs_pnbu_m1       [wrd]; stopp=1; continue };
   if(gcs_pnbu_m2       [wrd]) { rett = gcs_pnbu_m2       [wrd]; stopp=1; continue };
   if(gcs_pnbu_m3       [wrd]) { rett = gcs_pnbu_m3       [wrd]; stopp=1; continue };
   if(gcs_pnpa_mn       [wrd]) { rett = gcs_pnpa_mn       [wrd]; stopp=1; continue };
   if(gcs_pnpo_mn       [wrd]) { rett = gcs_pnpo_mn       [wrd]; stopp=1; continue };
   if(gcs_vz_nebu_m1    [wrd]) { rett = gcs_vz_nebu_m1    [wrd]; stopp=1; continue };
   if(gcs_vz_nebu_m2    [wrd]) { rett = gcs_vz_nebu_m2    [wrd]; stopp=1; continue };
   if(gcs_vz_nebu_m3    [wrd]) { rett = gcs_vz_nebu_m3    [wrd]; stopp=1; continue };
   if(gcs_vz_nepa_mn    [wrd]) { rett = gcs_vz_nepa_mn    [wrd]; stopp=1; continue };
   if(gcs_vz_nepo_mn    [wrd]) { rett = gcs_vz_nepo_mn    [wrd]; stopp=1; continue };
   if(geo_edsrim        [wrd]) { rett = geo_edsrim        [wrd]; stopp=1; continue };
   if(geo_mnim          [wrd]) { rett = geo_mnim          [wrd]; stopp=1; continue };
   if(gn_nein           [wrd]) { rett = gn_nein           [wrd]; stopp=1; continue };
   if(gn_nein_bz        [wrd]) { rett = gn_nein_bz        [wrd]; stopp=1; continue };
   if(gn_nena_e1        [wrd]) { rett = gn_nena_e1        [wrd]; stopp=1; continue };
   if(gn_nena_e2        [wrd]) { rett = gn_nena_e2        [wrd]; stopp=1; continue };
   if(gn_nena_e3        [wrd]) { rett = gn_nena_e3        [wrd]; stopp=1; continue };
   if(gn_nena_m1        [wrd]) { rett = gn_nena_m1        [wrd]; stopp=1; continue };
   if(gn_nena_m2        [wrd]) { rett = gn_nena_m2        [wrd]; stopp=1; continue };
   if(gn_nena_m3        [wrd]) { rett = gn_nena_m3        [wrd]; stopp=1; continue };
   if(gn_nepa_edmu      [wrd]) { rett = gn_nepa_edmu      [wrd]; stopp=1; continue };
   if(gn_nepa_edsr      [wrd]) { rett = gn_nepa_edsr      [wrd]; stopp=1; continue };
   if(gn_nepa_edze      [wrd]) { rett = gn_nepa_edze      [wrd]; stopp=1; continue };
   if(gn_nepa_mn        [wrd]) { rett = gn_nepa_mn        [wrd]; stopp=1; continue };
   if(gn_nepo_ed        [wrd]) { rett = gn_nepo_ed        [wrd]; stopp=1; continue };
   if(gn_nepo_mn        [wrd]) { rett = gn_nepo_mn        [wrd]; stopp=1; continue };
   if(gn_pein           [wrd]) { rett = gn_pein           [wrd]; stopp=1; continue };
   if(gn_pein_bz        [wrd]) { rett = gn_pein_bz        [wrd]; stopp=1; continue };
   if(gn_pena_e1        [wrd]) { rett = gn_pena_e1        [wrd]; stopp=1; continue };
   if(gn_pena_e2        [wrd]) { rett = gn_pena_e2        [wrd]; stopp=1; continue };
   if(gn_pena_e3        [wrd]) { rett = gn_pena_e3        [wrd]; stopp=1; continue };
   if(gn_pena_m1        [wrd]) { rett = gn_pena_m1        [wrd]; stopp=1; continue };
   if(gn_pena_m2        [wrd]) { rett = gn_pena_m2        [wrd]; stopp=1; continue };
   if(gn_pena_m3        [wrd]) { rett = gn_pena_m3        [wrd]; stopp=1; continue };
   if(gn_pepa_edmu      [wrd]) { rett = gn_pepa_edmu      [wrd]; stopp=1; continue };
   if(gn_pepa_edsr      [wrd]) { rett = gn_pepa_edsr      [wrd]; stopp=1; continue };
   if(gn_pepa_edze      [wrd]) { rett = gn_pepa_edze      [wrd]; stopp=1; continue };
   if(gn_pepa_mn        [wrd]) { rett = gn_pepa_mn        [wrd]; stopp=1; continue };
   if(gn_pepo_ed        [wrd]) { rett = gn_pepo_ed        [wrd]; stopp=1; continue };
   if(gn_pepo_mn        [wrd]) { rett = gn_pepo_mn        [wrd]; stopp=1; continue };
   if(gn_pnin           [wrd]) { rett = gn_pnin           [wrd]; stopp=1; continue };
   if(gn_pnna_e1        [wrd]) { rett = gn_pnna_e1        [wrd]; stopp=1; continue };
   if(gn_pnna_e2        [wrd]) { rett = gn_pnna_e2        [wrd]; stopp=1; continue };
   if(gn_pnna_e3        [wrd]) { rett = gn_pnna_e3        [wrd]; stopp=1; continue };
   if(gn_pnna_m1        [wrd]) { rett = gn_pnna_m1        [wrd]; stopp=1; continue };
   if(gn_pnna_m2        [wrd]) { rett = gn_pnna_m2        [wrd]; stopp=1; continue };
   if(gn_pnna_m3        [wrd]) { rett = gn_pnna_m3        [wrd]; stopp=1; continue };
   if(gn_pnpa_edmu      [wrd]) { rett = gn_pnpa_edmu      [wrd]; stopp=1; continue };
   if(gn_pnpa_edsr      [wrd]) { rett = gn_pnpa_edsr      [wrd]; stopp=1; continue };
   if(gn_pnpa_edze      [wrd]) { rett = gn_pnpa_edze      [wrd]; stopp=1; continue };
   if(gn_pnpa_mn        [wrd]) { rett = gn_pnpa_mn        [wrd]; stopp=1; continue };
   if(gn_pnpo_ed        [wrd]) { rett = gn_pnpo_ed        [wrd]; stopp=1; continue };
   if(gn_pnpo_mn        [wrd]) { rett = gn_pnpo_mn        [wrd]; stopp=1; continue };
   if(gn_vz_ne_in       [wrd]) { rett = gn_vz_ne_in       [wrd]; stopp=1; continue };
   if(gn_vz_nein_bz     [wrd]) { rett = gn_vz_nein_bz     [wrd]; stopp=1; continue };
   if(gn_vz_nena_e1     [wrd]) { rett = gn_vz_nena_e1     [wrd]; stopp=1; continue };
   if(gn_vz_nena_e2     [wrd]) { rett = gn_vz_nena_e2     [wrd]; stopp=1; continue };
   if(gn_vz_nena_e3     [wrd]) { rett = gn_vz_nena_e3     [wrd]; stopp=1; continue };
   if(gn_vz_nena_m1     [wrd]) { rett = gn_vz_nena_m1     [wrd]; stopp=1; continue };
   if(gn_vz_nena_m2     [wrd]) { rett = gn_vz_nena_m2     [wrd]; stopp=1; continue };
   if(gn_vz_nena_m3     [wrd]) { rett = gn_vz_nena_m3     [wrd]; stopp=1; continue };
   if(gn_vz_nepa_edmu   [wrd]) { rett = gn_vz_nepa_edmu   [wrd]; stopp=1; continue };
   if(gn_vz_nepa_edsr   [wrd]) { rett = gn_vz_nepa_edsr   [wrd]; stopp=1; continue };
   if(gn_vz_nepa_edze   [wrd]) { rett = gn_vz_nepa_edze   [wrd]; stopp=1; continue };
   if(gn_vz_nepa_mn     [wrd]) { rett = gn_vz_nepa_mn     [wrd]; stopp=1; continue };
   if(gn_vz_nepo_ed     [wrd]) { rett = gn_vz_nepo_ed     [wrd]; stopp=1; continue };
   if(gn_vz_nepo_mn     [wrd]) { rett = gn_vz_nepo_mn     [wrd]; stopp=1; continue };
   if(gs_nebu_e1        [wrd]) { rett = gs_nebu_e1        [wrd]; stopp=1; continue };
   if(gs_nebu_e2        [wrd]) { rett = gs_nebu_e2        [wrd]; stopp=1; continue };
   if(gs_nebu_e3        [wrd]) { rett = gs_nebu_e3        [wrd]; stopp=1; continue };
   if(gs_nebu_m1        [wrd]) { rett = gs_nebu_m1        [wrd]; stopp=1; continue };
   if(gs_nebu_m2        [wrd]) { rett = gs_nebu_m2        [wrd]; stopp=1; continue };
   if(gs_nebu_m3        [wrd]) { rett = gs_nebu_m3        [wrd]; stopp=1; continue };
   if(gs_nein           [wrd]) { rett = gs_nein           [wrd]; stopp=1; continue };
   if(gs_nein_bz        [wrd]) { rett = gs_nein_bz        [wrd]; stopp=1; continue };
   if(gs_nepa_edmu      [wrd]) { rett = gs_nepa_edmu      [wrd]; stopp=1; continue };
   if(gs_nepa_edsr      [wrd]) { rett = gs_nepa_edsr      [wrd]; stopp=1; continue };
   if(gs_nepa_edze      [wrd]) { rett = gs_nepa_edze      [wrd]; stopp=1; continue };
   if(gs_nepa_mn        [wrd]) { rett = gs_nepa_mn        [wrd]; stopp=1; continue };
   if(gs_nepo_ed        [wrd]) { rett = gs_nepo_ed        [wrd]; stopp=1; continue };
   if(gs_nepo_mn        [wrd]) { rett = gs_nepo_mn        [wrd]; stopp=1; continue };
   if(gs_pebu_e1        [wrd]) { rett = gs_pebu_e1        [wrd]; stopp=1; continue };
   if(gs_pebu_e2        [wrd]) { rett = gs_pebu_e2        [wrd]; stopp=1; continue };
   if(gs_pebu_e3        [wrd]) { rett = gs_pebu_e3        [wrd]; stopp=1; continue };
   if(gs_pebu_m1        [wrd]) { rett = gs_pebu_m1        [wrd]; stopp=1; continue };
   if(gs_pebu_m2        [wrd]) { rett = gs_pebu_m2        [wrd]; stopp=1; continue };
   if(gs_pebu_m3        [wrd]) { rett = gs_pebu_m3        [wrd]; stopp=1; continue };
   if(gs_pein           [wrd]) { rett = gs_pein           [wrd]; stopp=1; continue };
   if(gs_pein_bz        [wrd]) { rett = gs_pein_bz        [wrd]; stopp=1; continue };
   if(gs_pepa_edmu      [wrd]) { rett = gs_pepa_edmu      [wrd]; stopp=1; continue };
   if(gs_pepa_edsr      [wrd]) { rett = gs_pepa_edsr      [wrd]; stopp=1; continue };
   if(gs_pepa_edze      [wrd]) { rett = gs_pepa_edze      [wrd]; stopp=1; continue };
   if(gs_pepa_mn        [wrd]) { rett = gs_pepa_mn        [wrd]; stopp=1; continue };
   if(gs_pepo_ed        [wrd]) { rett = gs_pepo_ed        [wrd]; stopp=1; continue };
   if(gs_pepo_mn        [wrd]) { rett = gs_pepo_mn        [wrd]; stopp=1; continue };
   if(gs_pnbu_e1        [wrd]) { rett = gs_pnbu_e1        [wrd]; stopp=1; continue };
   if(gs_pnbu_e2        [wrd]) { rett = gs_pnbu_e2        [wrd]; stopp=1; continue };
   if(gs_pnbu_e3        [wrd]) { rett = gs_pnbu_e3        [wrd]; stopp=1; continue };
   if(gs_pnbu_m1        [wrd]) { rett = gs_pnbu_m1        [wrd]; stopp=1; continue };
   if(gs_pnbu_m2        [wrd]) { rett = gs_pnbu_m2        [wrd]; stopp=1; continue };
   if(gs_pnbu_m3        [wrd]) { rett = gs_pnbu_m3        [wrd]; stopp=1; continue };
   if(gs_pnin           [wrd]) { rett = gs_pnin           [wrd]; stopp=1; continue };
   if(gs_pnpa_edmu      [wrd]) { rett = gs_pnpa_edmu      [wrd]; stopp=1; continue };
   if(gs_pnpa_edsr      [wrd]) { rett = gs_pnpa_edsr      [wrd]; stopp=1; continue };
   if(gs_pnpa_edze      [wrd]) { rett = gs_pnpa_edze      [wrd]; stopp=1; continue };
   if(gs_pnpa_mn        [wrd]) { rett = gs_pnpa_mn        [wrd]; stopp=1; continue };
   if(gs_pnpo_ed        [wrd]) { rett = gs_pnpo_ed        [wrd]; stopp=1; continue };
   if(gs_pnpo_mn        [wrd]) { rett = gs_pnpo_mn        [wrd]; stopp=1; continue };
   if(gs_vz_nebu_e1     [wrd]) { rett = gs_vz_nebu_e1     [wrd]; stopp=1; continue };
   if(gs_vz_nebu_e2     [wrd]) { rett = gs_vz_nebu_e2     [wrd]; stopp=1; continue };
   if(gs_vz_nebu_e3     [wrd]) { rett = gs_vz_nebu_e3     [wrd]; stopp=1; continue };
   if(gs_vz_nebu_m1     [wrd]) { rett = gs_vz_nebu_m1     [wrd]; stopp=1; continue };
   if(gs_vz_nebu_m2     [wrd]) { rett = gs_vz_nebu_m2     [wrd]; stopp=1; continue };
   if(gs_vz_nebu_m3     [wrd]) { rett = gs_vz_nebu_m3     [wrd]; stopp=1; continue };
   if(gs_vz_nein        [wrd]) { rett = gs_vz_nein        [wrd]; stopp=1; continue };
   if(gs_vz_nein_bz     [wrd]) { rett = gs_vz_nein_bz     [wrd]; stopp=1; continue };
   if(gs_vz_nepa_edmu   [wrd]) { rett = gs_vz_nepa_edmu   [wrd]; stopp=1; continue };
   if(gs_vz_nepa_edsr   [wrd]) { rett = gs_vz_nepa_edsr   [wrd]; stopp=1; continue };
   if(gs_vz_nepa_edze   [wrd]) { rett = gs_vz_nepa_edze   [wrd]; stopp=1; continue };
   if(gs_vz_nepa_mn     [wrd]) { rett = gs_vz_nepa_mn     [wrd]; stopp=1; continue };
   if(gs_vz_nepo_ed     [wrd]) { rett = gs_vz_nepo_ed     [wrd]; stopp=1; continue };
   if(gs_vz_nepo_mn     [wrd]) { rett = gs_vz_nepo_mn     [wrd]; stopp=1; continue };
   if(md_bz             [wrd]) { rett = md_bz             [wrd]; stopp=1; continue };
   if(md_ed             [wrd]) { rett = md_ed             [wrd]; stopp=1; continue };
   if(md_mn             [wrd]) { rett = md_mn             [wrd]; stopp=1; continue };
   if(mst_da            [wrd]) { rett = mst_da            [wrd]; stopp=1; continue };
   if(mst_ed_da         [wrd]) { rett = mst_ed_da         [wrd]; stopp=1; continue };
   if(mst_ed_im         [wrd]) { rett = mst_ed_im         [wrd]; stopp=1; continue };
   if(mst_ed_mu         [wrd]) { rett = mst_ed_mu         [wrd]; stopp=1; continue };
   if(mst_ed_mu_da      [wrd]) { rett = mst_ed_mu_da      [wrd]; stopp=1; continue };
   if(mst_ed_mu_im      [wrd]) { rett = mst_ed_mu_im      [wrd]; stopp=1; continue };
   if(mst_ed_mu_pr      [wrd]) { rett = mst_ed_mu_pr      [wrd]; stopp=1; continue };
   if(mst_ed_mu_ro      [wrd]) { rett = mst_ed_mu_ro      [wrd]; stopp=1; continue };
   if(mst_ed_mu_tv      [wrd]) { rett = mst_ed_mu_tv      [wrd]; stopp=1; continue };
   if(mst_ed_mu_vi      [wrd]) { rett = mst_ed_mu_vi      [wrd]; stopp=1; continue };
   if(mst_ed_pr         [wrd]) { rett = mst_ed_pr         [wrd]; stopp=1; continue };
   if(mst_ed_ro         [wrd]) { rett = mst_ed_ro         [wrd]; stopp=1; continue };
   if(mst_ed_sr         [wrd]) { rett = mst_ed_sr         [wrd]; stopp=1; continue };
   if(mst_ed_sr_da      [wrd]) { rett = mst_ed_sr_da      [wrd]; stopp=1; continue };
   if(mst_ed_sr_im      [wrd]) { rett = mst_ed_sr_im      [wrd]; stopp=1; continue };
   if(mst_ed_sr_pr      [wrd]) { rett = mst_ed_sr_pr      [wrd]; stopp=1; continue };
   if(mst_ed_sr_ro      [wrd]) { rett = mst_ed_sr_ro      [wrd]; stopp=1; continue };
   if(mst_ed_sr_tv      [wrd]) { rett = mst_ed_sr_tv      [wrd]; stopp=1; continue };
   if(mst_ed_sr_vi      [wrd]) { rett = mst_ed_sr_vi      [wrd]; stopp=1; continue };
   if(mst_ed_tv         [wrd]) { rett = mst_ed_tv         [wrd]; stopp=1; continue };
   if(mst_ed_vi         [wrd]) { rett = mst_ed_vi         [wrd]; stopp=1; continue };
   if(mst_ed_ze         [wrd]) { rett = mst_ed_ze         [wrd]; stopp=1; continue };
   if(mst_ed_ze_da      [wrd]) { rett = mst_ed_ze_da      [wrd]; stopp=1; continue };
   if(mst_ed_ze_im      [wrd]) { rett = mst_ed_ze_im      [wrd]; stopp=1; continue };
   if(mst_ed_ze_pr      [wrd]) { rett = mst_ed_ze_pr      [wrd]; stopp=1; continue };
   if(mst_ed_ze_ro      [wrd]) { rett = mst_ed_ze_ro      [wrd]; stopp=1; continue };
   if(mst_ed_ze_tv      [wrd]) { rett = mst_ed_ze_tv      [wrd]; stopp=1; continue };
   if(mst_ed_ze_vi      [wrd]) { rett = mst_ed_ze_vi      [wrd]; stopp=1; continue };
   if(mst_im            [wrd]) { rett = mst_im            [wrd]; stopp=1; continue };
   if(mst_mn            [wrd]) { rett = mst_mn            [wrd]; stopp=1; continue };
   if(mst_mn_da         [wrd]) { rett = mst_mn_da         [wrd]; stopp=1; continue };
   if(mst_mn_im         [wrd]) { rett = mst_mn_im         [wrd]; stopp=1; continue };
   if(mst_mn_pr         [wrd]) { rett = mst_mn_pr         [wrd]; stopp=1; continue };
   if(mst_mn_ro         [wrd]) { rett = mst_mn_ro         [wrd]; stopp=1; continue };
   if(mst_mn_tv         [wrd]) { rett = mst_mn_tv         [wrd]; stopp=1; continue };
   if(mst_mn_vi         [wrd]) { rett = mst_mn_vi         [wrd]; stopp=1; continue };
   if(mst_pr            [wrd]) { rett = mst_pr            [wrd]; stopp=1; continue };
   if(mst_ro            [wrd]) { rett = mst_ro            [wrd]; stopp=1; continue };
   if(mst_suw           [wrd]) { rett = mst_suw           [wrd]; stopp=1; continue };
   if(mst_tv            [wrd]) { rett = mst_tv            [wrd]; stopp=1; continue };
   if(mst_vi            [wrd]) { rett = mst_vi            [wrd]; stopp=1; continue };
   if(muc_edmuda        [wrd]) { rett = muc_edmuda        [wrd]; stopp=1; continue };
   if(muc_edmuim        [wrd]) { rett = muc_edmuim        [wrd]; stopp=1; continue };
   if(muc_edmupr        [wrd]) { rett = muc_edmupr        [wrd]; stopp=1; continue };
   if(muc_edmuro        [wrd]) { rett = muc_edmuro        [wrd]; stopp=1; continue };
   if(muc_edmutv        [wrd]) { rett = muc_edmutv        [wrd]; stopp=1; continue };
   if(muc_edmuvi        [wrd]) { rett = muc_edmuvi        [wrd]; stopp=1; continue };
   if(muc_edsrda        [wrd]) { rett = muc_edsrda        [wrd]; stopp=1; continue };
   if(muc_edsrim        [wrd]) { rett = muc_edsrim        [wrd]; stopp=1; continue };
   if(muc_edsrpr        [wrd]) { rett = muc_edsrpr        [wrd]; stopp=1; continue };
   if(muc_edsrro        [wrd]) { rett = muc_edsrro        [wrd]; stopp=1; continue };
   if(muc_edsrtv        [wrd]) { rett = muc_edsrtv        [wrd]; stopp=1; continue };
   if(muc_edsrvi        [wrd]) { rett = muc_edsrvi        [wrd]; stopp=1; continue };
   if(muc_edzeda        [wrd]) { rett = muc_edzeda        [wrd]; stopp=1; continue };
   if(muc_edzeim        [wrd]) { rett = muc_edzeim        [wrd]; stopp=1; continue };
   if(muc_edzepr        [wrd]) { rett = muc_edzepr        [wrd]; stopp=1; continue };
   if(muc_edzero        [wrd]) { rett = muc_edzero        [wrd]; stopp=1; continue };
   if(muc_edzetv        [wrd]) { rett = muc_edzetv        [wrd]; stopp=1; continue };
   if(muc_edzevi        [wrd]) { rett = muc_edzevi        [wrd]; stopp=1; continue };
   if(muc_mnda          [wrd]) { rett = muc_mnda          [wrd]; stopp=1; continue };
   if(muc_mnim          [wrd]) { rett = muc_mnim          [wrd]; stopp=1; continue };
   if(muc_mnpr          [wrd]) { rett = muc_mnpr          [wrd]; stopp=1; continue };
   if(muc_mnro          [wrd]) { rett = muc_mnro          [wrd]; stopp=1; continue };
   if(muc_mntv          [wrd]) { rett = muc_mntv          [wrd]; stopp=1; continue };
   if(muc_mnvi          [wrd]) { rett = muc_mnvi          [wrd]; stopp=1; continue };
   if(mzd               [wrd]) { rett = mzd               [wrd]; stopp=1; continue };
   if(nr_mest           [wrd]) { rett = nr_mest           [wrd]; stopp=1; continue };
   if(nr_nar            [wrd]) { rett = nr_nar            [wrd]; stopp=1; continue };
   if(nr_obce           [wrd]) { rett = nr_obce           [wrd]; stopp=1; continue };
   if(nr_obme           [wrd]) { rett = nr_obme           [wrd]; stopp=1; continue };
   if(nr_obna           [wrd]) { rett = nr_obna           [wrd]; stopp=1; continue };
   if(nr_obpr           [wrd]) { rett = nr_obpr           [wrd]; stopp=1; continue };
   if(nr_obvr           [wrd]) { rett = nr_obvr           [wrd]; stopp=1; continue };
   if(nr_opka           [wrd]) { rett = nr_opka           [wrd]; stopp=1; continue };
   if(nr_opsp           [wrd]) { rett = nr_opsp           [wrd]; stopp=1; continue };
   if(nr_opst           [wrd]) { rett = nr_opst           [wrd]; stopp=1; continue };
   if(nr_pv             [wrd]) { rett = nr_pv             [wrd]; stopp=1; continue };
   if(nr_srv            [wrd]) { rett = nr_srv            [wrd]; stopp=1; continue };
   if(nr_vopr           [wrd]) { rett = nr_vopr           [wrd]; stopp=1; continue };
   if(nrc_mest          [wrd]) { rett = nrc_mest          [wrd]; stopp=1; continue };
   if(nrc_nar           [wrd]) { rett = nrc_nar           [wrd]; stopp=1; continue };
   if(nrc_obce          [wrd]) { rett = nrc_obce          [wrd]; stopp=1; continue };
   if(nrc_obme          [wrd]) { rett = nrc_obme          [wrd]; stopp=1; continue };
   if(nrc_obna          [wrd]) { rett = nrc_obna          [wrd]; stopp=1; continue };
   if(nrc_obpr          [wrd]) { rett = nrc_obpr          [wrd]; stopp=1; continue };
   if(nrc_obvr          [wrd]) { rett = nrc_obvr          [wrd]; stopp=1; continue };
   if(nrc_opka          [wrd]) { rett = nrc_opka          [wrd]; stopp=1; continue };
   if(nrc_opsp          [wrd]) { rett = nrc_opsp          [wrd]; stopp=1; continue };
   if(nrc_opst          [wrd]) { rett = nrc_opst          [wrd]; stopp=1; continue };
   if(nrc_pv            [wrd]) { rett = nrc_pv            [wrd]; stopp=1; continue };
   if(nrc_srv           [wrd]) { rett = nrc_srv           [wrd]; stopp=1; continue };
   if(nrc_vopr          [wrd]) { rett = nrc_vopr          [wrd]; stopp=1; continue };
   if(pl_edmu_da        [wrd]) { rett = pl_edmu_da        [wrd]; stopp=1; continue };
   if(pl_edmu_im        [wrd]) { rett = pl_edmu_im        [wrd]; stopp=1; continue };
   if(pl_edmu_pr        [wrd]) { rett = pl_edmu_pr        [wrd]; stopp=1; continue };
   if(pl_edmu_ro        [wrd]) { rett = pl_edmu_ro        [wrd]; stopp=1; continue };
   if(pl_edmu_tv        [wrd]) { rett = pl_edmu_tv        [wrd]; stopp=1; continue };
   if(pl_edsr_da        [wrd]) { rett = pl_edsr_da        [wrd]; stopp=1; continue };
   if(pl_edsr_im        [wrd]) { rett = pl_edsr_im        [wrd]; stopp=1; continue };
   if(pl_edsr_pr        [wrd]) { rett = pl_edsr_pr        [wrd]; stopp=1; continue };
   if(pl_edsr_ro        [wrd]) { rett = pl_edsr_ro        [wrd]; stopp=1; continue };
   if(pl_edsr_tv        [wrd]) { rett = pl_edsr_tv        [wrd]; stopp=1; continue };
   if(pl_edze_dr        [wrd]) { rett = pl_edze_dr        [wrd]; stopp=1; continue };
   if(pl_edze_im        [wrd]) { rett = pl_edze_im        [wrd]; stopp=1; continue };
   if(pl_edze_tv        [wrd]) { rett = pl_edze_tv        [wrd]; stopp=1; continue };
   if(pl_edze_vi        [wrd]) { rett = pl_edze_vi        [wrd]; stopp=1; continue };
   if(pl_kred_mu        [wrd]) { rett = pl_kred_mu        [wrd]; stopp=1; continue };
   if(pl_kred_sr        [wrd]) { rett = pl_kred_sr        [wrd]; stopp=1; continue };
   if(pl_kred_ze        [wrd]) { rett = pl_kred_ze        [wrd]; stopp=1; continue };
   if(pl_krmn           [wrd]) { rett = pl_krmn           [wrd]; stopp=1; continue };
   if(pl_mn_da          [wrd]) { rett = pl_mn_da          [wrd]; stopp=1; continue };
   if(pl_mn_im          [wrd]) { rett = pl_mn_im          [wrd]; stopp=1; continue };
   if(pl_mn_ro          [wrd]) { rett = pl_mn_ro          [wrd]; stopp=1; continue };
   if(pl_mn_tv          [wrd]) { rett = pl_mn_tv          [wrd]; stopp=1; continue };
   if(pl_neiz           [wrd]) { rett = pl_neiz           [wrd]; stopp=1; continue };
   if(pl_pv_edmu_da     [wrd]) { rett = pl_pv_edmu_da     [wrd]; stopp=1; continue };
   if(pl_pv_edmu_im     [wrd]) { rett = pl_pv_edmu_im     [wrd]; stopp=1; continue };
   if(pl_pv_edmu_pr     [wrd]) { rett = pl_pv_edmu_pr     [wrd]; stopp=1; continue };
   if(pl_pv_edmu_ro     [wrd]) { rett = pl_pv_edmu_ro     [wrd]; stopp=1; continue };
   if(pl_pv_edmu_tv     [wrd]) { rett = pl_pv_edmu_tv     [wrd]; stopp=1; continue };
   if(pl_pv_edsr_da     [wrd]) { rett = pl_pv_edsr_da     [wrd]; stopp=1; continue };
   if(pl_pv_edsr_im     [wrd]) { rett = pl_pv_edsr_im     [wrd]; stopp=1; continue };
   if(pl_pv_edsr_pr     [wrd]) { rett = pl_pv_edsr_pr     [wrd]; stopp=1; continue };
   if(pl_pv_edsr_ro     [wrd]) { rett = pl_pv_edsr_ro     [wrd]; stopp=1; continue };
   if(pl_pv_edsr_tv     [wrd]) { rett = pl_pv_edsr_tv     [wrd]; stopp=1; continue };
   if(pl_pv_edze_dr     [wrd]) { rett = pl_pv_edze_dr     [wrd]; stopp=1; continue };
   if(pl_pv_edze_im     [wrd]) { rett = pl_pv_edze_im     [wrd]; stopp=1; continue };
   if(pl_pv_edze_tv     [wrd]) { rett = pl_pv_edze_tv     [wrd]; stopp=1; continue };
   if(pl_pv_edze_vi     [wrd]) { rett = pl_pv_edze_vi     [wrd]; stopp=1; continue };
   if(pl_pv_mn_da       [wrd]) { rett = pl_pv_mn_da       [wrd]; stopp=1; continue };
   if(pl_pv_mn_im       [wrd]) { rett = pl_pv_mn_im       [wrd]; stopp=1; continue };
   if(pl_pv_mn_pr       [wrd]) { rett = pl_pv_mn_pr       [wrd]; stopp=1; continue };
   if(pl_pv_mn_ro       [wrd]) { rett = pl_pv_mn_ro       [wrd]; stopp=1; continue };
   if(pl_pv_mn_tv       [wrd]) { rett = pl_pv_mn_tv       [wrd]; stopp=1; continue };
   if(pl_srv            [wrd]) { rett = pl_srv            [wrd]; stopp=1; continue };
   if(pq_edmu_da        [wrd]) { rett = pq_edmu_da        [wrd]; stopp=1; continue };
   if(pq_edmu_im        [wrd]) { rett = pq_edmu_im        [wrd]; stopp=1; continue };
   if(pq_edmu_pr        [wrd]) { rett = pq_edmu_pr        [wrd]; stopp=1; continue };
   if(pq_edmu_ro        [wrd]) { rett = pq_edmu_ro        [wrd]; stopp=1; continue };
   if(pq_edmu_tv        [wrd]) { rett = pq_edmu_tv        [wrd]; stopp=1; continue };
   if(pq_edsr_da        [wrd]) { rett = pq_edsr_da        [wrd]; stopp=1; continue };
   if(pq_edsr_im        [wrd]) { rett = pq_edsr_im        [wrd]; stopp=1; continue };
   if(pq_edsr_pr        [wrd]) { rett = pq_edsr_pr        [wrd]; stopp=1; continue };
   if(pq_edsr_ro        [wrd]) { rett = pq_edsr_ro        [wrd]; stopp=1; continue };
   if(pq_edsr_tv        [wrd]) { rett = pq_edsr_tv        [wrd]; stopp=1; continue };
   if(pq_edze_dr        [wrd]) { rett = pq_edze_dr        [wrd]; stopp=1; continue };
   if(pq_edze_im        [wrd]) { rett = pq_edze_im        [wrd]; stopp=1; continue };
   if(pq_edze_vi        [wrd]) { rett = pq_edze_vi        [wrd]; stopp=1; continue };
   if(pq_kred_mu        [wrd]) { rett = pq_kred_mu        [wrd]; stopp=1; continue };
   if(pq_kred_sr        [wrd]) { rett = pq_kred_sr        [wrd]; stopp=1; continue };
   if(pq_kred_ze        [wrd]) { rett = pq_kred_ze        [wrd]; stopp=1; continue };
   if(pq_krmn           [wrd]) { rett = pq_krmn           [wrd]; stopp=1; continue };
   if(pq_mn_da          [wrd]) { rett = pq_mn_da          [wrd]; stopp=1; continue };
   if(pq_mn_im          [wrd]) { rett = pq_mn_im          [wrd]; stopp=1; continue };
   if(pq_mn_ro          [wrd]) { rett = pq_mn_ro          [wrd]; stopp=1; continue };
   if(pq_mn_tv          [wrd]) { rett = pq_mn_tv          [wrd]; stopp=1; continue };
   if(pred_da           [wrd]) { rett = pred_da           [wrd]; stopp=1; continue };
   if(pred_im           [wrd]) { rett = pred_im           [wrd]; stopp=1; continue };
   if(pred_pr           [wrd]) { rett = pred_pr           [wrd]; stopp=1; continue };
   if(pred_ro           [wrd]) { rett = pred_ro           [wrd]; stopp=1; continue };
   if(pred_tv           [wrd]) { rett = pred_tv           [wrd]; stopp=1; continue };
   if(pred_vi           [wrd]) { rett = pred_vi           [wrd]; stopp=1; continue };
   if(predk             [wrd]) { rett = predk             [wrd]; stopp=1; continue };
   if(qall              [wrd]) { rett = qall              [wrd]; stopp=1; continue };
   if(qko_da            [wrd]) { rett = qko_da            [wrd]; stopp=1; continue };
   if(qko_ed_mu_da      [wrd]) { rett = qko_ed_mu_da      [wrd]; stopp=1; continue };
   if(qko_ed_mu_im      [wrd]) { rett = qko_ed_mu_im      [wrd]; stopp=1; continue };
   if(qko_ed_mu_pr      [wrd]) { rett = qko_ed_mu_pr      [wrd]; stopp=1; continue };
   if(qko_ed_mu_ro      [wrd]) { rett = qko_ed_mu_ro      [wrd]; stopp=1; continue };
   if(qko_ed_mu_tv      [wrd]) { rett = qko_ed_mu_tv      [wrd]; stopp=1; continue };
   if(qko_ed_mu_vi      [wrd]) { rett = qko_ed_mu_vi      [wrd]; stopp=1; continue };
   if(qko_ed_sr_da      [wrd]) { rett = qko_ed_sr_da      [wrd]; stopp=1; continue };
   if(qko_ed_sr_im      [wrd]) { rett = qko_ed_sr_im      [wrd]; stopp=1; continue };
   if(qko_ed_sr_pr      [wrd]) { rett = qko_ed_sr_pr      [wrd]; stopp=1; continue };
   if(qko_ed_sr_ro      [wrd]) { rett = qko_ed_sr_ro      [wrd]; stopp=1; continue };
   if(qko_ed_sr_tv      [wrd]) { rett = qko_ed_sr_tv      [wrd]; stopp=1; continue };
   if(qko_ed_sr_vi      [wrd]) { rett = qko_ed_sr_vi      [wrd]; stopp=1; continue };
   if(qko_ed_ze_da      [wrd]) { rett = qko_ed_ze_da      [wrd]; stopp=1; continue };
   if(qko_ed_ze_im      [wrd]) { rett = qko_ed_ze_im      [wrd]; stopp=1; continue };
   if(qko_ed_ze_pr      [wrd]) { rett = qko_ed_ze_pr      [wrd]; stopp=1; continue };
   if(qko_ed_ze_ro      [wrd]) { rett = qko_ed_ze_ro      [wrd]; stopp=1; continue };
   if(qko_ed_ze_tv      [wrd]) { rett = qko_ed_ze_tv      [wrd]; stopp=1; continue };
   if(qko_ed_ze_vi      [wrd]) { rett = qko_ed_ze_vi      [wrd]; stopp=1; continue };
   if(qko_im            [wrd]) { rett = qko_im            [wrd]; stopp=1; continue };
   if(qko_mn_da         [wrd]) { rett = qko_mn_da         [wrd]; stopp=1; continue };
   if(qko_mn_im         [wrd]) { rett = qko_mn_im         [wrd]; stopp=1; continue };
   if(qko_mn_pr         [wrd]) { rett = qko_mn_pr         [wrd]; stopp=1; continue };
   if(qko_mn_ro         [wrd]) { rett = qko_mn_ro         [wrd]; stopp=1; continue };
   if(qko_mn_tv         [wrd]) { rett = qko_mn_tv         [wrd]; stopp=1; continue };
   if(qko_mn_vi         [wrd]) { rett = qko_mn_vi         [wrd]; stopp=1; continue };
   if(qko_pr            [wrd]) { rett = qko_pr            [wrd]; stopp=1; continue };
   if(qko_ro            [wrd]) { rett = qko_ro            [wrd]; stopp=1; continue };
   if(qko_tv            [wrd]) { rett = qko_tv            [wrd]; stopp=1; continue };
   if(qko_vi            [wrd]) { rett = qko_vi            [wrd]; stopp=1; continue };
   if(qko_ze_im         [wrd]) { rett = qko_ze_im         [wrd]; stopp=1; continue };
   if(qko_ze_vi         [wrd]) { rett = qko_ze_vi         [wrd]; stopp=1; continue };
   if(qne_da            [wrd]) { rett = qne_da            [wrd]; stopp=1; continue };
   if(qne_im            [wrd]) { rett = qne_im            [wrd]; stopp=1; continue };
   if(qne_pr            [wrd]) { rett = qne_pr            [wrd]; stopp=1; continue };
   if(qne_ro            [wrd]) { rett = qne_ro            [wrd]; stopp=1; continue };
   if(qne_tv            [wrd]) { rett = qne_tv            [wrd]; stopp=1; continue };
   if(qne_vi            [wrd]) { rett = qne_vi            [wrd]; stopp=1; continue };
   if(qpo_ed_mu_da      [wrd]) { rett = qpo_ed_mu_da      [wrd]; stopp=1; continue };
   if(qpo_ed_mu_im      [wrd]) { rett = qpo_ed_mu_im      [wrd]; stopp=1; continue };
   if(qpo_ed_mu_pr      [wrd]) { rett = qpo_ed_mu_pr      [wrd]; stopp=1; continue };
   if(qpo_ed_mu_ro      [wrd]) { rett = qpo_ed_mu_ro      [wrd]; stopp=1; continue };
   if(qpo_ed_mu_tv      [wrd]) { rett = qpo_ed_mu_tv      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_da      [wrd]) { rett = qpo_ed_sr_da      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_im      [wrd]) { rett = qpo_ed_sr_im      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_pr      [wrd]) { rett = qpo_ed_sr_pr      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_ro      [wrd]) { rett = qpo_ed_sr_ro      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_tv      [wrd]) { rett = qpo_ed_sr_tv      [wrd]; stopp=1; continue };
   if(qpo_ed_sr_vi      [wrd]) { rett = qpo_ed_sr_vi      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_da      [wrd]) { rett = qpo_ed_ze_da      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_im      [wrd]) { rett = qpo_ed_ze_im      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_pr      [wrd]) { rett = qpo_ed_ze_pr      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_ro      [wrd]) { rett = qpo_ed_ze_ro      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_tv      [wrd]) { rett = qpo_ed_ze_tv      [wrd]; stopp=1; continue };
   if(qpo_ed_ze_vi      [wrd]) { rett = qpo_ed_ze_vi      [wrd]; stopp=1; continue };
   if(qpo_mn_da         [wrd]) { rett = qpo_mn_da         [wrd]; stopp=1; continue };
   if(qpo_mn_im         [wrd]) { rett = qpo_mn_im         [wrd]; stopp=1; continue };
   if(qpo_mn_pr         [wrd]) { rett = qpo_mn_pr         [wrd]; stopp=1; continue };
   if(qpo_mn_ro         [wrd]) { rett = qpo_mn_ro         [wrd]; stopp=1; continue };
   if(qpo_mn_tv         [wrd]) { rett = qpo_mn_tv         [wrd]; stopp=1; continue };
   if(qso_da            [wrd]) { rett = qso_da            [wrd]; stopp=1; continue };
   if(qso_im            [wrd]) { rett = qso_im            [wrd]; stopp=1; continue };
   if(qso_mu_da         [wrd]) { rett = qso_mu_da         [wrd]; stopp=1; continue };
   if(qso_mu_im         [wrd]) { rett = qso_mu_im         [wrd]; stopp=1; continue };
   if(qso_mu_pr         [wrd]) { rett = qso_mu_pr         [wrd]; stopp=1; continue };
   if(qso_mu_ro         [wrd]) { rett = qso_mu_ro         [wrd]; stopp=1; continue };
   if(qso_mu_tv         [wrd]) { rett = qso_mu_tv         [wrd]; stopp=1; continue };
   if(qso_mu_vi         [wrd]) { rett = qso_mu_vi         [wrd]; stopp=1; continue };
   if(qso_pr            [wrd]) { rett = qso_pr            [wrd]; stopp=1; continue };
   if(qso_ro            [wrd]) { rett = qso_ro            [wrd]; stopp=1; continue };
   if(qso_sr_da         [wrd]) { rett = qso_sr_da         [wrd]; stopp=1; continue };
   if(qso_sr_im         [wrd]) { rett = qso_sr_im         [wrd]; stopp=1; continue };
   if(qso_sr_pr         [wrd]) { rett = qso_sr_pr         [wrd]; stopp=1; continue };
   if(qso_sr_ro         [wrd]) { rett = qso_sr_ro         [wrd]; stopp=1; continue };
   if(qso_sr_tv         [wrd]) { rett = qso_sr_tv         [wrd]; stopp=1; continue };
   if(qso_sr_vi         [wrd]) { rett = qso_sr_vi         [wrd]; stopp=1; continue };
   if(qso_tv            [wrd]) { rett = qso_tv            [wrd]; stopp=1; continue };
   if(qso_vi            [wrd]) { rett = qso_vi            [wrd]; stopp=1; continue };
   if(qso_ze_da         [wrd]) { rett = qso_ze_da         [wrd]; stopp=1; continue };
   if(qso_ze_im         [wrd]) { rett = qso_ze_im         [wrd]; stopp=1; continue };
   if(qso_ze_pr         [wrd]) { rett = qso_ze_pr         [wrd]; stopp=1; continue };
   if(qso_ze_ro         [wrd]) { rett = qso_ze_ro         [wrd]; stopp=1; continue };
   if(qso_ze_tv         [wrd]) { rett = qso_ze_tv         [wrd]; stopp=1; continue };
   if(qso_ze_vi         [wrd]) { rett = qso_ze_vi         [wrd]; stopp=1; continue };
   if(qst               [wrd]) { rett = qst               [wrd]; stopp=1; continue };
   if(suyz              [wrd]) { rett = suyz              [wrd]; stopp=1; continue };
   if(sw_edmu_da        [wrd]) { rett = sw_edmu_da        [wrd]; stopp=1; continue };
   if(sw_edmu_im        [wrd]) { rett = sw_edmu_im        [wrd]; stopp=1; continue };
   if(sw_edmu_ne        [wrd]) { rett = sw_edmu_ne        [wrd]; stopp=1; continue };
   if(sw_edmu_pr        [wrd]) { rett = sw_edmu_pr        [wrd]; stopp=1; continue };
   if(sw_edmu_ro        [wrd]) { rett = sw_edmu_ro        [wrd]; stopp=1; continue };
   if(sw_edmu_tv        [wrd]) { rett = sw_edmu_tv        [wrd]; stopp=1; continue };
   if(sw_edmu_vi        [wrd]) { rett = sw_edmu_vi        [wrd]; stopp=1; continue };
   if(sw_edob_da        [wrd]) { rett = sw_edob_da        [wrd]; stopp=1; continue };
   if(sw_edob_im        [wrd]) { rett = sw_edob_im        [wrd]; stopp=1; continue };
   if(sw_edob_ne        [wrd]) { rett = sw_edob_ne        [wrd]; stopp=1; continue };
   if(sw_edob_pr        [wrd]) { rett = sw_edob_pr        [wrd]; stopp=1; continue };
   if(sw_edob_ro        [wrd]) { rett = sw_edob_ro        [wrd]; stopp=1; continue };
   if(sw_edob_tv        [wrd]) { rett = sw_edob_tv        [wrd]; stopp=1; continue };
   if(sw_edob_vi        [wrd]) { rett = sw_edob_vi        [wrd]; stopp=1; continue };
   if(sw_edsr_da        [wrd]) { rett = sw_edsr_da        [wrd]; stopp=1; continue };
   if(sw_edsr_im        [wrd]) { rett = sw_edsr_im        [wrd]; stopp=1; continue };
   if(sw_edsr_pr        [wrd]) { rett = sw_edsr_pr        [wrd]; stopp=1; continue };
   if(sw_edsr_ro        [wrd]) { rett = sw_edsr_ro        [wrd]; stopp=1; continue };
   if(sw_edsr_tv        [wrd]) { rett = sw_edsr_tv        [wrd]; stopp=1; continue };
   if(sw_edsr_vi        [wrd]) { rett = sw_edsr_vi        [wrd]; stopp=1; continue };
   if(sw_edze_da        [wrd]) { rett = sw_edze_da        [wrd]; stopp=1; continue };
   if(sw_edze_im        [wrd]) { rett = sw_edze_im        [wrd]; stopp=1; continue };
   if(sw_edze_pr        [wrd]) { rett = sw_edze_pr        [wrd]; stopp=1; continue };
   if(sw_edze_ro        [wrd]) { rett = sw_edze_ro        [wrd]; stopp=1; continue };
   if(sw_edze_tv        [wrd]) { rett = sw_edze_tv        [wrd]; stopp=1; continue };
   if(sw_edze_vi        [wrd]) { rett = sw_edze_vi        [wrd]; stopp=1; continue };
   if(sw_mn_da          [wrd]) { rett = sw_mn_da          [wrd]; stopp=1; continue };
   if(sw_mn_im          [wrd]) { rett = sw_mn_im          [wrd]; stopp=1; continue };
   if(sw_mn_ne          [wrd]) { rett = sw_mn_ne          [wrd]; stopp=1; continue };
   if(sw_mn_pr          [wrd]) { rett = sw_mn_pr          [wrd]; stopp=1; continue };
   if(sw_mn_ro          [wrd]) { rett = sw_mn_ro          [wrd]; stopp=1; continue };
   if(sw_mn_tv          [wrd]) { rett = sw_mn_tv          [wrd]; stopp=1; continue };
   if(sw_mn_vi          [wrd]) { rett = sw_mn_vi          [wrd]; stopp=1; continue };
   if(swn_edmu_da       [wrd]) { rett = swn_edmu_da       [wrd]; stopp=1; continue };
   if(swn_edmu_im       [wrd]) { rett = swn_edmu_im       [wrd]; stopp=1; continue };
   if(swn_edmu_me       [wrd]) { rett = swn_edmu_me       [wrd]; stopp=1; continue };
   if(swn_edmu_ne       [wrd]) { rett = swn_edmu_ne       [wrd]; stopp=1; continue };
   if(swn_edmu_pr       [wrd]) { rett = swn_edmu_pr       [wrd]; stopp=1; continue };
   if(swn_edmu_pt       [wrd]) { rett = swn_edmu_pt       [wrd]; stopp=1; continue };
   if(swn_edmu_ro       [wrd]) { rett = swn_edmu_ro       [wrd]; stopp=1; continue };
   if(swn_edmu_sq       [wrd]) { rett = swn_edmu_sq       [wrd]; stopp=1; continue };
   if(swn_edmu_tv       [wrd]) { rett = swn_edmu_tv       [wrd]; stopp=1; continue };
   if(swn_edob_da       [wrd]) { rett = swn_edob_da       [wrd]; stopp=1; continue };
   if(swn_edob_im       [wrd]) { rett = swn_edob_im       [wrd]; stopp=1; continue };
   if(swn_edob_ne       [wrd]) { rett = swn_edob_ne       [wrd]; stopp=1; continue };
   if(swn_edob_pr       [wrd]) { rett = swn_edob_pr       [wrd]; stopp=1; continue };
   if(swn_edob_ro       [wrd]) { rett = swn_edob_ro       [wrd]; stopp=1; continue };
   if(swn_edob_tv       [wrd]) { rett = swn_edob_tv       [wrd]; stopp=1; continue };
   if(swn_edob_vi       [wrd]) { rett = swn_edob_vi       [wrd]; stopp=1; continue };
   if(swn_edsr_da       [wrd]) { rett = swn_edsr_da       [wrd]; stopp=1; continue };
   if(swn_edsr_im       [wrd]) { rett = swn_edsr_im       [wrd]; stopp=1; continue };
   if(swn_edsr_ne       [wrd]) { rett = swn_edsr_ne       [wrd]; stopp=1; continue };
   if(swn_edsr_pr       [wrd]) { rett = swn_edsr_pr       [wrd]; stopp=1; continue };
   if(swn_edsr_ro       [wrd]) { rett = swn_edsr_ro       [wrd]; stopp=1; continue };
   if(swn_edsr_tv       [wrd]) { rett = swn_edsr_tv       [wrd]; stopp=1; continue };
   if(swn_edsr_vi       [wrd]) { rett = swn_edsr_vi       [wrd]; stopp=1; continue };
   if(swn_edze_da       [wrd]) { rett = swn_edze_da       [wrd]; stopp=1; continue };
   if(swn_edze_im       [wrd]) { rett = swn_edze_im       [wrd]; stopp=1; continue };
   if(swn_edze_me       [wrd]) { rett = swn_edze_me       [wrd]; stopp=1; continue };
   if(swn_edze_ne       [wrd]) { rett = swn_edze_ne       [wrd]; stopp=1; continue };
   if(swn_edze_pr       [wrd]) { rett = swn_edze_pr       [wrd]; stopp=1; continue };
   if(swn_edze_ro       [wrd]) { rett = swn_edze_ro       [wrd]; stopp=1; continue };
   if(swn_edze_tv       [wrd]) { rett = swn_edze_tv       [wrd]; stopp=1; continue };
   if(swn_edze_vi       [wrd]) { rett = swn_edze_vi       [wrd]; stopp=1; continue };
   if(swn_mn_da         [wrd]) { rett = swn_mn_da         [wrd]; stopp=1; continue };
   if(swn_mn_im         [wrd]) { rett = swn_mn_im         [wrd]; stopp=1; continue };
   if(swn_mn_ne         [wrd]) { rett = swn_mn_ne         [wrd]; stopp=1; continue };
   if(swn_mn_pr         [wrd]) { rett = swn_mn_pr         [wrd]; stopp=1; continue };
   if(swn_mn_ro         [wrd]) { rett = swn_mn_ro         [wrd]; stopp=1; continue };
   if(swn_mn_sq         [wrd]) { rett = swn_mn_sq         [wrd]; stopp=1; continue };
   if(swn_mn_tv         [wrd]) { rett = swn_mn_tv         [wrd]; stopp=1; continue };
   if(swo_edmu_da       [wrd]) { rett = swo_edmu_da       [wrd]; stopp=1; continue };
   if(swo_edmu_im       [wrd]) { rett = swo_edmu_im       [wrd]; stopp=1; continue };
   if(swo_edmu_ne       [wrd]) { rett = swo_edmu_ne       [wrd]; stopp=1; continue };
   if(swo_edmu_pr       [wrd]) { rett = swo_edmu_pr       [wrd]; stopp=1; continue };
   if(swo_edmu_ro       [wrd]) { rett = swo_edmu_ro       [wrd]; stopp=1; continue };
   if(swo_edmu_tv       [wrd]) { rett = swo_edmu_tv       [wrd]; stopp=1; continue };
   if(swo_edmu_zv       [wrd]) { rett = swo_edmu_zv       [wrd]; stopp=1; continue };
   if(swo_edob_da       [wrd]) { rett = swo_edob_da       [wrd]; stopp=1; continue };
   if(swo_edob_im       [wrd]) { rett = swo_edob_im       [wrd]; stopp=1; continue };
   if(swo_edob_ne       [wrd]) { rett = swo_edob_ne       [wrd]; stopp=1; continue };
   if(swo_edob_pr       [wrd]) { rett = swo_edob_pr       [wrd]; stopp=1; continue };
   if(swo_edob_ro       [wrd]) { rett = swo_edob_ro       [wrd]; stopp=1; continue };
   if(swo_edob_tv       [wrd]) { rett = swo_edob_tv       [wrd]; stopp=1; continue };
   if(swo_edob_vi       [wrd]) { rett = swo_edob_vi       [wrd]; stopp=1; continue };
   if(swo_edsr_da       [wrd]) { rett = swo_edsr_da       [wrd]; stopp=1; continue };
   if(swo_edsr_im       [wrd]) { rett = swo_edsr_im       [wrd]; stopp=1; continue };
   if(swo_edsr_pr       [wrd]) { rett = swo_edsr_pr       [wrd]; stopp=1; continue };
   if(swo_edsr_ro       [wrd]) { rett = swo_edsr_ro       [wrd]; stopp=1; continue };
   if(swo_edsr_tv       [wrd]) { rett = swo_edsr_tv       [wrd]; stopp=1; continue };
   if(swo_edsr_vi       [wrd]) { rett = swo_edsr_vi       [wrd]; stopp=1; continue };
   if(swo_edze_da       [wrd]) { rett = swo_edze_da       [wrd]; stopp=1; continue };
   if(swo_edze_im       [wrd]) { rett = swo_edze_im       [wrd]; stopp=1; continue };
   if(swo_edze_ne       [wrd]) { rett = swo_edze_ne       [wrd]; stopp=1; continue };
   if(swo_edze_pr       [wrd]) { rett = swo_edze_pr       [wrd]; stopp=1; continue };
   if(swo_edze_ro       [wrd]) { rett = swo_edze_ro       [wrd]; stopp=1; continue };
   if(swo_edze_tv       [wrd]) { rett = swo_edze_tv       [wrd]; stopp=1; continue };
   if(swo_edze_vi       [wrd]) { rett = swo_edze_vi       [wrd]; stopp=1; continue };
   if(swo_edze_zv       [wrd]) { rett = swo_edze_zv       [wrd]; stopp=1; continue };
   if(swo_mn_da         [wrd]) { rett = swo_mn_da         [wrd]; stopp=1; continue };
   if(swo_mn_im         [wrd]) { rett = swo_mn_im         [wrd]; stopp=1; continue };
   if(swo_mn_ne         [wrd]) { rett = swo_mn_ne         [wrd]; stopp=1; continue };
   if(swo_mn_pr         [wrd]) { rett = swo_mn_pr         [wrd]; stopp=1; continue };
   if(swo_mn_ro         [wrd]) { rett = swo_mn_ro         [wrd]; stopp=1; continue };
   if(swo_mn_sq         [wrd]) { rett = swo_mn_sq         [wrd]; stopp=1; continue };
   if(swo_mn_tv         [wrd]) { rett = swo_mn_tv         [wrd]; stopp=1; continue };
   if(vvodn             [wrd]) { rett = vvodn             [wrd]; stopp=1; continue };
   if(zvt               [wrd]) { rett = zvt               [wrd]; stopp=1; continue };
break };
return rett}
