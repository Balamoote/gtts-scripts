 ...
 if(iwrd=="замок" || iwrd=="замки") {
  ...
  if ( wb(-5,-1,"в во") && s(wbn,-1) ) {
   ...
   if ( vis(tn_ok) ) {
    ...
    { l[i]=omo2; Z[330]++; if(dbg){print "Z330"}; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[331]++; if(dbg){print "Z331"}; continue }; #edit
    ...
    { l[i]=omo2; Z[374]++; if(dbg){print "Z374"}; continue }; # check
    ...
    { l[i]=omo2; Z[381]++; if(dbg){print "Z381"}; continue }; #inspect
    if ( vgle(bb_n,tn-6,tn-2) &&
    ...
    { l[i]=omo2; Z[384]++; if(dbg){print "Z384"}; continue }; # _gl_v_lock-iv suw_vi suw_tv
    ...
    { l[i]=omo2; Z[385]++; if(dbg){print "Z385"}; continue }; # _gl_v_lock-iv suw_tv suw_vi
    ...
    { l[i]=omo2; Z[386]++; if(dbg){print "Z386"}; continue }; # _gl_v_lock-iv suw_tv suw_vi
    if ( veq(bb_n,tn-3) &&
           q(tn-2,"suw_vi suw_im mest_im mest_vi suw_tv mest_tv qast isname") &&
           q(tn-1,"suw_vi suw_tv qast nar_any") && s(tn-3,tn-1) )
    { l[i]=omo2; Z[387]++; if(dbg){print "Z387"}; continue }; #inspect
    if ( veq(bb_n,tn-2) &&
    ...
    { l[i]=omo2; Z[391]++; if(dbg){print "Z391"}; continue }; # _gl_v_lock-iv suw_vi PRE_VI suw_vi
    ...
    { l[i]=omo2; Z[392]++; if(dbg){print "Z392"}; continue }; # _gl_v_lock-iv suw_vi PRE_VI suw_vi
    ...
    { l[i]=omo2; Z[393]++; if(dbg){print "Z393"}; continue }; # _gl_v_lock-iv PRE_TV suw_tv
    if ( vgle(qb_n,tn-6,tn-3) && veq(bb_n,qb_n-1) &&
          qir(qb_n+1,tn-3,"prl_tv mest_tv mest_3e prq_tv qis_tv qik_tv") &&
            q(tn-2,"suw_tv mest_tv") &&
            q(tn-1,"suw_vi") && s(bb_n,tn-1) )
    { l[i]=omo2; Z[394]++; if(dbg){print "Z394"}; continue }; # _gl_v_lock-iv PRE_TV suw_tv suw_vi
    if ( vgle(qb_n,tn-7,tn-4) && veq(bb_n,qb_n-1) &&
          qir(qb_n+1,tn-4,"prl_tv mest_tv prq_tv qis_tv qik_tv") &&
            ...
            q(tn-1,"suw_vi") && s(bb_n,tn-1) )
    { l[i]=omo2; Z[395]++; if(dbg){print "Z395"}; continue }; # _gl_v_lock-iv PRE_TV suw_tv suw_vi
    ...
    { l[i]=omo2; Z[396]++; if(dbg){print "Z396"}; continue }; # _gl_v_lock-iv suw_vi PRE_TV suw_tv
    ...
    { l[i]=omo2; Z[397]++; if(dbg){print "Z397"}; continue }; # _gl_v_lock-iv suw_vi PRE_TV suw_tv
    ...
    { l[i]=omo2; Z[398]++; if(dbg){print "Z398"}; continue }; # _gl_v_lock-iv suw_vi PRE_RO suw_ro
    ...
    { l[i]=omo2; Z[399]++; if(dbg){print "Z399"}; continue }; # _gl_v_lock-iv <SUW_VI PRE_RO <suw_ro suw_vi
    ...
    { l[i]=omo2; Z[402]++; if(dbg){print "Z402"}; continue }; # _gl_v_lock-iv suw_vi suw_ro PRE_PR <suw_pr suw_ro
    ...
    { l[i]=omo2; Z[403]++; if(dbg){print "Z403"}; continue }; # _gl_v_lock-iv suw_vi suw_ro PRE_PR <suw_pr
    if ( vgle(qb_n,tn-6,tn-3) && veq(bb_n,qb_n-2) &&
            q(qb_n-1,"suw_vi prq_vi mest_vi mest_3e") &&
          qir(qb_n+1,tn-3,"prl_pr mcop_pr prq_pr qis_pr qik_pr") &&
            q(tn-2,"suw_pr") &&
            q(tn-1,"suw_vi prq_vi mest_vi mest_3e") && s(bb_n,tn-1) )
    { l[i]=omo2; Z[404]++; if(dbg){print "Z404"}; continue }; # _gl_v_lock-iv suw_vi suw_ro PRE_PR <suw_pr suw_vi
    if ( vgle(qb_n,tn-5,tn-3) && vgle(bb_n,qb_n-5,qb_n-1) &&
          ...
          qir(qb_n+1,tn-3,"prl_pr mcop_pr prq_pr qis_pr qik_pr") &&
            q(tn-2,"suw_pr") &&
            q(tn-1,"suw_ro mest_ro suw_vi mest_vi mest_3e") && s(bb_n,tn-1) )
    { l[i]=omo2; Z[405]++; if(dbg){print "Z405"}; continue }; # _gl_v_lock-iv PRE_PR <suw_pr suw_ro
    if ( vgle(qb_n,tn-5,tn-2) && veq(bb_n,qb_n-1) &&
          qir(qb_n+1,tn-2,"prl_pr mcop_pr prq_pr qis_pr qik_pr") &&
            q(tn-1,"suw_pr") && s(bb_n,tn-1) )
    ...
    { l[i]=omo2; Z[431]++; if(dbg){print "Z431"}; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[432]++; if(dbg){print "Z432"}; continue }; #check
    ...
    { l[i]=omo1; Z[471]++; if(dbg){print "Z471"}; continue }; #check
    if ( vgle(bb_n,tn-5,tn-2) &&
            q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
          qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
    { l[i]=omo1; Z[472]++; if(dbg){print "Z472"}; continue }; #check
    if ( vgle(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[633]++; if(dbg){print "Z633"}; continue };
  ...
  if ( bf_n ) {
    ...
    { bf_n=""; Z[881]++; if(dbg){print "Z881"}; };
    if ( vgle(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
            q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
    { bf_n=bf_n+1; Z[882]++; if(dbg){print "Z882"}; };
...
#   { l[i]=omo1; Z[887]++; if(dbg){print "Z887"}; continue };
    ...
    { l[i]=omo2; Z[901]++; if(dbg){print "Z901"}; continue };
 ...
 #
    ...
    if ( ba(-2,"_lock_prl") && prl_ro(-2) &&
          Q(-1,"suw_any sz_iili") && s(-2,-1) )
    { l[i]=omo2; Z[914]++; if(dbg){print "Z914"}; continue };
...
#   # _castle_lock_after:start
 ...
 #
    ...
    if ( prl_tv(-1) &&
             ba(-1,"_castle_prl_geo") && s(-1)  )
    { l[i]=omo1; Z[951]++; if(dbg){print "Z951"}; continue };
    if ( prl_tv(-2) &&
             ba(-2,"_castle_prl_geo") &&
              Q(-1,"suw_any") && s(-2,-1)  )
 ...
 #
  ...
  # _suw_v_castle-vp:end
    ...
    if ( ba(-3,"_lock_prl") && prl_da(-3) &&
          q(-2,"prl_any prq_any mest_any qast") && W(-2,"как") &&
          q(-1,"prl_any prq_any mest_any qast") && s(-3,-1)  )
    { l[i]=omo2; Z[971]++; if(dbg){print "Z971"}; continue };
    if ( ba(1,"_lock_prl") && s(0) && prl_da(1) && !isname(1) )
    ...
    if ( prl_da(1) &&
             ba(1,"_castle_prl_geo") && s(0) && !prl_da(1))
    { l[i]=omo1; Z[975]++; if(dbg){print "Z975"}; continue };
    # _castle_prl_geo:start
...
#   { l[i]=omo2; Z[977]++; if(dbg){print "Z977"}; continue };
#   # _castle_lock_after:start
 ...
 { l[i]=omo1; Z[985]++; if(dbg){print "Z985"}; continue };
 if ( z(0) &&
 ...
 { l[i]=omo2; Z[990]++; if(dbg){print "Z990"}; continue };
...
# escape:end
 ...
 if ( w(1,"на") &&
    bfa(2,5,"_lock_na_suw") && s(0,bfn-1) &&
    qir(2,bfn-1,"prl_vi prl_pr mest_vi mest_pr prq_vi prq_pr") )
 { l[i]=omo2; Z[999]++; if(dbg){print "Z999"}; continue }; #check ислючения "в"
 if ( w(1,"у") && Ww_(-1,"в") &&
 ...
 if ( gist(-4,2) &&
         ...
         w(-1,"а но и или") && s(0) )
 { l[i]=omo2; Z[1007]++; if(dbg){print "Z1007"}; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[1015]++; if(dbg){print "Z1015", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_vi") )
    { tn_ok = 1; Z[1016]++; if(dbg){print "Z1016", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[1017]++; if(dbg){print "Z1017", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
    ...
    if ( vis(tn_ok) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"даже") ) )
    { tn = xsn; Z[1023]++; if(dbg){print "Z1023", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vis(tn_ok) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[1024]++; if(dbg){print "Z1024", "-- tn_ok OFF", el, "замок"};};
    if ( vis(tn_ok) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[1025]++; if(dbg){print "Z1025", "-- tn_ok OFF", el, "замок"};};
}
