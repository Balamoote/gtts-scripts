...
#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue
 ...
 if ( isname(1) && ba(1,"_castle_isname_forced") && s(0) )
 { l[i]=omo1; Z[15]++; if(dbg){print "Z15"}; continue };
 if ( isname(1) && ba(1,"_castle_numbered_royalty-ro") && roman(1) && s(0) )
 { l[i]=omo1; Z[16]++; if(dbg){print "Z16"}; continue };

 if ( bba(-3,-1,"_castle_prl_forced") && isname(bbn) && FN_castle_prl_f("_castle_prl_forced") )
 { Z[17]++; if(dbg){print "Z17"}; continue };
 if ( bfa(1,3,"_castle_prl_forced") && isname(bbn) && FN_castle_prl_f("_castle_prl_forced") )
 { Z[18]++; if(dbg){print "Z18"}; continue };

 if ( FN_lock_prl_true_f() )
...
#{ l[i]=omo1; Z[54]++; if(dbg){print "Z54"}; continue };
 ...
 if ( bb(-4,-1,cst) &&
       w(1,"в") &&
     vgl(Y["n+_castlelike_things"],2,5) && s(bbn,Y["n+_castlelike_things"]-1) &&
 ...
 if ( wf(2,6,"собой") && bw(wfn-1,"представлять") ) { wf_n=wfn;
   ...
   if ( bfa(wf_n+1,wf_n+4,"_locklike_things") ) { bf_n=bfn;
     ...
     if ( vgl(wf_n,2,6) && vgl(bf_n,wfn+1,wfn+4) && s(1,bf_n-1) &&
          ...
          qiz(wf_n+1,bf_n-1,"prl_vi prq_vi mest_vi mest_3e","qast") &&
          qiz(bf_n+1,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
...
sz_iili(-2) &&
 ...
 if ( bfa(1,4,"_locklike_things") ) { bf_n=bfn;
    ...
    if ( z(0) && veq(bf_n,1) && !(isname(1) && bw(1,"люк")) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[159]++; if(dbg){print "Z159"}; continue } else
...
sz_iili(1) &&
 ...
 if ( FN_zamok_suwro_f() )
 { Z[175]++; if(dbg){print "Z175", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( wf(1,6,"для") ) { wf_n=Y["n+_pre_dlya"]=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
      ...
      { wf_n=bf_n=""; Z[177]++; if(dbg){print "Z177"}; };
      ...
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[180]++; if(dbg){print "Z180", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[181]++; if(dbg){print "Z181", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[182]++; if(dbg){print "Z182", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wb(-5,-2,"для") ) { wb_n=Y["n-_pre_dlya"]=wbn;
   if ( wba(wb_n+1,-1,"_castle_suw-ro1") ) { bb_n=wbn;
    ...
    { l[i]=omo1; Z[185]++; if(dbg){print "Z185"}; continue }; #checkro2
...
#
   ...
   if ( wfa(wb_n+1,wb_n+4,"_castle_na_suw-pr") ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_castle_na_suw-pr") ) { bf_n=bfn;
       ...
       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
            ...
            qir(bf_n+1,-1,"prl_edmuim prl_mnim prq_edmuim prq_mnim mest_vi mest_3e prl_kred_sr nar_any qast suw_ro prq_ro mest_ro prq_ro suw_tv prl_tv mest_tv") )
 ...
 if ( bba(1,3,"_gl_4_castle_na_suw-pr") ) { bf_n=bbn;
   ...
   if ( vgl(bf_n,1,3) && zs(0) &&
        ...
        qir(bf_n+2,wfn-1,"prl_vi prl_pr otsz_vi otsz_pr mest_vi mest_pr mest_pr prq_vi prq_pr") && s(1,wfn-1) )
   ...
   if ( vgl(bf_n,1,3) && zs(0) &&
        ...
        wf2(wfn+1,wfn+4,cst1) &&
        qir(1,bf_n-1,"nar_any qast_any mod_any gl_aux_be prl_kred_sr") &&
        qir(bf_n+2,wfn-1,"prl_vi prl_pr otsz_vi otsz_pr mest_vi mest_pr mest_pr prq_vi prq_pr") &&
 ...
 if ( w(1,"на") && wfa(2,5,"_lock_na_suw-pr") ) { wf_n=wfn;
        ...
        cst0="держать остаться унести";
        if ( bb(-4,-1,cst0) && s(bbn,-1) &&
           mwba(wf_n,bbn,"M_lock_na_suw-pr_gl_castle_pe") )
        { wf_n=""; Z[246]++; if(dbg){print "Z246"}; };
        if ( bf(-4,-1,cst0) && s(wb_n,-1) &&
...
#       { Z[256]++; if(dbg){print "Z256"}; continue };
 ...
 cst="веками вдали издали вдалеке";
 if ( w(1,cst) && s(0) )
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[280]++; if(dbg){print "Z280", "-- TN_na_zamok_f"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[313]++; if(dbg){print "Z313"}; };
    ...
    if ( bb(-5,-1,cst) ) { wb_n=bbn;
      ...
      if ( wba(wb_n-5,wb_n-1,"_lock_na_suw-pr") ) { bb_n=wbn;
       ...
       { l[i]=omo2; Z[322]++; if(dbg){print "Z322"}; continue };#urhere
    };};
  ...
  ######################################################################################################
    ...
    { Z[339]++; if(dbg){print "Z339", "-- FN_castle_prl_f"}; continue };

    # основные прилагательные -- lock
    if ( FN_lock_prl_f("_lock_prl") )
    { Z[340]++; if(dbg){print "Z340", "-- FN_lock_prl_f"}; continue };
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[341]++; if(dbg){print "Z341", "-- TN_v_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[345]++; if(dbg){print "Z345", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[356]++; if(dbg){print "Z356", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[357]++; if(dbg){print "Z357", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[358]++; if(dbg){print "Z358", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[359]++; if(dbg){print "Z359", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[360]++; if(dbg){print "Z360", "-- TN_qerez_zamok_f"}; continue };
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    cst="вдалеке вдали вблизи наконец";
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[497]++; if(dbg){print "Z497"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_da") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[509]++; if(dbg){print "Z509"}; continue }; #check
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
               w(1,"на") &&
             wfa(2,5,"_lock_na_suw-vi") && s(0,wfn-1) &&
             qir(2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") )
        { l[i]=omo2; Z[510]++; if(dbg){print "Z510"}; continue }; #check
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[521]++; if(dbg){print "Z521"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[554]++; if(dbg){print "Z554"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"];
       ...
       { bf_n=""; Z[578]++; if(dbg){print "Z578"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[579]++; if(dbg){print "Z579"}; };
       ...
       { l[i]=omo2; Z[589]++; if(dbg){print "Z589"}; continue };
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"ediv") &&
          prq_any(bf_n) && !q(bf_n,"prq_edmuim prq_edmutv prq_kred_mu") &&
             !exb(-1,"тот ни другой ") )
         { bf_n=""; Z[590]++; if(dbg){print "Z590"}; };
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
               ba(-1,"_castle_prl_geo") && s(-1) )
         { bf_n=""; Z[591]++; if(dbg){print "Z591"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 # доработка прилагательных
 if ( wb(-3,-1,"самый") )  { wb_n=wbn;

    cst="современный надежный";
    if ( veq(wb_n,-2) && w(-1,cst) && s(-2,-1) )
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[602]++; if(dbg){print "Z602", "-- TN_do_zamka_f"}; continue };
 ...
 { Z[647]++; if(dbg){print "Z647", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[647]++; if(dbg){print "Z647", "-- FN_lock_prl_after_f"}; continue };
...
#   if ( qy(1,"gl_aux_be qast_any") && q(1+qyn,"prl_edmuim prl_edmutv prl_kred_mu") && ba(1+qyn,"_lock_prl_after") && s(0) )
#   { l[i]=omo2; Z[651]++; if(dbg){print "Z651"}; continue };
    # _lock_after:end
# };
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[655]++; if(dbg){print "Z655"}; continue } else
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[663]++; if(dbg){print "Z663", "-- is_CAP"}; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[664]++; if(dbg){print "Z664", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[665]++; if(dbg){print "Z665", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[666]++; if(dbg){print "Z666", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[667]++; if(dbg){print "Z667", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[668]++; if(dbg){print "Z668", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[669]++; if(dbg){print "Z669", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[670]++; if(dbg){print "Z670", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[671]++; if(dbg){print "Z671", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[672]++; if(dbg){print "Z672", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[673]++; if(dbg){print "Z673", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[674]++; if(dbg){print "Z674", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-3) &&
#           q(-2,"prq_vi") && q(-1,"suw_vi") )
#   { tn_ok=fufu=1; Z[675]++; if(dbg){print "Z675", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-4) &&
#           q(-3,"prq_vi") && q(-2,"suw_vi") && q(-1,"prl_vi mest_vi") )
#   { tn_ok=fufu=1; Z[676]++; if(dbg){print "Z676", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[677]++; if(dbg){print "Z677", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[678]++; if(dbg){print "Z678", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[679]++; if(dbg){print "Z679", "-- tn_ok OFF", el, "замок"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    { l[i]=omo2; Z[707]++; if(dbg){print "Z707"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[708]++; if(dbg){print "Z708"}; stopp=ret=1; continue }; #edit
    ...
    { l[i]=omo1; Z[725]++; if(dbg){print "Z725"}; stopp=ret=1; continue };
    # _suw_v_castle-vp:end
    ...
    { l[i]=omo1; Z[731]++; if(dbg){print "Z731"}; stopp=ret=1; continue };
    if ( qxs(tn-1,cst,"нет") ||
         qxs(tn-1,"нет",cst) ||
         qxs(tn-1,"какими","судьбами") ||
         qxs(tn-1,"ни","ногой") )
    { l[i]=omo1; Z[732]++; if(dbg){print "Z732"}; stopp=ret=1; continue };

    if ( qxs(1,"больше более","ни","ногой")||
         qxs(1,"ни","ногой")||
           w(1,"ни-ни") )
    { l[i]=omo1; Z[733]++; if(dbg){print "Z733"}; stopp=ret=1; continue };
    # _suw_v_lock:start
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[749]++; if(dbg){print "Z749"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[751]++; if(dbg){print "Z751"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[757]++; if(dbg){print "Z757"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[763]++; if(dbg){print "Z763"}; };
...
!(narph_any(bbn)||w(bbn-1,"в на")) )
    ...
    { l[i]=omo2; Z[797]++; if(dbg){print "Z797"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[798]++; if(dbg){print "Z798"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst3) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[833]++; if(dbg){print "Z833"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[834]++; if(dbg){print "Z834"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[907]++; if(dbg){print "Z907"}; stopp=ret=1; continue };
    ...
    if ( bf(1,3,cst) ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,3) &&
            ...
            qir(1,bf_n-1,"mod_any gl_aux_be") &&
            qir(bf_n+1,bfn-1,"prl_vi mest_vi mesr_3e") )
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[926]++; if(dbg){print "Z926"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[935]++; if(dbg){print "Z935", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
   ...
   if ( bw(-2,cst) &&
    ...
    if ( bba(tn-4,tn-1,"_v_suw-vi_pod_lock") ) { bb_n=bbn;
       if ( vgl(bb_n,tn-4,tn-1) &&
            ...
            qir(bb_n+1,tn-1,"gl_peed gl_peed suw_vi prl_vi mest_vi mest_3e qast") )
...
function zamok_vi_pointesque_f(    ret, stopp) { stopp=Y["zamok_vi_pointesque_f"];
  while ( !stopp ) {
   ...
   { l[i]=omo1; Z[1047]++; if(dbg){print "Z1047"}; stopp=ret=1; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1065]++; if(dbg){print "Z1065", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) && wy(-1,"же") &&
#         qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
#   { tn_ok = 1; Z[1066]++; if(dbg){print "Z1066", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-1,"suw_tv mest_tv") )
#   { tn_ok = 1; Z[1067]++; if(dbg){print "Z1067", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-2,"suw_tv qik_tv") &&
#           q(-1,cst3) )
#   { tn_ok = 1; Z[1068]++; if(dbg){print "Z1068", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_vi") &&
#         qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
#           q(-1,"mest_vip3e suw_vi") )
#   { tn_ok = 1; Z[1069]++; if(dbg){print "Z1069", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_tv") &&
#         qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
#           q(-1,"suw_tv") )
#   { tn_ok = 1; Z[1070]++; if(dbg){print "Z1070", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1071]++; if(dbg){print "Z1071", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1072]++; if(dbg){print "Z1072", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1073]++; if(dbg){print "Z1073", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1074]++; if(dbg){print "Z1074", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1075]++; if(dbg){print "Z1075", "-- tn_ok ON", el, "замка"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1076]++; if(dbg){print "Z1076", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1077]++; if(dbg){print "Z1077", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1078]++; if(dbg){print "Z1078", "-- tn_ok OFF", el, "замка"};};
}
...
function FN_castle_prl_f(str_is_castle_prl,    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_is_castle_prl "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {

#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1083]++; if(dbg){print "Z1083"}; stopp=ret=1; continue };

    # _castle_prl:start
    if ( q(-1,"prl_edmuim prl_mnim prl_kred_mu") && ba(-1,str_is_castle_prl) && s(-1) &&
      !(ba(-2,"_lock_pe") && s(-2)) )
    { l[i]=omo1; Z[1084]++; if(dbg){print "Z1084"}; stopp=ret=1; continue };
    if ( q(-2,"prl_edmuim prl_mnim prl_kred_mu") && ba(-2,str_is_castle_prl) &&
         q(-1,"prl_any prq_any mest_any qast") && !(bam(-1,"_lock_prl _lock_pe")) && s(-2,-1) )
    { l[i]=omo1; Z[1085]++; if(dbg){print "Z1085"}; stopp=ret=1; continue };
    if ( q(-3,"prl_edmuim prl_mnim prl_kred_mu") && ba(-3,str_is_castle_prl) &&
    ...
    if ( q(-4,"prl_edmuim prl_mnim prl_kred_mu") && ba(-4,str_is_castle_prl) &&
    ...
    if ( q(1,"prl_edmuim prl_kred_mu") && w(0,"замок") && ba(1,str_is_castle_prl) && s(0) &&
    ...
    { l[i]=omo1; Z[1088]++; if(dbg){print "Z1088"}; stopp=ret=1; continue };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
             ...
             { wf_n=bf_n=""; Z[1090]++; if(dbg){print "Z1090"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bba(wf_n+1,wf_n+5,str_is_castle_prl) ) { bb_n=bbn;
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                 bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                 vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[1095]++; if(dbg){print "Z1095"}; };
    ...
    if ( bb(-3,-1,cst2) &&
          q(1,"prl_tv") && ba(1,str_is_castle_prl) && s(bbn,0) )
    { l[i]=omo1; Z[1098]++; if(dbg){print "Z1098"}; stopp=ret=1; continue };
    # _castle_prl:end
break}; Y[stopp_str]=1; return ret }

function FN_lock_prl_f(str_is_lock_prl,    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_is_lock_prl "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    # _lock_prl:escape
    cst="сложить";
    cst1="быть ведь же казаться стать считаться являться";
    ...
    { Z[1101]++; if(dbg){print "Z1101", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1102]++; if(dbg){print "Z1102", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1103]++; if(dbg){print "Z1103", "-- TN_v_zamok_f"}; stopp=ret=1; continue };

    if ( iwrd == "замок" ) {

       if ( q(-1,"prl_edmuim prl_kred_mu") && ba(-1,str_is_lock_prl) && s(-1) &&
          !wa(-1,"X_lock_prl") )
       { l[i]=omo2; Z[1104]++; if(dbg){print "Z1104"}; stopp=ret=1; continue };
       if ( q(-2,"prl_edmuim prl_kred_mu") && ba(-2,str_is_lock_prl) && !wa(-2,"X_lock_prl") &&
            q(-1,"prl_edmuim prq_edmuim mest_edmuvi prq_edmuim mest_3e qast_any nar_any prl_kred_sr gl_aux_be isname") && zs(-2) && s(-1)  )
       { l[i]=omo2; Z[1105]++; if(dbg){print "Z1105"}; stopp=ret=1; continue };
       if ( q(-3,"prl_edmuim prl_kred_mu") && ba(-3,str_is_lock_prl) && zs(-3) &&
       ...
       { l[i]=omo2; Z[1106]++; if(dbg){print "Z1106"}; stopp=ret=1; continue };

       cst1="быть казаться оказаться стать считаться являться";
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1107]++; if(dbg){print "Z1107", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) ||
               ( bfa(wf_n+1,bf_n-1,"_castle_prl") && Q(bfn,"prl_kred_sr") ) ||
                 vex(Y["tn_ok_vi"]) ||
            ( isname(0) && s(-1) ) )
             { wf_n=bf_n=""; Z[1108]++; if(dbg){print "Z1108"}; };

             if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+5) && s(0,bf_n-1) &&
                    q(bf_n,"prl_edmuim prl_mnim prl_edmutv prl_kred_mu") &&
                  qir(1,wf_n-1,"nar_any qast_any") &&
                  qir(wf_n+1,bf_n-1,"nar_any prq_kred_mu qast_any prl_edmuim prl_mnim prl_kred_sr predik prl_edmutv mest_im mest_tv mest_3e") )
             { l[i]=omo2; Z[1109]++; if(dbg){print "Z1109"}; stopp=ret=1; continue };
             if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
                    q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
             { l[i]=omo2; Z[1110]++; if(dbg){print "Z1110"}; stopp=ret=1; continue };

       };};
       cst1="ведь же там тут";
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1111]++; if(dbg){print "Z1111", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) || vex(Y["tn_ok_vi"]) )
            { wf_n=bf_n=""; Z[1112]++; if(dbg){print "Z1112"}; };
       ...
       if ( q(1,"prl_edmuim prl_kred_mu") && ba(1,str_is_lock_prl) && s(0) &&
       ...
       if ( q(1,"nar_mest gl_aux_be prl_edmuim") &&
            q(2,"prl_edmuim prl_kred_mu") && ba(2,str_is_lock_prl) && s(0,1) &&
       ...
       if ( q(1,"nar_mest gl_aux_be") &&
            ...
            q(4,"sz_iili") &&
            q(5,"prl_edmutv prl_kred_mu") && ba(5,str_is_lock_prl) && s(0,4) )
       ...
       # true
          if ( qf(2,6,"sz_i") && qiz(1,qfn-1,"gl_aux_be qast_any nar_any prl_kred_sr muk_edmuim muk_edmutv","prl_edmuim prl_edmutv") &&
                q(qfn+1,"prl_edmuim prl_edmutv") && ba(qfn+1,str_is_lock_prl) && s(0,qfn) )
          { l[i]=omo2; Z[1129]++; if(dbg){print "Z1129"}; stopp=ret=1; continue };
          if ( q(1,"prl_edmuim prl_edmutv qast_any gl_aux_be") && qm(2,"prl_edmuim prl_edmutv",str_is_lock_prl) && Q_w(3,"suw_edvi suw_edim suw_edtv") &&
              (zs(0)||se(0," — ")) && zs(1) )
          { l[i]=omo2; Z[1124]++; if(dbg){print "Z1124"}; stopp=ret=1; continue };
          cst="сделать оставить делать оставлять";
          if ( bb(-3,-1,cst) ) { bb_n=bbn;
              if ( qf(1,3,"prl_edmutv") && ba(qfn,str_is_lock_prl) && s(bb_n,qfn-1) && Q_w(qfn+1,"suw_edtv") &&
    ...
    if ( iwrd == "замки" ) {
       ...
       if ( q(-1,"prl_mnim prl_kred_mu") && ba(-1,str_is_lock_prl) && s(-1) && !wa(-1,"X_lock_prl") )
       { l[i]=omo2; Z[1118]++; if(dbg){print "Z1118"}; stopp=ret=1; continue };

       if ( q(1,"prl_edmuim prl_mnim prl_kred_mu") && ba(1,str_is_lock_prl) && s(0) && Q(2,"isname suw_im") )
...
function FN_lock_prl_true_f(    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_lock_prl_true_f"]);
  while ( !stopp ) {
    # _lock_prl:escape

       if ( s(-1) && q(-1,"prl_edmu prl_edmuvi prl_mn prl_kred_mu") && ba(-1,"_lock_prl_true") )
       { l[i]=omo2; Z[1120]++; if(dbg){print "Z1120"}; stopp=ret=1; continue };
#      if ( s(0) && prl_any(1) && ba(1,"_lock_prl_true") )
#      { l[i]=omo2; Z[1121]++; if(dbg){print "Z1121"}; stopp=ret=1; continue };

       if ( iwrd == "замок") {
          if ( zs(-2) && s(-1) && qm(-2,"prl_edmuim prl_kred_mu","_lock_prl_true") && q(-1,"prl_edmuim mest_edmuvi qast_any gl_aux_be") )
          { l[i]=omo2; Z[1122]++; if(dbg){print "Z1122"}; stopp=ret=1; continue };
          if ( z(-3) && s(-2,-1) && qm(-3,"prl_edmuim","_lock_prl_true") && q(-2,"prl_edmuim prl_kred_sr nar_any sz_i") && q(-1,"prl_edmuim") )
          { l[i]=omo2; Z[1123]++; if(dbg){print "Z1123"}; stopp=ret=1; continue };
          if ( q(1,"prl_edmuim prl_edmutv qast_any gl_aux_be") && qm(2,"prl_edmuim prl_edmutv","_lock_prl_true") && Q_w(3,"suw_edvi suw_edim suw_edtv") &&
              (zs(0)||se(0," — ")) && zs(1) )
          { l[i]=omo2; Z[1124]++; if(dbg){print "Z1124"}; stopp=ret=1; continue };
          if ( qm(1,"prl_edmuim","_lock_prl_true") && (s(0)||se(0," — ")||sc(0,":")) && Q_w(2,"suw_edvi suw_edim suw_edtv") )
          { l[i]=omo2; Z[1125]++; if(dbg){print "Z1125"}; stopp=ret=1; continue };
          if ( qf(1,5,"prl_edmuim prl_edmutv prl_kred_mu") && qir(1,qfn-1,"gl_aux_be qast_any nar_any prl_kred_sr muk_edmuim") &&
               ba(qfn,"_lock_prl_true") && (zs(0)||se(0," — ")) && s(1,qfn-1) && Q_w(qfn+1,"suw_edvi suw_edim suw_edtv") )
          { l[i]=omo2; Z[1126]++; if(dbg){print "Z1126"}; stopp=ret=1; continue };
          if ( qb(1,5,"prl_edmuim prl_edmutv prl_kred_mu") && qir(1,qbn-1,"gl_aux_be qast_any nar_any prl_kred_sr prl_edmuim prl_edmutv muk_edmuim") &&
               ba(qbn,"_lock_prl_true") && (zs(0)||se(0," — ")) && s(1,qbn-1) && Q(qbn+1,"suw_edvi suw_edim suw_edtv") )
          { l[i]=omo2; Z[1127]++; if(dbg){print "Z1127"}; stopp=ret=1; continue };

          if ( q(1,"vvod") && z(0) && z(1) && qm(2,"prl_edmuim prl_kred_mu","_lock_prl_true") && Q_w(3,"suw_edvi suw_edim") )
          { l[i]=omo2; Z[1127]++; if(dbg){print "Z1127"}; stopp=ret=1; continue };
          if ( q(1,"nar_mest") && s(0) && q(2,"vvod") && z(1) && z(2) && qm(3,"prl_edmuim prl_kred_mu","_lock_prl_true") && Q_w(4,"suw_edvi suw_edim") )
          { l[i]=omo2; Z[1127]++; if(dbg){print "Z1127"}; stopp=ret=1; continue };
          if ( q(1,"nar_mest") && s(0) && q(2,"vvod") && z(1) && z(2) && q(3,"nar_spos") && qm(4,"prl_edmuim prl_kred_mu","_lock_prl_true") && Q_w(5,"suw_edvi suw_edim") )
          { l[i]=omo2; Z[1127]++; if(dbg){print "Z1127"}; stopp=ret=1; continue };

          if ( qf(1,5,"prl_edmuim prl_edmutv") && qir(1,qfn-1,"gl_aux_be qast_any nar_any prl_kred_sr") && z(qfn) &&
                q(qfn+1,"prl_edmuim prl_edmutv") && ba(qfn+1,"_lock_prl_true") && s(0,qfn-1) )
          { l[i]=omo2; Z[1128]++; if(dbg){print "Z1128"}; stopp=ret=1; continue };
          if ( qf(2,6,"sz_i") && qiz(1,qfn-1,"gl_aux_be qast_any nar_any prl_kred_sr muk_edmuim muk_edmutv","prl_edmuim prl_edmutv") &&
                q(qfn+1,"prl_edmuim prl_edmutv") && ba(qfn+1,"_lock_prl_true") && s(0,qfn) )
          { l[i]=omo2; Z[1129]++; if(dbg){print "Z1129"}; stopp=ret=1; continue };

          cst="казался оказался остается остался показался";
          if ( wf(1,3,cst) ) { wf_n=wfn;
              if ( qf(wf_n+1,wf_n+3,"prl_edmutv") && ba(qfn,"_lock_prl_true") && s(0,qfn-1) && Q_w(qfn+1,"suw_edtv") &&
                  qir(1,wf_n-1,"nar_any qast_any") && qir(wf_n+1,qfn-1,"nar_any qast_any") )
              { l[i]=omo2; Z[1130]++; if(dbg){print "Z1130"}; stopp=ret=1; continue };
              if ( qb(wf_n+1,wf_n+3,"prl_edmutv") && ba(qbn,"_lock_prl_true") && s(0,qbn-1) && Q_w(qfn+1,"suw_edtv") &&
              ...
              if ( qf(wf_n+1,wf_n+3,"prl_edmuim") && ba(qfn,"_lock_prl_true") && s(0,qfn-1) && Q_w(qfn+1,"suw_edvi suw_edim") &&
                  qir(1,wf_n-1,"nar_any qast_any") && qir(wf_n+1,qfn-1,"nar_any qast_any") )
              { l[i]=omo2; Z[1132]++; if(dbg){print "Z1132"}; stopp=ret=1; continue };
              if ( qb(wf_n+1,wf_n+3,"prl_edmuim") && ba(qbn,"_lock_prl_true") && s(0,qbn-1) && Q_w(qfn+1,"suw_edvi suw_edim") &&
          ...
          cst="сделать оставить делать оставлять";
          if ( bb(-3,-1,cst) ) { bb_n=bbn;
              if ( qf(1,3,"prl_edmutv") && ba(qfn,"_lock_prl_true") && s(bb_n,qfn-1) && Q_w(qfn+1,"suw_edtv") &&
                  qir(1,qfn-1,"nar_any qast_any") && qir(bb_n+1,-1,"nar_any qast_any mest_im suw_im isname prl_edmuvi mest_edmuvi mest_3e") )
              { l[i]=omo2; Z[1130]++; if(dbg){print "Z1130"}; stopp=ret=1; continue };
          };
       }; #замок
       if ( iwrd == "замка") {
          if ( s(-2,-1) && prl_edmuro(-2) && q(-1,"prl_edmuro") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1134]++; if(dbg){print "Z1134"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замку") {
          if ( s(-2,-1) && prl_edmuda(-2) && q(-1,"prl_edmuda") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1135]++; if(dbg){print "Z1135"}; stopp=ret=1; continue };
          if ( q(1,"prl_edmuda") && (s(0)||se(0," — ")||sc(0,":")) && Q_w(2,"suw_edda suw_edda") )
          { l[i]=omo2; Z[1136]++; if(dbg){print "Z1136"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замком") {
          if ( s(-2,-1) && prl_edmutv(-2) && q(-1,"prl_edmutv") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1137]++; if(dbg){print "Z1137"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замке") {
          if ( s(-2,-1) && prl_edmupr(-2) && q(-1,"prl_edmupr") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1138]++; if(dbg){print "Z1138"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замки") {
          if ( s(-2,-1) && prl_mnim(-2) && q(-1,"prl_mnim") && ba(-2,"_lock_prl_true") )
          ...
          if ( q(1,"prl_mnim prl_mntv qast_any") && q(2,"prl_mnim prl_mntv") &&
              ba(2,"_lock_prl_true") && (zs(0)||se(0," — ")) && zs(1) )
          ...
          if ( qb(1,5,"prl_mnim prl_mntv prl_krmn") && qir(1,qbn-1,"gl_aux_be qast_any nar_any prl_krmn muk_mnim") &&
               ba(qbn,"_lock_prl_true") && (zs(0)||se(0," — ")) && s(1,qbn-1) && Q(qbn+1,"suw_mnim suw_mntv") )
          { l[i]=omo2; Z[1142]++; if(dbg){print "Z1142"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замков") {
          if ( s(-2,-1) && prl_mnro(-2) && q(-1,"prl_mnro") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1143]++; if(dbg){print "Z1143"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замкам") {
          if ( s(-2,-1) && prl_mnda(-2) && q(-1,"prl_mnda") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1144]++; if(dbg){print "Z1144"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замками") {
          if ( s(-2,-1) && prl_mntv(-2) && q(-1,"prl_mntv") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1145]++; if(dbg){print "Z1145"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замках") {
          if ( s(-2,-1) && prl_mnpr(-2) && q(-1,"prl_mnpr") && ba(-2,"_lock_prl_true") )
          { l[i]=omo2; Z[1146]++; if(dbg){print "Z1146"}; stopp=ret=1; continue };
       };
    # _lock_prl:end
break}; Y["FN_lock_prl_true_f"]=1; return ret }
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1147]++; if(dbg){print "Z1147"}; }; #check_!под
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1148]++; if(dbg){print "Z1148"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[1149]++; if(dbg){print "Z1149"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
