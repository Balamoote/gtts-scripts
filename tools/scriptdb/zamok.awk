...
#{ l[i]=omo1; Z[42]++; if(dbg){print "Z42"}; continue };
 ...
 { l[i]=omo1; Z[46]++; if(dbg){print "Z46"}; stopp=ret=1; continue };
 ...
 if ( vgl(Y["n+_castlelike_things"],1,7) ) { bf_n=Y["n+_castlelike_things"]; wf_n=Y["n-_lock_pe"];
    ...
    if ( !bw(wf_n,cst0)  ) {
      ...
      { Z[55]++; if(dbg){print "Z55"}; continue };
    };
 ...
 if ( veq(Y["iwrd_case"],"ro") ) {
    ...
    { Z[142]++; if(dbg){print "Z142", "-- TN_vnutri_zamka_f"}; continue};
    if ( vex(Y["tn_ok_vokrug"]) && TN_vokrug_zamka_f() )
    { Z[142]++; if(dbg){print "Z142", "-- TN_vokrug_zamka_f"}; continue};
    if ( vex(Y["tn_ok_u"]) && TN_u_zamka_f() )
    { Z[142]++; if(dbg){print "Z142", "-- TN_u_zamka_f"}; continue};
 };
 # suw_ro escape:end

 if ( FN_zamok_suwro_f() )
 { Z[143]++; if(dbg){print "Z143", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( w(1,"из") ) { wf_n=1;
    ...
    { Z[152]++; if(dbg){print "Z152", "-- TN_na_zamok_f"}; continue};
...
#     { wf_n=bf_n=""; Z[157]++; if(dbg){print "Z157"}; };
#     if ( wa(1,"_lock_suw-ro") && s(0) )
#     { wf_n=bf_n=""; Z[158]++; if(dbg){print "Z158"}; };
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[159]++; if(dbg){print "Z159"}; };
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[160]++; if(dbg){print "Z160", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[161]++; if(dbg){print "Z161", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[162]++; if(dbg){print "Z162", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[167]++; if(dbg){print "Z167"}; };
      ...
      cst0="гермодвери двери";
       ...
       { Z[168]++; if(dbg){print "Z168", "-- TN_v_zamok_f"}; continue };
...
#   { Z[175]++; if(dbg){print "Z175"}; continue };
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
     ...
     { Z[284]++; if(dbg){print "Z284", "-- TN_na_zamok_f"}; continue };
 ...
 if ( wf(1,4,"с со") ) { wf_n=wfn;
   ...
   if ( wfa(wf_n+1,wf_n+4,"_lock_s_suw-tv") ) { bf_n=wfn;
      ...
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[305]++; if(dbg){print "Z305", "-- TN_v_zamok_f"}; continue };
...
#   { l[i]=omo2; Z[313]++; if(dbg){print "Z313"}; continue };
 ...
 { l[i]=omo1; Z[316]++; if(dbg){print "Z316"}; continue };
  ...
  ######################################################################################################
    ...
    { Z[317]++; if(dbg){print "Z317", "-- FN_castle_prl_f(\"_castle_prl\")"}; continue };
    ...
    { Z[319]++; if(dbg){print "Z319", "-- FN_lock_prl_f(\"_lock_prl\")"}; continue };
...
#      { l[i]=omo1; Z[362]++; if(dbg){print "Z362"}; continue };
  ...
  if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"];
    ...
    if ( TN_v_zamok_f() )
    { Z[366]++; if(dbg){print "Z366", "-- TN_v_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_na_ist"],-8,-1) ) { wb_n=Y["n_pre_na_ist"];
    ...
    if ( TN_na_zamok_f() )
    { Z[368]++; if(dbg){print "Z368", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[371]++; if(dbg){print "Z371", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[372]++; if(dbg){print "Z372", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[373]++; if(dbg){print "Z373", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[374]++; if(dbg){print "Z374", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[375]++; if(dbg){print "Z375", "-- TN_qerez_zamok_f"}; continue };
...
#   if ( TN_do_zamka_f() )
#   { Z[537]++; if(dbg){print "Z537", "-- TN_do_zamka_f"}; continue };
#   if ( TN_ot_zamka_f() )
#   { Z[538]++; if(dbg){print "Z538", "-- TN_ot_zamka_f"}; continue };
#   if ( TN_iz_zamka_f() )
#   { Z[539]++; if(dbg){print "Z539", "-- TN_iz_zamka_f"}; continue };
 ...
 #
  #
    if ( TN_k_zamku_f() )
    { Z[547]++; if(dbg){print "Z547", "-- TN_k_zamku_f"}; continue };
...
#   { l[i]=omo2; Z[549]++; if(dbg){print "Z549"}; continue };
 ...
 #
    ...
    if ( vex(Y["tn_ok_pod_tv"]) && TN_pod_zamkom_f() )
    { Z[553]++; if(dbg){print "Z553", "-- TN_pod_zamkom_f"}; continue};
 ...
 #
    ...
    { Z[564]++; if(dbg){print "Z564", "-- TN_v_zamke_f"}; continue};
 ...
 { Z[596]++; if(dbg){print "Z596", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[597]++; if(dbg){print "Z597", "-- FN_lock_prl_f(\"_lock_prl_after\")"}; continue };
...
function DEF_TN_PREVI_ZAMOK(num,el,    fufu) { # определение компонентов между "pre_vi" и "замок"
    ...
    { tn_ok = 1; Z[611]++; if(dbg){print "Z611", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[612]++; if(dbg){print "Z612", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[613]++; if(dbg){print "Z613", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
    ...
    { tn = xsn; Z[622]++; if(dbg){print "Z622", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vex(tn_ok) && z(tn-1) && w(tn+1,"который") )
    { tn_ok = ""; Z[623]++; if(dbg){print "Z623", "-- tn_ok OFF", el, "замок"};};
    ...
    { tn_ok = ""; Z[624]++; if(dbg){print "Z624", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[625]++; if(dbg){print "Z625", "-- tn_ok OFF", el, "замок"};};
}
...
function DEF_TN_PREDA_ZAMKU(num,el,    fufu) { # определение компонентов между "pre_da" и "замку"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[626]++; if(dbg){print "Z626", "-- tn_ok ON", el, "замку"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[627]++; if(dbg){print "Z627", "-- tn_ok MOV", el, "замку"};};
}
...
function DEF_TN_PRETV_ZAMKOM(num,el,    fufu) { # определение компонентов между "pre_tv" и "замком"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[628]++; if(dbg){print "Z628", "-- tn_ok ON", el, "замком"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[629]++; if(dbg){print "Z629", "-- tn_ok MOV", el, "замком"};};
}
...
function DEF_TN_PREPR_ZAMKE(num,el) { # определение компонентов между "pre_pr" и "замке"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[630]++; if(dbg){print "Z630", "-- tn_ok ON", el, "замке"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[631]++; if(dbg){print "Z631", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[632]++; if(dbg){print "Z632", "-- tn_ok ON", el, "замке"};};
    ...
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[636]++; if(dbg){print "Z636", "-- tn_ok MOV", el, "замке"};};
}
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1047]++; if(dbg){print "Z1047", "-- tn_ok MOV", el, "замка"};};
}
...
function FN_lock_prl_f(str_lock,    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp_str="FN" str_lock "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1114]++; if(dbg){print "Z1114", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };
    ...
    # _lock_prl:start
       ...
       if ( iwrd == "замок") {
          ...
          if ( vex(Y["tn_ok_v"]) && qb(Y["n_pre_v"]+1,-1,"prl_im") && !wa(qbn,"_lock_prl_true") ) {
            ...
            { Z[1115]++; if(dbg){print "Z1115", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
...
function FN_hw_zamka_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,QB_n,QF_n,wb_n,wf_n, fufu) { stopp=v2s(Y["FN_hw_zamka_f"]);
  while ( !stopp ) {
    ...
    { fufu = 1; Z[1156]++; if(dbg){print "Z1156","hw_castle < hw_lock"}; };
    if ( vex(qf_n) && vlt(qf_n,-1) && bba(qf_n+1,-1,"_castle_prl") )
    { fufu = 2; Z[1157]++; if(dbg){print "Z1157","hw_castle < _lock_prl_true"}; };
    if ( vex(qb_n) && vlt(qb_n,-1) && bba(qb_n+1,-1,"_lock_prl_true") )
    { fufu = 1; Z[1158]++; if(dbg){print "Z1158","hw_castle < _lock_prl_true"}; };
#   if ( w(1,"на") &&
#      wfa(2,4,"_lock_na_suw-pr") && s(0,wfn-1) )
#   { fufu = 1; Z[1159]++; if(dbg){print "Z1159","hw_castle > _lock_na_suw-pr"}; };

    cst="глубина";
    if ( weq(QB_n,cst) && qxs(-1,"на","всю","глубину") )
    { l[i]=omo2; Z[1160]++; if(dbg){print "Z1160"}; stopp=ret=1; continue };
    cst="шум скрип звук шорох щель";
 ...
 #===============================================================================================
  ...
  if ( vex(Y["n+_lock_pe"]) ) { ef_n=Y["n+_lock_pe"]; EF_n=Y["w+_lock_pe"];
   ...
   if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       if ( veq(ef_n,1) &&
             bw(-1,cst3) && s(-1,0) )
       { l[i]=omo2; Z[1234]++; if(dbg){print "Z1234"}; stopp=ret=1; continue };
       if ( z(ef_n) &&
...
#      { l[i]=omo1; Z[382]++; if(dbg){print "Z382"}; continue };
         ...
         { l[i]=omo1; Z[378]++; if(dbg){print "Z378"}; stopp=ret=1; continue };
   ...
   if ( weq(EB_n,cst) && veq(lvl,0) ) { wb_n=Y["n-_castlelike_things"];
     ...
     if (  vgl(eb_n,-4,-1) && vlt(wb_n,eb_n) && !sos(wb_n,-1) &&
           ...
           qir(eb_n+1,-1,"prl_vi mest_vi mest_3e prq_vi") )
     { l[i]=omo1; Z[1293]++; if(dbg){print "Z1293"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) && s(eb_n+1,-1) &&
          w(eb_n+1,cst1) &&
        qir(eb_n+2,-1,"prl_edmuvi mest_edmuvi mest_3e") )
   { l[i]=omo2; Z[1309]++; if(dbg){print "Z1309"}; stopp=ret=1; continue };
...
#   { l[i]=omo2; Z[1329]++; if(dbg){print "Z1329"}; stopp=ret=1; continue };
  ...
  if ( vex(Y["n+_gl_zamok"]) ) { ef_n=Y["n+_gl_zamok"]; EF_n=Y["w+_gl_zamok"];
      ...
      if ( wf(ef_n+1,ef_n+3,"в во на") ) { wf_n=wfn;
        if ( wf(wf_n+1,wf_n+4,cst2) ) { bf_n=wfn;
          if ( wf(bf_n+1,bf_n+4,cst3) ) { qf_n=wfn;
           ...
           { l[i]=omo1; Z[376]++; if(dbg){print "Z376"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) && veq(lvl,5) ) {
      ...
      { l[i]=omo1; Z[381]++; if(dbg){print "Z381"}; stopp=ret=1; continue };
    };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      ...
      cst2="крепким простым сложным надежным";
      if ( qxs(ef_n+1,"довольно достаточно",cst2) && !ba(-1,"_castle_prl")  )
      { l[i]=omo2; Z[1346]++; if(dbg){print "Z1346"}; stopp=ret=1; continue };
      cst2="странный чудной";
      if ( qxs(1,"на","ней нем") && veq(ef_n,xsn+1) &&
            wf(ef_n+1,ef_n+3,cst2) && s(0,wfn-1) &&
           qir(ef_n+1,wfn-1,"mest_vi prl_vi") )
      { l[i]=omo2; Z[1347]++; if(dbg){print "Z1347"}; stopp=ret=1; continue };
    };
...
#   cst1="бедным безжизненным большим величественным мрачным";
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      ...
      { l[i]=omo1; Z[1367]++; if(dbg){print "Z1367"}; stopp=ret=1; continue };
    };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      ...
      if ( q(1,"suw_da mest_da") &&
           w(2,"не") &&
         veq(ef_n,3) && s(0,2) )
      { l[i]=omo1; Z[1375]++; if(dbg){print "Z1375"}; stopp=ret=1; continue };
      if ( veq(ef_n,1) &&
             q(2,"suw_ro mest_ro mest_3e") &&
             w(3,"к ко") && s(0,2) )
      { l[i]=omo1; Z[1376]++; if(dbg){print "Z1376"}; stopp=ret=1; continue };
   ...
   if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       if ( veq(ef_n,1) &&
             bw(-1,cst3) && s(-1,0) )
       { l[i]=omo2; Z[1380]++; if(dbg){print "Z1380"}; stopp=ret=1; continue };
       if ( z(ef_n) &&
          qxw(ef_n+1,"для","освобождения очистки","места земли территории") )
   ...
   if ( weq(EF_n,cst) && !wa(-1,"_lock_prl_true") ) {fufu="";
     ...
     { fufu=1; Z[1382]++; if(dbg){print "Z1382"}; };
     if ( vex(Y["n-_gl_v_lock-iv"]) && vex(Y["tn_ok_v"]) )
     { fufu=1; Z[1383]++; if(dbg){print "Z1383"}; };
   ...
   if ( weq(EF_n,cst) && veq(lvl,0) ) {
    ...
    { l[i]=omo1; Z[1393]++; if(dbg){print "Z1393"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) && veq(lvl,0) ) {
     ...
     if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       if ( w(ef_n+1,"на") &&
          ...
          qir(ef_n+2,wfn-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo2; Z[1408]++; if(dbg){print "Z1408"}; stopp=ret=1; continue };
    };
     ...
     if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       { l[i]=omo2; Z[1409]++; if(dbg){print "Z1409"}; stopp=ret=1; continue };
     };
     ...
     if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       { l[i]=omo2; Z[1410]++; if(dbg){print "Z1410"}; stopp=ret=1; continue };
     };

     cst="представлять";
     if ( weq(EF_n,cst) && w(ef_n+1,"собой собою") ) {
       if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[1411]++; if(dbg){print "Z1411", "-- TN_v_zamok_f"}; stopp=ret=1; continue};
    ...
    if ( vex(Y["n+_castlelike_things"]) ) { qf_n=Y["n+_castlelike_things"];
      ...
      { l[i]=omo1; Z[1413]++; if(dbg){print "Z1413"}; stopp=ret=1; continue };
    ...
    if ( vex(Y["n+_locklike_things"]) ) { qf_n=Y["n+_locklike_things"];
      ...
      { l[i]=omo2; Z[1414]++; if(dbg){print "Z1414"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      ...
      { l[i]=omo2; Z[1415]++; if(dbg){print "Z1415"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      if ( veq(ef_n,3) && s(0,2) &&
             w(1,"с со") &&
             q(2,"suw_ro") )
      { l[i]=omo2; Z[1418]++; if(dbg){print "Z1418"}; stopp=ret=1; continue };
      if ( veq(ef_n,4) && s(0,3) &&
             w(1,"с со") &&
             q(2,"suw_ro") &&
...
#    if ( weq(EF_n,cst8) ) { if ( s(ef_n,3) && bf(ef_n+1,4,cst) ) ef_n=bfn };
     ...
     if ( bfa(ef_n+1,ef_n+5,"_castlelike_things")||
        ...
        ( bfa(ef_n+1,ef_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) { if (bfn) qb_n=bfn; if (wfn) qb_n=wfn;
         ...
         { qb_n=""; Z[1424]++; if(dbg){print "Z1424"}; };
         cst="ключом";
         if ( zs(0) && s(1,qb_n-1) &&
               w(qb_n,cst) && !w(qb_n+1,"к") )
         { qb_n=""; Z[1425]++; if(dbg){print "Z1425"}; };
   ...
   if ( qf(ef_n+1,ef_n+4,"prl_kred_mu prq_kred_mu") ) { qf_n=qfn;
       ...
       if ( w(qf_n+1,"от") &&
          ...
          qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
          qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       { l[i]=omo2; Z[1431]++; if(dbg){print "Z1431"}; stopp=ret=1; continue };
       if ( w(qf_n+1,"с со") &&
           wf(qf_n+2,qf_n+5,cst4) && s(0,wfn-1) &&
          qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
          qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       { l[i]=omo2; Z[1432]++; if(dbg){print "Z1432"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
       ...
       if ( vgt(wb_n,ef_n)||wf(ef_n+1,ef_n+5,cst2) ) { if (wb_n) qf_n=wb_n; if (wfn) qf_n=wfn;
          ...
          if ( vgl(ef_n,1,3) && zs(0) && s(1,qf_n-1) &&
               vex(Y["tn_ok_vi"]) )
          { qf_n=""; Z[1433]++; if(dbg){print "Z1433"}; };
...
#           bfa(wfn+1,wfn+4,"_locklike_things") && s(0,bfn-1) &&
       ...
       { l[i]=omo2; Z[1436]++; if(dbg){print "Z1436"}; stopp=ret=1; continue };
       ...
       { l[i]=omo2; Z[1437]++; if(dbg){print "Z1437"}; stopp=ret=1; continue };
       if ( zs(0) && s(1,qf_n-1) &&
             q(qf_n,"suw_ro") &&
           qir(1,ef_n-1,"otsz_any mod_any gl_aux_be qast nar_any suw_ro isname") &&
       ...
       { l[i]=omo2; Z[1438]++; if(dbg){print "Z1438"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) && veq(lvl,0) ) {
      if ( q(ef_n,"prq_kred_mu") && wf(ef_n+1,ef_n+4,cst2) ) { qf_n=wfn;
        ...
        if ( s(0,qf_n-1) &&
           qir(1,ef_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
           qir(ef_n+1,qf_n-1,"prl_tv prq_tv mest_tv mest_3e") )
        { l[i]=omo1; Z[1440]++; if(dbg){print "Z1440"}; stopp=ret=1; continue };
  ...
  if ( weq(EF_n,cst) && veq(lvl,0) ) {
    ...
    { l[i]=omo2; Z[1442]++; if(dbg){print "Z1442"}; stopp=ret=1; continue };
