...
#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue
 ...
 if ( prl_any(-1) && ba(-1,"_lock_forced") && s(-1) )
 ...
 if ( q(-1,"isname") && q(0,"isname") && zs(-1) && zs(-2) &&
      Q(-1,"pre_any") &&
...
sz_iili(-2) &&
 ...
 if ( (qxs(-1,"а но","не и")||
       qxs(-1,"а но","также","и")||
       qxs(-1,"как так","и") ) &&
        ba(xsn-1,"_locklike_things") && z(xsn-1) )
...
#    qir(wfn+1,-1,"gl_need gl_vzed gl_aux_be prl_vi mest_vi mest_3e prq_vi") &&
#      s(wbn,-1) && sv(-1,"\\(") && Q_w(1,"isname") )
#{ l[i]=omo2; Z[122]++; if(dbg){print "Z122"}; continue }; #edit по "ниша, в которой ... замок"
 # _lock_v_suw-pr:end
...
sz_iili(-3) &&
 ...
 { l[i]=omo1; Z[201]++; if(dbg){print "Z201"}; continue }; #check_!под
 ...
 { l[i]=omo1; Z[232]++; if(dbg){print "Z232",qfn}; continue };
 if ( w(-2,"с со") &&
 ...
 if(iwrd=="замок" || iwrd=="замки") {
  ...
  if ( wb(-5,-1,"в во") && s(wbn,-1) ) {
   ...
   if ( vis(tn_ok) ) {
    ...
    { l[i]=omo2; Z[327]++; if(dbg){print "Z327"}; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
 ...
 narph_any(2) &&
    ...
    { l[i]=omo1; Z[560]++; if(dbg){print "Z560"}; continue }; # без tn!
    if ( ba(-3,"_gl_v_castle-iv") &&
          w(-2,"в") &&
          q(-1,"mest_vip3e mest_vi") && s(-3,-1) )
    { l[i]=omo1; Z[561]++; if(dbg){print "Z561"}; continue }; # без tn!
  ...
  if ( wb(-5,-1,"на") && s(wbn,-1) ) {
   ...
   if ( vis(tn_ok) ) {
    ...
    if ( bfa(1,3,"_gl_na_castle") && s(0,bfn-1) &&
          Xw(bfn,"X_castle_pe") && Qf(0,bfn-1,"sz_iili pre_any") )
...
#   { l[i]=omo2; Z[593]++; if(dbg){print "Z593"}; continue };
    ...
    if ( qy(tn-1,"sz_iili") &&
        ...
        qir(wbn+1,tn-2-qyn,"prl_ro mest_vi mest_3e prq_vi") )
...
#   { l[i]=omo2; Z[629]++; if(dbg){print "Z629"}; continue };
  ...
  if ( bf_n ) {
    ...
    { l[i]=omo1; Z[776]++; if(dbg){print "Z776"}; continue };
    if ( w(-2,cst2) && s(-2,-1) &&
        bw(-1,cst1) )
    ...
    if ( z(0) && !((bba(-4,-1,"_lock_pe") || bba(-4,-1,"_gl_v_lock-iv")) && s(bbn,-1)) &&
       bfa(2,6,"_castle_pe") && qir(1,bfn-1,"otsz_mu nar_any mest_any qast mod_any isname") &&
 ...
 #
  ...
  #
    ...
    if ( ba(-2,"_castle_prl") && prl_da(-2) &&
          q(-1,"prl_any prq_any mest_any qast") && s(-2,-1)  )
    { l[i]=omo1; Z[918]++; if(dbg){print "Z918"}; continue };
    ...
    { l[i]=omo2; Z[921]++; if(dbg){print "Z921"}; continue };
...
#   # _castle_lock_after:start
 ...
 #
    ...
    { l[i]=omo2; Z[938]++; if(dbg){print "Z938"}; continue };
    if ( ba(-2,"_lock_prl") && prl_tv(-2) &&
          Q(-1,"suw_any sz_iili") && s(-2,-1) )
    { l[i]=omo2; Z[939]++; if(dbg){print "Z939"}; continue };
    if ( ba(-3,"_lock_prl") && prl_tv(-3) &&
          q(-2,"prl_any prq_any mest_any qast") && W(-2,"как") &&
          q(-1,"prl_any prq_any mest_any qast") && s(-3,-1)  )
    { l[i]=omo2; Z[940]++; if(dbg){print "Z940"}; continue };
...
#   # _castle_lock_after:start
 ...
 #
    ...
    { l[i]=omo1; Z[950]++; if(dbg){print "Z950"}; continue };
  ...
  # _suw_v_castle-vp:end
    ...
    # _lock_prl:start
    if ( ba(-1,"_lock_prl") && s(-1) && prl_da(-1) )
    { l[i]=omo2; Z[960]++; if(dbg){print "Z960"}; continue };
    if ( ba(-2,"_lock_prl") && prl_da(-2) &&
          Q(-1,"suw_any sz_iili") && s(-2,-1) )
    ...
    if ( ba(-3,"_lock_prl") && prl_da(-3) &&
          q(-2,"prl_any prq_any mest_any qast") && W(-2,"как") &&
          q(-1,"prl_any prq_any mest_any qast") && s(-3,-1)  )
    { l[i]=omo2; Z[962]++; if(dbg){print "Z962"}; continue };
    if ( ba(1,"_lock_prl") && s(0) && prl_da(1) && !isname(1) )
...
#   # _castle_lock_after:start
 ...
 { l[i]=omo2; Z[981]++; if(dbg){print "Z981"}; continue };
 if ( z(0) && q(1,"prq_any") && ba(1,"_lock_pe") && s(1) )
 { l[i]=omo2; Z[982]++; if(dbg){print "Z982"}; continue };
 if ( q(-1,"prq_any") && ba(-1,"_lock_pe") && s(-1) )
 { l[i]=omo2; Z[983]++; if(dbg){print "Z983"}; continue };
 # _lock_pe:end
 ...
 { l[i]=omo1; Z[984]++; if(dbg){print "Z984"}; continue };
...
# escape:end
 ...
 if ( vgle(wb_n,-6,-2) && vgle(bb_n,wb_n+1,-1) &&
       qir(wb_n+1,bb_n-1,"prl_pr mest_pr mest_3e prq_pr") && s(wb_n,bb_n-1) &&
       qia(bb_n+1,-1,"qast","prl_any mest_any nar_any suw_ro isname") && ds(bb_n) && s(bb_n+1,-1) )
 { l[i]=omo1; Z[986]++; if(dbg){print "Z986"}; continue };
 ...
 { l[i]=omo1; Z[987]++; if(dbg){print "Z987"}; continue };
 # похожий
 ...
 { l[i]=omo2; Z[988]++; if(dbg){print "Z988"}; continue };
 if ( ba(-2,"_lock_na_suw") && se(-2," — ") &&
       w(-1,"на") && s(-1) )
 { l[i]=omo2; Z[989]++; if(dbg){print "Z989"}; continue };
 if ( w(1,"на") &&
    bfa(2,5,"_lock_na_suw") && s(0,bfn-1) &&
    qir(2,bfn-1,"prl_vi prl_pr mest_vi mest_pr prq_vi prq_pr") )
 { l[i]=omo2; Z[990]++; if(dbg){print "Z990"}; continue }; #check ислючения "в"
 if ( w(1,"у") && Ww_(-1,"в") &&
      q(2,"suw_vi mest_vi isname") &&
      w(3,"на") &&
    qir(4,5,"prl_vi prl_pr mest_vi mest_pr mest_3e prq_vi prq_pr") &&
    bfa(4,7,"_lock_na_suw") && s(0,bfn-1) )
 { l[i]=omo2; Z[991]++; if(dbg){print "Z991"}; continue };
 ...
 if ( z(0) && w(1,"где") )
 { l[i]=omo1; Z[995]++; if(dbg){print "Z995"}; continue };
 ...
 if ( gist(-4,2) &&
         ...
         w(-1,"а но и или") && s(0) )
 { l[i]=omo2; Z[998]++; if(dbg){print "Z998"}; continue };
 ...
 if ( (qxs(1,"де","ла")||
       qxs(1,"фон","дер")) &&
    isname(xsn+1) && s(xsn) )
 { l[i]=omo1; Z[1001]++; if(dbg){print "Z1001"}; continue };
 if ( w(1,"д л") && sc(1,"[\x27’]") &&
 isname(2) && s(0) )
 { l[i]=omo1; Z[1002]++; if(dbg){print "Z1002"}; continue };
 if ( q(1,"pre_vi pre_ro pre_pr") &&
 isname(2) && s(0,1) && !(bb(-4,-1,"ключ ключик")) )
 { l[i]=omo1; Z[1003]++; if(dbg){print "Z1003"}; continue };
 if ( isname(-1) && ( (s(-2,-1) && sv(-2,")"))||(z(-2) && s(-1)) ) )
 { l[i]=omo1; Z[1004]++; if(dbg){print "Z1004"}; continue };
 if ( isname(0) && s(-1) )
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[1006]++; if(dbg){print "Z1006", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_vi") )
    { tn_ok = 1; Z[1007]++; if(dbg){print "Z1007", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[1008]++; if(dbg){print "Z1008", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[1009]++; if(dbg){print "Z1009", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[1010]++; if(dbg){print "Z1010", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[1011]++; if(dbg){print "Z1011", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vis(tn_ok) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1012]++; if(dbg){print "Z1012", "-- tn_ok ON", el, "замок"};};
    if ( !vis(tn_ok) && veq(tn,-2) && wc(-1,"ский$") )
    { tn_ok = 1; Z[1013]++; if(dbg){print "Z1013", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vis(tn_ok) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"даже") ) )
    { tn = xsn; Z[1014]++; if(dbg){print "Z1014", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vis(tn_ok) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[1015]++; if(dbg){print "Z1015", "-- tn_ok OFF", el, "замок"};};
    if ( vis(tn_ok) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[1016]++; if(dbg){print "Z1016", "-- tn_ok OFF", el, "замок"};};
}
