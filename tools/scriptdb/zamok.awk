...
#
 ...
 if ( vgl(Y["n_pre_na"],-8,-2) ) { wb_n=Y["n_pre_na"];
   if ( wfa(wb_n+1,-1,"_lock_na_suw-vi") ) { bf_n=wfn;
      if ( vgl(Y["n-_castle_pe"],wfn+1,-1) )
      { bf_n=""; Z[252]++; if(dbg){print "Z252"}; };
#       cst0="держать";
#       if ( bb(wb_n-4,wb_n-1,cst0) && s(bbn,-1) &&
#           mwba(bf_n,bbn,"M_lock_na_suw-vi_gl_castle_pe") )
#       { wb_n=bf_n=""; Z[253]++; if(dbg){print "Z253"}; };
...
#       { Z[257]++; if(dbg){print "Z257"}; continue };
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[282]++; if(dbg){print "Z282", "-- TN_na_zamok_f"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_lock_s_suw-tv") ) { bf_n=wfn;
      ...
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[293]++; if(dbg){print "Z293", "-- TN_v_zamok_f"}; continue };
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[315]++; if(dbg){print "Z315"}; };
    ...
    if ( bb(-5,-1,cst) ) { wb_n=bbn;
      ...
      if ( wba(wb_n-5,wb_n-1,"_lock_na_suw-pr") ) { bb_n=wbn;
       ...
       { l[i]=omo2; Z[324]++; if(dbg){print "Z324"}; continue };#urhere
    };};
  ...
  ######################################################################################################
    ...
    { Z[341]++; if(dbg){print "Z341", "-- FN_castle_prl_f"}; continue };

    # основные прилагательные -- lock
    if ( FN_lock_prl_f("_lock_prl") )
    { Z[342]++; if(dbg){print "Z342", "-- FN_lock_prl_f"}; continue };
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[343]++; if(dbg){print "Z343", "-- TN_v_zamok_f"}; continue };
    ...
    { l[i]=omo1; Z[345]++; if(dbg){print "Z345"}; continue }; # без tn!
    if ( ba(-3,"_gl_v_castle-iv") &&
          w(-2,"в") &&
          q(-1,"mest_vip3e mest_vi") && s(-3,-1) )
    { l[i]=omo1; Z[346]++; if(dbg){print "Z346"}; continue }; # без tn!
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[347]++; if(dbg){print "Z347", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[358]++; if(dbg){print "Z358", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[359]++; if(dbg){print "Z359", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[360]++; if(dbg){print "Z360", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[361]++; if(dbg){print "Z361", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[362]++; if(dbg){print "Z362", "-- TN_qerez_zamok_f"}; continue };
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[501]++; if(dbg){print "Z501"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_da") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[513]++; if(dbg){print "Z513"}; continue }; #check
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
               w(1,"на") &&
             wfa(2,5,"_lock_na_suw-vi") && s(0,wfn-1) &&
             qir(2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") )
        { l[i]=omo2; Z[514]++; if(dbg){print "Z514"}; continue }; #check
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[525]++; if(dbg){print "Z525"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[558]++; if(dbg){print "Z558"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"];
       ...
       { bf_n=""; Z[582]++; if(dbg){print "Z582"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[583]++; if(dbg){print "Z583"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 if ( w(-1,"самый") && W_w(-2,"тот этот") && s(-1) )
 ...
 if ( wb(-5,-2,"самый") )  { wb_n=wbn;
    ...
    if ( veq(wb_n,-1) && W_w(-2,"тот этот") && s(-1) )
    { wb_n=""; Z[605]++; if(dbg){print "Z605"}; };
    if ( veq(wb_n,-1) && qxs(-2,"тот этот","же") && s(-1) )
    { wb_n=""; Z[606]++; if(dbg){print "Z606"}; };
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[618]++; if(dbg){print "Z618", "-- TN_do_zamka_f"}; continue };
 ...
 if ( bfa(1,5,"_lock_parts") ) { bf_n=bfn;
    ...
    { bf_n=""; Z[642]++; if(dbg){print "Z642"}; };
    if ( vgl(bf_n,1,5) && s(1,bf_n-1) && (s(0)||se(0," — ")) && q(-1,"isname") && p(-2) )
    { bf_n=""; Z[643]++; if(dbg){print "Z643"}; };
 ...
 { l[i]=omo2; Z[649]++; if(dbg){print "Z649"}; continue };
 ...
 { Z[664]++; if(dbg){print "Z664", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[665]++; if(dbg){print "Z665", "-- FN_lock_prl_after_f"}; continue };
...
# };
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[673]++; if(dbg){print "Z673"}; continue } else
...
#   { tn_ok=fufu=1; Z[693]++; if(dbg){print "Z693", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-4) &&
#           q(-3,"prq_vi") && q(-2,"suw_vi") && q(-1,"prl_vi mest_vi") )
#   { tn_ok=fufu=1; Z[694]++; if(dbg){print "Z694", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[695]++; if(dbg){print "Z695", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[697]++; if(dbg){print "Z697", "-- tn_ok OFF", el, "замок"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    { l[i]=omo2; Z[725]++; if(dbg){print "Z725"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[726]++; if(dbg){print "Z726"}; stopp=ret=1; continue }; #edit
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[767]++; if(dbg){print "Z767"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[769]++; if(dbg){print "Z769"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[775]++; if(dbg){print "Z775"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[781]++; if(dbg){print "Z781"}; };
...
!(narph_any(bbn)||w(bbn-1,"в на")) )
    ...
    { l[i]=omo2; Z[815]++; if(dbg){print "Z815"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[816]++; if(dbg){print "Z816"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst3) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[851]++; if(dbg){print "Z851"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[852]++; if(dbg){print "Z852"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[925]++; if(dbg){print "Z925"}; stopp=ret=1; continue };
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[944]++; if(dbg){print "Z944"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[954]++; if(dbg){print "Z954", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1084]++; if(dbg){print "Z1084", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) && wy(-1,"же") &&
#         qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
#   { tn_ok = 1; Z[1085]++; if(dbg){print "Z1085", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-1,"suw_tv mest_tv") )
#   { tn_ok = 1; Z[1086]++; if(dbg){print "Z1086", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-2,"suw_tv qik_tv") &&
#           q(-1,cst3) )
#   { tn_ok = 1; Z[1087]++; if(dbg){print "Z1087", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_vi") &&
#         qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
#           q(-1,"mest_vip3e suw_vi") )
#   { tn_ok = 1; Z[1088]++; if(dbg){print "Z1088", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_tv") &&
#         qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
#           q(-1,"suw_tv") )
#   { tn_ok = 1; Z[1089]++; if(dbg){print "Z1089", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1090]++; if(dbg){print "Z1090", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1091]++; if(dbg){print "Z1091", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1092]++; if(dbg){print "Z1092", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
    ...
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1095]++; if(dbg){print "Z1095", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1096]++; if(dbg){print "Z1096", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1097]++; if(dbg){print "Z1097", "-- tn_ok OFF", el, "замка"};};
}
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1102]++; if(dbg){print "Z1102"}; stopp=ret=1; continue };
    ...
    { l[i]=omo1; Z[1107]++; if(dbg){print "Z1107"}; stopp=ret=1; continue };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
             ...
             { wf_n=bf_n=""; Z[1109]++; if(dbg){print "Z1109"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                 bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                 vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[1114]++; if(dbg){print "Z1114"}; };
...
function FN_lock_prl_f(str_is_lock_prl,    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_is_lock_prl "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1121]++; if(dbg){print "Z1121", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1122]++; if(dbg){print "Z1122", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1123]++; if(dbg){print "Z1123", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    ...
    if ( iwrd == "замок" ) {
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1127]++; if(dbg){print "Z1127", "-- TN_v_zamok_f"}; stopp=ret=1; continue};
          ...
          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) ||
               ( bfa(wf_n+1,bf_n-1,"_castle_prl") && Q(bfn,"prl_kred_sr") ) ||
                 vex(Y["tn_ok_vi"]) ||
            ( isname(0) && s(-1) ) )
             { wf_n=bf_n=""; Z[1128]++; if(dbg){print "Z1128"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1131]++; if(dbg){print "Z1131", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) || vex(Y["tn_ok_vi"]) )
            { wf_n=bf_n=""; Z[1132]++; if(dbg){print "Z1132"}; };
...
#      { l[i]=omo2; Z[1144]++; if(dbg){print "Z1144"}; stopp=ret=1; continue };
       ...
       if ( iwrd == "замок") {
          ...
          if ( wf(1,3,cst) ) { wf_n=wfn;
              ...
              if ( qf(wf_n+1,wf_n+3,"prl_edmuim prl_kred_mu") && ba(qfn,"_lock_prl_true") && s(0,qfn-1) && Q_w(qfn+1,"suw_edvi suw_edim") &&
                  qir(1,wf_n-1,"nar_any qast_any") && qir(wf_n+1,qfn-1,"nar_any qast_any") )
              { l[i]=omo2; Z[1158]++; if(dbg){print "Z1158"}; stopp=ret=1; continue };
              if ( qb(wf_n+1,wf_n+3,"prl_edmuim prl_kred_mu") && ba(qbn,"_lock_prl_true") && s(0,qbn-1) && Q_w(qfn+1,"suw_edvi suw_edim") &&
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1174]++; if(dbg){print "Z1174"}; }; #check_!под
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1175]++; if(dbg){print "Z1175"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[1176]++; if(dbg){print "Z1176"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
