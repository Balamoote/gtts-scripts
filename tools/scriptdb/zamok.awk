...
#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue
 ...
 if ( isname(1) && ba(1,"_castle_numbered_royalty-ro") && roman(2) && s(0,1) )
...
#{ l[i]=omo1; Z[177]++; if(dbg){print "Z177"}; continue };
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[183]++; if(dbg){print "Z183"}; }; #check_!под

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[184]++; if(dbg){print "Z184"}; continue }; #check_!под
 };
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { l[i]=omo2; Z[205]++; if(dbg){print "Z205"}; continue };
       ...
       { Z[209]++; if(dbg){print "Z209"}; continue };
...
#   { l[i]=omo2; Z[211]++; if(dbg){print "Z211"}; continue };
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[228]++; if(dbg){print "Z228"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
...
#    wa(1,"_lock_s_suw-tv") &&
#    bw(2,"который") && s(1) )
#{ l[i]=omo1; Z[245]++; if(dbg){print "Z245"}; continue }; #check!
 # _lock_s_suw-tv:end
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( w(-1,"дисковой") && s(1) )
    { l[i]=omo2; Z[256]++; if(dbg){print "Z256"}; stopp=ret=1; continue };
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[264]++; if(dbg){print "Z264"}; };
       ...
       if ( bw(1,cst1) ) { wf_n=1;
          if ( bfa(2,5,"_lock_prl") ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],2,bf_n-1) || vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[298]++; if(dbg){print "Z298"}; };
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[322]++; if(dbg){print "Z322", "-- TN_v_zamok_f"}; continue };
    ...
    { l[i]=omo1; Z[323]++; if(dbg){print "Z323"}; continue }; # без tn!
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[326]++; if(dbg){print "Z326", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[338]++; if(dbg){print "Z338", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[339]++; if(dbg){print "Z339", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[340]++; if(dbg){print "Z340", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[341]++; if(dbg){print "Z341", "-- TN_qerez_zamok_f"}; continue };
  ...
  # ниже "замок" без собственного предлога
   ...
   { l[i]=omo1; Z[352]++; if(dbg){print "Z352"}; continue };
    ...
    { l[i]=omo2; Z[360]++; if(dbg){print "Z360"}; stopp=ret=1; continue };
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[487]++; if(dbg){print "Z487"}; };
...
#      { bf_n=""; Z[547]++; if(dbg){print "Z547"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[548]++; if(dbg){print "Z548"}; };
...
#   # _castle_lock_after:start
 ...
 if ( bfa(1,5,"_lock_parts") ) { bf_n=bfn;
    ...
    { bf_n=""; Z[650]++; if(dbg){print "Z650"}; };

    if ( vgl(bf_n,1,5) && s(1,bf_n-1) && (s(0)||se(0," — ")) && qf(1,bfn,"isname") )
    { bf_n=""; Z[651]++; if(dbg){print "Z651"}; };
 ...
 if ( w(1,"на") &&
    bfa(2,5,"_lock_na_suw") && s(0,bfn-1) &&
    qir(2,bfn-1,"prl_vi prl_pr mest_vi mest_pr prq_vi prq_pr") )
 { l[i]=omo2; Z[670]++; if(dbg){print "Z670"}; continue }; #check ислючения "в"
 if ( w(1,"у") && Ww_(-1,"в") &&
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[690]++; if(dbg){print "Z690"}; continue } else
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn = xsn; Z[710]++; if(dbg){print "Z710", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[711]++; if(dbg){print "Z711", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[712]++; if(dbg){print "Z712", "-- tn_ok OFF", el, "замок"};};
}
...
function TN_v_zamok_f(    ret, tn, stopp) { stopp=v2s(Y["TN_v_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_v"]) ) { tn=Y["n_pre_v"];
    ...
    { l[i]=omo2; Z[743]++; if(dbg){print "Z743"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[744]++; if(dbg){print "Z744"}; stopp=ret=1; continue }; #edit
    ...
    if ( vgl(Y["tn-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["tn-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[794]++; if(dbg){print "Z794"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[802]++; if(dbg){print "Z802"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[808]++; if(dbg){print "Z808"}; };
    ...
    cst="быстрее быстро все назад обратно сейчас скорее сначала сперва срочно теперь";
    cst1="прямо сразу";
    if ( (w(tn-1,cst)||w(tn,cst1)) &&
          p(tn-2,",") && p(0) )
    ...
    { l[i]=omo2; Z[848]++; if(dbg){print "Z848"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[849]++; if(dbg){print "Z849"}; stopp=ret=1; continue }; #check
    ...
    if ( vgl(Y["tn-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["tn-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[888]++; if(dbg){print "Z888"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[889]++; if(dbg){print "Z889"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
function TN_na_zamok_f(    ret, tn, stopp) { stopp=v2s(Y["TN_na_zamok_f"]);
   ...
   if ( vex(Y["tn_ok_na"]) ) { tn=Y["n_pre_na"];
    ...
    { l[i]=omo1; Z[1005]++; if(dbg){print "Z1005", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
