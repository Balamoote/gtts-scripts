...
#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue
 ...
 if ( esca() )
 { Z[22]++; if(dbg){print "Z22", "-- is_a_phrase!"}; continue };

#if ( q(1,"isname") && s(0) )
#{ print l[i+1] >> "_isname.txt"; Z[23]++; if(dbg){print "Z23"}; }; continue
 ...
 if ( iwrd=="замок" || iwrd=="замки" )  { Y["iwrd_case"]="iv"; if ( iwrd=="замок" ) {Y["iwrd_form"]="ediv"} else {Y["iwrd_form"]="mniv"}; if(dbg){print "замок: им+вин, ед+мн"};
    ...
    { Z[24]++; if(dbg){print "Z24", "-- is_escape!"}; continue };
    # pointesque
...
#{ print book[b] >> "_vna.txt"; Z[25]++; if(dbg){print "Z25"}; }; continue
...
#{ l[i]=omo1; Z[54]++; if(dbg){print "Z54"}; continue };
 ...
 { l[i]=omo1; Z[56]++; if(dbg){print "Z56"}; stopp=ret=1; continue };
 ...
 if ( vgl(Y["n+_castlelike_things"],1,7) ) { bf_n=Y["n+_castlelike_things"]; wf_n=Y["n-_lock_pe"];
    ...
    if ( !bw(wf_n,cst0)  ) {
      ...
      { Z[68]++; if(dbg){print "Z68"}; continue };
    ...
    if ( vgl(bf_n,2,7) && z(0) &&
        (qxw(1,"а","не","просто только")||
         ...
         qir(xwn+1,bf_n-1,"prl_any prq_any mest_any") )
 ...
 if ( wf(2,6,"собой") && bw(wfn-1,"представлять") ) { wf_n=wfn;
      ...
      { Z[87]++; if(dbg){print "Z87", "-- TN_v_zamok_f"}; continue};
 ...
 if ( bf(1,4,cst) ) { bf_n=bfn;
   ...
   if ( bfa(bf_n+1,bf_n+5,"_castlelike_things")||
      ...
      ( bfa(bf_n+1,bf_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) {
       ...
       { bf_n=bb_n=""; Z[92]++; if(dbg){print "Z92"}; };
       if ( vgl(bf_n,1,4) && zs(0) && s(1,bb_n-1) &&
 ...
 if (bb(1,3,cst)) { bf_n=bbn;
   if ( bfa(bf_n+1,bf_n+5,"_locklike_things")||wf(bf_n+1,bf_n+5,cst2) ) { if (bfn) bb_n=bfn; if (wfn) bb_n=wfn;
       ...
       if ( vgl(bf_n,1,3) && zs(0) && s(1,bb_n-1) &&
            vex(Y["tn_ok_vi"]) )
       { bf_n=bb_n=""; Z[104]++; if(dbg){print "Z104"}; };
 ...
 if ( wb(-4,-1,"и ни") && z(wbn-1) && s(wbn,-1) &&
     wb2(wbn-4,wbn-1,"и ни") && s(wbn2,wbn-2) &&
      ba(wbn-1,"_castle_environs") &&
     qir(wbn+1,-1,"prl_any mest_any prq_any qast_any") &&
     qir(wbn2+1,wbn-2,"prl_any mest_any prq_any qast_any") )
 ...
 if ( bfa(1,6,"_castle_environs") ) { bf_n=Y["n+_castle_environs"]=bfn; qb_n=Y["n-_lock_pe"]; bb_n=Y["n-_gl_v_lock-iv"];
   ...
   { var_do_tn=1; Z[121]++; if(dbg){print "Z121"}; };
   if ( wa(bf_n,"_castle_u_suw-ro") && s(1,bf_n-1) && wf(1,bf_n-1,"у") )
   { var_do_tn=1; Z[122]++; if(dbg){print "Z122"}; };
#  if ( vex(Y["tn_ok_v"]) && vgl(bb_n,Y["n_pre_v"]-8,Y["n_pre_v"]-1) && s(1,bf_n-1) && wf(1,bf_n-1,"в во") )
#  { var_do_tn=1; Z[123]++; if(dbg){print "Z123"}; };
   ...
   if ( !var_do_tn ) {
    ...
    if ( vgl(bf_n,1,6) &&
       ...
       !(bfa(1,qfn-1,"_lock_pe") || wfa(1,qfn-1,"_lock_suw-ro")) &&
         qia(qfn+1,bf_n-1,"prl_kred_sr","prl_any mest_any qast_any") )
    ...
    if ( vgl(bf_n,2,6) && sc(0,"[,—]") && s(1,bf_n-1) &&
...
sz_iili(-2) &&
 ...
 if ( bfa(1,4,"_locklike_things") ) { bf_n=bfn;
    ...
    if ( qq(-1,1) && z(0) && s(-1) &&
         veq(bf_n,2) && s(1) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[161]++; if(dbg){print "Z161"}; continue } else
    ...
    if ( q(1,"sz_iili") &&
       ...
       veq(bf_n,3) && s(0,2) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[168]++; if(dbg){print "Z168"}; continue } else
    ...
    if ( z(0) && veq(bf_n,1) && !(isname(1) && bw(1,"люк")) ) {
       if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[174]++; if(dbg){print "Z174"}; continue }
    else { l[i]=omo2; Z[175]++; if(dbg){print "Z175"}; continue };};
 };
...
sz_iili(1) &&
 ...
 if ( qb(-4,-1,"sz_iili") ) { qb_n=qbn;
   ...
   { qb_n=bb_n=""; Z[180]++; if(dbg){print "Z180"}; };
   if ( vgl(Y["n-_castle_pe"],bb_n+1,qb_n-1) && s(Y["n-_castle_pe"],-1) )
   { qb_n=bb_n=""; Z[181]++; if(dbg){print "Z181"}; };
 ...
 if ( veq(Y["iwrd_case"],"tv") ) {
    ...
    { Z[192]++; if(dbg){print "Z192", "-- TN_pod_zamkom_f"}; continue};
 };
 # suw_ro escape:end

 if ( FN_zamok_suwro_f() )
 { Z[193]++; if(dbg){print "Z193", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( w(1,"из") ) { wf_n=1;
    ...
    { wf_n=""; Z[201]++; if(dbg){print "Z201"}; continue };
 ...
 if ( wf(1,6,"для") ) { wf_n=Y["n+_pre_dlya"]=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
      ...
      { wf_n=bf_n=""; Z[206]++; if(dbg){print "Z206"}; };
      if ( wa(1,"_lock_suw-ro") && s(0) )
      { wf_n=bf_n=""; Z[207]++; if(dbg){print "Z207"}; };
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[208]++; if(dbg){print "Z208"}; };
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[209]++; if(dbg){print "Z209", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[210]++; if(dbg){print "Z210", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[211]++; if(dbg){print "Z211", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wb(-5,-2,"для") ) { wb_n=Y["n-_pre_dlya"]=wbn;
   if ( wba(wb_n+1,-1,"_castle_suw-ro1") ) { bb_n=wbn;
    ...
    { l[i]=omo1; Z[214]++; if(dbg){print "Z214"}; continue }; #checkro2
   ...
   if ( wba(wb_n+1,-1,"_castle_suw-ro2") ) { bb_n=wbn;
    ...
    if ( vgl(wb_n,-5,-1) && vgl(bb_n,wb_n+1,-1) && s(wb_n-1,-1) &&
         ...
         qir(wb_n+1,bb_n-1,"prl_ro mest_ro mest_3e prq_ro") &&
         qir(bb_n+1,-1,"prl_any mest_any") )
    { l[i]=omo1; Z[215]++; if(dbg){print "Z215"}; continue }; #checkro2
 ...
 if ( wf(1,4,"в во") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[221]++; if(dbg){print "Z221"}; };
    cst0="ванной";
    if ( vgl(wf_n,1,4) && s(0,wf_n-1) &&
          wf(wf_n+1,wf_n+3,cst0) &&
         qir(wf_n+1,wfn-1,"prl_vi mest_vi mest_3e") )
    { wf_n=""; Z[222]++; if(dbg){print "Z222"}; };
    cst0="отправить";
    if ( vgl(wf_n,1,4) && s(0,wf_n-1) &&
        (veq(Y["n-_lock_pe"],-1) && s(-1) && !bw(-1,cst0) ) )
    { wf_n=""; Z[223]++; if(dbg){print "Z223"}; };

    if ( veq(wf_n,1) &&
          qf(wf_n+1,wf_n+4,"isname") &&
 ...
 if ( wf(1,4,"в во") ) { wf_n=wfn;
    ...
    { Z[228]++; if(dbg){print "Z228"}; continue };
 ...
 if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"]; wf_n=Y["n-_lock_v_suw-pr"];
    if ( vgl(wb_n,-8,-2) && vgl(wf_n,wb_n+1,-1) && s(wb_n,-1) &&
         ...
         qir(wf_n+1,-1,"prl_vi mest_vi mest_3e prq_vi isname nar_step") )
...
#
 ...
 if ( wf(1,3,"на") ) { wf_n=wfn;
   ...
   if ( vgl(wf_n,1,3) &&
        ...
        wf2(wfn+1,wfn+4,cst1) && s(0,wfn2-1) &&
        qir(1,wf_n-1,"suw_ro suw_edne isname") &&
        qir(wf_n+2,wfn-1,"prl_pr mest_pr mest_3e nar_any prq_pr") &&
        qir(wfn+1,wfn2-1,"prl_ro mest_ro prq_ro isname") )
 ...
 if ( vgl(Y["n_pre_na"],-8,-2) ) { wb_n=Y["n_pre_na"];
   if ( wfa(wb_n+1,-1,"_lock_na_suw-pr") ) { bf_n=wfn; qf_n=Y["n-_castle_pe"];
 ...
 if ( w(1,"на") && wfa(2,5,"_lock_na_suw-pr") ) { wf_n=wfn; qb_n=Y["n-_castle_pe"]
        ...
        { wf_n=""; Z[286]++; if(dbg){print "Z286"}; };
        cst0="весь целый";
        if ( w(-1,cst0) && vgl(wf_n,2,5) && s(-1,wf_n-1) )
        { wf_n=""; Z[287]++; if(dbg){print "Z287"}; };
 ...
 if ( w(1,"на") ) {
   ...
   if ( bba(-5,-1,"_gl_4_lock_na_suw-vi") ) { bb_n=bbn;
     ...
     { l[i]=omo2; Z[298]++; if(dbg){print "Z298"}; continue };#uphere
   };};
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
     ...
     { Z[311]++; if(dbg){print "Z311", "-- TN_na_zamok_f"}; continue };
 ...
 { l[i]=omo1; Z[334]++; if(dbg){print "Z334"}; continue };
  ...
  ######################################################################################################
    ...
    { Z[335]++; if(dbg){print "Z335", "-- FN_castle_prl_f"}; continue };
    ...
    { Z[337]++; if(dbg){print "Z337", "-- FN_lock_prl_f(\"_lock_prl\")"}; continue };
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( wb(-4,-1,"с со") && wb2(wbn-4,wbn-1,cst) && s(wbn2,-1) &&
        qir(wbn2+1,wbn-1,"nar_step qast_any") &&
        qir(wbn+1,-1,"prl_vi mest_vi mest_3e prq_vi") )
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[358]++; if(dbg){print "Z358"}; };
    ...
    if ( bw(1,cst) &&
          w(2,"из") && s(0,1) )
    { l[i]=omo1; Z[370]++; if(dbg){print "Z370"}; stopp=ret=1; continue };
    if ( !vex(Y["tn_ok_v"]) && bw(1,cst) &&
            q(2,"nar_spos nar_kaq prl_kred_sr") && s(0,1) )
    { l[i]=omo1; Z[371]++; if(dbg){print "Z371"}; stopp=ret=1; continue };
    if ( bw(1,cst1) &&
         bf(2,5,cst) &&
    ...
    if ( bw(1,cst) && s(0) &&
       (qxs(2,"вовсе совсем никак отнюдь","не")||
        qxs(2,"не") ) &&
          w(xsn+1,"из") && s(xsn+1) )
    { l[i]=omo1; Z[373]++; if(dbg){print "Z373"}; stopp=ret=1; continue };
    if ( wb(-7,-4,"из") && s(wbn,-1) &&
          q(wbn+1,"otsz_ro") &&
         bw(wbn+2,cst1) &&
         bw(wbn+3,cst) &&
        qir(wbn+4,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[374]++; if(dbg){print "Z374"}; stopp=ret=1; continue };
    # сложен
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[390]++; if(dbg){print "Z390", "-- TN_v_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[394]++; if(dbg){print "Z394", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[405]++; if(dbg){print "Z405", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[406]++; if(dbg){print "Z406", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[407]++; if(dbg){print "Z407", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[408]++; if(dbg){print "Z408", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[409]++; if(dbg){print "Z409", "-- TN_qerez_zamok_f"}; continue };
  ...
  # ниже "замок" без собственного предлога
   ...
   if ( wb(-5,-2,"на") &&
       ...
       qir(wbn+1,wbn2-1,"prl_vi mest_vi mest_3e qin_vi") &&
   ...
   if ( wb(-5,-2,"на") &&
       ...
       qir(wbn2+1,-1,"prl_ro suw_ro prl_vi mest_vi mest_3e") )
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[563]++; if(dbg){print "Z563"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_da") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[575]++; if(dbg){print "Z575"}; continue }; #check
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
               w(1,"на") &&
             wfa(2,5,"_lock_na_suw-vi") && s(0,wfn-1) &&
             qir(2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") )
        { l[i]=omo2; Z[576]++; if(dbg){print "Z576"}; continue }; #check
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[587]++; if(dbg){print "Z587"}; };
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
              w(qb_n,"в во") && qir(qb_n+1,-1,"isname") && ba(bb_n,"_gl_v_castle-iv") )
       { bb_n=qb_n=""; Z[588]++; if(dbg){print "Z588"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[621]++; if(dbg){print "Z621"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"]; wf_n=Y["n-_gl_v_lock-iv"];
       ...
       { bf_n=""; Z[646]++; if(dbg){print "Z646"};};
       if ( vgl(bf_n,1,6) && xw(bf_n,"X_prl_castle_pe") && !eos(0,bf_n-1) )
       { bf_n=""; Z[647]++; if(dbg){print "Z647"};};
       ...
       { bf_n=""; Z[649]++; if(dbg){print "Z649"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[650]++; if(dbg){print "Z650"}; };
       ...
       { l[i]=omo1; Z[667]++; if(dbg){print "Z667"}; continue };
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"ediv") &&
          prq_any(bf_n) && !q(bf_n,"prq_edmuim prq_edmutv prq_kred_mu") &&
             !exb(-1,"тот ни другой ") )
         { bf_n=""; Z[668]++; if(dbg){print "Z668"}; };
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
               ba(-1,"_castle_prl_geo") && s(-1) )
         { bf_n=""; Z[669]++; if(dbg){print "Z669"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[697]++; if(dbg){print "Z697", "-- TN_do_zamka_f"}; continue };
 ...
 #
  #
    if ( TN_k_zamku_f() )
    { Z[705]++; if(dbg){print "Z705", "-- TN_k_zamku_f"}; continue };
 ...
 #
    ...
    if ( vex(Y["tn_ok_pod_tv"]) && TN_pod_zamkom_f() )
    { Z[711]++; if(dbg){print "Z711", "-- TN_pod_zamkom_f"}; continue};
 ...
 #
    ...
    { Z[717]++; if(dbg){print "Z717", "-- TN_v_zamke_f"}; continue};
 ...
 { l[i]=omo2; Z[729]++; if(dbg){print "Z729"}; continue };
 ...
 if ( bf_(1,5,cst) &&
        w(bf2+1,"на") &&
      bfa(bf2+2,bf2+5,"_castlelike_things") && !eos(0,bfn-1) &&
      qir(1,bf2-1,"gl_aux_be isname otsz_any") &&
      qir(bf2+2,bfn-1,"prl_vi mest_vi mest_3e prq_vi") )
 ...
 { Z[747]++; if(dbg){print "Z747", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[748]++; if(dbg){print "Z748", "-- FN_lock_prl_f(\"_lock_prl_after\")"}; continue };
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[761]++; if(dbg){print "Z761", "-- is_CAP"}; continue };
...
function DEFINE_TN_OK_PREVI_ZAMOK(num,el,    fufu) { # определение компонентов между "pre_vi" и "замок"
    ...
    { tn_ok = 1; Z[762]++; if(dbg){print "Z762", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[763]++; if(dbg){print "Z763", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[764]++; if(dbg){print "Z764", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[765]++; if(dbg){print "Z765", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[766]++; if(dbg){print "Z766", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[767]++; if(dbg){print "Z767", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[768]++; if(dbg){print "Z768", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[769]++; if(dbg){print "Z769", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[770]++; if(dbg){print "Z770", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[771]++; if(dbg){print "Z771", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[772]++; if(dbg){print "Z772", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[773]++; if(dbg){print "Z773", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[774]++; if(dbg){print "Z774", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[775]++; if(dbg){print "Z775", "-- tn_ok OFF", el, "замок"};};
}
...
function DEFINE_TN_OK_PREDA_ZAMKU(num,el,    fufu) { # определение компонентов между "pre_da" и "замку"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[776]++; if(dbg){print "Z776", "-- tn_ok ON", el, "замку"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[777]++; if(dbg){print "Z777", "-- tn_ok MOV", el, "замку"};};
    # выключение tn_ok
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[778]++; if(dbg){print "Z778", "-- tn_ok OFF", el, "замку"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[779]++; if(dbg){print "Z779", "-- tn_ok OFF", el, "замку"};};
}
...
function DEFINE_TN_OK_PRETV_ZAMKOM(num,el,    fufu) { # определение компонентов между "pre_tv" и "замком"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[780]++; if(dbg){print "Z780", "-- tn_ok ON", el, "замком"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[781]++; if(dbg){print "Z781", "-- tn_ok MOV", el, "замком"};};
    # выключение tn_ok
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[782]++; if(dbg){print "Z782", "-- tn_ok OFF", el, "замком"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[783]++; if(dbg){print "Z783", "-- tn_ok OFF", el, "замком"};};
}
...
function DEFINE_TN_OK_PREPR_ZAMKE(num,el) { # определение компонентов между "pre_pr" и "замке"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[784]++; if(dbg){print "Z784", "-- tn_ok ON", el, "замке"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[785]++; if(dbg){print "Z785", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[786]++; if(dbg){print "Z786", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[787]++; if(dbg){print "Z787", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[788]++; if(dbg){print "Z788", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[789]++; if(dbg){print "Z789", "-- tn_ok ON", el, "замке"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[790]++; if(dbg){print "Z790", "-- tn_ok MOV", el, "замке"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    if ( bb(tn-4,tn-1,cst) &&
        bb_(bbn-4,bbn-1,cst1) && s(bb2,tn-1) &&
        qir(bbn+1,tn-1,"nar_any qast_any") &&
        qir(bb2+1,bbn-1,"suw_ro suw_edne isname prl_ro mest_ro mest_3e nar_any qast_any") )
    ...
    { l[i]=omo2; Z[819]++; if(dbg){print "Z819"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[820]++; if(dbg){print "Z820"}; stopp=ret=1; continue }; #edit
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       { bb_n=""; Z[859]++; if(dbg){print "Z859"}; };
       if ( vgl(bb_n,tn-9,tn-1) && s(bb_n,tn-1) &&
            bba(tn+1,-1,"_castle_prl") )
       { bb_n=""; Z[860]++; if(dbg){print "Z860"}; };
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[863]++; if(dbg){print "Z863"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[871]++; if(dbg){print "Z871"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[878]++; if(dbg){print "Z878"}; };
    ...
    { l[i]=omo2; Z[914]++; if(dbg){print "Z914"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst) &&
        bf_(bfn+1,bfn+3,cst1) && s(1,bf2-1) &&
        qir(1,bfn-1,"nar_any") &&
        qir(bfn+1,bf2-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") )
    { l[i]=omo2; Z[915]++; if(dbg){print "Z915"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[916]++; if(dbg){print "Z916"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst2) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[952]++; if(dbg){print "Z952"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[953]++; if(dbg){print "Z953"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
    ...
    if ( bfa(1,6,"_gl_v_castle-iv") ) { bf_n=bfn;
       ...
       { bf_n=""; Z[1007]++; if(dbg){print "Z1007"}; };
   ...
   if ( vex(Y["tn_ok_na"]) ) { tn=Y["n_pre_na"];
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[1045]++; if(dbg){print "Z1045"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[1058]++; if(dbg){print "Z1058", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
  ...
  if ( vex(Y["tn_ok_pod"]) ) { tn=Y["n_pre_pod"];
   ...
   if ( bw(-2,cst) &&
    ...
    if ( bw(-2,cst4) &&
         ...
         bw(3,cst1) && s(-2,2) )
    { l[i]=omo2; Z[1107]++; if(dbg){print "Z1107"}; stopp=ret=1; continue };
    ...
    if ( bw(-6,cst4) &&
         ...
         bw(-2,cst1) &&
          w(-1,"под") && s(-6,-1) )
    { l[i]=omo2; Z[1108]++; if(dbg){print "Z1108"}; stopp=ret=1; continue };
...
function TN_o_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["TN_o_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_o"]) ) { tn=Y["n_pre_o"];
    ...
    { l[i]=omo1; Z[1158]++; if(dbg){print "Z1158"}; stopp=ret=1; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1172]++; if(dbg){print "Z1172", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) && wy(-1,"же") &&
#         qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
#   { tn_ok = 1; Z[1173]++; if(dbg){print "Z1173", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-1,"suw_tv mest_tv") )
#   { tn_ok = 1; Z[1174]++; if(dbg){print "Z1174", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-2,"suw_tv qik_tv") &&
#           q(-1,cst3) )
#   { tn_ok = 1; Z[1175]++; if(dbg){print "Z1175", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_vi") &&
#         qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
#           q(-1,"mest_vip3e suw_vi") )
#   { tn_ok = 1; Z[1176]++; if(dbg){print "Z1176", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_tv") &&
#         qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
...
#   { tn_ok = 1; Z[1177]++; if(dbg){print "Z1177", "-- tn_ok ON", el, "замка"};};
    ...
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1183]++; if(dbg){print "Z1183", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1184]++; if(dbg){print "Z1184", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1185]++; if(dbg){print "Z1185", "-- tn_ok OFF", el, "замка"};};
}
...
function TN_v_zamke_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["TN_v_zamke_f"]);
  ...
  if ( vex(Y["tn_ok_v_pr"]) ) { tn=Y["n_pre_v_pr"];
    ...
    if ( bba(tn-5,tn-1,"_gl_v_castle-pr") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1190]++; if(dbg){print "Z1190"}; stopp=ret=1; continue };
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_pr mest_pr mest_3e","suw_ro suw_edne") )
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
                if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                     ...
                     vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1209]++; if(dbg){print "Z1209"}; };
                if ( s(bf_n,bf_n+1) && sz_iili(bf_n+1) && ba(bf_n+2,"_lock_prl") )
                { wf_n=bf_n=""; Z[1210]++; if(dbg){print "Z1210"}; };
                ...
                { l[i]=omo1; Z[1212]++; if(dbg){print "Z1212"}; stopp=ret=1; continue };
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
               if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                    bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                    vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1215]++; if(dbg){print "Z1215"}; };
...
function FN_lock_prl_f(str_lock,    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_lock "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1240]++; if(dbg){print "Z1240", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1241]++; if(dbg){print "Z1241", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1242]++; if(dbg){print "Z1242", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    # _lock_prl:escape
    ...
    # _lock_prl:start
       ...
       if ( iwrd == "замок") {
          ...
          { l[i]=omo2; Z[1243]++; if(dbg){print "Z1243"}; stopp=ret=1; continue };
          ...
          { l[i]=omo2; Z[1248]++; if(dbg){print "Z1248"}; stopp=ret=1; continue };
          ...
          { l[i]=omo2; Z[1253]++; if(dbg){print "Z1253"}; stopp=ret=1; continue };
          ...
          { l[i]=omo2; Z[1254]++; if(dbg){print "Z1254"}; stopp=ret=1; continue };
          ...
          if ( bf(1,3,cst) ) { wf_n=bfn;
              ...
              { l[i]=omo2; Z[1256]++; if(dbg){print "Z1256"}; stopp=ret=1; continue };
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1281]++; if(dbg){print "Z1281"}; };
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1282]++; if(dbg){print "Z1282"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step qip_ro qik_im") )
    { l[i]=omo1; Z[1283]++; if(dbg){print "Z1283"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
