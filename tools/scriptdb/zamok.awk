...
#===============================================================================================================================================================
 ...
 if ( FN_hw_zamka_f() )
 { Z[25]++; if(dbg){print "Z25", "-- is_HW"}; continue };
...
#{ l[i]=omo1; Z[44]++; if(dbg){print "Z44"}; continue };
 ...
 { l[i]=omo1; Z[48]++; if(dbg){print "Z48"}; stopp=ret=1; continue };
 ...
 if ( bfa(1,5,"_locklike_things") ) { bf_n=bfn;
    ...
    { Z[118]++; if(dbg){print "Z118"}; continue };
    ...
    if ( veq(bf_n,1) && tz(0) && (tz(3)||q_w(4,"sz_iili")||p(3)) &&
           q(2,"prl_kred_sr") &&
           q(3,"prl_im") )
    { l[i]=omo2; Z[135]++; if(dbg){print "Z135"}; continue };
...
sz_iili(1) &&
 ...
 if ( qb(-4,-1,"sz_iili") ) { qb_n=qbn;
   ...
   { qb_n=bb_n=""; Z[140]++; if(dbg){print "Z140"}; };
   if ( vgl(Y["n-_castle_pe"],bb_n+1,qb_n-1) && s(Y["n-_castle_pe"],-1) )
   { qb_n=bb_n=""; Z[141]++; if(dbg){print "Z141"}; };
 ...
 if ( veq(Y["iwrd_case"],"tv") ) {
    ...
    { Z[148]++; if(dbg){print "Z148", "-- TN_pod_zamkom_f"}; continue};
 };
 if ( veq(Y["iwrd_case"],"ro") ) {
    if ( vex(Y["tn_ok_do"]) && TN_do_zamka_f() )
    { Z[149]++; if(dbg){print "Z149", "-- TN_do_zamka_f"}; continue};
    if ( vex(Y["tn_ok_ot"]) && TN_ot_zamka_f() )
    { Z[150]++; if(dbg){print "Z150", "-- TN_ot_zamka_f"}; continue};
    if ( vex(Y["tn_ok_iz"]) && TN_iz_zamka_f() )
    { Z[151]++; if(dbg){print "Z151", "-- TN_iz_zamka_f"}; continue};
 };
 # suw_ro escape:end

 if ( FN_zamok_suwro_f() )
 { Z[152]++; if(dbg){print "Z152", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( w(1,"из") ) { wf_n=1;
    ...
    { wf_n=""; Z[160]++; if(dbg){print "Z160"}; continue };

    if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
    { Z[161]++; if(dbg){print "Z161", "-- TN_na_zamok_f"}; continue};
...
#     { wf_n=bf_n=""; Z[166]++; if(dbg){print "Z166"}; };
#     if ( wa(1,"_lock_suw-ro") && s(0) )
#     { wf_n=bf_n=""; Z[167]++; if(dbg){print "Z167"}; };
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[168]++; if(dbg){print "Z168"}; };
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[169]++; if(dbg){print "Z169", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[170]++; if(dbg){print "Z170", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[171]++; if(dbg){print "Z171", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[176]++; if(dbg){print "Z176"}; };
      ...
      cst0="гермодвери двери";
       ...
       { Z[177]++; if(dbg){print "Z177", "-- TN_v_zamok_f"}; continue };
...
#       { l[i]=omo1; Z[191]++; if(dbg){print "Z191"}; continue };
    ...
    { Z[192]++; if(dbg){print "Z192"}; continue };
 ...
 if ( wf(1,3,"под") ) {wf_n=wfn;
     if ( wfa(wf_n+1,wf_n+4,"_castle_pod_suw-tv") ) { bf_n=wfn; bb_n=Y["n-_lock_pe"];
       ...
       { wf_n=""; Z[198]++; if(dbg){print "Z198"}; };

       cst="названием";
       if ( w(bf_n,cst) && !isname(bf_n+1) && s(0,bf_n) )
       { wf_n=""; Z[199]++; if(dbg){print "Z199"}; };
       cst="ударом";
       cst1="молота";
 ...
 if ( wf(1,3,"на") ) { wf_n=wfn;
     ...
     { Z[235]++; if(dbg){print "Z235"}; continue };
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-2) ) { wb_n=Y["n_pre_na_ist"];
   if ( wfa(wb_n+1,wb_n+4,"_lock_na_suw-pr") ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_lock_na_suw-pr") ) { bf_n=bfn;
       ...
       { wb_n=wf_n=bf_n=""; Z[243]++; if(dbg){print "Z243"}; };
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-2) ) { wb_n=Y["n_pre_na_ist"]; wf_n=Y["n_pre_v_ist"]
   ...
   if ( wfa(wb_n+1,-1,"_lock_na_suw-pr") ) { bf_n=wfn; qf_n=Y["n-_castle_pe"]; qb_n=Y["n-_lock_pe"];
     ...
     if ( bba(-8,-1,"M_iscastle_lock_na_suw-pr_gl_list") ) { bb_n=bbn;
        ...
        { bf_n=""; Z[254]++; if(dbg){print "Z254"}; };
        if ( s(-1) && bw(-1,"весь целый") )
        { wb_n=bf_n=""; Z[255]++; if(dbg){print "Z255"}; };
...
#       { Z[261]++; if(dbg){print "Z261"}; continue };
...
#  };};
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
     ...
     { Z[291]++; if(dbg){print "Z291", "-- TN_na_zamok_f"}; continue };
 ...
 if ( wf(1,4,"с со") ) { wf_n=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_lock_s_suw-tv") ) { bf_n=wfn;
      ...
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[310]++; if(dbg){print "Z310", "-- TN_v_zamok_f"}; continue };
      ...
      { wf_n=bf_n=""; Z[313]++; if(dbg){print "Z313"}; };
#     cst2="деталями";
...
#   { l[i]=omo2; Z[318]++; if(dbg){print "Z318"}; continue };
 ...
 { l[i]=omo1; Z[321]++; if(dbg){print "Z321"}; continue };
  ...
  ######################################################################################################
    ...
    { Z[322]++; if(dbg){print "Z322", "-- FN_castle_prl_f(\"_castle_prl\")"}; continue };
    ...
    { Z[324]++; if(dbg){print "Z324", "-- FN_lock_prl_f(\"_lock_prl\")"}; continue };
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    { l[i]=omo1; Z[328]++; if(dbg){print "Z328"}; continue };b
    if ( se(0," — ") &&
        qsf(1,5,",") && se(sfn+1," — ") &&
    ...
    if ( bf(1,4,cst) ) { bf_n=bfn;
      ...
      if ( bfa(bf_n+1,bf_n+5,"_castlelike_things")||
         ...
         ( bfa(bf_n+1,bf_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) {
          ...
          { bf_n=bb_n=""; Z[335]++; if(dbg){print "Z335"}; };
    ...
    if ( bw(1,cst) &&
          w(2,"из") && s(0,1) )
    { l[i]=omo1; Z[361]++; if(dbg){print "Z361"}; stopp=ret=1; continue };
    if ( !vex(Y["tn_ok_v"]) && bw(1,cst) &&
            q(2,"nar_spos nar_kaq prl_kred_sr") && s(0,1) )
    { l[i]=omo1; Z[362]++; if(dbg){print "Z362"}; stopp=ret=1; continue };
    if ( bw(1,cst1) &&
         bf(2,5,cst) &&
    ...
    if ( bw(1,cst) && s(0) &&
       (qxs(2,"вовсе совсем никак отнюдь","не")||
        qxs(2,"не") ) &&
          w(xsn+1,"из") && s(xsn+1) )
    { l[i]=omo1; Z[364]++; if(dbg){print "Z364"}; stopp=ret=1; continue };
    if ( wb(-7,-4,"из") && s(wbn,-1) &&
          q(wbn+1,"otsz_ro") &&
         bw(wbn+2,cst1) &&
         bw(wbn+3,cst) &&
        qir(wbn+4,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[365]++; if(dbg){print "Z365"}; stopp=ret=1; continue };
    # сложен
...
#      { l[i]=omo1; Z[371]++; if(dbg){print "Z371"}; continue };
  ...
  if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"];
    ...
    if ( TN_v_zamok_f() )
    { Z[375]++; if(dbg){print "Z375", "-- TN_v_zamok_f"}; continue };
    ...
    { l[i]=omo1; Z[376]++; if(dbg){print "Z376"}; continue }; # без tn!
  ...
  if ( vgl(Y["n_pre_na_ist"],-8,-1) ) { wb_n=Y["n_pre_na_ist"];
    ...
    if ( TN_na_zamok_f() )
    { Z[377]++; if(dbg){print "Z377", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[380]++; if(dbg){print "Z380", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[381]++; if(dbg){print "Z381", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[382]++; if(dbg){print "Z382", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[383]++; if(dbg){print "Z383", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[384]++; if(dbg){print "Z384", "-- TN_qerez_zamok_f"}; continue };
...
#   cst1="врезать встроить вырезать";
    ...
    if ( bb(-6,-1,cst1) ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,-6,-1) && s(bb_n,0) &&
              w(1,cst4) &&
            qir(bb_n+1,-1,"prl_vi mest_3e mest_vi") )
...
#           vex(Y["tn_ok_vi"]) )
#      { bb_n=qb_n=""; Z[497]++; if(dbg){print "Z497"}; };
#      if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
#             w(qb_n,"в во") && qir(qb_n+1,-1,"isname") && ba(bb_n,"_gl_v_castle-iv") )
#      { bb_n=qb_n=""; Z[498]++; if(dbg){print "Z498"}; };
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=qb_n=""; Z[499]++; if(dbg){print "Z499"}; };
...
#      { bb_n=""; Z[516]++; if(dbg){print "Z516"};};
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[534]++; if(dbg){print "Z534"}; };
    ...
    if ( vgl(Y["n-_lock_pe"],-7,-1) ) { bb_n=Y["n-_lock_pe"]; BB_n=Y["w-_lock_pe"];
      ...
      { bb_n=bb_n-1; Z[538]++; if(dbg){print "Z538", "-- bb_n-1"}; };
      # _lock_pe escape <<<---:start
      # _lock_pe escape <<<---:end

      if ( veq(bb_n,-1) && q(bb_n,"predik vvod") && s(-1) && zs(-2) )
      { bb_n=""; Z[539]++; if(dbg){print "Z539"};};
...
#         { bf_n=""; Z[571]++; if(dbg){print "Z571"}; };
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo2; Z[578]++; if(dbg){print "Z578"}; continue };
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"ediv") &&
          prq_any(bf_n) && !q(bf_n,"prq_edmuim prq_edmutv prq_kred_mu") &&
             !exb(-1,"тот ни другой ") )
         { bf_n=""; Z[579]++; if(dbg){print "Z579"}; };
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
               ba(-1,"_castle_prl_geo") && s(-1) )
         { bf_n=""; Z[580]++; if(dbg){print "Z580"}; };
       ...
       { l[i]=omo2; Z[586]++; if(dbg){print "Z586"}; stopp=ret=1; continue };
       cst="спине";
...
#   if ( TN_do_zamka_f() )
#   { Z[608]++; if(dbg){print "Z608", "-- TN_do_zamka_f"}; continue };
#   if ( TN_ot_zamka_f() )
#   { Z[609]++; if(dbg){print "Z609", "-- TN_ot_zamka_f"}; continue };
#   if ( TN_iz_zamka_f() )
#   { Z[610]++; if(dbg){print "Z610", "-- TN_iz_zamka_f"}; continue };
 ...
 #
  #
    if ( TN_k_zamku_f() )
    { Z[618]++; if(dbg){print "Z618", "-- TN_k_zamku_f"}; continue };
...
#   { l[i]=omo2; Z[620]++; if(dbg){print "Z620"}; continue };
 ...
 #
    ...
    if ( vex(Y["tn_ok_pod_tv"]) && TN_pod_zamkom_f() )
    { Z[624]++; if(dbg){print "Z624", "-- TN_pod_zamkom_f"}; continue};
      ...
      if ( bfa(bf_n+1,bf_n+5,"_castlelike_things")||
         ...
         ( bfa(bf_n+1,bf_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) {
          ...
          if ( vgl(bf_n,1,4) && zs(0) && s(1,bb_n-1) &&
               vgl(Y["n-_lock_pe"],-4,-1) && s(Y["n-_lock_pe"],-1) &&
               qir(bb_n+1,-1,"prl_edmuim prl_mnim mest_vi mest_3e qast_any") )
          { bf_n=bb_n=""; Z[630]++; if(dbg){print "Z630"}; };
          if ( vgl(bf_n,1,4) && zs(0) && s(1,bb_n-1) &&
                 w(bb_n,"ключом") && !w(bb_n+1,"к") )
          { bf_n=bb_n=""; Z[631]++; if(dbg){print "Z631"}; };
 ...
 #
    ...
    { Z[635]++; if(dbg){print "Z635", "-- TN_v_zamke_f"}; continue};
 ...
 if ( wf(1,6,"в во") ) { wf_n=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_v_suw-pr") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,6) && vgl(bf_n,wf_n+1,wf_n+4) && zs(0) && s(1,bfn-1) &&
         ...
         qir(wf_n+1,bf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
    { l[i]=omo2; Z[650]++; if(dbg){print "Z650"}; continue };
    if ( vgl(wf_n,1,6) && vgl(bf_n,wf_n+1,wf_n+4) && zs(0) && s(1,bfn-1) &&
          bw(wf_n-1,cst1) &&
         qir(1,wf_n-2,"prl_any mest_any isname qast_ne") &&
         qir(wf_n+1,bf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
 ...
 { l[i]=omo1; Z[661]++; if(dbg){print "Z661"}; stopp=ret=1; continue };
 ...
 { Z[667]++; if(dbg){print "Z667", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[668]++; if(dbg){print "Z668", "-- FN_lock_prl_f(\"_lock_prl_after\")"}; continue };
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[681]++; if(dbg){print "Z681", "-- is_CAP"}; continue };
...
function DEF_TN_PREVI_ZAMOK(num,el,    fufu) { # определение компонентов между "pre_vi" и "замок"
    ...
    { tn_ok = 1; Z[682]++; if(dbg){print "Z682", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[683]++; if(dbg){print "Z683", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[684]++; if(dbg){print "Z684", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[685]++; if(dbg){print "Z685", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[686]++; if(dbg){print "Z686", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[687]++; if(dbg){print "Z687", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[688]++; if(dbg){print "Z688", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[689]++; if(dbg){print "Z689", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[690]++; if(dbg){print "Z690", "-- tn_ok ON", el, "замок"};};
    ...
    { tn = xsn; Z[693]++; if(dbg){print "Z693", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vex(tn_ok) && z(tn-1) && w(tn+1,"который") )
    { tn_ok = ""; Z[694]++; if(dbg){print "Z694", "-- tn_ok OFF", el, "замок"};};
    ...
    { tn_ok = ""; Z[695]++; if(dbg){print "Z695", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[696]++; if(dbg){print "Z696", "-- tn_ok OFF", el, "замок"};};
}
...
function DEF_TN_PREDA_ZAMKU(num,el,    fufu) { # определение компонентов между "pre_da" и "замку"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[697]++; if(dbg){print "Z697", "-- tn_ok ON", el, "замку"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[698]++; if(dbg){print "Z698", "-- tn_ok MOV", el, "замку"};};
}
...
function DEF_TN_PRETV_ZAMKOM(num,el,    fufu) { # определение компонентов между "pre_tv" и "замком"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[699]++; if(dbg){print "Z699", "-- tn_ok ON", el, "замком"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[700]++; if(dbg){print "Z700", "-- tn_ok MOV", el, "замком"};};
}
...
function DEF_TN_PREPR_ZAMKE(num,el) { # определение компонентов между "pre_pr" и "замке"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[701]++; if(dbg){print "Z701", "-- tn_ok ON", el, "замке"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[702]++; if(dbg){print "Z702", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[703]++; if(dbg){print "Z703", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[704]++; if(dbg){print "Z704", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[705]++; if(dbg){print "Z705", "-- tn_ok ON", el, "замке"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[706]++; if(dbg){print "Z706", "-- tn_ok ON", el, "замке"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[707]++; if(dbg){print "Z707", "-- tn_ok MOV", el, "замке"};};
}
...
function TN_v_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_v_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_v"]) ) { tn=Y["n_pre_v"];
    ...
    if ( q(tn-2,"mest_im mest_vi mest_3e suw_odim suw_odvi") && Q(tn-2,"gl_po") &&
         ...
         p(tn-3) && p(0) && s(tn-2,-1) )
    { l[i]=omo1; Z[721]++; if(dbg){print "Z721"}; stopp=ret=1; continue };
    if ( q(tn-2,"nar_any prl_kred_sr") &&
         q(tn-1,"mest_im mest_vi mest_3e suw_odim suw_odvi") && Q(tn-1,"gl_po") &&
         p(tn-3) && p(0) && s(tn-2,-1) )
    { l[i]=omo1; Z[722]++; if(dbg){print "Z722"}; stopp=ret=1; continue };
    ...
    if ( bf(1,5,cst) ) { bf_n=bfn;
      ...
      { l[i]=omo1; Z[723]++; if(dbg){print "Z723"}; stopp=ret=1; continue };
      if ( vgl(bf_n,1,5) &&
             w(bf_n-1,cst1) && s(0,bf_n-1) &&
           qir(1,bf_n-2,"mest_im otsz_any gl_aux_be mod_any") )
      { l[i]=omo1; Z[724]++; if(dbg){print "Z724"}; stopp=ret=1; continue };
    };
    ...
    { l[i]=omo2; Z[738]++; if(dbg){print "Z738"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[739]++; if(dbg){print "Z739"}; stopp=ret=1; continue }; #edit
...
#   { l[i]=omo1; Z[743]++; if(dbg){print "Z743"}; stopp=ret=1; continue };
    if ( bw(tn-1,cst) &&
         bw(tn-2,cst1) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[744]++; if(dbg){print "Z744"}; stopp=ret=1; continue };
    ...
    { l[i]=omo1; Z[745]++; if(dbg){print "Z745"}; stopp=ret=1; continue };
    ...
    { l[i]=omo1; Z[756]++; if(dbg){print "Z756"}; stopp=ret=1; continue };
    ...
    { l[i]=omo1; Z[757]++; if(dbg){print "Z757"}; stopp=ret=1; continue };
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       { l[i]=omo2; Z[787]++; if(dbg){print "Z787"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[790]++; if(dbg){print "Z790"}; stopp=ret=1; continue };
       ...
       { l[i]=omo2; Z[794]++; if(dbg){print "Z794"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[801]++; if(dbg){print "Z801"}; };
      ...
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
        ...
        if ( vgl(qb_n,tn-8,tn-4) && veq(bb_n,qb_n-1) &&
             ...
             qir(qb_n+2,tn-3,"prl_vi mcop_vi prq_vi qis_vi qik_vi") &&
               q(tn-2,"suw_vi") &&
               q(tn-1,"suw_ro") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[817]++; if(dbg){print "Z817"}; stopp=ret=1; continue };
    };};
    ...
    if ( bfa(1,4,"_gl_v_lock-iv") ) { bf_n=bfn;
       ...
       { l[i]=omo2; Z[818]++; if(dbg){print "Z818"}; stopp=ret=1; continue };

#      if ( vgl(bf_n,1,4) && s(0,bf_n-1) && Qf(1,bf_n-1,"sz_iili") &&
#             q(tn-1,"suw_tv nar_spos") && s(tn-1) )
#      { l[i]=omo2; Z[819]++; if(dbg){print "Z819"}; stopp=ret=1; continue };
    };
    ...
    if ( bba(tn-5,tn-2,"_v_zamok") &&
         qir(bbn+1,tn-2,"prl_vi prq_vi mcop_vi nar_mest") &&
           w(tn-1,cst1) && s(bbn,tn-1) )
    { l[i]=omo2; Z[825]++; if(dbg){print "Z825"}; stopp=ret=1; continue };
    if ( ba(tn-4,"_v_zamok") &&
          w(tn-3,cst1) &&
     pre_pr(tn-2) &&
...
#   { l[i]=omo2; Z[829]++; if(dbg){print "Z829"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[845]++; if(dbg){print "Z845"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[850]++; if(dbg){print "Z850"}; stopp=ret=1; continue };
    if ( bw(tn-2,cst1) &&
         bw(tn-1,cst) && s(tn-2,tn-1) )
...
#      { bb_n=""; Z[871]++; if(dbg){print "Z871"}; };
    ...
    if ( bfa(1,6,"_gl_v_castle-iv") ) { bf_n=bfn;
       ...
       { bf_n=""; Z[931]++; if(dbg){print "Z931"}; };

       if ( vgl(bf_n,1,6) && s(0,bf_n-1) && Qf(1,bf_n,"sz_iili") )
       { l[i]=omo1; Z[932]++; if(dbg){print "Z932"}; stopp=ret=1; continue };
       if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
            sos(tn-9,tn-1) &&
...
function TN_na_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_na_zamok_f"]);
   ...
   if ( vex(Y["tn_ok_na"]) ) { tn=Y["n_pre_na"];
    ...
    { l[i]=omo2; Z[951]++; if(dbg){print "Z951"}; stopp=ret=1; continue };

    cst="пасти пасть";
    if ( w(tn-1,cst) && s(tn-1) )
    { l[i]=omo2; Z[952]++; if(dbg){print "Z952"}; stopp=ret=1; continue };

    cst="закрывать закрыть запереть запирать";
    cst2="дорогу";
    if ( bw(tn-2,cst) &&
          w(tn-1,cst2) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[953]++; if(dbg){print "Z953"}; stopp=ret=1; continue };
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[972]++; if(dbg){print "Z972"}; stopp=ret=1; };
    ...
    if ( bfa(1,5,"_gl_na_lock") ) { bf_n=bfn;
       ...
       { l[i]=omo2; Z[1004]++; if(dbg){print "Z1004"}; stopp=ret=1; continue };
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
            qir(1,bf_n-1,"nar_any prl_kred_sr") )
       { l[i]=omo2; Z[1005]++; if(dbg){print "Z1005"}; stopp=ret=1; continue };
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
...
function TN_za_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_za_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_za"]) ) { tn=Y["n_pre_za"];
   ...
   if ( bw(-5,cst1) && z(-5) &&
    ...
    { l[i]=omo1; Z[1018]++; if(dbg){print "Z1018"}; stopp=ret=1; continue };
    ...
    if ( wb(-5,-3,"нужно пора") &&
          w(-2,cst) &&
          w(-1,"за") && s(wbn,-1) )
    { l[i]=omo1; Z[1022]++; if(dbg){print "Z1022"}; stopp=ret=1; continue };
    if ( bw(-3,"хотеть") &&
          w(-2,cst) &&
          w(-1,"за") && s(-3,-1) )
    { l[i]=omo1; Z[1023]++; if(dbg){print "Z1023"}; stopp=ret=1; continue };
    ...
    if ( bba(tn-7,tn-1,"_gl_za_lock") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=""; Z[1025]++; if(dbg){print "Z1025"}; };
    ...
    { l[i]=omo1; Z[1033]++; if(dbg){print "Z1033"}; stopp=ret=1; continue };
    # _gl_za_castle --->>>:end
...
function TN_pod_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_pod_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_pod"]) ) { tn=Y["n_pre_pod"];
    ...
    if ( bw(tn-3,cst4) &&
          w(tn-2,"на") &&
         bw(tn-1,cst1) && s(tn-3,tn-1) )
    { l[i]=omo2; Z[1041]++; if(dbg){print "Z1041"}; stopp=ret=1; continue };
    if ( bw(tn-1,cst4) &&
          w(1,"в") &&
...
#      { l[i]=omo1; Z[1071]++; if(dbg){print "Z1071"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[1078]++; if(dbg){print "Z1078"}; stopp=ret=1; continue };
    # _suw_pod_lock:end

    if ( qxs(1,"на","хранение") )
    { l[i]=omo2; Z[1079]++; if(dbg){print "Z1079"}; stopp=ret=1; continue };

    if ( w(tn-2,"в во") &&
         q(tn-1,"suw_vi") && s(tn-2,tn-1) )
    { l[i]=omo2; Z[1080]++; if(dbg){print "Z1080"}; stopp=ret=1; continue };
    if ( w(tn-3,"в во") &&
         q(tn-2,"suw_vi") &&
         w(tn-1,"не") && s(tn-3,tn-1) )
    { l[i]=omo2; Z[1081]++; if(dbg){print "Z1081"}; stopp=ret=1; continue };
    if ( w(1,"в во") && s(tn-1) &&
         q(2,"suw_vi") && s(0,1) )
    { l[i]=omo2; Z[1082]++; if(dbg){print "Z1082"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[1093]++; if(dbg){print "Z1093"}; stopp=ret=1; continue };
   Y["tn_ok_pod"]="";
   } # tn_ok <<== end
...
function TN_o_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_o_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_o"]) ) { tn=Y["n_pre_o"];
    ...
    if ( bba(tn-5,tn-1,"_gl_o_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1099]++; if(dbg){print "Z1099"}; stopp=ret=1; continue };
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
    ...
    if ( bfa(1,5,"_gl_o_lock") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo2; Z[1102]++; if(dbg){print "Z1102"}; stopp=ret=1; continue };
   ...
   sz_iili(2) &&
    ...
    { l[i]=omo2; Z[1103]++; if(dbg){print "Z1103"}; stopp=ret=1; continue };
   Y["tn_ok_o"]="";
   }; # tn_ok <<== end
...
function TN_po_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_po_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_po"]) ) { tn=Y["n_pre_po"];
    ...
    if ( bba(tn-5,tn-1,"_gl_po_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1104]++; if(dbg){print "Z1104"}; stopp=ret=1; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1110]++; if(dbg){print "Z1110", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1111]++; if(dbg){print "Z1111", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1112]++; if(dbg){print "Z1112", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1113]++; if(dbg){print "Z1113", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1114]++; if(dbg){print "Z1114", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1115]++; if(dbg){print "Z1115", "-- tn_ok ON", el, "замка"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1116]++; if(dbg){print "Z1116", "-- tn_ok MOV", el, "замка"};};
}
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1142]++; if(dbg){print "Z1142"}; stopp=ret=1; continue };
    if ( iwrd == "замок") {
          ...
          if ( bf(1,4,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
                if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                     ...
                     vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1150]++; if(dbg){print "Z1150"}; };
                if ( s(bf_n,bf_n+1) && sz_iili(bf_n+1) && ba(bf_n+2,"_lock_prl") )
                { wf_n=bf_n=""; Z[1151]++; if(dbg){print "Z1151"}; };
                ...
                { l[i]=omo1; Z[1153]++; if(dbg){print "Z1153"}; stopp=ret=1; continue };
                if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
                       q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
                { l[i]=omo1; Z[1154]++; if(dbg){print "Z1154"}; stopp=ret=1; continue };
                ...
                if ( vgl(wf_n,1,4) && vgl(bf_n,wf_n+1,wf_n+5) &&
                     ...
                     qir(wf_n+1,bf_n-1,"nar_any prq_kred_mu qast_any prl_edmuim prl_mnim prl_kred_sr predik prl_edmutv mest_im mest_tv mest_3e mest_da isname suw_da") )
                { l[i]=omo1; Z[1155]++; if(dbg){print "Z1155"}; stopp=ret=1; continue };
#               if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
#                      q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
#               { l[i]=omo1; Z[1156]++; if(dbg){print "Z1156"}; stopp=ret=1; continue };
          };};
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bba(wf_n+1,wf_n+5,str_castle) ) { bb_n=bbn;
                ...
                { l[i]=omo1; Z[1157]++; if(dbg){print "Z1157"}; stopp=ret=1; continue };
          };};
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
               if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                    bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                    vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1158]++; if(dbg){print "Z1158"}; };
                ...
                { l[i]=omo1; Z[1159]++; if(dbg){print "Z1159"}; stopp=ret=1; continue };
#               if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
#                      q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
#               { l[i]=omo1; Z[1160]++; if(dbg){print "Z1160"}; stopp=ret=1; continue };
          };};
    ...
    if ( iwrd == "замка") {
       if ( q(-1,"prl_edmuro") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1162]++; if(dbg){print "Z1162"}; stopp=ret=1; continue };
...
#      { l[i]=omo1; Z[1166]++; if(dbg){print "Z1166"}; stopp=ret=1; continue };
    };
    if ( iwrd == "замком") {
       if ( q(-1,"prl_edmutv") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1167]++; if(dbg){print "Z1167"}; stopp=ret=1; continue };
       if ( s(-2,-1) && prl_edmutv(-2) && q(-1,"prl_edmutv") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1168]++; if(dbg){print "Z1168"}; stopp=ret=1; continue };
    };
    if ( iwrd == "замке") {
       if ( q(-1,"prl_edmupr") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1169]++; if(dbg){print "Z1169"}; stopp=ret=1; continue };
       if ( s(-2,-1) && prl_edmupr(-2) && q(-1,"prl_edmupr") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1170]++; if(dbg){print "Z1170"}; stopp=ret=1; continue };
    };
    if ( iwrd == "замки") {
       if ( s(-1) && prl_mnim(-1) && ba(-1,str_castle) )
       { l[i]=omo1; Z[1171]++; if(dbg){print "Z1171"}; stopp=ret=1; continue };
    ...
    if ( iwrd == "замкам") {
       ...
       { l[i]=omo1; Z[1177]++; if(dbg){print "Z1177"}; stopp=ret=1; continue };
       if ( s(-2,-1) && prl_mnda(-2) && q(-1,"prl_mnda") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1178]++; if(dbg){print "Z1178"}; stopp=ret=1; continue };
    };
    if ( iwrd == "замками") {
    ...
    if ( iwrd == "замках") {
       ...
       { l[i]=omo1; Z[1182]++; if(dbg){print "Z1182"}; stopp=ret=1; continue };
    };
    # _castle_prl:end
...
function FN_lock_prl_f(str_lock,    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp_str="FN" str_lock "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1183]++; if(dbg){print "Z1183", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1184]++; if(dbg){print "Z1184", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1185]++; if(dbg){print "Z1185", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    # _lock_prl:escape
    ...
    # _lock_prl:start
       ...
       if ( iwrd == "замок") {
          ...
          { l[i]=omo2; Z[1187]++; if(dbg){print "Z1187"}; stopp=ret=1; continue }; #uphere
...
#         };
       ...
       if ( iwrd == "замки") {
          ...
          { l[i]=omo2; Z[1215]++; if(dbg){print "Z1215"}; stopp=ret=1; continue };
          if ( qb(1,5,"prl_mnim prl_mntv prl_krmn") && qir(1,qbn-1,"gl_aux_be qast_any nar_any prl_krmn muk_mnim") &&
               ba(qbn,str_lock) && (zs(0)||se(0," — ")) && s(1,qbn-1) && Q(qbn+1,"suw_mnim suw_mntv") )
          { l[i]=omo2; Z[1216]++; if(dbg){print "Z1216"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замков") {
          if ( s(-1) && q(-1,"prl_mnro") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1217]++; if(dbg){print "Z1217"}; stopp=ret=1; continue };
          if ( s(-2,-1) && prl_mnro(-2) && q(-1,"prl_mnro") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1218]++; if(dbg){print "Z1218"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замкам") {
          ...
          { l[i]=omo2; Z[1220]++; if(dbg){print "Z1220"}; stopp=ret=1; continue };
       };
       if ( iwrd == "замками") {
       ...
       if ( iwrd == "замках") {
          ...
          { l[i]=omo2; Z[1223]++; if(dbg){print "Z1223"}; stopp=ret=1; continue };
          if ( s(-2,-1) && prl_mnpr(-2) && q(-1,"prl_mnpr") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1224]++; if(dbg){print "Z1224"}; stopp=ret=1; continue };
       };
    # _lock_prl:end
break}; Y[stopp_str]=1; return ret }
...
function FN_hw_zamka_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n, fufu) { stopp=v2s(Y["FN_hw_zamka_f"]);
  while ( !stopp ) {
    ...
    { fufu = 1; Z[1225]++; if(dbg){print "Z1225","hw_castle < hw_lock"}; };
    if ( bba(qb_n+1,-1,"_lock_prl_true") )
    { fufu = 1; Z[1226]++; if(dbg){print "Z1226","hw_caste < _lock_prl_true"}; };
...
#        !(w(1,"на") && wa(2,"_lock_na_suw-pr") && s(0,1)) &&
    ...
    { l[i]=omo1; Z[1227]++; if(dbg){print "Z1227"}; stopp=ret=1; continue };
    if ( vgl(qf_n,-5,-1) &&
         qir(qf_n+1,-1,"prl_ro prq_ro mest_ro mest_3e nar_any prl_kred_sr") && s(qf_n,-1) )
    { l[i]=omo2; Z[1228]++; if(dbg){print "Z1228"}; stopp=ret=1; continue };

    cst="сторона часть";
    cst1="ближний восточный дальний западный исторический нижний привилегированный северный северо-восточный северо-западны центральный юго-восточный юго-западный \
          южный";
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
       ...
       if ( vgl(wf_n,1,5) && s(0,wf_n-1) && !( isname(wf_n)||isname(1) ) &&
            qir(1,wf_n-1,"prl_ro mest_ro prq_ro qik_im qip_ro") )
       { l[i]=omo2; Z[1236]++; if(dbg){print "Z1236"}; stopp=ret=1; continue };

 };
 # _lock_suw-ro:end
break}; Y["FN_zamok_suwro_f"]=1; return ret }

function FN_zamok_vi_esc_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["FN_zamok_vi_esc_f"]);
  while ( !stopp ) {

 if ( qxw(-3,"часть","ворот двери") && z(0) &&
      qxw(-1,"а","именно") && se(-1," — ") )
 { l[i]=omo2; Z[1237]++; if(dbg){print "Z1237"}; stopp=ret=1; continue };
  ...
  if ( vex(Y["n-_castle_pe"]) ) { eb_n=Y["n-_castle_pe"]; EB_n=Y["w-_castle_pe"];
    ...
    if ( weq(EB_n,cst) ) {
      ...
      { l[i]=omo2; Z[1238]++; if(dbg){print "Z1238"}; stopp=ret=1; continue };
    };
 ...
 #===============================================================================================
  if ( vex(Y["n-_lock_pe"]) ) { eb_n=Y["n-_lock_pe"]; EB_n=Y["w-_lock_pe"];
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[1239]++; if(dbg){print "Z1239"}; stopp=ret=1; continue };
  ...
  cst="снять";
    ...
    if ( weq(EB_n,cst) ) {
      ...
      { l[i]=omo1; Z[1256]++; if(dbg){print "Z1256"}; stopp=ret=1; continue };
      cst1="здесь нынче";
      ...
      { l[i]=omo1; Z[1259]++; if(dbg){print "Z1259"}; stopp=ret=1; continue };
  ...
  cst="потрясти";
   ...
   if ( weq(EF_n,cst) ) {
      ...
      { l[i]=omo1; Z[1272]++; if(dbg){print "Z1272"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo1; Z[1273]++; if(dbg){print "Z1273"}; stopp=ret=1; continue };
    if ( vgl(ef_n,1,3) && s(0,ef_n) &&
           w(ef_n+1,"для") &&
    ...
    { l[i]=omo1; Z[1275]++; if(dbg){print "Z1275"}; stopp=ret=1; continue };
    if ( vgl(ef_n,1,3) &&
  ...
  if ( vex(Y["n-_gl_zamok"]) ) { eb_n=Y["n-_gl_zamok"]; EB_n=Y["w-_gl_zamok"];
   ...
   if ( weq(EB_n,cst) ) { bb_n=Y["n-_castle_pe"];
       if ( vgl(bb_n,-7,-3) && q(bb_n+1,"sz_iili") && veq(bb_n+2,eb_n) && s(bb_n,-1) &&
            qir(bb_n+3,-1,"prl_vi mest_vi mest_3e prq_vi") )
       { l[i]=omo1; Z[1286]++; if(dbg){print "Z1286"}; stopp=ret=1; continue };
   };
   cst="ломать";
   cst2="воздушный";
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[1287]++; if(dbg){print "Z1287"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) ) {
    if ( ba(-2,"_gl_dvizh") && sc(-2,"—") && s(-1) &&
        veq(eb_n,-1) )
    { l[i]=omo1; Z[1288]++; if(dbg){print "Z1288"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) ) {
      if ( s(eb_n,-1) &&
         ...
         qir(eb_n+1,-1,"prl_vi prq_vi mest_vi mest_3e") &&
         qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
      { l[i]=omo1; Z[1289]++; if(dbg){print "Z1289"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) ) {
    if ( ba(-2,"_gl_dvizh") && sc(-2,"—") && s(-1) &&
        veq(eb_n,-1) )
    { l[i]=omo1; Z[1290]++; if(dbg){print "Z1290"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[1295]++; if(dbg){print "Z1295"}; stopp=ret=1; continue };
     if (  vgl(eb_n,-4,-1) &&
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[1298]++; if(dbg){print "Z1298"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EB_n,cst) && s(eb_n+1,-1) &&
          w(eb_n+1,cst1) &&
        qir(eb_n+2,-1,"prl_edmuvi mest_edmuvi mest_3e") )
   { l[i]=omo2; Z[1311]++; if(dbg){print "Z1311"}; stopp=ret=1; continue };
...
#  qb_n=Y["n-_lock_pe"];
   ...
   if ( weq(EB_n,cst) && veq(eb_n,-3) && q(-3,"prq_any") &&
          q(-2,"prl_tv prq_tv") &&
          q(-1,"suw_tv") && s(-3,-1) )
   { l[i]=omo1; Z[1316]++; if(dbg){print "Z1316"}; stopp=ret=1; continue };
   ...
   if ( weq(EB_n,cst) ) { bf_n=wfn="";
     ...
     if ( vgl(Y["n+_castlelike_things"],1,5) ) { bf_n=Y["n+_castlelike_things"] } else {
      ...
      { l[i]=omo1; Z[1317]++; if(dbg){print "Z1317"}; stopp=ret=1; continue };
  ...
  if ( weq(EB_n,cst) ) {
    ...
    { l[i]=omo1; Z[1321]++; if(dbg){print "Z1321"}; stopp=ret=1; continue };
    ...
    { l[i]=omo1; Z[1322]++; if(dbg){print "Z1322"}; stopp=ret=1; continue };
    ...
    if ( (qxs(-1,"в",cst2)||
          ...
          qxs(-1,"на","мушку") ) &&
          veq(eb_n,xsn-1) && s(xsn-1) )
    { l[i]=omo2; Z[1339]++; if(dbg){print "Z1339"}; stopp=ret=1; continue };
    if ( veq(eb_n,-1) &&
        (qxs(1,"в",cst2)||
         qxs(1,"в","правую левую свою свои обе",cst2)||
         qxs(1,"на","мушку") ) && s(-1) )
    { l[i]=omo2; Z[1340]++; if(dbg){print "Z1340"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[1341]++; if(dbg){print "Z1341"}; stopp=ret=1; continue };
    ...
    if ( veq(eb_n,-1) &&
           ...
           w(3,"из") &&
           w(4,cst2) && s(-1,3) )
    { l[i]=omo2; Z[1342]++; if(dbg){print "Z1342"}; stopp=ret=1; continue };
    ...
    if ( veq(eb_n,-1) && s(-3,-1) &&
           ...
           w(-2,cst2) )
    { l[i]=omo2; Z[1344]++; if(dbg){print "Z1344"}; stopp=ret=1; continue };
    if ( veq(eb_n,-1) &&
        sz_i(1) &&
          bw(2,"сказать") &&
           q(3,"suw_odda mest_da isname") && s(-1,2) )
    { l[i]=omo2; Z[1345]++; if(dbg){print "Z1345"}; stopp=ret=1; continue };
  ...
  if ( vex(Y["n+_gl_zamok"]) ) { ef_n=Y["n+_gl_zamok"]; EF_n=Y["w+_gl_zamok"];
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo2; Z[1348]++; if(dbg){print "Z1348"}; stopp=ret=1; continue };
   ...
   cst2="мужчины отмычек отмычки техника";
   if ( weq(EF_n,cst) ) {
    if ( vgl(ef_n,1,3) &&
           q(ef_n+1,"suw_tv") &&
           w(ef_n+2,"для к") &&
    ...
    { l[i]=omo1; Z[1353]++; if(dbg){print "Z1353"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo1; Z[1354]++; if(dbg){print "Z1354"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) ) { bb_n=Y["n-_castle_pe"];
     if ( s(0,ef_n) &&
          q(ef_n+1,"mest_mnvi") &&
         Qf(1,ef_n-1,"sz_iili pre_any") )
     { l[i]=omo1; Z[1357]++; if(dbg){print "Z1357"}; stopp=ret=1; continue };
     cst1="крышей";
     ...
     { l[i]=omo1; Z[1358]++; if(dbg){print "Z1358"}; stopp=ret=1; continue };
       cst2="конструктора лего";
       if ( zs(0) && qxs(ef_n+1,"из",cst2) )
       { l[i]=omo1; Z[1359]++; if(dbg){print "Z1359"}; stopp=ret=1; continue };
   ...
   if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo1; Z[1360]++; if(dbg){print "Z1360"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) ) {
       if ( s(0,ef_n) &&
          ...
          qir(ef_n+1,wfn-1,"prl_vi mest_3e") &&
           Qf(1,ef_n-1,"sz_iili pre_any") )
       { l[i]=omo1; Z[1361]++; if(dbg){print "Z1361"}; stopp=ret=1; continue };
   };
   ...
   if ( weq(EF_n,cst) ) {
       if ( s(0,ef_n) &&
          ...
          qir(ef_n+1,bfn-1,"mest_da suw_da isname prl_vi prl_ro mest_vi mest_ro mest_3e") &&
           Qf(1,ef_n-1,"sz_iili pre_any") )
       { l[i]=omo1; Z[1362]++; if(dbg){print "Z1362"}; stopp=ret=1; continue };
   };

   cst="разбить";
   if ( weq(EF_n,cst) ) {
     if ( qxs(ef_n+1,"в","пыль прах") )
     { l[i]=omo1; Z[1363]++; if(dbg){print "Z1363"}; stopp=ret=1; continue };
   };
     ...
     if ( weq(EF_n,cst) ) {
      if ( bba(-5,-1,"_castle_platz") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,-5,-1) &&
              ...
              q(bb_n+2,"otsz_any") )
       { l[i]=omo1; Z[1364]++; if(dbg){print "Z1364"}; stopp=ret=1; continue };
      };
       cst1="высоко";
       if ( vgl(ef_n,1,3) && z(0) && s(1,ef_n) &&
              w(ef_n+1,cst1) &&
            qir(1,ef_n-1,"otsz_any nar_any qast_any") )
       { l[i]=omo1; Z[1365]++; if(dbg){print "Z1365"}; stopp=ret=1; continue };
    };
     ...
     if ( weq(EF_n,cst) && w(ef_n+1,"собой") ) {
       if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[1368]++; if(dbg){print "Z1368", "-- TN_v_zamok_f"}; stopp=ret=1; continue};
      ...
      if ( zs(0) && s(1,qf_n-1) &&
          ...
          qir(wfn+1,wfN-1,"prl_ro prq_ro mest_ro mest_3e") )
      { l[i]=omo1; Z[1369]++; if(dbg){print "Z1369"}; stopp=ret=1; continue };
    ...
    if ( vex(Y["n+_castlelike_things"]) ) { qf_n=Y["n+_castlelike_things"];
      ...
      { l[i]=omo1; Z[1370]++; if(dbg){print "Z1370"}; stopp=ret=1; continue };
    ...
    if ( vex(Y["n+_locklike_things"]) ) { qf_n=Y["n+_locklike_things"];
      ...
      if ( zs(0) && s(1,qf_n-1) &&
          ...
          qiz(ef_n+2,qf_n-1,"prl_vi prq_vi mest_vi mest_3e","qast") )
      { l[i]=omo2; Z[1371]++; if(dbg){print "Z1371"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) ) {
      ...
      { l[i]=omo2; Z[1372]++; if(dbg){print "Z1372"}; stopp=ret=1; continue };
...
#    if ( weq(EF_n,cst8) ) { if ( s(ef_n,3) && bf(ef_n+1,4,cst) ) ef_n=bfn };
     ...
     if ( bfa(ef_n+1,ef_n+5,"_castlelike_things")||
        ...
        ( bfa(ef_n+1,ef_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) { if (bfn) qb_n=bfn; if (wfn) qb_n=wfn;
         ...
         { qb_n=""; Z[1381]++; if(dbg){print "Z1381"}; };
         cst="ключом";
         if ( zs(0) && s(1,qb_n-1) &&
               w(qb_n,cst) && !w(qb_n+1,"к") )
         { qb_n=""; Z[1382]++; if(dbg){print "Z1382"}; };
      ...
      { l[i]=omo1; Z[1386]++; if(dbg){print "Z1386"}; stopp=ret=1; continue };
   ...
   if ( qf(ef_n+1,ef_n+4,"prl_kred_mu prq_kred_mu") ) { qf_n=qfn;
       ...
       if (  w(qf_n+1,"по") &&
           ...
           qir(qf_n+2,wfn-1,"prl_da prq_da mest_da mest_3e") )
       { l[i]=omo1; Z[1387]++; if(dbg){print "Z1387"}; stopp=ret=1; continue };
       if ( w(qf_n+1,"от") &&
           wf(qf_n+2,qf_n+5,cst2) && s(0,wfn-1) &&
          qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
          qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       ...
       if ( w(qf_n+1,"с со") &&
          ...
          qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
          qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       { l[i]=omo2; Z[1389]++; if(dbg){print "Z1389"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) ) {
       wfn=bfn=""; if ( bfa(ef_n+1,ef_n+5,"_locklike_things")||wf(ef_n+1,ef_n+5,cst2) ) { if (bfn) qf_n=bfn; if (wfn) qf_n=wfn;
          ...
          if ( vgl(ef_n,1,3) && zs(0) && s(1,qf_n-1) &&
               vex(Y["tn_ok_vi"]) )
          { qf_n=""; Z[1390]++; if(dbg){print "Z1390"}; };
       ...
       if ( zs(0) && w(qf_n,cst0) && s(1,qf_n) ) {
          ...
          { l[i]=omo1; Z[1391]++; if(dbg){print "Z1391"}; stopp=ret=1; continue };
       ...
       { l[i]=omo2; Z[1394]++; if(dbg){print "Z1394"}; stopp=ret=1; continue };
       if ( zs(0) && s(1,qf_n-1) &&
             q(qf_n,"suw_ro") &&
           qir(1,ef_n-1,"otsz_any mod_any gl_aux_be qast nar_any suw_ro isname") &&
       ...
       { l[i]=omo2; Z[1395]++; if(dbg){print "Z1395"}; stopp=ret=1; continue };
    ...
    if ( weq(EF_n,cst) ) {
      if ( wf(ef_n+1,ef_n+4,cst2) ) { qf_n=wfn;
        ...
        { l[i]=omo2; Z[1396]++; if(dbg){print "Z1396"}; stopp=ret=1; continue };
  ...
  if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo2; Z[1399]++; if(dbg){print "Z1399"}; stopp=ret=1; continue };

    if ( veq(ef_n,1) &&
        sz_i(2) &&
          ba(3,"_lock_bratj_i_gl") && zs(0) && s(1,2) )
    { l[i]=omo2; Z[1400]++; if(dbg){print "Z1400"}; stopp=ret=1; continue };
    if ( veq(ef_n,1) &&
           w(2,"у") &&
           q(3,"suw_ro mest_ro mest_3e") &&
          bw(4,"кровь") && s(0,3) )
    { l[i]=omo2; Z[1401]++; if(dbg){print "Z1401"}; stopp=ret=1; continue };

    if ( veq(ef_n,1) &&
          ba(2,"_lock_parts") && s(0,1) )
    { l[i]=omo2; Z[1402]++; if(dbg){print "Z1402"}; stopp=ret=1; continue };
    if ( ba(-2,"_lock_parts") &&
          q(-1,"prl_vi mest_vi mest_3e") &&
          w(1,"не") &&
        veq(ef_n,2) && s(-2,1) )
