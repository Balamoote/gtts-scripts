...
#{ print l[i-1] >> "_prl_a.txt"; Z[1]++; if(dbg){print "Z1"}; }; continue #mute
#if ( w(0,"замок") && wb(-4,-1,"в во") && s(wbn,-1) && qir(wbn+1,-1,"prl_vi mest_vi mest_3e prq_vi qik_vi qis_vi qast") )
#{ print book[b]; Z[2]++; if(dbg){print "Z2"}; }; continue #mute
#{ print book[b] >> "_va_zamok.txt"; Z[3]++; if(dbg){print "Z3"}; }; continue #mute
#if ( w(0,"замок") && wb(-4,-1,"под") && s(wbn,-1) )
#{ print book[b] >> "_pod_zamok.txt"; Z[4]++; if(dbg){print "Z4"}; }; continue #mute
#{ getBFx(-2,-2,"_glo.txt"); Z[5]++; if(dbg){print "Z5"}; }; continue #mute
#if ( w(0,"замок") && bb(-9,-1,"заменить") && s(bbn,-1) )
#{ print book[b] >> "_pod_zamok.txt"; Z[6]++; if(dbg){print "Z6"}; }; continue #mute
#if ( q(1,"isname") && q(2,"isname") && q(3,"isname") && s(0,2) )
#{ print l[i+1] sep[i+1] l[i+2] sep[i+2] l[i+3] sep[i+3] >> "_isname.txt"; Z[7]++; if(dbg){print "Z7"}; }; continue #mute
#if ( w(1,"между") &&
#     q(2,"prl_tv prq_tv mest_tv mest_3e") &&
#     q(3,"suw_tv") && s(0,2) && Q(3,"prl_tv")  )
#{ print l[i+3] >> "_ss_tv.txt"; Z[8]++; if(dbg){print "Z8"}; }; continue #mute
#if ( w(1,"между") &&
#     q(2,"prl_tv prq_tv mest_tv mest_3e") &&
#     q(3,"prl_tv prq_tv mest_tv mest_3e") &&
#     q(4,"suw_tv") && s(0,3) && Q(4,"prl_tv")  )
#{ print l[i+4] >> "_ss_tv.txt"; Z[9]++; if(dbg){print "Z9"}; }; continue #mute
#{ print l[i+1] >> "_sw.txt"; Z[10]++; if(dbg){print "Z10"}; }; continue #mute

#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue #mute
 # forced:start
#if ( q(1,"isname") && s(0) )
#{ print l[i+1] >> "_isname.txt"; Z[12]++; if(dbg){print "Z12"}; }; continue #mute
 ...
 { l[i]=omo1; Z[13]++; if(dbg){print "Z13"}; continue }; #hit_253
 if ( w2x(1,"_castle_w_w") && s(0,1) )
 { l[i]=omo1; Z[14]++; if(dbg){print "Z14"}; continue }; #hit_9412
 if ( w2x(1,"_lock_w_w") && s(0,1) )
 { l[i]=omo2; Z[15]++; if(dbg){print "Z15"}; continue }; #hit_1
...
if ( bfa(1,6,"_locklike_things")     ) { Y["n+_locklike_things"]    = bfn; Y["w+_locklike_things"]    = BFn; };
 ...
 if ( iwrd=="замок" || iwrd=="замки" )  { Y["iwrd_case"]="iv"; if ( iwrd=="замок" ) {Y["iwrd_form"]="ediv"} else {Y["iwrd_form"]="mniv"}; if(dbg){print "замок: им+вин, ед+мн"};
   ...
   cst="в во на за под подо о по через";
   if ( wb(-8,-1,cst) ) { qb_n=wbn;
    ...
    bfa(1,6,  "_gl_zamok");  Y["n+_gl_zamok"]   = bf_n = bfn; Y["w+_gl_zamok"]   = BFn;
       ...
       cst="казаться являться оказаться считаться";
...
#{ print book[b] >> "_vna.txt"; Z[16]++; if(dbg){print "Z16"}; }; continue #mute

    # pointesque <<-- складываем сюда исключения или случаи с опорными словами
    if ( FN_zamok_vi_esc_f() )
    { Z[17]++; if(dbg){print "Z17", "-- is_escape!"}; continue }; #hit_530
    # pointesque
 ...
 if (iwrd=="замка" || iwrd=="замков" ) { Y["iwrd_case"]="ro"; if ( iwrd=="замка" ) {Y["iwrd_form"]="edro"} else {Y["iwrd_form"]="mnro"}; if(dbg){print "замок: ро, ед+мн"};
    # определить, прикреплен ли к омографу предлог
   cst="до от ото из изо";
   if ( wb(-5,-1,cst) ) { qb_n=wbn;
    ...
    # hwro
    if ( bba(-5,-1,"_hw_castle-ro") ) { Y["_hw_castle-ro"] = bbn };
    if ( bba(-5,-1,"_hw_lock-ro") )   { Y["_hw_lock-ro"]   = bbn };
...
#===============================================================================================================================================================
 ...
 { l[i]=omo1; Z[18]++; if(dbg){print "Z18"}; continue }; #hit_189
 if ( isname(1) && wa(1,"_castle_numbered_royalty-ro") && roman(1) && s(0) )
 { l[i]=omo1; Z[19]++; if(dbg){print "Z19"}; continue }; #hit_27

 if ( isname(-1) && s(-1) && ba(-1,"_castle_isname_prl_forced") )
 { l[i]=omo1; Z[20]++; if(dbg){print "Z20"}; continue }; #hit_2800
 cst="верхолетный каменный магический центральный";
 if ( isname(-1) && s(-2,-1) && bw(-1,cst) )
 { l[i]=omo1; Z[21]++; if(dbg){print "Z21"}; continue }; #hit_235
 if ( isname(0) && s(-1) && bw(-1,cst) )
 { l[i]=omo1; Z[22]++; if(dbg){print "Z22"}; continue }; #hit_7

 if ( bba(-3,-1,"_lock_prl_forced") && FN_lock_prl_f("_lock_prl_forced") )
 { Z[23]++; if(dbg){print "Z23"}; continue }; #hit_6546
 if ( bfa(1,3,"_lock_prl_forced") && FN_lock_prl_f("_lock_prl_forced") )
 { Z[24]++; if(dbg){print "Z24"}; continue }; #hit_93

 if ( FN_hw_zamka_f() )
 { Z[25]++; if(dbg){print "Z25", "-- is_HW"}; continue }; #hit_110952

 if ( FN_lock_prl_f("_lock_prl_true") )
 { Z[26]++; if(dbg){print "Z26"}; continue }; #hit_30158
 ...
 if ( scw(0,cst) )
 { l[i]=omo1; Z[34]++; if(dbg){print "Z34"}; continue }; #hit_87
 if ( scw(-1,cst) )
 { l[i]=omo1; Z[35]++; if(dbg){print "Z35"}; continue }; #hit_88
 if ( s(0) && q(1,"isname") && sc(1,cst) )
 { l[i]=omo1; Z[36]++; if(dbg){print "Z36"}; continue }; #hit_17
 cst="Sesamee|Magzip|Zip|Lock|Fox|lock|Cartier|Mongoose";
 if ( s(0) && sc(0,cst) )
 { l[i]=omo2; Z[37]++; if(dbg){print "Z37"}; continue }; #hit_7
 cst="йейла чабба";
 if ( s(0) && isname(1) && w(1,cst) )
 { l[i]=omo2; Z[38]++; if(dbg){print "Z38"}; continue }; #hit_14
 cst="отрада светоч";
 if ( s(0) && isname(1) && w(1,cst) && sc(0,"[\x22«]") && sc(1,"[\x22»]") )
 { l[i]=omo1; Z[39]++; if(dbg){print "Z39"}; continue }; #hit_3
 cst="максима";
 if ( s(0) && isname(1) && w(1,cst) && sc(0,"[\x22«]") && sc(1,"[\x22»]") )
 { l[i]=omo2; Z[40]++; if(dbg){print "Z40"}; continue }; #hit_1
 if ( s(0) && isname(2) && qxs(1,"от",cst) && sc(1,"[\x22«]") && sc(2,"[\x22»]") )
 { l[i]=omo2; Z[41]++; if(dbg){print "Z41"}; continue }; #hit_7


 if ( q(0,"isname") && q(1,"isname") && zs(0) && zs(-1) &&
    !(s(0) && wa(1,"_lock_suw-ro") )  )
 { l[i]=omo1; Z[43]++; if(dbg){print "Z43"}; continue }; #hit_3813
 if ( prq_any(-1) && ba(-1,"_prq_lock") && s(-1) )
 { l[i]=omo2; Z[44]++; if(dbg){print "Z44"}; continue }; #hit_549

 if ( prq_any(-1) && ba(-1,"_prq_castle") && s(-1) )
 { l[i]=omo1; Z[45]++; if(dbg){print "Z45"}; continue }; #hit_364
 if ( q(1,"gl_aux_be") &&
      q(2,"prq_any") && ba(2,"_prq_castle") && s(0,1) )
 { l[i]=omo1; Z[46]++; if(dbg){print "Z46"}; continue }; #hit_249
 ...
 if ( q(-1,"isname") && q(0,"isname") && s(-1) && zs(-2) &&
   ...
   !(zs(0) && qy(1,"sz_iili") && s(0+qyn) && ba(1+qyn,"_lock_parts") ) &&
    !(s(0) && wa(1,"_lock_suw-ro") ) )
 { l[i]=omo1; Z[47]++; if(dbg){print "Z47"}; continue }; #hit_3261

 cst="окружить";
 if ( prq_any(-2) && bw(-2,cst) &&
            q(-1,"suw_tv mest_tv") && s(-2,-1) )
 { l[i]=omo1; Z[48]++; if(dbg){print "Z48"}; continue }; #hit_22
...
#{ l[i]=omo1; Z[52]++; if(dbg){print "Z52"}; continue }; #hit_1
 ...
 if ( wc(-1,cst) && s(-1) )
 { l[i]=omo2; Z[55]++; if(dbg){print "Z55"}; continue }; #hit_88

 cst="воздушный волшебный";
 if ( bw(-1,cst) &&
      sc(-2,"[\x22«]") && sc(0,"[\x22»]") && s(-1) &&
    !( z(0) && q(1,"prq_any") && ba(1,"_lock_pe") ) )
 { l[i]=omo1; Z[56]++; if(dbg){print "Z56"}; stopp=ret=1; continue }; #hit_172

 cst="дюжин дюжины сотени сотни тысяч тысячи";
 if ( w(1,cst) &&
      q(2,"suw_mnro") && s(0,1) )
 { l[i]=omo1; Z[57]++; if(dbg){print "Z57"}; continue }; #hit_23

 cst="выглядеть казаться оказаться показаться представлять";
 if ( q(-1,"nar_mest nar_napr") &&
     bf(1,3,cst) && s(-1,bfn-1) )
 { l[i]=omo1; Z[58]++; if(dbg){print "Z58"}; continue }; #hit_186

 if ( qxs(1,"средней","руки") )
 { l[i]=omo1; Z[60]++; if(dbg){print "Z60"}; continue }; #hit_2
 if ( qxw(-1,"да","ну","его") && z(-1) )
 { l[i]=omo1; Z[61]++; if(dbg){print "Z61"}; continue }; #hit_1
 ...
 if ( vgl(Y["n-_castlelike_things"],-6,-1) ) { bb_n=Y["n-_castlelike_things"]; BB_n=Y["w-_castlelike_things"];
    ...
    { l[i]=omo1; Z[63]++; if(dbg){print "Z63"}; continue }; #hit_267
    cst="более больше скорее";
    if (  w(-3,cst) && s(-3) &&
         veq(bb_n,-2) && z(-2) &&
          w(-1,"чем") && s(1) )
    { l[i]=omo1; Z[65]++; if(dbg){print "Z65"}; continue }; #hit_13
    if ( sc(-1,"-") && veq(bb_n,-1) )
    { l[i]=omo1; Z[79]++; if(dbg){print "Z79"}; continue }; #hit_339
 ...
 if ( vgl(Y["n+_castlelike_things"],1,7) ) { bf_n=Y["n+_castlelike_things"]; wf_n=Y["n-_lock_pe"];
    ...
    { l[i]=omo1; Z[64]++; if(dbg){print "Z64"}; continue }; #hit_4
    ...
    if ( !bw(wf_n,cst0)  ) {
      ...
      { bf_n=""; Z[66]++; if(dbg){print "Z66"}; }; #hit_1284
    ...
    { l[i]=omo1; Z[71]++; if(dbg){print "Z71"}; continue }; #hit_11
    ...
    { l[i]=omo1; Z[72]++; if(dbg){print "Z72"}; continue }; #hit_13
    if ( veq(bf_n,2) && q(-1,"qast_ne") && z(0) && s(-1) && s(1) &&
           q(1,"qast_ne") )
    { l[i]=omo1; Z[73]++; if(dbg){print "Z73"}; continue }; #hit_45
    if ( veq(bf_n,3) && q(-1,"qast_ne") && z(1) && s(-1,0) && s(2) &&
           q(1,"gl_aux_be") &&
           w(2,"а но") )
    { l[i]=omo1; Z[74]++; if(dbg){print "Z74"}; continue }; #hit_2
    ...
    { l[i]=omo1; Z[75]++; if(dbg){print "Z75"}; continue }; #hit_297

    if ( vgl(bf_n,3,7) && zs(0) && s(-1,bf_n-1) &&
     sz_iili(1) &&
         bqq(-1,2) )
    { l[i]=omo1; Z[76]++; if(dbg){print "Z76"}; continue }; #hit_159
    # дефис
    if ( sc(0,"-") && veq(Y["n+_castlelike_things"],1) )
    { l[i]=omo1; Z[78]++; if(dbg){print "Z78"}; continue }; #hit_629

    if ( vgl(bf_n,2,7) && w(1,"как") && z(0) && s(1,bf_n-1) &&
          qir(2,bf_n-1,"prl_im prl_vi prq_im prq_vi") )
    { l[i]=omo1; Z[92]++; if(dbg){print "Z92"}; continue }; #hit_44
 ...
 { l[i]=omo1; Z[77]++; if(dbg){print "Z77"}; continue }; #hit_1
 ...
 { l[i]=omo1; Z[111]++; if(dbg){print "Z111"}; continue }; #hit_1
 # _castlelike_acro:start
 ...
 if ( q(-1,"prl_edmuim mest_edmuim") && z(0) && s(-1) && s(1) &&
    ...
    !bw(2,cst0) )
 { l[i]=omo2; Z[115]++; if(dbg){print "Z115"}; continue }; #hit_37
 # escapes:end

 #
 if ( vgl(Y["n-_castle_environs"],-6,-2) ) { bb_n=Y["n-_castle_environs"];
   if ( veq(bb_n,-3) && s(-3) &&
          q(-2,"suw_ro mest_ro isname") && sc(-2,"—") &&
          q(-1,"suw_ro mest_ro isname") && s(-1) )
   { l[i]=omo1; Z[116]++; if(dbg){print "Z116"}; continue }; #hit_11

   if ( qq(-3,-1) && veq(bb_n,-2) && z(-2) && s(-3) && s(-1) )
   { l[i]=omo1; Z[117]++; if(dbg){print "Z117"}; continue }; #hit_238
   ...
   { l[i]=omo1; Z[118]++; if(dbg){print "Z118"}; continue }; #hit_67
   if ( vgl(bb_n,-6,-3) &&
   ...
   { l[i]=omo1; Z[119]++; if(dbg){print "Z119"}; continue }; #hit_20
   if ( veq(bb_n,-3) &&
    sz_iili(-2) &&
         qq(-4,-1) && s(-4,-1) )
   { l[i]=omo1; Z[120]++; if(dbg){print "Z120"}; continue }; #hit_101
   if ( q(-4,"prl_any mest_any prq_any") &&
      veq(bb_n,-3) &&
  sz_iili(-2) &&
       qq(-5,-1) && s(-5,-1) )
   { l[i]=omo1; Z[121]++; if(dbg){print "Z121"}; continue }; #hit_26
   if ( veq(bb_n,-4) &&
    sz_iili(-3) &&
          q(-1,"prl_any mest_any prq_any") &&
         qq(-5,-2) && s(-5,-1) )
   { l[i]=omo1; Z[122]++; if(dbg){print "Z122"}; continue }; #hit_27
   if ( veq(bb_n,-5) &&
    sz_iili(-4) &&
          w(-3,"даже") &&
          q(-1,"prl_any mest_any prq_any") &&
         qq(-6,-2) && s(-6,-1) )
   { l[i]=omo1; Z[123]++; if(dbg){print "Z123"}; continue }; #hit_1
   if ( veq(bb_n,-3) && qq(-4,-1) && qq(-5,-2) && z(-3) && s(-5,-4) && s(-2,-1)  )
   { l[i]=omo1; Z[124]++; if(dbg){print "Z124"}; continue }; #hit_87
 };
 if ( vgl(Y["n+_castle_environs"],1,6) ) { bf_n=Y["n+_castle_environs"]; BF_n=Y["w+_castle_environs"];
      ...
      if ( vgl(Y["n_pre_v_ist"],-8,-1) && vgl(Y["n-_lock_v_suw-pr"],Y["n_pre_v_ist"]+1,-1) && s(Y["n_pre_v_ist"],-1) &&
           qir(Y["n-_lock_v_suw-pr"]+1,-1,"gl_in gl_ed gl_mn qast_any") )
      { bf_n=""; Z[125]++; if(dbg){print "Z125"}; }; #hit_7
   var_do_tn="";
   ...
   if ( !var_do_tn ) {
    ...
    { bf_n=""; Z[133]++; if(dbg){print "Z133"}; }; #hit_1012
    ...
    { l[i]=omo1; Z[134]++; if(dbg){print "Z134"}; continue }; #hit_5697
    if ( veq(bf_n,2) && z(0) && s(1) &&
           q(1,"prl_any mcop_any") &&
           Q(2,"isname") )
    { l[i]=omo1; Z[135]++; if(dbg){print "Z135"}; continue }; #hit_549

    if ( veq(bf_n,2) && z(1) && s(0) &&
           q(1,"isname") )
    { l[i]=omo1; Z[136]++; if(dbg){print "Z136"}; continue }; #hit_160
    if ( pre_vi(-1) && s(-1) && z(0) &&
         pre_vi(1) && W(1,"на под") &&
            veq(bf_n,2) && qq(-1,1) && s(1) )
    { l[i]=omo1; Z[137]++; if(dbg){print "Z137"}; continue }; #hit_57
    ...
    if ( vgl(bf_n,2,7) && z(0) &&
        (qxw(1,"а","не","просто только")||
         ...
         qir(xwn+1,bf_n-1,"prl_any prq_any mest_any mest_3e") )
    { l[i]=omo1; Z[146]++; if(dbg){print "Z146"}; continue }; #hit_58

    if ( veq(bf_n,1) && sc(0,"[,—]") && s(1,2) && sz_iili(2) &&
          qf(3,6,"suw_any") &&
         qir(3,qfn-1,"prl_any mest_any prq_any") )
    { l[i]=omo1; Z[147]++; if(dbg){print "Z147"}; continue }; #hit_166
    if ( vgl(bf_n,3,6) && sc(0,"[,—]") && s(1,2) && sz_iili(2) &&
     suw_any(1) && Q(1,"gl_in") &&
         qir(3,qfn-1,"prl_any mest_any prq_any") )
    { l[i]=omo1; Z[148]++; if(dbg){print "Z148"}; continue }; #hit_59
 ...
 { l[i]=omo2; Z[149]++; if(dbg){print "Z149"}; continue }; #hit_41
 if ( sc(0,"[/-]") && ba(1,"_locklike_hyphens") )
 { l[i]=omo2; Z[150]++; if(dbg){print "Z150"}; continue }; #hit_269
 if ( sc(-1,"[/-]") && isacro(-1) && wa(-1,"_locklike_acro") )
 { l[i]=omo2; Z[151]++; if(dbg){print "Z151"}; continue }; #hit_9
 ...
 { l[i]=omo2; Z[152]++; if(dbg){print "Z152"}; continue }; #hit_17
 if ( sc(0,"[/-]") && w(1,"ключ") && s(-1) && bw(-1,cst) )
 { l[i]=omo2; Z[153]++; if(dbg){print "Z153"}; continue }; #hit_2
 # _locklike_hyphens:start

 # _locklike_things:start
 if ( bba(-3,-1,"_locklike_things") ) { bb_n=bbn;
   if ( veq(bb_n,-1) && se(-1," — ") )
   { l[i]=omo2; Z[155]++; if(dbg){print "Z155"}; continue }; #hit_111
   if ( veq(bb_n,-2) && s(-2) &&
          q(-1,"suw_ro") && se(-1," — ") )
   { l[i]=omo2; Z[156]++; if(dbg){print "Z156"}; continue }; #hit_3
   if ( z(-1) && veq(bb_n,-1) && !(isname(1) && bw(1,"люк")) )
   { l[i]=omo2; Z[157]++; if(dbg){print "Z157"}; continue }; #hit_642

   if ( qq(-4,-1) &&
         q(-2,"sz_iili") &&
       veq(bb_n,-3) && s(-4,-1) )
   { l[i]=omo2; Z[158]++; if(dbg){print "Z158"}; continue }; #hit_48
 };

 if ( bfa(1,5,"_locklike_things") ) { bf_n=bfn;
    ...
    { Z[159]++; if(dbg){print "Z159"}; continue }; #hit_124
    ...
    if ( z(0) && veq(bf_n,1) && s(1) && bw(1,cst1) &&
     ( qxw(2,"которого")||
       qxw(2,"от","которого") ) )
    { l[i]=omo1; Z[160]++; if(dbg){print "Z160"}; continue }; #hit_1
    cst1="дверь механизм";
    ...
    if ( bw(bf_n,cst) && z(bf_n) &&
          w(bf_n+1,"включая") &&
          w(bf_n+2,cst1) )
    { l[i]=omo1; Z[163]++; if(dbg){print "Z163"}; continue }; #hit_1
    if ( veq(bf_n,4) && bw(bf_n,cst) &&
         qxs(1,"и","все","его") )
    { l[i]=omo1; Z[164]++; if(dbg){print "Z164"}; continue }; #hit_1

    if ( qq(-1,2) &&
          q(1,"sz_iili") &&
        veq(bf_n,3) && s(-1,2) )
    { l[i]=omo2; Z[165]++; if(dbg){print "Z165"}; continue }; #hit_74
    if ( qq(-1,1) && z(0) && s(-1) &&
        veq(bf_n,2) && s(1) )
    { l[i]=omo2; Z[166]++; if(dbg){print "Z166"}; continue }; #hit_232
    if ( qq(-1,2) && qq(-2,1) && z(0) && s(-2,-1) &&
        veq(bf_n,3) && s(1,2) )
    { l[i]=omo2; Z[167]++; if(dbg){print "Z167"}; continue }; #hit_45
    if ( qq(-1,2) && zs(0) && s(-1) &&
    sz_iili(1) &&
        veq(bf_n,3) && s(1,2) )
    { l[i]=omo2; Z[168]++; if(dbg){print "Z168"}; continue }; #hit_3
    if ( qq(-1,1) && z(0) && s(-1) &&
          q(2,"prl_any mest_any prq_any nar_any") &&
        veq(bf_n,3) && s(1,2) )
    { l[i]=omo2; Z[169]++; if(dbg){print "Z169"}; continue }; #hit_28
    if ( qq(-1,2) && z(0) && s(-1) &&
          w(1,"а но однако ни и") &&
        veq(bf_n,3) && s(1,2) )
    { l[i]=omo2; Z[170]++; if(dbg){print "Z170"}; continue }; #hit_15
    if ( qq(-1,2) && z(0) && s(-1) &&
          w(1,"а но однако ни") &&
          q(3,"prl_any mest_any prq_any nar_any") &&
        veq(bf_n,4) && s(1,3) )
    { l[i]=omo2; Z[171]++; if(dbg){print "Z171"}; continue }; #hit_5

    if (  vgl(bf_n,2,4) && s(0,bf_n-1) &&
            q(1,"sz_iili") &&
          qir(2,bf_n-1,"prl_any mest_any prq_any") )
    { l[i]=omo2; Z[172]++; if(dbg){print "Z172"}; continue }; #hit_2237
    cst="вроде типа";
    if ( vgl(bf_n,2,4) &&
           w(bf_n-1,cst) && q(bf_n,"suw_ro") && s(0,bf_n-1) &&
         qir(1,bf_n-2,"prl_edmuim prl_kred_mu") )
    { l[i]=omo2; Z[173]++; if(dbg){print "Z173"}; continue }; #hit_1
    cst0="загадка";
    if ( veq(bf_n,1) && tz(0) && (tz(1)||q_w(2,"sz_iili")||p(1)) && W(1,cst0) )
    { l[i]=omo2; Z[174]++; if(dbg){print "Z174"}; continue }; #hit_303
    if ( veq(bf_n,1) && tz(0) && (tz(2)||q_w(3,"sz_iili")||p(2)) &&
           q(2,"suw_ro prl_vi prl_im") )
    { l[i]=omo2; Z[175]++; if(dbg){print "Z175"}; continue }; #hit_10
    if ( veq(bf_n,1) && tz(0) && (tz(3)||q_w(4,"sz_iili")||p(3)) &&
           q(2,"prl_kred_sr") &&
           q(3,"prl_im") )
    { l[i]=omo2; Z[176]++; if(dbg){print "Z176"}; continue }; #hit_1
 ...
 { l[i]=omo2; Z[177]++; if(dbg){print "Z177"}; continue }; #hit_13
 if ( z(0) &&
   (qxw(1,"а но","не и")||
    qxw(1,"а но","также","и")||
    qxw(1,"так","и") ) &&
 ...
 { l[i]=omo2; Z[178]++; if(dbg){print "Z178"}; continue }; #hit_6
 ...
 if ( qb(-4,-1,"sz_iili") ) { qb_n=qbn;
   ...
   { l[i]=omo2; Z[183]++; if(dbg){print "Z183"}; continue }; #hit_1722
 };};
 ...
 { l[i]=omo2; Z[184]++; if(dbg){print "Z184"}; continue }; #hit_1
 ...
 if ( wf(1,4,cst) ) { wf_n=wfn;
   ...
   { l[i]=omo1; Z[185]++; if(dbg){print "Z185"}; continue }; #hit_6
 ...
 { l[i]=omo1; Z[188]++; if(dbg){print "Z188"}; continue }; #hit_6

 if ( qxs(1,"с","лица","земли")||
      qxs(1,"снизу сверху","доверху донизу")||
      qxs(1,"с","ног","на","голову") )
 { l[i]=omo1; Z[189]++; if(dbg){print "Z189"}; continue }; #hit_71

 if ( veq(Y["iwrd_case"],"tv") ) {
    if ( vex(Y["tn_ok_pod_tv"]) && TN_pod_zamkom_f() )
    { Z[193]++; if(dbg){print "Z193", "-- TN_pod_zamkom_f"}; continue}; #hit_1808
 };
 if ( veq(Y["iwrd_case"],"ro") ) {
    if ( vex(Y["tn_ok_do"]) && TN_do_zamka_f() )
    { Z[194]++; if(dbg){print "Z194", "-- TN_do_zamka_f"}; continue}; #hit_2978
    if ( vex(Y["tn_ok_ot"]) && TN_ot_zamka_f() )
    { Z[195]++; if(dbg){print "Z195", "-- TN_ot_zamka_f"}; continue}; #hit_1524
    if ( vex(Y["tn_ok_iz"]) && TN_iz_zamka_f() )
    { Z[196]++; if(dbg){print "Z196", "-- TN_iz_zamka_f"}; continue}; #hit_5995
 };
 # suw_ro escape:end

 if ( FN_zamok_suwro_f() )
 { Z[197]++; if(dbg){print "Z197", "-- FN_zamok_suwro_f"}; continue }; #hit_24416
 ...
 { l[i]=omo1; Z[198]++; if(dbg){print "Z198"}; continue }; #hit_195
 ...
 { l[i]=omo2; Z[199]++; if(dbg){print "Z199"}; continue }; #hit_14
 if ( w(1,"без") &&
    wfa(2,5,"_lock_bez_suw-ro") && s(0,wfn-1) &&
    qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
 { l[i]=omo2; Z[200]++; if(dbg){print "Z200"}; continue }; #hit_103
 # _lock_bez_suw-ro:end

 # _castle_iz_suw-ro:start
 cst="построить сложить состоять строить быть";
 ...
 if ( wf2(bf_n+1,bf_n+5,"из изо") &&
      ...
      qir(bf_n+1,wfN-1,"prq_kred_mu gl_aux_be isname suw_ro suw_edne nar_any") &&
      qir(wfN+1,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
 { l[i]=omo1; Z[201]++; if(dbg){print "Z201"}; continue }; #hit_1353

 if ( wb(-5,-1,"из изо") ) { wb_n=wbn;
    if ( vgl(wb_n,-5,-1) && s(wb_n,-1) &&
         qir(wb_n+1,-2,"prl_ro prq_ro mest_ro mest_3e") &&
          wa(-1,"_castle_iz_suw-ro") )
    { l[i]=omo1; Z[202]++; if(dbg){print "Z202"}; continue }; #hit_66
 ...
 if ( w(1,"из") ) { wf_n=1;
    ...
    { wf_n=""; Z[205]++; if(dbg){print "Z205"}; continue }; #hit_11

    if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
    { Z[206]++; if(dbg){print "Z206", "-- TN_na_zamok_f"}; continue}; #hit_24

    if ( veq(wf_n,1) &&
         wfa(2,5,"_lock_iz_suw-ro") && s(0,wfn-1) &&
         qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
    { l[i]=omo2; Z[207]++; if(dbg){print "Z207"}; continue }; #hit_103
 };
 # _lock_iz_suw-ro:end
 ...
 if ( wf(1,6,"для") ) { wf_n=Y["n+_pre_dlya"]=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    { l[i]=omo1; Z[210]++; if(dbg){print "Z210"}; continue }; #hit_83

    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
#     if ( bam(-1,"_lock_prl _lock_prl_true") && s(-1) )
#     { wf_n=bf_n=""; Z[211]++; if(dbg){print "Z211"}; }; #mute
#     if ( wa(1,"_lock_suw-ro") && s(0) )
#     { wf_n=bf_n=""; Z[212]++; if(dbg){print "Z212"}; }; #mute
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[213]++; if(dbg){print "Z213"}; }; #hit_1
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[214]++; if(dbg){print "Z214", "-- TN_v_zamok_f"}; continue}; #hit_14
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[215]++; if(dbg){print "Z215", "-- TN_na_zamok_f"}; continue}; #hit_2
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[216]++; if(dbg){print "Z216", "-- TN_pod_zamok_f"}; continue}; #hit_1

      if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) &&
           qir(1,wf_n-1,"nar_any prl_kred_sr prl_kred_mu prq_kred_sr prq_kred_mu suw_vi prl_vi prl_tv pre_vi qast_any") &&
           qir(wf_n+1,bf_n-1,"prl_ro mest_ro mest_3e prq_ro") )
      { l[i]=omo1; Z[217]++; if(dbg){print "Z217"}; continue }; #hit_192
   };
 };
   # продолжаем "для"
   ...
   if ( bf(wf_n+2,wf_n+5,cst) ) { bf_n=bfn;
     if ( wf(bf_n+1,bf_n+4,cst2) ) { wb_n=wfn;
       ...
       { l[i]=omo2; Z[218]++; if(dbg){print "Z218"}; continue }; #hit_1
 ...
 if ( wb(-5,-2,"для") ) { wb_n=Y["n-_pre_dlya"]=wbn;
   if ( wba(wb_n+1,-1,"_castle_suw-ro1") ) { bb_n=wbn;
    ...
    { l[i]=omo1; Z[219]++; if(dbg){print "Z219"}; continue }; #hit_38
   ...
   if ( wba(wb_n+1,-1,"_castle_suw-ro2") ) { bb_n=wbn;
    ...
    if ( vgl(wb_n,-5,-1) && vgl(bb_n,wb_n+1,-1) && s(wb_n-1,-1) &&
         ...
         qir(wb_n+1,bb_n-1,"prl_ro mest_ro mest_3e prq_ro") &&
         qir(bb_n+1,-1,"prl_any mest_any") )
    { l[i]=omo1; Z[220]++; if(dbg){print "Z220"}; continue }; #hit_3
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[221]++; if(dbg){print "Z221"}; }; #hit_12

      cst0="гермодвери двери";
       if ( vgl(wf_n,1,5) && s(0,wf_n-1) && W(wf_n,cst0) &&
            vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[222]++; if(dbg){print "Z222", "-- TN_v_zamok_f"}; continue }; #hit_353

    if ( vgl(wf_n,2,5) && s(0,wf_n-1) &&
           w(1,"для у от") &&
         qir(2,wf_n-1,"prl_ro prq_ro mest_ro mest_3e") )
    { l[i]=omo2; Z[223]++; if(dbg){print "Z223"}; continue }; #hit_459
 };
 # _lock_suw-ro:end
 #######################################################################################################################
 ...
 if ( vgl(Y["n_pre_v_ist"],-6,-3) ) { wb_n=Y["n_pre_v_ist"];
    ...
    if ( q(wb_n+1,"isname") && s(wb_n,-1) &&
       ...
       qir(wb_n+3,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[224]++; if(dbg){print "Z224"}; continue }; #hit_32
 };
 if ( vgl(Y["n_pre_na_ist"],-6,-3) ) { wb_n=Y["n_pre_na_ist"];
    ...
    if ( q(wb_n+1,"isname") && s(wb_n,-1) &&
         q(wb_n+2,"gl_aux_be nar_any") &&
       qir(wb_n+3,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[225]++; if(dbg){print "Z225"}; continue }; #hit_7
 };
 # в географической местности
...
#       { l[i]=omo1; Z[235]++; if(dbg){print "Z235"}; continue }; #mute
 ...
 if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"]; wf_n=Y["n-_lock_v_suw-pr"];
    ...
    { l[i]=omo2; Z[239]++; if(dbg){print "Z239"}; continue }; #hit_122
    ...
    { l[i]=omo2; Z[240]++; if(dbg){print "Z240"}; continue }; #hit_68
    ...
    { l[i]=omo2; Z[241]++; if(dbg){print "Z241"}; continue }; #hit_2
 };
 # _lock_v_suw-pr:end
 ...
 if ( wf(1,3,"под") ) {wf_n=wfn;
     if ( wfa(wf_n+1,wf_n+4,"_castle_pod_suw-tv") ) { bf_n=wfn; bb_n=Y["n-_lock_pe"];
       ...
       { wf_n=""; Z[242]++; if(dbg){print "Z242"}; }; #hit_11

       cst="названием";
       if ( w(bf_n,cst) && !isname(bf_n+1) && s(0,bf_n) )
       { wf_n=""; Z[243]++; if(dbg){print "Z243"}; }; #hit_8
       cst="ударом";
       cst1="молота";
       if ( w(bf_n,cst) && w(bf_n+1,cst1) && s(0,bf_n) )
       { l[i]=omo2; Z[244]++; if(dbg){print "Z244"}; continue }; #hit_1
       ...
       { l[i]=omo1; Z[245]++; if(dbg){print "Z245"}; continue }; #hit_347
    };
    if ( vgl(wf_n,1,3) &&
           q(wf_n+1,"isname") && s(0,wf_n) )
    { l[i]=omo1; Z[246]++; if(dbg){print "Z246"}; continue }; #hit_155
   };
 if ( w(-2,"под") &&
     wa(-1,"_castle_pod_suw-tv") && s(-2,-1) )
 { l[i]=omo1; Z[247]++; if(dbg){print "Z247"}; continue }; #hit_42
 ...
 { l[i]=omo2; Z[248]++; if(dbg){print "Z248"}; continue }; #hit_18
 if ( q(1,"nar_spos qast") && zs(0) &&
      w(2,"под") &&
    wfa(3,6,"_lock_pod_suw-tv") && s(1,wfn-1) &&
    qir(3,wfn-1,"prl_tv prq_tv mest_tv mest_3e qast") )
 { l[i]=omo2; Z[249]++; if(dbg){print "Z249"}; continue }; #hit_1
 ...
 if ( wba(-5,-1,"_lock_pod_suw-tv") ) { bb_n=wbn;
   if ( wb(bb_n-4,bb_n-1,"под") ) { wb_n=wbn;
     ...
     { l[i]=omo2; Z[250]++; if(dbg){print "Z250"}; continue }; #hit_1
 };};
 ...
 if ( w(1,"над") &&
    ...
    qir(3,wfn-1,"prl_tv prq_tv mest_tv mest_3e qast") )
 { l[i]=omo1; Z[251]++; if(dbg){print "Z251"}; continue }; #hit_66
 if ( w(1,"над") &&
      q(2,"isname") && s(0,1) )
 { l[i]=omo1; Z[252]++; if(dbg){print "Z252"}; continue }; #hit_56
 if ( q(1,"qast") &&
      w(2,"над") &&
    wfa(3,6,"_castle_nad_suw-tv") && s(0,wfn-1) &&
    qir(3,wfn-1,"prl_tv prq_tv mest_tv mest_3e qast") )
 { l[i]=omo1; Z[253]++; if(dbg){print "Z253"}; continue }; #hit_1
 if ( w(-2,"над") &&
     wa(-1,"_castle_nad_suw-tv") && s(-2,-1) )
 { l[i]=omo1; Z[254]++; if(dbg){print "Z254"}; continue }; #hit_31
 ...
 { l[i]=omo1; Z[255]++; if(dbg){print "Z255"}; continue }; #hit_35
 cst="месте";
 if ( qxw(1,"на",cst) &&
      bfa(xwn+1,xwn+4,"_castle_environs") && s(0,bfn-1) &&
      qir(xwn+1,bfn-1,"prl_pr mest_pr mest_3e nar_any prq_pr") )
 { l[i]=omo1; Z[256]++; if(dbg){print "Z256"}; continue }; #hit_4
 ...
 #
  ...
  { l[i]=omo2; Z[259]++; if(dbg){print "Z259"}; continue }; #hit_2
  if ( wb(-5,-2,"на") && s(wbn,-1) &&
        w(wbn+1,"земле") &&
      qir(wbn+2,-1,"prl_vi mest_vi mest_3e prq_vi") )
  { l[i]=omo2; Z[260]++; if(dbg){print "Z260"}; continue }; #hit_2
 ...
 if ( wba(-7,-1,"_castle_na_suw-pr") ) { bb_n=wbn;
   ...
   cst0="двери";
    ...
    { l[i]=omo1; Z[261]++; if(dbg){print "Z261"}; continue }; #hit_640
    ...
    { l[i]=omo1; Z[262]++; if(dbg){print "Z262"}; continue }; #hit_91
    ...
    { l[i]=omo1; Z[263]++; if(dbg){print "Z263"}; continue }; #hit_21
    ...
    { l[i]=omo1; Z[269]++; if(dbg){print "Z269"}; continue }; #hit_1
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-3) ) { wb_n=Y["n_pre_na_ist"];
   if ( wfa(wb_n+1,wb_n+4,"_castle_na_suw-pr") ) { wf_n=wfn;
       ...
       { l[i]=omo1; Z[270]++; if(dbg){print "Z270"}; continue }; #hit_6
       if (  pre_ro(wf_n+1) &&
            q(-2,"suw_ro mest_ro") &&
           q(-1,"gl_aux_be") && s(wb_n,-1) )
       { l[i]=omo1; Z[271]++; if(dbg){print "Z271"}; continue }; #hit_2
       if ( pre_ro(wf_n+1) && s(wb_n,-1) &&
       ...
       { l[i]=omo1; Z[272]++; if(dbg){print "Z272"}; continue }; #hit_1
       ...
       { l[i]=omo1; Z[273]++; if(dbg){print "Z273"}; continue }; #hit_2
     ...
     if ( bfa(wf_n+1,-1,"_gl_4_castle_na_suw-pr") ) { bf_n=bfn;
       ...
       { l[i]=omo1; Z[274]++; if(dbg){print "Z274"}; continue }; #hit_398
       ...
       { l[i]=omo1; Z[275]++; if(dbg){print "Z275"}; continue }; #hit_85
       ...
       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
            ...
            qir(bf_n+1,-1,"prl_edmuim prl_mnim prq_edmuim prq_mnim mest_vi mest_3e prl_kred_sr nar_any qast suw_ro prq_ro mest_ro suw_tv prl_tv mest_tv") )
       { l[i]=omo1; Z[276]++; if(dbg){print "Z276"}; continue }; #hit_3
       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
              q(wf_n+1,"pre_tv") &&
            qir(wb_n+1,wf_n-1,"prl_pr prq_pr mest_pr mest_3e muk_pr") &&
   ...
   if ( wf(wb_n+1,wb_n+4,cst) ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_castle_na_suw-pr") ) { bf_n=bfn;
       ...
       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
            ...
            qir(bf_n+1,-1,"prl_edmuim prl_mnim prq_edmuim prq_mnim mest_vi mest_3e prl_kred_sr nar_any qast suw_ro prq_ro mest_ro suw_tv prl_tv mest_tv") )
       { l[i]=omo1; Z[279]++; if(dbg){print "Z279"}; continue }; #hit_12
   ...
   if ( wf(wb_n+1,wb_n+4,cst) ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_castle_na_suw-pr") ) { bf_n=bfn;
       ...
       { l[i]=omo1; Z[280]++; if(dbg){print "Z280"}; continue }; #hit_2
 ...
 if ( wf(1,3,"на") ) { wf_n=wfn;
     ...
     { Z[281]++; if(dbg){print "Z281"}; continue }; #hit_101
   ...
   { l[i]=omo2; Z[282]++; if(dbg){print "Z282"}; continue }; #hit_6

   if ( vgl(wf_n,1,3) &&
        wfa(wf_n+1,wf_n+4,"_castle_na_suw-pr") && s(0,wfn-1) &&
        qir(1,wf_n-1,"suw_ro suw_edne isname") &&
        qir(wf_n+1,wfn-1,"prl_pr mest_pr mest_3e nar_any prq_pr") )
   { l[i]=omo1; Z[283]++; if(dbg){print "Z283"}; continue }; #hit_3022
   ...
   { l[i]=omo1; Z[284]++; if(dbg){print "Z284"}; continue }; #hit_44
 };

 if ( bba(1,3,"_gl_4_castle_na_suw-pr") ) { bf_n=bbn;

     if ( bba(-3,-1,"_lock_prl") && s(bbn,-1) )
     { bf_n=""; Z[285]++; if(dbg){print "Z285"}; }; #hit_99
   ...
   { l[i]=omo1; Z[286]++; if(dbg){print "Z286"}; continue }; #hit_603
   if ( vgl(bf_n,1,3) && qsf(0,bf_n-1,"[,—]") && s(0,sfn-1) &&
        ...
        qir(bf_n+2,wfn-1,"prl_vi prl_pr otsz_vi otsz_pr mest_vi mest_pr prq_vi prq_pr") )
   { l[i]=omo1; Z[287]++; if(dbg){print "Z287"}; continue }; #hit_407
   ...
   { l[i]=omo1; Z[288]++; if(dbg){print "Z288"}; continue }; #hit_22
 };
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-2) ) { wb_n=Y["n_pre_na_ist"];
   if ( wfa(wb_n+1,wb_n+4,"_lock_na_suw-pr") ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_lock_na_suw-pr") ) { bf_n=bfn;
       ...
       { wb_n=wf_n=bf_n=""; Z[289]++; if(dbg){print "Z289"}; }; #hit_3

       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
            qir(wb_n+1,wf_n-1,"prl_pr prq_pr mest_pr mest_3e muk_pr") &&
            qir(wf_n+1,bf_n-1,"suw_ro mest_ro mest_3e isname prl_kred_sr nar_any mest_im suw_im suw_edne mod_any gl_aux_be suw_da mest_da prl_da prq_da qast") &&
            qir(bf_n+1,-1,"prl_edmuim prl_mnim prq_edmuim prq_mnim mest_vi mest_3e prl_kred_sr nar_any qast suw_ro prq_ro mest_ro suw_tv prl_tv mest_tv") )
       { l[i]=omo2; Z[290]++; if(dbg){print "Z290"}; continue }; #hit_846
     ...
     if ( bba(wb_n-3,wb_n-1,"_gl_4_lock_na_suw-pr") ) { bf_n=bbn;
       ...
       { l[i]=omo2; Z[291]++; if(dbg){print "Z291"}; continue }; #hit_84
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-2) ) { wb_n=Y["n_pre_na_ist"]; wf_n=Y["n_pre_v_ist"]
        ...
        { l[i]=omo1; Z[292]++; if(dbg){print "Z292"}; continue }; #hit_1
     ...
     if ( bba(-8,-1,"M_iscastle_lock_na_suw-pr_gl_list") ) { bb_n=bbn;
      ...
      if ( vgl(wb_n,-8,-2) && vgl(bf_n,wb_n+1,-1) && s(wb_n,-1) &&
           ...
           qir(bf_n+1,-1,"gl_need gl_vzed gl_aux_be prl_vi mest_vi mest_3e prq_vi qast_any suw_ro") &&
            sv(-1,"\\(") && Q_w(1,"isname") )
      { l[i]=omo2; Z[302]++; if(dbg){print "Z302"}; continue }; #hit_511
      ...
      { l[i]=omo2; Z[303]++; if(dbg){print "Z303"}; continue }; #hit_2
 };};


 if ( wa(-2,"_lock_na_suw-pr") && se(-2," — ") &&
       w(-1,"на") && s(-1) )
 { l[i]=omo2; Z[304]++; if(dbg){print "Z304"}; continue }; #hit_11
...
#       { wf_n=""; Z[312]++; if(dbg){print "Z312"}; }; #mute
        ...
        { wf_n=""; Z[314]++; if(dbg){print "Z314"}; }; #hit_9

     if ( vgl(wf_n,2,5) && ts(0) && s(1,wf_n-1) &&
          qir(2,wf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
     { l[i]=omo2; Z[315]++; if(dbg){print "Z315"}; continue }; #hit_6639
 };
 # _lock_na_suw-pr:end
   ...
   if ( wfa(wb_n+1,wb_n+4,"_lock_na_suw-vi") ) { wf_n=wfn;
     if ( bfa(wf_n+1,-1,"_gl_4_lock_na_suw-vi") ) { bf_n=bfn;
       ...
       if ( vgl(wb_n,-8,-3) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wf_n+1,-1) && s(wb_n,-1) &&
            ...
            qir(wf_n+1,bf_n-1,"suw_ro mest_ro mest_3e isname prl_kred_sr nar_any mest_im suw_im suw_edne mod_any gl_aux_be suw_da mest_da prl_da prq_da qast") &&
            qir(bf_n+1,-1,"prl_edmuim prl_mnim prq_edmuim prq_mnim mest_vi mest_3e prl_kred_sr nar_any qast suw_ro prq_ro mest_ro suw_tv prl_tv mest_tv") )
       { l[i]=omo2; Z[316]++; if(dbg){print "Z316"}; continue }; #hit_12
     ...
     if ( bba(wb_n-3,wb_n-1,"_gl_4_lock_na_suw-vi") ) { bf_n=bbn;
       ...
       if ( vgl(wb_n,-8,-2) && vgl(wf_n,wb_n+1,wb_n+4) && vgl(bf_n,wb_n-3,wb_n-1) && s(bf_n,-1) &&
            ...
            qir(wb_n+1,wf_n-1,"prl_vi prq_vi mest_vi mest_3e muk_vi") &&
            qir(wf_n+1,-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo2; Z[317]++; if(dbg){print "Z317"}; continue }; #hit_32
 ...
 if ( vgl(Y["n_pre_na_ist"],-8,-2) ) { wb_n=Y["n_pre_na_ist"];
   if ( wfa(wb_n+1,-1,"_lock_na_suw-vi") ) { bf_n=wfn;
      ...
      { bf_n=""; Z[318]++; if(dbg){print "Z318"}; }; #hit_58
      ...
      if ( vgl(wb_n,-8,-2) && vgl(bf_n,wb_n+1,-1) && s(wb_n,-1) &&
           ...
           qir(bf_n+1,-1,"gl_ed gl_in gl_mn gl_vzed gl_vzmn gl_po gl_aux_be prl_vi mest_vi mest_3e prq_vi qast_any") &&
            sv(-1,"\\(") && Q_w(1,"isname") )
      { l[i]=omo2; Z[319]++; if(dbg){print "Z319"}; continue }; #hit_430
 };};

 if ( wa(-2,"_lock_na_suw-vi") && se(-2," — ") &&
       w(-1,"на") && s(-1) )
 { l[i]=omo2; Z[320]++; if(dbg){print "Z320"}; continue }; #hit_43
...
#    { Z[321]++; if(dbg){print "Z321"}; continue }; #mute
#    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_suw_v_castle-vp"]) &&
#        mwba(wf_n,Y["n-_suw_v_castle-vp"],"M_lock_na_suw-vi_suw_v_castle-vp") && TN_v_zamok_f() )
#    { Z[322]++; if(dbg){print "Z322"}; continue }; #mute

     if ( vgl(wf_n,2,5) && s(0,wf_n-1) &&
          qir(2,wf_n-1,"prl_vi mest_vi mest_3e prq_vi") )
     { l[i]=omo2; Z[323]++; if(dbg){print "Z323"}; continue }; #hit_602
   };
...
#    { l[i]=omo2; Z[324]++; if(dbg){print "Z324"}; continue }; #mute
#  };};
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
     ...
     { l[i]=omo1; Z[338]++; if(dbg){print "Z338"}; continue }; #hit_154
     if ( vgl(wf_n,1,3) && qsf(0,wf_n-1,",") && s(0,sfn-1) &&
          ...
          qir(wf_n+1,wfn-1,"prl_ro prq_ro mest_ro mest_3e qik_ro") )
     { l[i]=omo1; Z[339]++; if(dbg){print "Z339"}; continue }; #hit_20
     # _castle_s_suw-ro:end

     # _castle_s_suw-tv:start
     if ( veq(wf_n,1) && qxs(2,"острым","краем") )
     { l[i]=omo2; Z[340]++; if(dbg){print "Z340"}; continue }; #hit_1
     ...
     { l[i]=omo1; Z[341]++; if(dbg){print "Z341"}; continue }; #hit_2122
     if ( vgl(wf_n,1,3) && zs(wf_n-1) && s(0,wf_n-2) &&
     ...
     { l[i]=omo1; Z[342]++; if(dbg){print "Z342"}; continue }; #hit_5
     # _castle_s_suw-tv:end
 };
 ...
 { l[i]=omo1; Z[343]++; if(dbg){print "Z343"}; continue }; #hit_9
 ...
 if ( wf(1,3,"за") ) { wf_n=wfn;
      ...
      if ( vgl(wf_n,1,3) && zs(wf_n-1) && s(0,wf_n-2) &&
           ...
           qir(1,wf_n-1,"suw_ro suw_edne isname sz_i") &&
           qir(wf_n+1,wfn-1,"prl_tv prq_tv mest_tv mest_3e qik_tv") )
     { l[i]=omo1; Z[344]++; if(dbg){print "Z344"}; continue }; #hit_94
     if ( vgl(wf_n,1,3) && zs(wf_n-1) && s(0,wf_n-2) &&
            q(wf_n+1,"prl_tv") && s(wf_n) && zs(wf_n+1) &&
            q(wf_n+2,"prq_tv") &&
          wfa(wf_n+3,wf_n+7,"_castle_za_suw-tv") && s(wf_n+2,wfn-1) &&
          qir(wf_n+3,wfn-1,"prl_tv prq_tv mest_tv mest_3e suw_tv suw_ro") )
     { l[i]=omo1; Z[345]++; if(dbg){print "Z345"}; continue }; #hit_1
 };
 ...
 { l[i]=omo1; Z[346]++; if(dbg){print "Z346"}; continue }; #hit_37
 # _castle_u_suw-ro:start
 ...
 if ( w(1,cst) &&
    ...
    qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e qik_ro") )
 { l[i]=omo1; Z[347]++; if(dbg){print "Z347"}; continue }; #hit_106
 if ( wb(-6,-2,cst) &&
      wa(wbn+1,"_castle_sredi_suw-ro") && s(wbn,-1) &&
     qia(wbn+2,-1,"gl_aux_be","prl_vi mest_3e mest_vi prq_vi") )
 { l[i]=omo1; Z[348]++; if(dbg){print "Z348"}; continue }; #hit_12
 # _castle_sredi_suw-ro:start
 ...
 if ( w(1,cst) &&
    ...
    qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e qik_ro") )
 { l[i]=omo2; Z[349]++; if(dbg){print "Z349"}; continue }; #mute
 if ( wb(-6,-2,cst) &&
      wa(wbn+1,"_lock_sredi_suw-ro") && s(wbn,-1) &&
     qia(wbn+2,-1,"gl_aux_be","prl_vi mest_3e mest_vi prq_vi") )
 { l[i]=omo2; Z[350]++; if(dbg){print "Z350"}; continue }; #hit_1
 # _castle_sredi_suw-ro:start

 # _castle_bez_suw-ro:start
 if ( w(-2,"без") &&
     wa(-1,"_castle_bez_suw-ro") && s(-2,-1) )
 { l[i]=omo1; Z[351]++; if(dbg){print "Z351"}; continue }; #hit_10
 # _castle_bez_suw-ro:start
 ...
 if ( wb(-8,-2,"с со") ) { wb_n=wbn;
   if ( wfa(wb_n+1,-1,"_lock_s_suw-ro") ) { bb_n=wfn;
      ...
      { l[i]=omo2; Z[352]++; if(dbg){print "Z352"}; continue }; #hit_116

 };};
 if ( w(1,"с со") &&
    wfa(2,5,"_lock_s_suw-ro") && s(0,wfn-1) &&
    qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e qik_ro") && W_w(wfn+1,"до по") )
 { l[i]=omo2; Z[353]++; if(dbg){print "Z353"}; continue }; #hit_420
 # _lock_s_suw-ro:end
 ...
 if ( w(1,"у") &&
    ...
    qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e qik_ro") && W_w(wfn+1,"до по") )
 { l[i]=omo2; Z[354]++; if(dbg){print "Z354"}; continue }; #hit_6
 if ( w(-2,"у") &&
     wa(-1,"_lock_u_suw-ro") && s(-2,-1) )
 { l[i]=omo2; Z[355]++; if(dbg){print "Z355"}; continue }; #hit_10
 # _lock_u_suw-ro:end
...
#     { l[i]=omo1; Z[360]++; if(dbg){print "Z360"}; continue }; #mute
    ...
    { l[i]=omo2; Z[361]++; if(dbg){print "Z361"}; continue }; #hit_1030
    ...
    { l[i]=omo2; Z[362]++; if(dbg){print "Z362"}; continue }; #hit_5
    if ( qsf(1,3,",") && vgl(wf_n,2,4) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,sfn-1) && z(sfn) && s(sfn+1,bf_n-1) &&
         ...
         qir(wf_n+1,bf_n-1,"prl_tv prq_tv mest_tv mest_3e qik_tv suw_tv") )
    { l[i]=omo2; Z[363]++; if(dbg){print "Z363"}; continue }; #hit_13
#   if ( vgl(wf_n,1,4) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) &&
#      mqast(1) && veq(xwn,wf_n-1) &&
#        qir(wf_n+1,bf_n-1,"prl_tv prq_tv mest_tv mest_3e qik_tv") && W_w(bf_n+1,"до по") )
#   { l[i]=omo2; Z[364]++; if(dbg){print "Z364"}; continue }; #mute
 ...
 { l[i]=omo1; Z[365]++; if(dbg){print "Z365"}; continue }; #hit_99

 if ( sc(0,"[XIV]+") &&
       w(1,"века веков столетия столетий") && s(0) )
 { l[i]=omo1; Z[366]++; if(dbg){print "Z366"}; continue }; #hit_94
 if ( sc(0,"[XIV]+") &&
       w(1,"в вв") && s(0) && sc(1,"\\.") )
 { l[i]=omo1; Z[367]++; if(dbg){print "Z367"}; continue }; #hit_43

  ######################################################################################################
    # основные прилагательные -- castle

    if ( FN_castle_prl_f("_castle_prl") )
    { Z[368]++; if(dbg){print "Z368", "-- FN_castle_prl_f(\"_castle_prl\")"}; continue }; #hit_26198
    if ( q(-1,"prl_pv_any") && ba(-1,"_castle_prl_pv") )
    { l[i]=omo1; Z[369]++; if(dbg){print "Z369"}; continue }; #hit_29

    # основные прилагательные -- lock
    if ( FN_lock_prl_f("_lock_prl") )
    { Z[370]++; if(dbg){print "Z370", "-- FN_lock_prl_f(\"_lock_prl\")"}; continue }; #hit_1678

    if ( mbba(-1,2,"M_lock_2prl") && s(-1) && z(0) && s(1) && q(1,"nar_step") )
    { l[i]=omo2; Z[371]++; if(dbg){print "Z371"}; continue }; #hit_1
    if ( mbba(-2,-1,"M_lock_2prl") && s(-2,-1) )
    { l[i]=omo2; Z[372]++; if(dbg){print "Z372"}; continue }; #hit_10
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    { l[i]=omo1; Z[373]++; if(dbg){print "Z373"}; continue };b #hit_363
    ...
    { l[i]=omo1; Z[374]++; if(dbg){print "Z374"}; continue };b #hit_104
    if ( se(0," — ") &&
        qsf(1,5,",") && se(sfn+1," — ") &&
         ba(sfn+1,"_castlelike_things") )
    { l[i]=omo1; Z[375]++; if(dbg){print "Z375"}; continue }; #hit_3
    if ( q(-1,"mest_it") && s(-1) &&
        ba(-2,"_castlelike_things") && se(-2," — ") )
    { l[i]=omo1; Z[376]++; if(dbg){print "Z376"}; continue }; #hit_11
    ...
    { l[i]=omo1; Z[377]++; if(dbg){print "Z377"}; continue }; #hit_3
    ...
    { l[i]=omo1; Z[378]++; if(dbg){print "Z378"}; continue }; #hit_1
    if ( sc(0,"\\(") && sc(1,"\\)") && veq(Y["n+_castlelike_things"],1) )
    { l[i]=omo1; Z[379]++; if(dbg){print "Z379"}; continue }; #hit_9
    ...
    { l[i]=omo1; Z[380]++; if(dbg){print "Z380"}; continue }; #hit_2
    ...
    { l[i]=omo1; Z[381]++; if(dbg){print "Z381"}; continue }; #hit_2
    ...
    if ( wb(-4,-1,"с со") && wb2(wbn-4,wbn-1,cst) && s(wbN,-1) &&
        ...
        qir(wbn+1,-1,"prl_vi mest_vi mest_3e prq_vi") )
    { l[i]=omo1; Z[387]++; if(dbg){print "Z387"}; continue }; #hit_92
    ...
    if ( narph_spos(1) && s(0) && z(xwn) &&
                  w(xwn+1,cst1) &&
                  q(xwn+2,"nar_spos") )
    { l[i]=omo2; Z[394]++; if(dbg){print "Z394"}; continue }; #hit_1
    ...
    if ( wf(2,6,cst1) ) { bf_n=wfn;
       ...
       { l[i]=omo1; Z[395]++; if(dbg){print "Z395"}; continue }; #hit_87
    };
    if ( qxs(-1,"до",cst1) )
    { l[i]=omo1; Z[396]++; if(dbg){print "Z396"}; continue }; #hit_9
    cst1="булыжники груду камешки камни камушки кирпичи кирпичики клочья куски кусочки пыль щепки щепы";
    if ( wf(2,5,cst1) ) { bf_n=wfn;
       if ( vgl(bf_n,2,5) && s(0,bf_n-1) &&
              w(1,"в во на") &&
            qir(2,bf_n-1,"prl_vi") )
       { l[i]=omo1; Z[397]++; if(dbg){print "Z397"}; continue }; #hit_36
    };
    if ( wb(-4,-1,cst1) ) { wb_n=wbn;
       ...
       { l[i]=omo1; Z[398]++; if(dbg){print "Z398"}; continue }; #hit_10
    };
    ...
    if ( wf(2,5,cst1) ) { bf_n=wfn;
       if ( vgl(bf_n,2,5) && s(0,bf_n-1) &&
              w(1,"по") &&
            qir(2,bf_n-1,"prl_da") )
       { l[i]=omo1; Z[399]++; if(dbg){print "Z399"}; continue }; #hit_83
    };
    ...
    if ( bw(1,cst) &&
          w(2,"из") && s(0,1) )
    { l[i]=omo1; Z[405]++; if(dbg){print "Z405"}; stopp=ret=1; continue }; #hit_2
    if ( !vex(Y["tn_ok_v"]) && bw(1,cst) &&
            q(2,"nar_spos nar_kaq prl_kred_sr") && s(0,1) )
    { l[i]=omo1; Z[406]++; if(dbg){print "Z406"}; stopp=ret=1; continue }; #hit_1
    if ( bw(1,cst1) &&
         bf(2,5,cst) &&
          w(bfn+1,"в из по") && s(0,bfn) && Qf(2,bfn-1,"pre_any sz_iili") )
    { l[i]=omo1; Z[407]++; if(dbg){print "Z407"}; continue }; #hit_5
    if ( bw(1,cst) && s(0) &&
       (qxs(2,"вовсе совсем никак отнюдь","не")||
        qxs(2,"не") ) &&
          w(xsn+1,"из") && s(xsn+1) )
    { l[i]=omo1; Z[408]++; if(dbg){print "Z408"}; stopp=ret=1; continue }; #hit_1
    if ( wb(-7,-4,"из") && s(wbn,-1) &&
          q(wbn+1,"otsz_ro") &&
         bw(wbn+2,cst1) &&
         bw(wbn+3,cst) &&
        qir(wbn+4,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[409]++; if(dbg){print "Z409"}; stopp=ret=1; continue }; #hit_8
    # сложен
    ...
    if ( w(-4,"у") &&
         ...
         q(-2,"nar_step") &&
         w(-1,cst1) && s(-4,-1) )
    { l[i]=omo1; Z[410]++; if(dbg){print "Z410"}; continue }; #hit_5
    if ( w(-3,"у") &&
         q(-2,"mest_ro suw_ro") &&
         w(-1,cst1) && s(-3,-1) )
    { l[i]=omo1; Z[411]++; if(dbg){print "Z411"}; continue }; #hit_25

    # _lock_prq_kak_gl:start
    cst1="быть ведь же казаться стать считаться являться";
#   if ( q(-1,"prq_im prq_kred_mu") && bw(-2,cst1) && ba(-1,"_lock_prq_kak_gl") && s(-2,-1) )
#   { l[i]=omo2; Z[414]++; if(dbg){print "Z414"}; continue }; #mute
    if ( q(2,"prq_im prq_kred_mu") && bw(1,cst1) && ba(2,"_lock_prq_kak_gl") && s(0,1) &&
      (!ba(-1,"_castle_prl") && s(-1)) )
    { l[i]=omo2; Z[415]++; if(dbg){print "Z415"}; continue }; #hit_3
    ...
    if ( wb(-5,-1,cst) ) { wb_n=wbn;
       ...
       { l[i]=omo1; Z[417]++; if(dbg){print "Z417"}; continue }; #hit_3411
#      if ( veq(wb_n,-2) &&
#            wc(-1,"ский$") && s(-2,-1) )
#      { l[i]=omo1; Z[418]++; if(dbg){print "Z418"}; continue }; #hit_2
    };

    if ( w(1,cst) && s(0) && p(1) )
    { l[i]=omo1; Z[419]++; if(dbg){print "Z419"}; continue }; #hit_10
    cst="весь целиком";
    if ( w(1,cst) && se(0," — ") && ( p(1)||z(1) ) )
    { l[i]=omo1; Z[420]++; if(dbg){print "Z420"}; continue }; #hit_1
    # весь
    ...
    if ( ba(-5,"_lock_pe") &&
         ...
         bw(-3,cst1) &&
          w(-2,"в") &&
         bw(-1,cst2) && s(-5,-1) )
    { l[i]=omo2; Z[425]++; if(dbg){print "Z425"}; continue }; #hit_1

  # в замок --->>>
  if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"];

    if ( TN_v_zamok_f() )
    { Z[426]++; if(dbg){print "Z426", "-- TN_v_zamok_f"}; continue }; #hit_51292
    ...
    { l[i]=omo1; Z[428]++; if(dbg){print "Z428"}; continue }; # без tn! #hit_4
  ...
  if ( vgl(Y["n_pre_na_ist"],-8,-1) ) { wb_n=Y["n_pre_na_ist"];
    ...
    if ( TN_na_zamok_f() )
    { Z[430]++; if(dbg){print "Z430", "-- TN_na_zamok_f"}; continue }; #hit_14873

    if ( veq(wb_n,-3) &&
          ba(-4,"_suw_na_castle") && s(-4,-1) &&
          wa(-2,"_castle_environs") &&
        sz_i(-1) )
    { l[i]=omo1; Z[431]++; if(dbg){print "Z431"}; continue }; #hit_1
    if ( veq(wb_n,-3) &&
          ba(-4,"_gl_na_lock") &&
           q(-2,"prl_vi prq_vi") &&
           q(-1,"suw_vi") && s(-4,-1) )
    { l[i]=omo2; Z[432]++; if(dbg){print "Z432"}; continue }; #hit_1
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[433]++; if(dbg){print "Z433", "-- TN_za_zamok_f"}; continue }; #hit_387
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[434]++; if(dbg){print "Z434", "-- TN_pod_zamok_f"}; continue }; #hit_4127
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[435]++; if(dbg){print "Z435", "-- TN_o_zamok_f"}; continue }; #hit_32
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[436]++; if(dbg){print "Z436", "-- TN_po_zamok_f"}; continue }; #hit_1
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[437]++; if(dbg){print "Z437", "-- TN_qerez_zamok_f"}; continue }; #hit_21
  ...
  # ниже "замок" без собственного предлога
   ...
   #
    ...
    { l[i]=omo2; Z[483]++; if(dbg){print "Z483"}; continue }; #hit_6
    ...
    { l[i]=omo1; Z[494]++; if(dbg){print "Z494"}; continue }; #hit_72
    ...
    { l[i]=omo1; Z[495]++; if(dbg){print "Z495"}; continue }; #hit_108
    ...
    if ( bf(1,3,cst) ) { bf_n=bfn;
      ...
      { l[i]=omo2; Z[496]++; if(dbg){print "Z496"}; continue }; #hit_1
      ...
      { l[i]=omo1; Z[497]++; if(dbg){print "Z497"}; continue }; #hit_4
    ...
    if ( vgl(Y["n_pre_v_ist"],-8,-1) ) { wb_n=Y["n_pre_v_ist"];
       ...
       { l[i]=omo1; Z[498]++; if(dbg){print "Z498"}; continue }; #hit_2
    };
    ...
    { l[i]=omo1; Z[504]++; if(dbg){print "Z504"}; continue }; #hit_23
    if ( w(-2,cst2) && s(-2,-1) &&
        bw(-1,cst1) )
    { l[i]=omo1; Z[505]++; if(dbg){print "Z505"}; continue }; #hit_1
    ...
    if ( bw(-4,cst1) && z(-4) &&
     pre_tv(-3) &&
          q(-2,"otsz_tv") &&
         bw(-1,cst2) && s(-3,-1) )
    { l[i]=omo2; Z[506]++; if(dbg){print "Z506"}; continue }; #hit_1

    cst1="пасть";
    if ( bw(1,cst1) && s(0) &&
        qxs(2,"смертью","храбрых") )
    { l[i]=omo2; Z[507]++; if(dbg){print "Z507"}; continue }; #hit_1

    if ( bw(1,"ходить") &&
       (qxs(2,"на","ушах")||
          w(2,"ходуном") ) && s(0,1) )
    { l[i]=omo1; Z[519]++; if(dbg){print "Z519"}; continue }; #hit_4

    cst="открывать открываться открыть открыться";
    if ( bw(-3,cst) &&
          w(-2,"перед") &&
          q(-1,"suw_tv mest_tv") && s(-3,-1) )
    { l[i]=omo1; Z[520]++; if(dbg){print "Z520"}; continue }; #hit_3
    if ( w(-3,"перед") &&
         q(-2,"suw_tv mest_tv") &&
        bw(-1,cst) && s(-3,-1) )
    { l[i]=omo1; Z[521]++; if(dbg){print "Z521"}; continue }; #hit_3

    cst="потянуть тянуть";
    if ( w(1,"не") &&
        bw(2,cst) && s(0,1) )
    { l[i]=omo1; Z[522]++; if(dbg){print "Z522"}; continue }; #hit_1
    cst1="башня время сила стена";
    if ( bw(1,cst) &&
         bw(2,cst1) && s(0,1) )
    { l[i]=omo1; Z[523]++; if(dbg){print "Z523"}; continue }; #hit_2
    if ( q(1,"suw_da mest_da") &&
         w(2,"не") &&
        bw(3,cst) && s(0,2) )
    { l[i]=omo1; Z[524]++; if(dbg){print "Z524"}; continue }; #hit_1
    if ( bw(1,cst) &&
          q(2,"suw_ro mest_ro mest_3e") &&
          w(3,"к ко") && s(0,2) )
    { l[i]=omo1; Z[525]++; if(dbg){print "Z525"}; continue }; #hit_1
    if ( bf(1,5,cst) && Qf(1,bfn-1,"sz_iili pre_any") &&
          w(bfn+1,"из") &&
          q(bfn+2,"suw_ro mest_ro mest_3e") && s(0,bfn+1) )
    { l[i]=omo1; Z[526]++; if(dbg){print "Z526"}; continue }; #hit_1
    if ( bf(1,3,cst) && Qf(1,bfn-1,"sz_iili pre_any") &&
          q(bfn+1,"suw_vi mest_vi mest_3e") &&
          q(bfn+2,"nar_any prl_kred_sr") && s(0,bfn+1) )
    { l[i]=omo1; Z[527]++; if(dbg){print "Z527"}; continue }; #hit_1
    ...
    if ( bw(-1,cst) &&
          w(1,"от") &&
         bw(2,cst1) && s(-1,1) )
    { l[i]=omo1; Z[530]++; if(dbg){print "Z530"}; continue }; #hit_6
    if ( bw(-1,cst) &&
          w(1,"от") &&
          q(2,"prl_ro prq_ro") &&
         bw(3,cst1) && s(-1,2) )
    { l[i]=omo1; Z[531]++; if(dbg){print "Z531"}; continue }; #hit_1
      ...
      if ( bb(-3,-1,cst) ) { bb_n=bbn;
        if ( vgl(bb_n,-3,-1) && s(bb_n,wf_n-1) &&
           ( qxs(wf_n,"из","его ее её их ваших цепких","лап рук")||
             ...
             qir(1,wf_n-1,"nar_any prl_kred_sr isname") )
        { l[i]=omo1; Z[532]++; if(dbg){print "Z532"}; continue }; #hit_1
    };};
    ...
    { l[i]=omo1; Z[533]++; if(dbg){print "Z533"}; continue }; #hit_1
    ...
    if ( bw(1,cst) &&
          w(2,"в во") &&
         bw(3,cst1) && s(0,2) )
    { l[i]=omo2; Z[534]++; if(dbg){print "Z534"}; continue }; #hit_1
    ...
    if ( bw(-2,cst1) &&
          w(-1,cst) && s(-2,-1) )
    { l[i]=omo1; Z[540]++; if(dbg){print "Z540"}; continue }; #hit_1
    ...
    if ( qxs(1,"заместитель","командира")||
         qxs(1,"начтыла") )
    { l[i]=omo2; Z[549]++; if(dbg){print "Z549"}; continue }; #hit_2

    cst1="капитулировать сдаться";
    cst2="возня минута секунда";
    if ( bw(-1,cst2) &&
         bw(1,cst1) && s(-1,0) )
    { l[i]=omo2; Z[550]++; if(dbg){print "Z550"}; continue }; #hit_4
    if ( bw(1,cst1) &&
          w(2,"через после") &&
         bf(3,6,cst2) && s(0,bfn-1) )
    { l[i]=omo2; Z[551]++; if(dbg){print "Z551"}; continue }; #hit_4
    ...
    if ( bf(1,3,cst1) ) { bf_n=bfn;
       ...
       { l[i]=omo2; Z[552]++; if(dbg){print "Z552"}; continue }; #hit_1
       if ( vgl(bf_n,1,3) && wf(bf_n+1,bf_n+4,cst3) && zs(0) && s(1,wfn-1) &&
            qir(bf_n+1,wfn-1,"prl_tv mest_tv mest_3e") &&
             Qf(1,bf_n-1,"sz_iili pre_any") )
       { l[i]=omo2; Z[553]++; if(dbg){print "Z553"}; continue }; #mute
    };
    ...
    if ( bf(1,3,cst1) ) { bf_n=bfn;
       ...
       { l[i]=omo1; Z[554]++; if(dbg){print "Z554"}; continue }; #hit_1
    };

    if ( bw(1,"загореться") &&
          w(2,"зеленым красным") && s(0,1) )
    { l[i]=omo2; Z[555]++; if(dbg){print "Z555"}; continue }; #hit_2
    ...
    if ( vgl(Y["n-_lock_pe"],-7,-1) ) { bb_n=Y["n-_lock_pe"]; BB_n=Y["w-_lock_pe"];
       ...
       { l[i]=omo1; Z[556]++; if(dbg){print "Z556"}; continue }; #hit_1
       cst="заменить";
       if ( vgl(bb_n,-7,-1) && s(bb_n,-1) &&
       ( isname(0) || q_w(1,"isname") ) &&
             bw(bb_n,cst) )
       { l[i]=omo1; Z[557]++; if(dbg){print "Z557"}; continue }; #hit_4
#      cst="поискать";
#      if ( vgl(bb_n,-7,-1) && bw(bb_n,cst) && s(bb_n,0) && q(1,"isname") &&
#           qir(bb_n+1,-1,"prl_vi mest_vi mest_3e") )
#      { l[i]=omo1; Z[558]++; if(dbg){print "Z558"}; continue }; #mute
       cst="достать";
       if ( vgl(bb_n,-7,-1) && bw(bb_n,cst) && s(bb_n,-1) && z(0) &&
              w(bb_n-1,"настолько так") && w(1,"что") &&
            qir(bb_n+1,-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[559]++; if(dbg){print "Z559"}; continue }; #hit_1
       cst="собирать собрать";
       if ( vgl(bb_n,-7,-1) && bw(bb_n,cst) && s(bb_n,-1) &&
              w(bb_n+1,"с со") &&
            qir(bb_n+2,-1,"prl_tv mest_tv mest_3e isname") )
       { l[i]=omo1; Z[560]++; if(dbg){print "Z560"}; continue }; #hit_2
       if ( veq(bb_n,-1) && bw(bb_n,cst) && isname(0) && s(-1) )
       { l[i]=omo1; Z[561]++; if(dbg){print "Z561"}; continue }; #hit_1
       cst0="разбирать";
       if ( vgl(bb_n,-7,-3) && q(bb_n+1,"sz_iili") && bw(bb_n+2,cst) && s(bb_n,-1) && bw(bb_n,cst0) &&
            qir(bb_n+3,-1,"prl_vi mest_vi mest_3e prq_vi") )
       { l[i]=omo2; Z[562]++; if(dbg){print "Z562"}; continue }; #hit_1
       ...
       if ( vgl(bb_n,-7,-1) && s(bb_n,-1) &&
             bw(bb_n,cst) &&
              w(bb_n+1,cst1) &&
            qir(bb_n+2,-1,"prl_vi prq_vi mest_vi mest_3e qast nar_any prl_kred_sr") )
       { l[i]=omo1; Z[566]++; if(dbg){print "Z566"}; continue }; #hit_3
       ...
       { l[i]=omo1; Z[567]++; if(dbg){print "Z567"}; continue }; #hit_1
    };
    # esc+_lock_pe
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"]; BF_n=Y["w+_lock_pe"];
       ...
       { l[i]=omo1; Z[568]++; if(dbg){print "Z568"}; continue }; #hit_1
       ...
       { l[i]=omo1; Z[569]++; if(dbg){print "Z569"}; continue }; #hit_2
#      cst="валяться висеть лежать";
#      if ( bw(bf_n,cst) &&
#            w(bf_n+1,"за") &&
#           wf(bf_n+2,bf_n+5,"дверью") && s(0,wfn-1) )
#      { l[i]=omo2; Z[570]++; if(dbg){print "Z570"}; continue }; #mute
       ...
       { l[i]=omo1; Z[571]++; if(dbg){print "Z571"}; continue }; #hit_1
       if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
             bw(bf_n,cst) &&
              w(bf_n+1,cst2) &&
            qir(1,bf_n-1,"suw_ro suw_edne isname") )
       { l[i]=omo1; Z[572]++; if(dbg){print "Z572"}; continue }; #hit_1
       ...
       { l[i]=omo1; Z[573]++; if(dbg){print "Z573"}; continue }; #hit_4
       ...
       if ( vgl(bf_n,1,6) && veq(Y["n-_pre_dlya"],-2) && s(-2,bf_n) &&
             ...
             bw(bf_n,cst) &&
              w(bf_n+1,cst3) )
       { l[i]=omo1; Z[574]++; if(dbg){print "Z574"}; continue }; #hit_1
       if ( vgl(bf_n,1,6) &&
             weq(BF_n,cst) &&
             wf(bf_n+1,bf_n+4,cst1) && s(0,wfn-1) &&
            qir(1,bf_n-1,"isname nar_any") &&
            qir(bf_n+1,wfn-1,"prl_vi mest_vi mest_da suw_da isname") )
       { l[i]=omo1; Z[575]++; if(dbg){print "Z575"}; continue }; #hit_11
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
       ...
       { l[i]=omo1; Z[576]++; if(dbg){print "Z576"}; continue }; #hit_1
       ...
       { l[i]=omo1; Z[577]++; if(dbg){print "Z577"}; continue }; #hit_2
       if ( vgl(bf_n,1,6) && s(0,bf_n) &&
             bw(bf_n,cst) &&
             wf(bf_n+1,bf_n+5,"от") &&
             wf(TN+1,TN+5,cst3) )
       { l[i]=omo1; Z[578]++; if(dbg){print "Z578"}; continue }; #hit_1
       cst="треснуть";
       if ( bw(bf_n,cst) && s(0,bf_n-1) &&
           qxs(bf_n+1,"по","швам") )
       { l[i]=omo1; Z[579]++; if(dbg){print "Z579"}; continue }; #hit_1
       if ( bw(bf_n,cst) &&
             w(bf_n+1,"от") &&
            wf(bf_n+2,bf_n+6,"людей гостей") && s(0,wfn-1) )
       { l[i]=omo1; Z[580]++; if(dbg){print "Z580"}; continue }; #hit_1
       cst="висеть";
       if ( bw(bf_n,cst) &&
             w(bf_n+1,"в") &&
            wf(bf_n+2,bf_n+5,"пространстве небе воздухе") && s(0,wfn-1) )
       { l[i]=omo1; Z[581]++; if(dbg){print "Z581"}; continue }; #hit_3
       if ( bw(bf_n,cst) &&
       ...
       { l[i]=omo1; Z[582]++; if(dbg){print "Z582"}; continue }; #hit_1
    ...
    if ( vgl(Y["n-_castle_pe"],-5,-1) ) { bb_n=Y["n-_castle_pe"];
       ...
       if ( vgl(bb_n,-5,-1) && s(bb_n,0) &&
            ...
            qir(bb_n+1,-1,"prl_vi mest_vi mest_3e prl_kred_sr nar_any qast_any") &&
              q(1,"suw_any") )
       { l[i]=omo1; Z[583]++; if(dbg){print "Z583"}; continue }; #hit_441
       cst="встречать";
       if ( vgl(bb_n,-5,-1) && s(bb_n-1,-1) &&
             bw(bb_n,cst) &&
              q(bb_n-1,"mest_vi suw_vi isname") &&
            qxs(bb_n-2,"на","входе") )
       { l[i]=omo2; Z[584]++; if(dbg){print "Z584"}; continue }; #hit_1
      ...
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) && !sc(-1,"\\(") &&
            ...
            qir(qb_n+1,-2,"prl_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") &&
              q(-1,"suw_vi") )
       { l[i]=omo1; Z[585]++; if(dbg){print "Z585"}; continue }; #hit_80
       ...
       { l[i]=omo1; Z[586]++; if(dbg){print "Z586"}; continue }; #hit_44
...
#       { l[i]=omo2; Z[588]++; if(dbg){print "Z588"}; continue }; #mute
    ...
    if ( qb(-7,-2,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[591]++; if(dbg){print "Z591"}; continue }; #hit_258
      ...
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
             ...
             qir(qb_n+1,-2,"prl_tv mest_tv prq_tv mest_3e prl_kred_sr nar_any qast_any") &&
               q(-1,"suw_tv") )
        { l[i]=omo1; Z[592]++; if(dbg){print "Z592"}; continue }; #hit_27
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
             qir(bb_n+1,qb_n-1,"suw_vi suw_edne isname prl_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") &&
               q(qb_n-1,"suw_tv") &&
             qir(qb_n+2,-1,"prl_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") )
        { l[i]=omo1; Z[593]++; if(dbg){print "Z593"}; continue }; #hit_1

        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
             qir(bb_n+1,qb_n-1,"suw_vi suw_edne isname prl_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") &&
               q(qb_n+1,"suw_tv") &&
             qir(qb_n+2,-1,"suw_ro suw_edne isname prl_ro mest_ro prq_ro prl_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") )
        { l[i]=omo1; Z[594]++; if(dbg){print "Z594"}; continue }; #hit_36
    ...
    if ( qb(-7,-2,"pre_pr") ) { qb_n=qbn;
      ...
      cst="расположить";
       ...
       { l[i]=omo2; Z[595]++; if(dbg){print "Z595"}; continue }; #hit_1
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"suw_da prl_da mest_da prq_da nar_mest prl_kred_sr") &&
            qir(qb_n+1,-2,"prl_pr prq_pr mest_pr mest_3e") &&
              q(-1,"suw_pr") )
       { l[i]=omo1; Z[596]++; if(dbg){print "Z596"}; continue }; #hit_93
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"suw_da prl_da mest_da prq_da nar_mest prl_kred_sr") &&
              q(qb_n+1,"suw_pr isname") &&
            qir(qb_n+2,-1,"suw_ro prl_vi prq_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[597]++; if(dbg){print "Z597"}; continue }; #hit_78
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"suw_da prl_da mest_da prq_da nar_mest prl_kred_sr") &&
              q(qb_n+1,"mest_pr prl_pr prq_pr mest_3e") &&
              q(qb_n+2,"suw_pr isname") &&
            qir(qb_n+3,-1,"suw_ro prl_vi prq_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[598]++; if(dbg){print "Z598"}; continue }; #hit_32
...
#           vex(Y["tn_ok_vi"]) )
#      { bb_n=qb_n=""; Z[599]++; if(dbg){print "Z599"}; }; #mute
#      if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
#             w(qb_n,"в во") && qir(qb_n+1,-1,"isname") && ba(bb_n,"_gl_v_castle-iv") )
#      { bb_n=qb_n=""; Z[600]++; if(dbg){print "Z600"}; }; #mute
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=qb_n=""; Z[601]++; if(dbg){print "Z601"}; }; #hit_8
...
#      { l[i]=omo2; Z[609]++; if(dbg){print "Z609"}; continue }; #mute
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
              w(bb_n+1,"к") &&
            qir(bb_n+2,qb_n-2,"prl_da mest_da mest_3e prq_da") &&
              q(qb_n-1,"suw_da mest_da") &&
            qir(qb_n+1,-1,"prl_vi mest_vi prq_vi suw_vi") )
       { l[i]=omo2; Z[610]++; if(dbg){print "Z610"}; continue }; #hit_1
      ...
      if ( bba(qb_n-5,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-5,-1) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
            ...
            qir(qb_n+1,-2,"prl_ro mest_ro mest_3e") &&
              q(-1,"suw_ro mest_ro") )
       { l[i]=omo2; Z[611]++; if(dbg){print "Z611"}; continue }; #hit_95
       if ( vgl(qb_n,-5,-1) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"suw_vi prl_vi prq_vi mest_vi mest_3e nar_any") &&
              q(qb_n+1,"suw_ro mest_ro") &&
            qir(qb_n+2,-1,"prl_vi mest_vi mest_3e prq_vi suw_ro") )
       { l[i]=omo2; Z[612]++; if(dbg){print "Z612"}; continue }; #hit_32
      ...
      if ( bba(qb_n-5,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-5,-1) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
            ...
            qir(qb_n+1,-2,"prl_tv mest_tv mest_3e") &&
              q(-1,"suw_tv mest_tv") )
       { l[i]=omo2; Z[613]++; if(dbg){print "Z613"}; continue }; #hit_149
       if ( vgl(qb_n,-5,-1) && vgl(bb_n,qb_n-7,qb_n-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"suw_vi prl_vi prq_vi mest_vi mest_3e nar_any") &&
              q(qb_n+1,"suw_tv mest_tv") &&
            qir(qb_n+2,-1,"prl_vi mest_vi mest_3e prq_vi") )
       { l[i]=omo2; Z[614]++; if(dbg){print "Z614"}; continue }; #hit_12
...
#      { bb_n=""; Z[618]++; if(dbg){print "Z618"};}; #mute
       ...
       { bb_n=""; Z[622]++; if(dbg){print "Z622"};}; #hit_5
         ...
         if ( vex(Y["n_pre_v_ist"]) ) { wf_n=Y["n_pre_v_ist"];
            ...
            { l[i]=omo1; Z[635]++; if(dbg){print "Z635"}; continue }; #hit_2
         };
       ...
       { l[i]=omo1; Z[636]++; if(dbg){print "Z636"}; continue }; #hit_4
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[637]++; if(dbg){print "Z637"}; }; #hit_67
         ...
         { l[i]=omo1; Z[638]++; if(dbg){print "Z638"}; continue }; #hit_85
    ...
    if ( vgl(Y["n-_lock_pe"],-7,-1) ) { bb_n=Y["n-_lock_pe"]; BB_n=Y["w-_lock_pe"];
      ...
      { bb_n=""; Z[646]++; if(dbg){print "Z646"};}; #hit_3
       ...
       { l[i]=omo2; Z[657]++; if(dbg){print "Z657"}; continue }; #hit_26
       if ( (qxw(-1,"его ее их еще","и") ||
             qxw(-1,"и",cst1) ) &&
             veq(bb_n,xwn-1) && s(xwn-1) && s(-1) && p(0) )
       { l[i]=omo2; Z[658]++; if(dbg){print "Z658"}; continue }; #hit_3
    };
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[661]++; if(dbg){print "Z661"}; continue }; #hit_50
       if ( vgl(qb_n,-5,-1) && vgl(bb_n,qbn-5,qbn-1) && s(bb_n,-1) &&
            qir(bb_n+1,qb_n-1,"prl_vi prq_vi mest_vi mest_3e qast nar_any prl_kred_sr") &&
            qir(qb_n+1,-1,"prl_vi prq_vi mest_vi mest_3e qast nar_any prl_kred_sr") )
       { l[i]=omo2; Z[662]++; if(dbg){print "Z662"}; continue }; #hit_88
...
#   { l[i]=omo2; Z[663]++; if(dbg){print "Z663"}; continue }; #mute
    # _lock_pe <<<---:end
...
#         { bf_n=""; Z[677]++; if(dbg){print "Z677"}; }; #mute
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { bf_n=""; Z[681]++; if(dbg){print "Z681"};}; #hit_10

       if ( vgl(bf_n,2,6) && s(0,bf_n-1) &&
              w(1,"за") &&
            qir(2,bf_n-1,"suw_tv mest_tv prl_tv isname qast_any") && !(isname(0) && zs(-1) ) )
       { l[i]=omo2; Z[682]++; if(dbg){print "Z682"}; continue }; #hit_25
       if ( vgl(bf_n,3,5) && s(0,bf_n-1) &&
              w(1,"на") &&
              q(2,"suw_vi mest_vi") &&
              s(0,bf_n-1) && Qf(3,bf_n-1,"sz_iili pre_any") )
       { l[i]=omo2; Z[683]++; if(dbg){print "Z683"}; continue }; #hit_56
...
#   cst1="быть ведь же стать считаться являться";
    ...
    { l[i]=omo1; Z[702]++; if(dbg){print "Z702"}; continue }; #hit_941
    if ( prl_im(-2) && ba(-2,"_castle_prl_geo") &&
              Q(-1,"suw_any") && s(-2,-1)  )
    { l[i]=omo1; Z[703]++; if(dbg){print "Z703"}; continue }; #hit_10
    if ( prl_im(1) && ba(1,"_castle_prl_geo") && s(0) && !prl_ro(1))
    { l[i]=omo1; Z[704]++; if(dbg){print "Z704"}; continue }; #hit_29
    # _castle_prl_geo:start
 ...
 if ( wb(-5,-1,"самый") )  { wb_n=wbn;
    ...
    { l[i]=omo1; Z[706]++; if(dbg){print "Z706"}; continue }; #hit_23

    if ( veq(wb_n,-2) && w(-1,"же") )
    { l[i]=omo1; Z[707]++; if(dbg){print "Z707"}; continue }; #hit_1

    if ( veq(wb_n,-1) && qxs(-2,"тот этот","же") && s(-1) )
    { wb_n=""; Z[708]++; if(dbg){print "Z708"}; }; #mute

    if ( veq(wb_n,-2) && wa(-1,"_samyj_prl_castle") && s(wb_n,-1) )
    { l[i]=omo1; Z[709]++; if(dbg){print "Z709"}; continue }; #hit_51
    cst="большой дорогой";
    ...
    { l[i]=omo1; Z[710]++; if(dbg){print "Z710"}; continue }; #hit_4

    if ( veq(wb_n,-2) && wa(-1,"_samyj_prl_lock") && s(wb_n,-1) )
    { l[i]=omo2; Z[711]++; if(dbg){print "Z711"}; continue }; #hit_21

    if ( veq(wb_n,-4) && q(-3,"prl_edmuvi") && q(-2,"sz_i") && wa(-1,"_samyj_prl_lock") && s(wb_n,-1) )
    { l[i]=omo2; Z[712]++; if(dbg){print "Z712"}; continue }; #hit_1
 ...
 if ( wf(1,4,"самый самым") )  { wf_n=wfn;
    ...
    { l[i]=omo1; Z[713]++; if(dbg){print "Z713"}; continue }; #hit_6
    if ( vgl(wf_n,1,3) && wa(wf_n+1,"_samyj_prl_lock") && s(0,wf_n) &&
         qir(1,wf_n-1,"nar_any gl_aux_be qast_any") )
    { l[i]=omo2; Z[714]++; if(dbg){print "Z714"}; continue }; #hit_17
    ...
    { l[i]=omo1; Z[715]++; if(dbg){print "Z715"}; continue }; #hit_1
    if ( vgl(wf_n,2,4) && qm(wf_n+1,"prl_edmuim prl_edmutv","_samyj_prl_lock") && s(0,wf_n-1) &&
          bw(1,cst5) && qir(2,wf_n-1,"nar_any gl_aux_be qast_any") )
    { l[i]=omo2; Z[716]++; if(dbg){print "Z716"}; continue }; #hit_8
 ...
 #
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn; if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[719]++; if(dbg){print "Z719"}; continue }; #hit_1130
    ...
    if ( wb(-5,-1,cst) ) { wb_n=wbn;
       ...
       { l[i]=omo1; Z[722]++; if(dbg){print "Z722"}; continue }; #hit_315

    };


#   if ( TN_do_zamka_f() )
#   { Z[723]++; if(dbg){print "Z723", "-- TN_do_zamka_f"}; continue }; #mute
#   if ( TN_ot_zamka_f() )
#   { Z[724]++; if(dbg){print "Z724", "-- TN_ot_zamka_f"}; continue }; #mute
#   if ( TN_iz_zamka_f() )
#   { Z[725]++; if(dbg){print "Z725", "-- TN_iz_zamka_f"}; continue }; #mute

    # _castle_prl:start
    if ( ba(-1,"_castle_prl") && prl_ro(-1) && s(-1)  )
    { l[i]=omo1; Z[726]++; if(dbg){print "Z726"}; continue }; #hit_51
    if ( ba(-2,"_castle_prl") && prl_ro(-2) &&
          q(-1,"prl_any prq_any mest_any qast") && s(-2,-1)  )
    { l[i]=omo1; Z[727]++; if(dbg){print "Z727"}; continue }; #hit_24
    if ( ba(1,"_castle_prl") && prl_ro(1) && s(0) )
    { l[i]=omo1; Z[728]++; if(dbg){print "Z728"}; continue }; #hit_86
    # _castle_prl:start

    # _lock_prl:start
    if ( ba(-1,"_lock_prl") && s(-1) && prl_ro(-1) )
    { l[i]=omo2; Z[729]++; if(dbg){print "Z729"}; continue }; #hit_16
    if ( ba(1,"_lock_prl") && s(0) && prl_ro(1) && !isname(1) )
    { l[i]=omo2; Z[730]++; if(dbg){print "Z730"}; continue }; #hit_7
    # _lock_prl:end

    # _castle_prl_geo:start
    if ( prl_ro(-1) &&
             ba(-1,"_castle_prl_geo") && s(-1)  )
    { l[i]=omo1; Z[731]++; if(dbg){print "Z731"}; continue }; #hit_623
    if ( prl_ro(-2) &&
             ba(-2,"_castle_prl_geo") &&
              Q(-1,"suw_any") && s(-2,-1)  )
    { l[i]=omo1; Z[732]++; if(dbg){print "Z732"}; continue }; #hit_14
    # _castle_prl_geo:start
 ...
 #
  #
    if ( TN_k_zamku_f() )
    { Z[733]++; if(dbg){print "Z733", "-- TN_k_zamku_f"}; continue }; #hit_756


    # _castle_prl:start
    if ( ba(-1,"_castle_prl") && prl_da(-1) && s(-1)  )
    { l[i]=omo1; Z[734]++; if(dbg){print "Z734"}; continue }; #hit_4
    # _castle_prl:start

    # _lock_prl:start
#   if ( ba(-1,"_lock_prl") && s(-1) && prl_da(-1) )
#   { l[i]=omo2; Z[735]++; if(dbg){print "Z735"}; continue }; #mute
    if ( ba(1,"_lock_prl") && s(0) && prl_da(1) && !isname(1) )
    { l[i]=omo2; Z[736]++; if(dbg){print "Z736"}; continue }; #hit_1
    # _lock_prl:end

    # _castle_prl_geo:start
    if ( prl_da(-1) &&
             ba(-1,"_castle_prl_geo") && s(-1)  )
    { l[i]=omo1; Z[737]++; if(dbg){print "Z737"}; continue }; #hit_158
    # _castle_prl_geo:start
 ...
 #
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn; if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[738]++; if(dbg){print "Z738"}; continue }; #hit_329

    };};

    if ( vex(Y["tn_ok_pod_tv"]) && TN_pod_zamkom_f() )
    { Z[739]++; if(dbg){print "Z739", "-- TN_pod_zamkom_f"}; continue}; #mute


 #
    # _castle_prl:start
    if ( ba(-1,"_castle_prl") && prl_tv(-1) && s(-1)  )
    { l[i]=omo1; Z[740]++; if(dbg){print "Z740"}; continue }; #hit_14
    if ( ba(-2,"_castle_prl") && prl_tv(-2) &&
          q(-1,"prl_any prq_any mest_any qast") && s(-2,-1)  )
    { l[i]=omo1; Z[741]++; if(dbg){print "Z741"}; continue }; #hit_4
    # _castle_prl:start

    # _lock_prl:start
    if ( ba(-1,"_lock_prl") && s(-1) && prl_tv(-1) )
    { l[i]=omo2; Z[742]++; if(dbg){print "Z742"}; continue }; #hit_1
    # _lock_prl:end

    # _castle_prl_geo:start
    if ( prl_tv(-1) &&
             ba(-1,"_castle_prl_geo") && s(-1)  )
    { l[i]=omo1; Z[743]++; if(dbg){print "Z743"}; continue }; #hit_169
    # _castle_prl_geo:start

    # _castle_tv:start
    if ( bba(-4,-1,"_castle_tv") && s(bbn,-1) &&
         qir(bbn+1,-1,"prl_tv prq_tv mest_tv qast_any") )
    { l[i]=omo1; Z[744]++; if(dbg){print "Z744"}; continue }; #hit_276
    # _castle_tv:start
 ...
 #
    ...
    { Z[745]++; if(dbg){print "Z745", "-- TN_v_zamke_f"}; continue}; #hit_6480
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn; if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[746]++; if(dbg){print "Z746"}; continue }; #hit_386
 ...
 if ( bfa(1,5,"_lock_parts") ) { bf_n=bfn; BF_n=BFn;
    ...
    { l[i]=omo2; Z[752]++; if(dbg){print "Z752"}; continue }; #hit_2936
 ...
 { l[i]=omo1; Z[753]++; if(dbg){print "Z753"}; continue }; #hit_979
 if ( z(0) &&
      q(1,"nar_spos prl_kred_sr") &&
      q(2,"prq_any") && ba(2,"_castle_pe") && s(1,2) )
 { l[i]=omo1; Z[754]++; if(dbg){print "Z754"}; continue }; #hit_37
 if ( q(-3,"prq_any") && ba(-3,"_castle_pe") &&
 pre_ro(-2) &&
      q(-1,"suw_ro") && s(-3,-1) )
 { l[i]=omo1; Z[755]++; if(dbg){print "Z755"}; continue }; #hit_38
 if ( q(-1,"prq_any") && ba(-1,"_castle_pe") && s(-1) )
 { l[i]=omo1; Z[756]++; if(dbg){print "Z756"}; continue }; #hit_1570
 # _castle_pe:end

 # _lock_pe:start
 if ( z(0) && q(1,"prq_any") && ba(1,"_lock_pe") && s(1) )
 { l[i]=omo2; Z[757]++; if(dbg){print "Z757"}; continue }; #hit_455
 if ( q(-1,"prq_any") && ba(-1,"_lock_pe") && s(-1) )
 { l[i]=omo2; Z[758]++; if(dbg){print "Z758"}; continue }; #hit_825
 # _lock_pe:end
 ...
 if ( wf(1,6,"в во") ) { wf_n=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_v_suw-pr") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,6) && vgl(bf_n,wf_n+1,wf_n+4) && zs(0) && s(1,bfn-1) &&
         ...
         qir(wf_n+1,bf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
    { l[i]=omo1; Z[759]++; if(dbg){print "Z759"}; continue }; #hit_954
    ...
    if ( vgl(wf_n,1,6) && vgl(bf_n,wf_n+1,wf_n+4) && zs(0) && s(1,bfn-1) &&
         ...
         qir(wf_n+1,bf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
    { l[i]=omo2; Z[760]++; if(dbg){print "Z760"}; continue }; #hit_1
    if ( vgl(wf_n,1,6) && vgl(bf_n,wf_n+1,wf_n+4) && zs(0) && s(1,bfn-1) &&
          bw(wf_n-1,cst1) &&
         qir(1,wf_n-2,"prl_any mest_any isname qast_ne") &&
         qir(wf_n+1,bf_n-1,"prl_pr mest_pr mest_3e prq_pr") )
    { l[i]=omo1; Z[761]++; if(dbg){print "Z761"}; continue }; #hit_99
 ...
 if ( vgl(Y["n_pre_v_ist"],-6,-2) ) { wb_n=Y["n_pre_v_ist"];
   if ( wba(wb_n+1,-1,"_castle_v_suw-pr") ) { bb_n=wbn;
    ...
    { l[i]=omo2; Z[762]++; if(dbg){print "Z762"}; continue }; #hit_1
    # escape:end

    if ( vgl(wb_n,-6,-2) && vgl(bb_n,wb_n+1,-1) &&
         qir(wb_n+1,bb_n-1,"prl_pr mest_pr mest_3e prq_pr") && s(wb_n,bb_n-1) &&
         qia(bb_n+1,-1,"qast","prl_any mest_any nar_any suw_ro isname") && ds(bb_n) && s(bb_n+1,-1) )
    { l[i]=omo1; Z[763]++; if(dbg){print "Z763"}; continue }; #hit_99
 ...
 { l[i]=omo1; Z[764]++; if(dbg){print "Z764"}; continue }; #hit_106
 if ( wf(1,4,"как") &&
     ...
     qir(bfN+2,bfn-1,"prl_vi mest_vi mest_3e prq_vi") )
 { l[i]=omo1; Z[765]++; if(dbg){print "Z765"}; continue }; #hit_1
 # похожий II

 if ( wf(1,4,"дома") && s(0,wfn-1) &&
     qir(1,wfn-1,"prl_ro mest_ro prq_ro") &&
    !wfa(1,wfn-1,"_lock_suw-ro") && Ww_(-1,"под") )
 { l[i]=omo1; Z[766]++; if(dbg){print "Z766"}; continue }; #hit_36
 ...
 if ( vgl(Y["n_pre_v_ist"],-8,-2) ) { wb_n=Y["n_pre_v_ist"];
    ...
    { l[i]=omo1; Z[772]++; if(dbg){print "Z772"}; continue }; #hit_25
    if ( vgl(wb_n,-7,-3) && s(wb_n,-1) &&
      digits(wb_n+1) &&
           w(wb_n+2,"м х") &&
           w(wb_n+3,"году годах") &&
    ...
    { l[i]=omo1; Z[773]++; if(dbg){print "Z773"}; continue }; #hit_1
    if ( vgl(wb_n,-6,-2) && s(wb_n+1,-1) &&
       roman(wb_n) &&
           w(wb_n+1,"веке веках") &&
         qir(wb_n+2,-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo1; Z[774]++; if(dbg){print "Z774"}; continue }; #hit_6
 ...
 { l[i]=omo1; Z[775]++; if(dbg){print "Z775"}; continue }; #hit_1
 if ( qb(-6,-2,"pre_ro") && s(qbn+1,-1) &&
   roman(qbn) &&
       w(qbn+1,"века веков") &&
     qir(qbn+2,-1,"prl_vi mest_vi mest_3e") )
 { l[i]=omo1; Z[776]++; if(dbg){print "Z776"}; continue }; #hit_1

 if ( FN_castle_prl_f("_castle_prl_after") )
 { Z[777]++; if(dbg){print "Z777", "-- FN_castle_prl_after_f"}; continue }; #hit_5928

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[778]++; if(dbg){print "Z778", "-- FN_lock_prl_f(\"_lock_prl_after\")"}; continue }; #hit_1631


 # перенос назад
    if ( z(-1) && veq(Y["n-_castle_environs"],-1) )
    { l[i]=omo1; Z[779]++; if(dbg){print "Z779"}; continue }; #hit_955
    if ( z(0) && veq(Y["n+_castle_environs"],1) && Q(1,"isname") )
    { l[i]=omo1; Z[780]++; if(dbg){print "Z780"}; continue }; #hit_672
 # перенос назад
 ...
 if ( gist(-4,2) &&
         ...
         w(-1,"а но и или") && s(0) )
 { l[i]=omo2; Z[781]++; if(dbg){print "Z781"}; continue }; #hit_2
 ...
 { l[i]=omo1; Z[782]++; if(dbg){print "Z782"}; continue }; #hit_19758
 ...
 { l[i]=omo1; Z[783]++; if(dbg){print "Z783"}; continue }; #hit_13

 if ( (qxs(1,"де","ла")||
       qxs(1,"фон","дер")) &&
    isname(xsn+1) && s(xsn) )
 { l[i]=omo1; Z[784]++; if(dbg){print "Z784"}; continue }; #hit_18

 if ( isname(2) && s(0) ) {
    if ( w(1,"де дю фон ди да дель ла ло ле лё ля") && s(1) )
    { l[i]=omo1; Z[785]++; if(dbg){print "Z785"}; continue }; #hit_371
    if ( w(1,"д л") && sc(1,"[\x27’]") )
    { l[i]=omo1; Z[786]++; if(dbg){print "Z786"}; continue }; #hit_74
    if ( w(1,"ал") && se(1,"-") )
    { l[i]=omo1; Z[787]++; if(dbg){print "Z787"}; continue }; #hit_4
    if ( q(1,"pre_vi pre_ro pre_pr") && s(1) &&
       !bb(-4,-1,"ключ ключик ключ-карта") )
    { l[i]=omo1; Z[788]++; if(dbg){print "Z788"}; continue }; #hit_915
 };

 if ( isname(1) && prl_any(1) && s(0) && (p(1)||q_w(1,"sz_iili")) )
 { l[i]=omo1; Z[789]++; if(dbg){print "Z789"}; continue }; #hit_70
 if ( isname(-1) && ( (s(-2,-1) && sv(-2,"\x29"))||(z(-2) && s(-1)) ) )
 { l[i]=omo1; Z[790]++; if(dbg){print "Z790"}; continue }; #hit_5305
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[791]++; if(dbg){print "Z791", "-- is_CAP"}; continue }; #hit_6810
...
function DEF_TN_PREVI_ZAMOK(num,el,    fufu) { # определение компонентов между "pre_vi" и "замок"
    ...
    { tn_ok = 1; Z[792]++; if(dbg){print "Z792", "-- tn_ok ON", el, "замок"};}; #hit_109292
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[793]++; if(dbg){print "Z793", "-- tn_ok ON", el, "замок"};}; #hit_134
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[794]++; if(dbg){print "Z794", "-- tn_ok ON", el, "замок"};}; #hit_66
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[795]++; if(dbg){print "Z795", "-- tn_ok ON", el, "замок"};}; #hit_23
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[796]++; if(dbg){print "Z796", "-- tn_ok ON", el, "замок"};}; #hit_18
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[797]++; if(dbg){print "Z797", "-- tn_ok ON", el, "замок"};}; #hit_16
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[798]++; if(dbg){print "Z798", "-- tn_ok ON", el, "замок"};}; #hit_12
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[799]++; if(dbg){print "Z799", "-- tn_ok ON", el, "замок"};}; #hit_18
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[800]++; if(dbg){print "Z800", "-- tn_ok ON", el, "замок"};}; #hit_375
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[801]++; if(dbg){print "Z801", "-- tn_ok ON", el, "замок"};}; #hit_40
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[802]++; if(dbg){print "Z802", "-- tn_ok ON", el, "замок"};}; #hit_15
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[803]++; if(dbg){print "Z803", "-- tn_ok MOV", el, "замок"};}; #hit_560
    # выключение tn_ok
    if ( vex(tn_ok) && z(tn-1) && w(tn+1,"который") )
    { tn_ok = ""; Z[804]++; if(dbg){print "Z804", "-- tn_ok OFF", el, "замок"};}; #hit_4
    ...
    { tn_ok = ""; Z[805]++; if(dbg){print "Z805", "-- tn_ok OFF", el, "замок"};}; #hit_45
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[806]++; if(dbg){print "Z806", "-- tn_ok OFF", el, "замок"};}; #hit_4
}
...
function DEF_TN_PREDA_ZAMKU(num,el,    fufu) { # определение компонентов между "pre_da" и "замку"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[807]++; if(dbg){print "Z807", "-- tn_ok ON", el, "замку"};}; #hit_17223
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[808]++; if(dbg){print "Z808", "-- tn_ok MOV", el, "замку"};}; #hit_205
}
...
function DEF_TN_PRETV_ZAMKOM(num,el,    fufu) { # определение компонентов между "pre_tv" и "замком"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[811]++; if(dbg){print "Z811", "-- tn_ok ON", el, "замком"};}; #hit_7634
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[812]++; if(dbg){print "Z812", "-- tn_ok MOV", el, "замком"};}; #hit_63
}
...
function DEF_TN_PREPR_ZAMKE(num,el) { # определение компонентов между "pre_pr" и "замке"
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[815]++; if(dbg){print "Z815", "-- tn_ok ON", el, "замке"};}; #hit_116151
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[816]++; if(dbg){print "Z816", "-- tn_ok ON", el, "замке"};}; #hit_3
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[817]++; if(dbg){print "Z817", "-- tn_ok ON", el, "замке"};}; #hit_17
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[818]++; if(dbg){print "Z818", "-- tn_ok ON", el, "замке"};}; #hit_660
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[819]++; if(dbg){print "Z819", "-- tn_ok ON", el, "замке"};}; #hit_86
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ском$|ном$") )
    { tn_ok = 1; Z[820]++; if(dbg){print "Z820", "-- tn_ok ON", el, "замке"};}; #hit_29
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[821]++; if(dbg){print "Z821", "-- tn_ok MOV", el, "замке"};}; #hit_949
}
...
function TN_v_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_v_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_v"]) ) { tn=Y["n_pre_v"];
    ...
    { l[i]=omo2; Z[822]++; if(dbg){print "Z822"}; stopp=ret=1; continue }; #hit_1

    cst="поставить";
    cst1="гарнизон конь";
    if ( bw(tn-1,cst) &&
         bw(1,cst1) && s(tn-1) )
    { l[i]=omo1; Z[823]++; if(dbg){print "Z823"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[824]++; if(dbg){print "Z824"}; stopp=ret=1; continue }; #hit_4

    cst="переходить";
    cst2="ствол";
    if ( bb(tn-4,tn-2,cst2) &&
         bw(tn-1,cst) && s(bbn,tn-1) )
    { l[i]=omo2; Z[825]++; if(dbg){print "Z825"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( qxs(tn-1,"ни не") && z(0) && s(tn-1) &&
        (qxw(1,"а но","не","в")||
         ...
         qir(xwn+1,bfn-1,"prl_vi mest_vi mest_3e prq_vi") )
    { l[i]=omo1; Z[826]++; if(dbg){print "Z826"}; stopp=ret=1; continue }; #hit_24
    if ( qxw(tn-3,"ни не","в") &&
          ba(tn-2,"_castle_environs") && z(tn-2) &&
         qxs(tn-1,"а но ни не") )
    { l[i]=omo1; Z[827]++; if(dbg){print "Z827"}; stopp=ret=1; continue }; #hit_2
    ...
    { l[i]=omo1; Z[828]++; if(dbg){print "Z828"}; stopp=ret=1; continue }; #hit_17

    cst="совать";
    if ( bb(tn-4,tn-2,cst) &&
        qir(bbn+1,tn-2,"prl_edmuvi mest_edmuvi mest_3e") &&
          w(tn-1,"нос") && s(bbn,tn-1) )
    { l[i]=omo1; Z[829]++; if(dbg){print "Z829"}; stopp=ret=1; continue }; #hit_1
    if ( bw(1,cst) &&
         wf(2,5,"нос") &&
        qir(wfn+1,wfn-1,"prl_edmuvi mest_edmuvi mest_3e") && s(1,wfn-1) )
    { l[i]=omo1; Z[830]++; if(dbg){print "Z830"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[831]++; if(dbg){print "Z831"}; stopp=ret=1; continue }; #hit_2

    if ( qxs(tn-1,"по","возвращению приезду прибытию") )
    { l[i]=omo1; Z[832]++; if(dbg){print "Z832"}; stopp=ret=1; continue }; #hit_91
    if ( qxs(tn-2,"в","одной","руке") &&
           q(tn-1,"suw_vi") &&
          zs(tn-1) )
    { l[i]=omo2; Z[833]++; if(dbg){print "Z833"}; stopp=ret=1; continue }; #hit_1

    if ( q(tn-1,"mest_im mest_vi mest_3e suw_odim suw_odvi") && Q(tn-1,"gl_po") &&
         p(tn-2) && p(0) && s(tn-1,-1) )
    { l[i]=omo1; Z[834]++; if(dbg){print "Z834"}; stopp=ret=1; continue }; #hit_168
    if ( q(tn-2,"mest_im mest_vi mest_3e suw_odim suw_odvi") && Q(tn-2,"gl_po") &&
         q(tn-1,"nar_any prl_kred_sr") &&
         p(tn-3) && p(0) && s(tn-2,-1) )
    { l[i]=omo1; Z[835]++; if(dbg){print "Z835"}; stopp=ret=1; continue }; #hit_22
    if ( q(tn-2,"nar_any prl_kred_sr") &&
         q(tn-1,"mest_im mest_vi mest_3e suw_odim suw_odvi") && Q(tn-1,"gl_po") &&
         p(tn-3) && p(0) && s(tn-2,-1) )
    { l[i]=omo1; Z[836]++; if(dbg){print "Z836"}; stopp=ret=1; continue }; #hit_12
    ...
    if ( bf(1,5,cst) ) { bf_n=bfn;
      ...
      { l[i]=omo1; Z[837]++; if(dbg){print "Z837"}; stopp=ret=1; continue }; #hit_1
      if ( vgl(bf_n,1,5) &&
             w(bf_n-1,cst1) && s(0,bf_n-1) &&
           qir(1,bf_n-2,"mest_im otsz_any gl_aux_be mod_any") )
      { l[i]=omo1; Z[838]++; if(dbg){print "Z838"}; stopp=ret=1; continue }; #hit_2
    };
    ...
    { # захват в -->>
        ...
        if ( bb(tn-3,tn-1,cst1) ) { bb_n=bbn;
           ...
           { l[i]=omo2; Z[839]++; if(dbg){print "Z839"}; stopp=ret=1; continue }; #hit_120
           if ( vgl(bb_n,tn-4,tn-1) && s(bbn,tn-1) &&
                qir(bb_n+1,tn-2,"prl_vi mcop_vi prq_vi") &&
                 bw(tn-1,cst2) )
           { l[i]=omo2; Z[840]++; if(dbg){print "Z840"}; stopp=ret=1; continue }; #hit_1307
           if ( veq(bb_n,tn-3) && s(tn-3,tn-1) &&
                 bw(tn-2,cst2) &&
                  q(tn-1,"suw_ro suw_edne isname") )
           { l[i]=omo2; Z[841]++; if(dbg){print "Z841"}; stopp=ret=1; continue }; #hit_36
           if ( veq(bb_n,tn-2) && s(tn-3,tn-1) &&
                  w(tn-3,cst3) &&
                  q(tn-1,"isname suw_vi") )
           { l[i]=omo2; Z[842]++; if(dbg){print "Z842"}; stopp=ret=1; continue }; #hit_1

        };
           if ( bw(tn-1,cst2) && se(tn-1, " — ") && p(0) )
           { l[i]=omo2; Z[843]++; if(dbg){print "Z843"}; stopp=ret=1; continue }; #hit_2

        if ( bw(tn-1,cst1) &&
             bf(1,3,cst2) && s(tn-1,bfn-1) )
        { l[i]=omo2; Z[844]++; if(dbg){print "Z844"}; stopp=ret=1; continue }; #hit_123

        cst="захват обхват";
        if ( bw(tn-2,cst) &&
             bw(tn-1,cst2) && s(tn-2,tn-1) )
        { l[i]=omo2; Z[845]++; if(dbg){print "Z845"}; stopp=ret=1; continue }; #hit_5
        if ( bw(tn-5,cst) &&
             bw(tn-4,cst2) &&
              q(tn-3,"suw_ro suw_edne isname") &&
              q(tn-2,"prl_tv mest_tv mest_3e prq_tv qis_tv qik_tv") &&
              q(tn-1,"suw_tv") && s(tn-5,tn-1) )
        { l[i]=omo2; Z[846]++; if(dbg){print "Z846"}; stopp=ret=1; continue }; #hit_2
        if ( bw(tn-1,cst) && s(tn-1) )
        { l[i]=omo2; Z[847]++; if(dbg){print "Z847"}; stopp=ret=1; continue }; #hit_2

        # в ... «замок» в кавычках
        if ( bw(tn-2,cst1) &&
              q(tn-1,"isname suw_vi") &&
             sc(-1,"[\x22«]") && sc(0,"[\x22»]") && s(tn-2,tn-1) )
        { l[i]=omo2; Z[848]++; if(dbg){print "Z848"}; stopp=ret=1; continue }; #hit_3
        if ( bw(tn-1,cst2) &&
             sc(tn-1,"[\x22«]") && sc(0,"[\x22»]") && s(tn-1) )
        { l[i]=omo2; Z[849]++; if(dbg){print "Z849"}; stopp=ret=1; continue }; #hit_13
        if ( bw(tn-2,cst2) &&
              q(tn-1,"qast_any") &&
             sc(tn-1,"[\x22«]") && sc(0,"[\x22»]") && s(tn-2,tn-1) )
        { l[i]=omo2; Z[850]++; if(dbg){print "Z850"}; stopp=ret=1; continue }; #hit_1

        if ( bw(tn-1,cst2) &&
              w(1,"за на") &&
             bw(2,cst2) && s(tn-1,1) )
        { l[i]=omo2; Z[851]++; if(dbg){print "Z851"}; stopp=ret=1; continue }; #hit_119
    ...
    { l[i]=omo2; Z[852]++; if(dbg){print "Z852"}; stopp=ret=1; continue }; #edit #hit_1
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[853]++; if(dbg){print "Z853"}; stopp=ret=1; continue }; #edit #hit_1
    ...
    { l[i]=omo1; Z[854]++; if(dbg){print "Z854"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[855]++; if(dbg){print "Z855"}; stopp=ret=1; continue }; #hit_4
    cst2="спичка";
    if ( bw(tn-2,cst) && s(tn-2,tn-1) &&
          q(tn-1,"suw_vi") && bw(tn-1,cst2) )
    { l[i]=omo2; Z[856]++; if(dbg){print "Z856"}; stopp=ret=1; continue }; #hit_1

    cst1="армия войско отряд";
#   if ( bb(tn-4,tn-1,cst) &&
#        bw(tn-1,cst1) && s(bbn,tn-1) &&
#       qir(bbn+1,tn-2,"prl_vi mest_vi mest_3e nar_any") )
#   { l[i]=omo1; Z[857]++; if(dbg){print "Z857"}; stopp=ret=1; continue }; #mute
    if ( bw(tn-1,cst) &&
         bw(tn-2,cst1) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[858]++; if(dbg){print "Z858"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[859]++; if(dbg){print "Z859"}; stopp=ret=1; continue }; #hit_12
    ...
    if ( bb(tn-5,tn-2,cst2) && s(bbn,tn-1) &&
         bw(tn-1,cst1) )
    { l[i]=omo1; Z[860]++; if(dbg){print "Z860"}; stopp=ret=1; continue }; #hit_1
    if ( bb(tn-5,tn-2,cst1) && s(bbn,tn-1) &&
         bw(tn-1,cst2) )
    { l[i]=omo1; Z[861]++; if(dbg){print "Z861"}; stopp=ret=1; continue }; #hit_3
    if ( bw(tn-2,cst2) && z(tn-2) &&
         bw(tn-1,cst1) && s(tn-1) )
    { l[i]=omo1; Z[862]++; if(dbg){print "Z862"}; stopp=ret=1; continue }; #hit_1
    if ( bf(1,4,cst1) &&
         bf(bfn+1,bfn+4,cst2) && s(0,bfn-1) )
    { l[i]=omo1; Z[863]++; if(dbg){print "Z863"}; stopp=ret=1; continue }; #hit_1

    cst1="бить ударить";
    if ( bb(tn-4,tn-2,cst1) && s(bbn,tn-1) &&
          q(tn-1,"suw_tv") &&
        qir(bbn+1,tn-2,"prl_tv mest_tv") )
    { l[i]=omo2; Z[864]++; if(dbg){print "Z864"}; stopp=ret=1; continue }; #hit_13
    ...
    { l[i]=omo2; Z[865]++; if(dbg){print "Z865"}; stopp=ret=1; continue }; #hit_1
    cst="ключ кобч-карта";
    if ( ba(tn-2,"_suw_v_castle-vp") &&
         bw(tn-1,cst) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[866]++; if(dbg){print "Z866"}; stopp=ret=1; continue }; #hit_1
    cst="ключ ключ-карта пропуск";
    if ( ba(tn-2,"_gl_v_lock-iv") &&
         bw(tn-1,cst) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[867]++; if(dbg){print "Z867"}; stopp=ret=1; continue }; #hit_2000

    #
    if ( ba(tn-1,"_suw_v_castle-vp") && s(tn-1) )
    { l[i]=omo1; Z[868]++; if(dbg){print "Z868"}; stopp=ret=1; continue }; #hit_5203
    if ( vvb(tn-5,tn-1) && ba(vvn,"_suw_v_castle-vp") )
    { l[i]=omo1; Z[869]++; if(dbg){print "Z869"}; stopp=ret=1; continue }; #hit_17
    if ( ba(tn-2,"_suw_v_castle-vp")  && s(tn-2) &&
          q(tn-1,"prl_kred_sr nar_any") && zs(tn-1) )
    { l[i]=omo1; Z[870]++; if(dbg){print "Z870"}; stopp=ret=1; continue }; #hit_79
    ...
    { l[i]=omo1; Z[871]++; if(dbg){print "Z871"}; stopp=ret=1; continue }; #hit_291

    cst="без для из с";
    if ( bba(tn-6,tn-3,"_suw_v_castle-vp") &&
           w(bbn+1,cst) &&
         qir(bbn+2,tn-1,"suw_ro prl_ro prq_ro mest_ro mest_3e isname") && s(bbn,tn-1) )
    { l[i]=omo1; Z[872]++; if(dbg){print "Z872"}; stopp=ret=1; continue }; #hit_74
    if ( ba(tn-1,"_suw_v_castle-vp") && se(tn-1," — ") )
    { l[i]=omo1; Z[873]++; if(dbg){print "Z873"}; stopp=ret=1; continue }; #hit_15
    cst="начаться начинаться";
    if ( bw(1,cst) &&
         ba(2,"_suw_v_castle-vp") && s(0,1) )
    { l[i]=omo1; Z[874]++; if(dbg){print "Z874"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( (qxs(tn-2,"за на",cst2) ||
          ...
          qxs(tn-2,"на",cst5,cst6) ) &&
           bw(tn-1,cst) && s(tn-1) )
    { l[i]=omo2; Z[875]++; if(dbg){print "Z875"}; stopp=ret=1; continue }; #hit_75
    if ( (qxs(tn-1,"за на",cst2) ||
          ...
          qxs(tn-1,"на",cst5,cst6) ) &&
           bw(xsn-1,cst) && s(tn-1) )
    { l[i]=omo2; Z[876]++; if(dbg){print "Z876"}; stopp=ret=1; continue }; #hit_159
    if ( bw(tn-1,cst) && s(tn-1) &&
       (qxs(1,"за на",cst2) ||
        ...
        qxs(1,"на",cst6) ||
        qxs(1,"на",cst5,cst6) ) )
    { l[i]=omo2; Z[877]++; if(dbg){print "Z877"}; stopp=ret=1; continue }; #hit_128
    if ( p(-3) && bw(-2,cst) && tn == -1 && (p(0)||sz_iili(1)) )
    { l[i]=omo2; Z[878]++; if(dbg){print "Z878"}; stopp=ret=1; continue }; #hit_23

    cst="рука";
    cst1="ладонями пальцами";
    if ( bw(tn-1,cst) && s(tn-1,0) && w(1,cst1) )
    { l[i]=omo2; Z[879]++; if(dbg){print "Z879"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[880]++; if(dbg){print "Z880"}; stopp=ret=1; continue }; #hit_5
    if ( qxs(tn-1,cst,"нет") ||
         qxs(tn-1,"нет",cst) ||
         qxs(tn-1,"какими","судьбами") ||
         qxs(tn-1,"ни","ногой") )
    { l[i]=omo1; Z[881]++; if(dbg){print "Z881"}; stopp=ret=1; continue }; #hit_2

    if ( qxs(1,"больше более","ни","ногой")||
         qxs(1,"ни","ногой")||
           w(1,"ни-ни") )
    { l[i]=omo1; Z[882]++; if(dbg){print "Z882"}; stopp=ret=1; continue }; #hit_6
    # _suw_v_lock:start
    if ( ba(tn-1,"_suw_v_lock") && s(tn-1) )
    { l[i]=omo2; Z[883]++; if(dbg){print "Z883"}; stopp=ret=1; continue }; #hit_2985
    if ( vvb(tn-5,tn-1) &&
          ba(vvn,"_suw_v_lock") )
    { l[i]=omo2; Z[884]++; if(dbg){print "Z884"}; stopp=ret=1; continue }; #hit_10
    if ( bba(tn-6,tn-2,"_suw_v_lock") &&
         qir(bbn+1,tn-2,"prl_ro prq_ro mcop_ro") &&
           q(tn-1,"suw_ro mest_ro mest_3e isname qast") && s(bbn,tn-1) )
    { l[i]=omo2; Z[885]++; if(dbg){print "Z885"}; stopp=ret=1; continue }; #hit_237
    if ( vvb(tn-5,tn-1) &&
         bba(vvn-6,vvn-1,"_suw_v_lock") &&
         qir(vvn+1,vvn-1,"prl_ro prq_ro mcop_ro") &&
           q(vvn,"suw_ro mest_ro mest_3e isname qast") && s(bbn,vvn-1) )
    { l[i]=omo2; Z[886]++; if(dbg){print "Z886"}; stopp=ret=1; continue }; #hit_2
    if ( ba(tn-3,"_suw_v_lock") &&
     pre_vi(tn-2) &&
          q(tn-1,"suw_vi") && s(tn-3,tn-1) )
    { l[i]=omo2; Z[887]++; if(dbg){print "Z887"}; stopp=ret=1; continue }; #hit_2
    # _suw_v_lock:end
    ...
    if ( w(tn-2,cst1) &&
        bw(tn-1,cst) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[888]++; if(dbg){print "Z888"}; stopp=ret=1; continue }; #hit_1
    if ( bw(tn-2,cst2) &&
         bw(tn-1,cst) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[889]++; if(dbg){print "Z889"}; stopp=ret=1; continue }; #hit_1
    if ( qb(tn-9,tn-3,"mest_impe suw_odim") &&
         bw(tn-2,cst) &&
          q(tn-1,"nar_napr") && s(qbn,tn-1) )
    { l[i]=omo1; Z[890]++; if(dbg){print "Z890"}; stopp=ret=1; continue }; #hit_1
    if ( qb(tn-7,tn-3,"gl_ed gl_mn") &&
    sz_iili(tn-2) &&
         bw(tn-1,cst) && s(qbn,tn-1) )
    { l[i]=omo1; Z[891]++; if(dbg){print "Z891"}; stopp=ret=1; continue }; #hit_1
    if ( bb(-7,-4,cst) &&
     pre_vi(bbn+1) &&
        qir(bbn+2,tn-2,"prl_vi prq_vi mcop_vi") &&
         bw(tn-1,cst3) && s(bbn,tn-1) )
    { l[i]=omo1; Z[892]++; if(dbg){print "Z892"}; stopp=ret=1; continue }; #hit_1

    cst="затолкать";
    if ( w(tn-3,"тебя меня вас нас") &&
        bw(tn-2,cst) &&
         q(tn-1,"nar_napr") && s(tn-3,tn-1) )
    { l[i]=omo1; Z[893]++; if(dbg){print "Z893"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       { bb_n=""; Z[894]++; if(dbg){print "Z894"}; }; #hit_1
       if ( vgl(bb_n,tn-9,tn-1) && s(bb_n,tn-1) &&
            bba(tn+1,-1,"_castle_prl") )
       { bb_n=""; Z[895]++; if(dbg){print "Z895"}; }; #hit_5
       if ( vgl(bb_n,tn-9,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=qb_n=""; Z[896]++; if(dbg){print "Z896"}; }; #hit_1


       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo2; Z[897]++; if(dbg){print "Z897"}; stopp=ret=1; continue }; #hit_2776
       ...
       { l[i]=omo2; Z[898]++; if(dbg){print "Z898"}; stopp=ret=1; continue }; #hit_2825
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[899]++; if(dbg){print "Z899"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bb_n,tn-7,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mest_vi mest_3e prl_kred_sr nar_step isname") &&
              q(tn-2,"suw_vi") &&
              q(tn-1,"suw_ro") && s(bb_n,tn-1) )
       { l[i]=omo2; Z[900]++; if(dbg){print "Z900"}; stopp=ret=1; continue }; #hit_40
       if ( veq(bb_n,tn-4) &&
       ...
       { l[i]=omo2; Z[901]++; if(dbg){print "Z901"}; stopp=ret=1; continue }; #inspect #hit_2
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[902]++; if(dbg){print "Z902"}; stopp=ret=1; continue }; #hit_37
       if ( vgl(bb_n,tn-7,tn-3) &&
              q(bb_n+1,"suw_vi") &&
       ...
       { l[i]=omo2; Z[903]++; if(dbg){print "Z903"}; stopp=ret=1; continue }; #hit_1
       ...
       { l[i]=omo2; Z[904]++; if(dbg){print "Z904"}; stopp=ret=1; continue }; #hit_42
       ...
       { l[i]=omo2; Z[905]++; if(dbg){print "Z905"}; stopp=ret=1; continue }; #hit_7
       ...
       { l[i]=omo2; Z[906]++; if(dbg){print "Z906"}; stopp=ret=1; continue }; #hit_1
       ...
       { l[i]=omo2; Z[907]++; if(dbg){print "Z907"}; stopp=ret=1; continue }; #hit_1
       if ( veq(bb_n,tn-3) &&
              q(tn-2,"suw_vi suw_im mest_im mest_vi suw_tv mest_tv qast isname") &&
              q(tn-1,"suw_vi suw_tv qast nar_any") && s(tn-3,tn-1) )
       { l[i]=omo2; Z[908]++; if(dbg){print "Z908"}; stopp=ret=1; continue }; #inspect #hit_43
       if ( veq(bb_n,tn-2) &&
              q(tn-1,"mest_vi mest_tv mest_im mest_3e prl_vi") && s(tn-2,tn-1) )
       { l[i]=omo2; Z[909]++; if(dbg){print "Z909"}; stopp=ret=1; continue }; #hit_928
       if ( veq(bb_n,tn-3) &&
       ...
       { l[i]=omo2; Z[910]++; if(dbg){print "Z910"}; stopp=ret=1; continue }; #hit_1
       if ( veq(bb_n,tn-2) &&
              q(tn-1,"suw_im prl_kred_sr prl_srav isname nar_any qis_vi") && s(tn-2,tn-1) )
       { l[i]=omo2; Z[911]++; if(dbg){print "Z911"}; stopp=ret=1; continue }; #hit_27
    };

    if ( vvb(tn-7,tn-1) && bba(vvn-6,vvn-1,"_gl_v_lock-iv") &&
         qir(bbn+1,vvn-1,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname") &&
           q(vvn,"suw_vi") && s(bbn,vvn-1) )
    { l[i]=omo2; Z[912]++; if(dbg){print "Z912"}; stopp=ret=1; continue }; #hit_4
    cst0="смотреть";
    ...
    { l[i]=omo2; Z[913]++; if(dbg){print "Z913"}; stopp=ret=1; continue }; #hit_4
      ...
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,tn-8,tn-3) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,tn-1) &&
            ...
            qir(qb_n+2,tn-2,"prl_tv prq_tv mest_tv qik_tv qis_tv") &&
              q(tn-1,"suw_tv") )
       { l[i]=omo2; Z[914]++; if(dbg){print "Z914"}; stopp=ret=1; continue }; #hit_2


       if ( qb(qb_n+1,tn-1,"prq_vi") && ba(qbn,"_gl_v_castle-iv") )
       { qb_n=bb_n=""; l[i]=omo2; Z[915]++; if(dbg){print "Z915"}; }; #hit_12
       ...
       { l[i]=omo2; Z[916]++; if(dbg){print "Z916"}; stopp=ret=1; continue }; #hit_21
    ...
    if ( qb(tn-8,tn-2,"pre_tv") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[917]++; if(dbg){print "Z917"}; stopp=ret=1; continue }; #hit_30
       if ( vgl(qb_n,tn-8,tn-3) && veq(bb_n,qb_n-1) &&
            qir(qb_n+1,tn-3,"prl_tv mest_tv mest_3e prq_tv qis_tv qik_tv") &&
              q(tn-2,"suw_tv mest_tv") &&
              q(tn-1,"suw_vi") && s(bb_n,tn-1) )
       { l[i]=omo2; Z[918]++; if(dbg){print "Z918"}; stopp=ret=1; continue }; #hit_75
       if ( vgl(qb_n,tn-8,tn-4) && veq(bb_n,qb_n-1) &&
            qir(qb_n+1,tn-4,"prl_tv mest_tv prq_tv qis_tv qik_tv") &&
              ...
              q(tn-1,"suw_vi") && s(bb_n,tn-1) )
       { l[i]=omo2; Z[919]++; if(dbg){print "Z919"}; stopp=ret=1; continue }; #hit_3
       ...
       { l[i]=omo2; Z[920]++; if(dbg){print "Z920"}; stopp=ret=1; continue }; #hit_119
       ...
       { l[i]=omo2; Z[921]++; if(dbg){print "Z921"}; stopp=ret=1; continue }; #hit_2
    ...
    if ( qb(tn-8,tn-2,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[922]++; if(dbg){print "Z922"}; stopp=ret=1; continue }; #hit_49
       ...
       { l[i]=omo2; Z[923]++; if(dbg){print "Z923"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( qb(tn-8,tn-2,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
        ...
        if ( vgl(qb_n,tn-8,tn-3) && veq(bb_n,qb_n-3) &&
               ...
               q(tn-2,"suw_pr") &&
               q(tn-1,"suw_tv") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[924]++; if(dbg){print "Z924"}; stopp=ret=1; continue }; #hit_1
        ...
        { l[i]=omo2; Z[925]++; if(dbg){print "Z925"}; stopp=ret=1; continue }; #hit_50
        ...
        { l[i]=omo2; Z[926]++; if(dbg){print "Z926"}; stopp=ret=1; continue }; #hit_8
        ...
        { l[i]=omo2; Z[927]++; if(dbg){print "Z927"}; stopp=ret=1; continue }; #hit_11
        if ( vgl(qb_n,tn-8,tn-2) && veq(bb_n,qb_n-1) &&
             qir(qb_n+1,tn-2,"prl_pr mcop_pr prq_pr qis_pr qik_pr") &&
               q(tn-1,"suw_pr") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[928]++; if(dbg){print "Z928"}; stopp=ret=1; continue }; #hit_20
        if ( vgl(qb_n,tn-8,tn-2) && veq(bb_n,qb_n-3) &&
               q(qb_n-2,"pre_ro") &&
               q(qb_n-1,"suw_ro mest_ro") &&
             qir(qb_n+1,tn-2,"prl_pr mcop_pr prq_pr qis_pr qik_pr") &&
               q(tn-1,"suw_pr") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[929]++; if(dbg){print "Z929"}; stopp=ret=1; continue }; #hit_1
        if ( vgl(qb_n,tn-8,tn-3) && veq(bb_n,qb_n-1) &&
               q(qb_n+1,"suw_pr") &&
             qir(qb_n+2,tn-2,"prl_vi mcop_vi prq_vi qis_vi qik_vi") &&
               q(tn-1,"suw_vi") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[930]++; if(dbg){print "Z930"}; stopp=ret=1; continue }; #hit_1
        if ( vgl(qb_n,tn-8,tn-4) && veq(bb_n,qb_n-1) &&
               q(qb_n+1,"suw_pr") &&
             qir(qb_n+2,tn-3,"prl_vi mcop_vi prq_vi qis_vi qik_vi") &&
               q(tn-2,"suw_vi") &&
               q(tn-1,"suw_ro") && s(bb_n,tn-1) )
        { l[i]=omo2; Z[931]++; if(dbg){print "Z931"}; stopp=ret=1; continue }; #hit_1
    };};
    ...
    if ( bfa(1,4,"_gl_v_lock-iv") ) { bf_n=bfn;
       ...
       { l[i]=omo2; Z[932]++; if(dbg){print "Z932"}; stopp=ret=1; continue }; #hit_137

#      if ( vgl(bf_n,1,4) && s(0,bf_n-1) && Qf(1,bf_n-1,"sz_iili") &&
#             q(tn-1,"suw_tv nar_spos") && s(tn-1) )
#      { l[i]=omo2; Z[933]++; if(dbg){print "Z933"}; stopp=ret=1; continue }; #mute
    };
    ...
    { l[i]=omo2; Z[934]++; if(dbg){print "Z934"}; stopp=ret=1; continue }; #hit_2
    # _gl_v_lock-iv:end
    ...
    if ( ba(1,"_v_zamok") &&
          w(2,cst1) && s(0,1) )
    { l[i]=omo2; Z[935]++; if(dbg){print "Z935"}; stopp=ret=1; continue }; #hit_1

    if ( ba(tn-2,"_v_zamok") &&
          w(tn-1,cst1) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[936]++; if(dbg){print "Z936"}; stopp=ret=1; continue }; #hit_7
    if ( ba(tn-1,"_v_zamok") && z(tn-1) &&
         wb(tn-5,tn-2,cst1) && s(wbn,tn-2) &&
        qir(wbn+1,tn-2,"qast_any nar_any") )
    { l[i]=omo2; Z[937]++; if(dbg){print "Z937"}; stopp=ret=1; continue }; #hit_1
    if ( bba(tn-6,tn-3,"_v_zamok") &&
           w(bbn+1,cst1) &&
         qir(bbn+2,tn-2,"prl_ro mest_ro mest_3e prq_ro") &&
           q(tn-1,"suw_ro suw_edne") && s(tn-3,tn-1) )
    { l[i]=omo2; Z[938]++; if(dbg){print "Z938"}; stopp=ret=1; continue }; #hit_3
    if ( bba(tn-5,tn-2,"_v_zamok") &&
         qir(bbn+1,tn-2,"prl_vi prq_vi mcop_vi nar_mest") &&
           w(tn-1,cst1) && s(bbn,tn-1) )
    { l[i]=omo2; Z[939]++; if(dbg){print "Z939"}; stopp=ret=1; continue }; #hit_4
    if ( ba(tn-4,"_v_zamok") &&
          w(tn-3,cst1) &&
     pre_pr(tn-2) &&
          q(tn-1,"suw_pr mest_pr") && s(tn-4,tn-1) )
    { l[i]=omo2; Z[940]++; if(dbg){print "Z940"}; stopp=ret=1; continue }; #hit_1
    if ( ba(tn-4,"_v_zamok") &&
          q(tn-3,"suw_vi mest_vi isname") &&
     pre_vi(tn-2) &&
          w(tn-1,cst1) && s(tn-4,tn-1) )
    { l[i]=omo2; Z[941]++; if(dbg){print "Z941"}; stopp=ret=1; continue }; #hit_1

    cst2="хватом";
    if ( ba(tn-2,"_v_zamok") &&
          w(tn-1,cst2) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[942]++; if(dbg){print "Z942"}; stopp=ret=1; continue }; #hit_1
    # _v_zamok:end

#   if ( bw(tn-2,"пуля") &&
#        bw(tn-1,"угодить") && s(tn-2,tn-1) )
#   { l[i]=omo2; Z[943]++; if(dbg){print "Z943"}; stopp=ret=1; continue }; #mute

    cst="быстрее быстро все и или назад обратно сейчас скорее сначала сперва срочно теперь";
    cst1="прямо сразу";
    if ( w(tn-1,cst) &&
         p(tn-2,",") && p(0) )
    { l[i]=omo1; Z[944]++; if(dbg){print "Z944"}; stopp=ret=1; continue }; #hit_62
    if ( w(tn,cst1) &&
         p(tn-1,",") && p(0) )
    { l[i]=omo1; Z[945]++; if(dbg){print "Z945"}; stopp=ret=1; continue }; #hit_11

    if ( p(-2,",") && veq(tn,-1) && cap(0) && p(0) )
    { l[i]=omo1; Z[946]++; if(dbg){print "Z946"}; stopp=ret=1; continue }; #hit_14
    ...
    { l[i]=omo2; Z[947]++; if(dbg){print "Z947"}; stopp=ret=1; continue }; #hit_3
    if ( bb(tn-4,tn-2,cst1) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"nar_any qast_any") &&
         bw(tn-1,cst) )
    { l[i]=omo2; Z[948]++; if(dbg){print "Z948"}; stopp=ret=1; continue }; #hit_83
    if ( bw(tn-3,cst1) && s(bbn+1,tn-1) && sc(bbn,"[,:]") &&
          q(tn-2,"mest_im") &&
         bw(tn-1,cst) )
    { l[i]=omo2; Z[949]++; if(dbg){print "Z949"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( ( qxs(tn-1,"а но что только","не")||
           ...
           vvb(xsn-5,xsn-1) &&
            bw(vvn,cst) )
    { l[i]=omo2; Z[958]++; if(dbg){print "Z958"}; stopp=ret=1; continue }; #hit_1

    cst="втягиваться";
    if ( bw(tn-1,cst) &&
          w(tn+1,"центральный") && s(tn-1) )
    { l[i]=omo2; Z[959]++; if(dbg){print "Z959"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bba(tn-6,tn-2,"_lock_parts") &&
          sQ(bbn,tn-2,"[….:;!?]") &&
          bw(tn-1,cst) && s(tn-1) )
    { l[i]=omo2; Z[960]++; if(dbg){print "Z960"}; stopp=ret=1; continue }; #hit_44
    if ( bba(tn-6,tn-2,"_lock_parts") &&
           W(bbn+1,"и") && sQ(bbn,tn-2,"[….:;!?]") && Q(0,"isname") &&
          bw(tn-1,cst1) && s(tn-1) )
    { l[i]=omo2; Z[961]++; if(dbg){print "Z961"}; stopp=ret=1; continue }; #hit_15
    if ( bba(tn-6,tn-2,"_lock_parts") &&
          bw(tn-1,"проникнуть просочиться") && s(bbn,tn-1) )
    { l[i]=omo2; Z[962]++; if(dbg){print "Z962"}; stopp=ret=1; continue }; #hit_2

    if ( bba(tn-3,tn-1,"_lock_parts") &&
          se(tn-1," — ") && s(bbn,tn-2) )
    { l[i]=omo2; Z[963]++; if(dbg){print "Z963"}; stopp=ret=1; continue }; #hit_4
    ...
    { l[i]=omo2; Z[964]++; if(dbg){print "Z964"}; stopp=ret=1; continue }; #hit_5
    if ( bw(tn-2,cst1) &&
         bw(tn-1,cst) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[965]++; if(dbg){print "Z965"}; stopp=ret=1; continue }; #hit_2
    if ( bw(tn-1,cst1) && s(-1) &&
         bw(1,cst) && s(0) )
    { l[i]=omo2; Z[966]++; if(dbg){print "Z966"}; stopp=ret=1; continue }; #hit_4
    if ( bw(tn-1,cst1) &&
          q(1,"mest_vi prl_vi qik_vi") &&
         bw(2,cst) && s(tn-1,1) )
    { l[i]=omo2; Z[967]++; if(dbg){print "Z967"}; stopp=ret=1; continue }; #hit_4
    # escape
    ...
    if ( bb(tn-5,tn-2,cst) && s(bbn,tn-1) &&
         bb(bbn-1,tn-1,cst1) )
    { l[i]=omo2; Z[969]++; if(dbg){print "Z969"}; stopp=ret=1; continue }; #hit_6
    ...
    if ( wb(tn-5,tn-1,"надо нужно") ) { wb_n=wbn;
       ...
       { l[i]=omo1; Z[970]++; if(dbg){print "Z970"}; stopp=ret=1; continue }; #hit_123
       if ( vgl(wb_n,tn-5,tn-1) &&
         pre_pr(wb_n+1) &&
            qir(wb_n+2,tn-2,"prl_pr mest_pr mest_3e prq_pr") &&
              q(tn-1,"suw_pr suw_edne isname") && s(wb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[971]++; if(dbg){print "Z971"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( wf(1,5,"надо нужно") ) { wf_n = wfn;
       ...
       { l[i]=omo1; Z[972]++; if(dbg){print "Z972"}; stopp=ret=1; continue }; #hit_38
       cst="будет было ведь все и не равно тоже";
       if ( vgl(wf_n,1,5) &&
            wir(1,wf_n-1,cst) && s(0,wf_n-1) )
       { l[i]=omo1; Z[973]++; if(dbg){print "Z973"}; stopp=ret=1; continue }; #hit_2
       if ( vgl(wf_n,2,5) &&
              q(1,"suw_da mest_da suw_vi mest_vi mest_3e") &&
            wir(2,wf_n-1,cst) && s(0,wf_n-1) )
       { l[i]=omo1; Z[974]++; if(dbg){print "Z974"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-5,tn-2) ) { bb_n = Y["n-_gl_v_castle-iv"] };
       if ( vgl(bb_n,tn-5,tn-2) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-2,"prl_any prq_any mest_any") &&
             ba(tn-1,"_lock_parts") )
       { l[i]=omo2; Z[975]++; if(dbg){print "Z975"}; stopp=ret=1; continue }; #hit_6
       if ( veq(Y["n-_gl_v_castle-iv"],tn-4) &&
             ba(tn-3,"_lock_parts") &&
              q(tn-2,"suw_ro") &&
              q(tn-1,"nar_napr") && s(tn-4,tn-1) )
       { l[i]=omo2; Z[976]++; if(dbg){print "Z976"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[977]++; if(dbg){print "Z977"}; stopp=ret=1; continue }; #hit_7
    if ( bb(tn-6,tn-3,cst) &&
        qir(bbn+1,tn-3,"qast nar_any") &&
         ba(tn-2,"_lock_parts") &&
          q(tn-1,"suw_ro") && s(bbn,tn-1) )
    { l[i]=omo2; Z[978]++; if(dbg){print "Z978"}; stopp=ret=1; continue }; #hit_1
    # _lock_parts:end
    ...
    { l[i]=omo1; Z[979]++; if(dbg){print "Z979"}; stopp=ret=1; continue }; #hit_2
    ...
    { l[i]=omo2; Z[980]++; if(dbg){print "Z980"}; stopp=ret=1; continue }; #hit_1
    if ( bw(tn-4,cst) &&
          q(tn-3,"nar_any mest_vi mest_3e suw_vi") &&
          q(tn-2,"prl_tv mest_tv mest_3e prq_tv") &&
          w(tn-1,cst1) && s(tn-4,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[981]++; if(dbg){print "Z981"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[982]++; if(dbg){print "Z982"}; stopp=ret=1; continue }; #hit_1

    cst="айда бегом брысь верхом домой надо надолго немедленно нужно обратно пешком ползком пора сюда";
    if ( w(tn-1,cst) &&
       (zs(tn-1)||sc(tn-1,"—")) )
    { l[i]=omo1; Z[983]++; if(dbg){print "Z983"}; stopp=ret=1; continue }; #hit_1472
    if ( w(1,cst) && s(0) )
    { l[i]=omo1; Z[984]++; if(dbg){print "Z984"}; stopp=ret=1; continue }; #hit_45
...
#      { bb_n=""; Z[986]++; if(dbg){print "Z986"}; }; #mute
       ...
       { l[i]=omo1; Z[987]++; if(dbg){print "Z987"}; stopp=ret=1; continue }; #hit_378
       if ( vgl(bb_n,tn-5,tn-2) &&
            qir(bb_n+1,tn-2,"prl_im prq_im mest_im") &&
              q(tn-1,"suw_im name_im_sy") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[988]++; if(dbg){print "Z988"}; stopp=ret=1; continue }; #hit_326
       if ( vgl(bb_n,tn-7,tn-3) &&
       ...
       { l[i]=omo1; Z[989]++; if(dbg){print "Z989"}; stopp=ret=1; continue }; #hit_76
       ...
       { l[i]=omo1; Z[990]++; if(dbg){print "Z990"}; stopp=ret=1; continue }; #hit_48
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[991]++; if(dbg){print "Z991"}; stopp=ret=1; continue }; #hit_3355
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[992]++; if(dbg){print "Z992"}; stopp=ret=1; continue }; #hit_629
       ...
       { l[i]=omo1; Z[993]++; if(dbg){print "Z993"}; stopp=ret=1; continue }; #hit_43
       if ( vgl(bb_n,tn-5,tn-2) &&
       ...
       { l[i]=omo1; Z[994]++; if(dbg){print "Z994"}; stopp=ret=1; continue }; #hit_304
       if ( vgl(bb_n,tn-5,tn-2) &&
       ...
       { l[i]=omo1; Z[995]++; if(dbg){print "Z995"}; stopp=ret=1; continue }; #hit_93
       if ( vgl(bb_n,tn-6,tn-3) &&
            qir(bb_n+1,tn-3,"mest_vi prq_vi suw_vi mest_3e name_vi_sy isname nar_any qast") &&
              ...
              q(tn-1,"nar_any") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[996]++; if(dbg){print "Z996"}; stopp=ret=1; continue }; #hit_8
       if ( vgl(bb_n,tn-6,tn-2) &&
              q(bb_n+1,"suw_vi suw_edne isname nar_any") &&
            qir(bb_n+2,tn-1,"suw_ro prq_ro suw_edne isname nar_any") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[997]++; if(dbg){print "Z997"}; stopp=ret=1; continue }; #hit_517
       if ( vgl(bb_n,tn-5,tn-2) &&
       ...
       { l[i]=omo1; Z[998]++; if(dbg){print "Z998"}; stopp=ret=1; continue }; #hit_161
       if ( vgl(bb_n,tn-5,tn-2) &&
       ...
       { l[i]=omo1; Z[999]++; if(dbg){print "Z999"}; stopp=ret=1; continue }; #hit_16
       if ( vgl(bb_n,tn-6,tn-2) &&
            ...
            qir(bb_n+2,tn-2,"prl_tv mest_tv mest_3e gl_in prl_kred_sr nar_any qast name_tv_sy isname") &&
              q(tn-1,"suw_tv mest_tv prq_tv prl_tv suw_edne mest_3e gl_in prl_kred_sr nar_any qast name_vi_sy isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1000]++; if(dbg){print "Z1000"}; stopp=ret=1; continue }; #hit_18
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1001]++; if(dbg){print "Z1001"}; stopp=ret=1; continue }; #hit_2
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1002]++; if(dbg){print "Z1002"}; stopp=ret=1; continue }; #hit_38
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1003]++; if(dbg){print "Z1003"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1004]++; if(dbg){print "Z1004"}; stopp=ret=1; continue }; #hit_2
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1005]++; if(dbg){print "Z1005"}; stopp=ret=1; continue }; #hit_8
       ...
       { l[i]=omo1; Z[1006]++; if(dbg){print "Z1006"}; stopp=ret=1; continue }; #hit_7
       if ( vgl(bb_n,tn-7,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mest_vi mest_3e") &&
              q(tn-2,"suw_vi suw_edne") &&
              q(tn-1,"suw_ro prl_kred_sr") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1007]++; if(dbg){print "Z1007"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(bb_n,tn-7,tn-4) &&
            qir(bb_n+1,tn-4,"prl_vi prq_vi mest_vi mest_3e") &&
              ...
              q(tn-1,"suw_ro prl_kred_sr") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1008]++; if(dbg){print "Z1008"}; stopp=ret=1; continue }; #hit_2

       if ( vgl(bb_n,tn-6,tn-3) &&
              q(bb_n+1,"suw_vi suw_edne mest_vi mest_3e") &&
            qir(bb_n+2,tn-2,"prl_da prq_da mest_da mest_3e") &&
              q(tn-1,"suw_da isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1009]++; if(dbg){print "Z1009"}; stopp=ret=1; continue }; #hit_1
       if ( vgl(bb_n,tn-4,tn-2) &&
            qir(bb_n+1,tn-1,"gl_in gl_aux_be qast_any") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1010]++; if(dbg){print "Z1010"}; stopp=ret=1; continue }; #hit_4
       if ( veq(bb_n,tn-1) && zs(tn-1) )
       { l[i]=omo1; Z[1011]++; if(dbg){print "Z1011"}; stopp=ret=1; continue }; #hit_28021
       if ( vvb(tn-5,tn-1) && veq(bb_n,vvn) )
       { l[i]=omo1; Z[1012]++; if(dbg){print "Z1012"}; stopp=ret=1; continue }; #hit_26
    };
       ...
       { l[i]=omo1; Z[1013]++; if(dbg){print "Z1013"}; stopp=ret=1; continue }; #hit_57
    ...
    if ( qb(tn-6,tn-2,"sz_iili") ) { qb_n=qbn; if ( bba(qb_n-6,qb_n-2,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1014]++; if(dbg){print "Z1014"}; stopp=ret=1; continue }; #hit_66
       if ( vgl(qb_n,tn-6,tn-2) && vgl(bb_n,qb_n-6,qb_n-2) &&
       ...
       { l[i]=omo1; Z[1015]++; if(dbg){print "Z1015"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(qb_n,tn-6,tn-2) && vgl(bb_n,qb_n-6,qb_n-2) &&
       ...
       { l[i]=omo1; Z[1016]++; if(dbg){print "Z1016"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( qb(tn-6,tn-2,"pre_da") ) { qb_n=qbn; if ( bba(qb_n-9,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1017]++; if(dbg){print "Z1017"}; stopp=ret=1; continue }; #hit_979
       if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-6,tn-3) &&
            ...
            qir(qb_n+1,tn-3,"prl_da mest_da mest_3e prq_da prl_kred_sr nar_any") &&
              ...
              q(tn-1,"nar_any") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1018]++; if(dbg){print "Z1018"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( qb(tn-7,tn-2,"pre_tv") ) { qb_n=qbn;
      if ( bba(qb_n-9,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1019]++; if(dbg){print "Z1019"}; stopp=ret=1; continue }; #hit_760
       ...
       { l[i]=omo1; Z[1020]++; if(dbg){print "Z1020"}; stopp=ret=1; continue }; #hit_1
        if ( vgl(bb_n,qb_n-7,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
             ...
             qir(qb_n+1,tn-2,"prl_tv mest_tv suw_tv mest_3e prq_tv prl_kred_sr qik_tv nar_any isname") &&
               q(tn-1,"mest_tv suw_tv prq_tv suw_edne isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1021]++; if(dbg){print "Z1021"}; stopp=ret=1; continue }; #hit_1
       if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1022]++; if(dbg){print "Z1022"}; stopp=ret=1; continue }; #hit_19
       if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
            qir(bb_n+1,qb_n-1,"mest_vi suw_vi suw_im mest_im mest_da prl_vi suw_da mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
              q(qb_n+1,"mest_tv suw_tv suw_edne isname") &&
            qir(qb_n+2,tn-2,"prl_ro mest_ro mest_3e prq_ro") &&
              q(tn-1,"suw_ro suw_edne") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1023]++; if(dbg){print "Z1023"}; stopp=ret=1; continue }; #hit_5
       if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo1; Z[1024]++; if(dbg){print "Z1024"}; stopp=ret=1; continue }; #hit_7
       ...
       { l[i]=omo1; Z[1025]++; if(dbg){print "Z1025"}; stopp=ret=1; continue }; #hit_24
    ...
    if ( qb(tn-7,tn-2,"pre_pr") ) { qb_n=qbn; if ( bba(qb_n-6,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1026]++; if(dbg){print "Z1026"}; stopp=ret=1; continue }; #hit_12
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
            qir(bb_n+1,qb_n-1,"mest_vi suw_vi mest_da suw_da mest_im suw_im prl_vi prq_vi mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
       ...
       { l[i]=omo1; Z[1027]++; if(dbg){print "Z1027"}; stopp=ret=1; continue }; #hit_1
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
            qir(bb_n+1,qb_n-1,"mest_vi suw_vi mest_da suw_da mest_im suw_im prl_vi prq_vi mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
       ...
       { l[i]=omo1; Z[1028]++; if(dbg){print "Z1028"}; stopp=ret=1; continue }; #hit_2
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
            qir(bb_n+1,qb_n-1,"mest_vi suw_vi mest_da suw_da mest_im suw_im prl_vi prq_vi mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
            qir(qb_n+1,tn-2,"prl_pr prq_pr mest_pr mest_3e qik_pr") &&
              q(tn-1,"mest_pr suw_pr suw_edne") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1029]++; if(dbg){print "Z1029"}; stopp=ret=1; continue }; #hit_93
       ...
       { l[i]=omo1; Z[1030]++; if(dbg){print "Z1030"}; stopp=ret=1; continue }; #hit_9
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
            qir(bb_n+1,qb_n-1,"mest_vi suw_vi mest_da suw_da mest_im suw_im prl_vi prq_vi mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
              q(qb_n+1,"suw_pr suw_edne") &&
            qir(qb_n+2,tn-1,"suw_ro suw_edne nar_any prl_ro prq_ro") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1031]++; if(dbg){print "Z1031"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
         pre_tv(bb_n+1) &&
       ...
       { l[i]=omo1; Z[1032]++; if(dbg){print "Z1032"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( qb(tn-9,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1033]++; if(dbg){print "Z1033"}; stopp=ret=1; continue }; #hit_548
       ...
       { l[i]=omo1; Z[1034]++; if(dbg){print "Z1034"}; stopp=ret=1; continue }; #hit_5
       if ( vgl(bb_n,qb_n-5,qb_n-1) && vgl(qb_n,tn-9,tn-2) &&
             qb(bb_n+1,qb_n-1,"pre_ro") &&
            qir(bb_n+1,qbn-1,"mest_vi suw_vi mest_da suw_da mest_im suw_im prl_vi prq_vi mest_3e qis_vi nar_any prl_kred_sr isname qast_any") &&
            qir(qbn+1,qb_n-1,"suw_ro prl_ro mest_ro mest_3e prq_ro prl_kred_sr nar_any isname") &&
            qir(qb_n+1,tn-1,"suw_vi suw_im suw_ro suw_mnsq prl_vi mest_vi mest_3e prq_vi qik_vi qis_vi nar_any prl_kred_sr isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[1035]++; if(dbg){print "Z1035"}; stopp=ret=1; continue }; #hit_9
       ...
       if ( vgl(bb_n,qb_n-7,qb_n-1) && vgl(qb_n,tn-9,tn-2) && qb(qb_n+1,tn-1,"sz_iili") ) { qf_n=qbn; qsf(bb_n,qf_n,","); wf_n=sfn;
          ...
          { l[i]=omo1; Z[1036]++; if(dbg){print "Z1036"}; stopp=ret=1; continue }; #hit_45
          ...
          { l[i]=omo1; Z[1037]++; if(dbg){print "Z1037"}; stopp=ret=1; continue }; #hit_11
    ...
    if ( qb(tn-7,tn-2,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-6,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
        ...
        { l[i]=omo1; Z[1038]++; if(dbg){print "Z1038"}; stopp=ret=1; continue }; #hit_322
        if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-7,tn-2) && s(bb_n,tn-2) && zs(tn-1) &&
        ...
        { l[i]=omo1; Z[1039]++; if(dbg){print "Z1039"}; stopp=ret=1; continue }; #hit_7
       };
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-9,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
        ...
        { l[i]=omo1; Z[1040]++; if(dbg){print "Z1040"}; stopp=ret=1; continue }; #hit_1

        if ( narph_any(tn-1) &&
                   veq(bb_n,xwn-1) && s(xnw-1,tn-1) )
        { l[i]=omo1; Z[1041]++; if(dbg){print "Z1041"}; stopp=ret=1; continue }; #hit_2
        if ( narph_any(tn-2) &&
                   veq(bb_n,xwn-1) &&
                     q(tn-1,"suw_vi suw_im mest_vi mest_im mest_3e isname nar_any") && s(xnw-1,tn-1) )
        { l[i]=omo1; Z[1042]++; if(dbg){print "Z1042"}; stopp=ret=1; continue }; #hit_2
        if ( narph_any(tn-1) &&
                   veq(bb_n,xwn-2) &&
                     q(xwn-1,"suw_vi mest_vi mest_3e isname nar_any") && s(xnw-2,tn-1) )
        { l[i]=omo1; Z[1043]++; if(dbg){print "Z1043"}; stopp=ret=1; continue }; #hit_1
    };

    if ( preph_ro(1) && zs(0) &&
                q(xwn+1,"isname") && s(xwn) )
    { l[i]=omo1; Z[1044]++; if(dbg){print "Z1044"}; stopp=ret=1; continue }; #hit_7

    if ( pre_tv(1) &&
              q(2,"suw_tv") &&
              q(3,"nar_spos") &&
             ba(4,"_gl_v_castle-iv") && s(0,3) )
    { l[i]=omo1; Z[1045]++; if(dbg){print "Z1045"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bfa(1,6,"_gl_v_castle-iv") ) { bf_n=bfn;
       ...
       { bf_n=""; Z[1046]++; if(dbg){print "Z1046"}; }; #hit_1

       if ( vgl(bf_n,1,6) && s(0,bf_n-1) && Qf(1,bf_n,"sz_iili") )
       { l[i]=omo1; Z[1047]++; if(dbg){print "Z1047"}; stopp=ret=1; continue }; #hit_4151
       if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
            sos(tn-9,tn-1) &&
          !(vgl(Y["n-_gl_v_lock-iv"],son,tn-1)||bba(son,tn-1,"_lock_parts") ) &&
             (w(bf_n-1,"и")||
            qxs(bf_n-2,"и","не") ) )
       { l[i]=omo1; Z[1048]++; if(dbg){print "Z1048"}; stopp=ret=1; continue }; #hit_36
    ...
    { l[i]=omo1; Z[1049]++; if(dbg){print "Z1049"}; stopp=ret=1; continue }; #hit_5
    if ( q(1,"suw_im mest_im isname") &&
         q(2,"narph_any") &&
       bfa(xwn+1,xwn+3,"_gl_v_castle-iv") && s(0,bfn-1) &&
       qir(xwn+1,bfn-1,"qast_any nar_any") )
    { l[i]=omo1; Z[1050]++; if(dbg){print "Z1050"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[1051]++; if(dbg){print "Z1051"}; stopp=ret=1; continue }; #hit_40
    if ( z(0) &&
         q(1,"sz") &&
         w(2,"к ко") &&
         q(3,"suw_odedda suw_odmnda isname mest_da") && s(1,2) )
    { l[i]=omo1; Z[1052]++; if(dbg){print "Z1052"}; stopp=ret=1; continue }; #hit_2
    ...
    { l[i]=omo1; Z[1053]++; if(dbg){print "Z1053"}; stopp=ret=1; continue }; #hit_83

    if ( q(tn-2,"mest_da suw_da") &&
         q(tn-1,"mod_ed mod_mn mod_bz") && s(tn-2,tn-1) )
    { l[i]=omo1; Z[1054]++; if(dbg){print "Z1054"}; stopp=ret=1; continue }; #hit_7

    if ( w(tn-2,"к ко") &&
         q(tn-1,"mest_da suw_da isname") && s(tn-2) && zs(tn-1) )
    { l[i]=omo1; Z[1055]++; if(dbg){print "Z1055"}; stopp=ret=1; continue }; #hit_88

    # prl как часть сказуемого
    cst="вхожий";
    if ( bw(tn-1,cst) && s(tn-1) )
    { l[i]=omo1; Z[1056]++; if(dbg){print "Z1056"}; stopp=ret=1; continue }; #hit_20
    if ( bw(1,cst) && s(0) )
    { l[i]=omo1; Z[1057]++; if(dbg){print "Z1057"}; stopp=ret=1; continue }; #hit_1
    if ( w(1,"не") &&
        bw(2,cst) && s(0,1) )
    { l[i]=omo1; Z[1058]++; if(dbg){print "Z1058"}; stopp=ret=1; continue }; #hit_2
    if ( q(1,"mest_im suw_im isname prl_kred_sr") &&
        bw(2,cst) && s(0,1) )
    { l[i]=omo1; Z[1059]++; if(dbg){print "Z1059"}; stopp=ret=1; continue }; #hit_1
    if ( q(1,"mest_im suw_im isname") &&
         w(2,"не") &&
        bw(3,cst) && s(0,2) )
    { l[i]=omo1; Z[1060]++; if(dbg){print "Z1060"}; stopp=ret=1; continue }; #hit_1
    #
    cst="ваш мой наш свой сей тот";
    if ( w(-1,cst) )
    { l[i]=omo1; Z[1061]++; if(dbg){print "Z1061"}; stopp=ret=1; continue }; #hit_140
    ...
    if ( veq(tn,-1) && s(-1) ) {
      ...
      { l[i]=omo2; Z[1062]++; if(dbg){print "Z1062"}; stopp=ret=1; continue }; #hit_6
      if ( p(-2) && p(0) )
      { l[i]=omo1; Z[1063]++; if(dbg){print "Z1063"}; stopp=ret=1; continue }; #hit_259
    };
...
function TN_na_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_na_zamok_f"]);
   ...
   if ( vex(Y["tn_ok_na"]) ) { tn=Y["n_pre_na"];
    ...
    cst="весь целый";
    if ( w(tn+1,cst) )
    { l[i]=omo1; Z[1064]++; if(dbg){print "Z1064"}; stopp=ret=1; continue }; #hit_14
    ...
    if ( ( ( w(tn-1,"как") && qxw(1,"так","и","на") && s(tn-1))||
         ...
         ( qxs(tn-1,"не") && qxw(1,"а но") ) ) && z(0) &&
           ...
           qir(xwn+1,bfn-1,"prl_vi mest_vi mest_3e prq_vi") )
    { l[i]=omo1; Z[1065]++; if(dbg){print "Z1065"}; stopp=ret=1; continue }; #hit_5
    ...
    { l[i]=omo2; Z[1066]++; if(dbg){print "Z1066"}; stopp=ret=1; continue }; #hit_15

    cst="пасти пасть";
    if ( w(tn-1,cst) && s(tn-1) )
    { l[i]=omo2; Z[1067]++; if(dbg){print "Z1067"}; stopp=ret=1; continue }; #hit_4

    cst="закрывать закрыть запереть запирать";
    cst2="дорогу";
    if ( bw(tn-2,cst) &&
          w(tn-1,cst2) && s(tn-2,tn-1) )
    { l[i]=omo2; Z[1068]++; if(dbg){print "Z1068"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bb(tn-6,tn-2,cst) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"prl_vi mest_vi") &&
          w(tn-1,cst1) )
    { l[i]=omo1; Z[1069]++; if(dbg){print "Z1069"}; stopp=ret=1; continue }; #hit_12
    ...
    if ( bb(tn-6,tn-2,cst) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"prl_edmuvi mest_edmuvi") &&
          w(tn-1,cst1) )
    { l[i]=omo1; Z[1070]++; if(dbg){print "Z1070"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( bb(tn-6,tn-2,cst) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"prl_edzevi mest_edzevi") &&
          w(tn-1,cst1) )
    { l[i]=omo2; Z[1071]++; if(dbg){print "Z1071"}; stopp=ret=1; continue }; #hit_47
    ...
    if ( bb(tn-4,tn-2,cst1) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"suw_odro isname") &&
         bw(tn-1,cst) )
    { l[i]=omo2; Z[1072]++; if(dbg){print "Z1072"}; stopp=ret=1; continue }; #hit_2

    if ( bw(tn-2,"раскатать") &&
          w(tn-1,"губу губы") && s(tn-2,tn-1) )
    { l[i]=omo1; Z[1073]++; if(dbg){print "Z1073"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bb(tn-3,tn-1,cst) ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1074]++; if(dbg){print "Z1074"}; stopp=ret=1; continue }; #hit_2

       if ( veq(bb_n,tn-1) && s(tn-1) &&
             bw(1,cst1) && s(0) )
       { l[i]=omo1; Z[1075]++; if(dbg){print "Z1075"}; stopp=ret=1; continue }; #hit_9
       if ( veq(bb_n,tn-1) && s(tn-1) &&
              q(1,"prl_vi mest_vi mest_3e") &&
             bw(2,cst1) && s(0,1) )
       { l[i]=omo1; Z[1076]++; if(dbg){print "Z1076"}; stopp=ret=1; continue }; #hit_7
       if ( veq(bb_n,tn-2) && s(tn-2,tn-1) &&
             bw(tn-1,cst1) )
       { l[i]=omo1; Z[1077]++; if(dbg){print "Z1077"}; stopp=ret=1; continue }; #hit_5

       if ( veq(bb_n,tn-1) && s(tn-1) &&
             bw(1,cst2) && s(0) )
       { l[i]=omo2; Z[1078]++; if(dbg){print "Z1078"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bf(1,3,cst) ) { bf_n=bfn;
       ...
       { l[i]=omo1; Z[1079]++; if(dbg){print "Z1079"}; stopp=ret=1; continue }; #hit_1
       if ( vgl(bf_n,1,3) &&
             bf(bf_n+1,bf_n+4,cst1) && s(0,bf_n-1) &&
            qir(1,bf_n-1,"mod_any gl_aux_be") &&
            qir(bf_n+1,bfn-1,"mod_any gl_aux_be") )
       { l[i]=omo1; Z[1080]++; if(dbg){print "Z1080"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(bf_n,1,3) &&
             bf(bf_n+1,bf_n+4,cst2) && s(0,bf_n-1) &&
            qir(1,bf_n-1,"mod_any gl_aux_be") &&
            qir(bf_n+1,bfn-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo2; Z[1081]++; if(dbg){print "Z1081"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bb(tn-5,tn-2,cst) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"mest_vi prl_vi") &&
          w(tn-1,cst2) )
    { l[i]=omo2; Z[1082]++; if(dbg){print "Z1082"}; stopp=ret=1; continue }; #hit_19
    if ( bb(tn-5,tn-1,cst) && s(bbn,0) &&
        qir(bbn+1,tn-1,"mest_vi prl_vi") &&
          w(1,cst2) )
    { l[i]=omo2; Z[1083]++; if(dbg){print "Z1083"}; stopp=ret=1; continue }; #hit_4
    if ( bb(tn-5,tn-2,cst) && s(bbn,tn-1) &&
        qir(bbn+1,tn-2,"mest_vi prl_vi") &&
          w(tn-1,cst1) )
    { l[i]=omo1; Z[1084]++; if(dbg){print "Z1084"}; stopp=ret=1; continue }; #hit_1

    # _suw_na_castle:start
    if ( vgl(Y["n-_suw_na_castle"],tn-5,tn-1) ) { bb_n=Y["n-_suw_na_castle"];

       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo1; Z[1085]++; if(dbg){print "Z1085"}; stopp=ret=1; continue }; #hit_825
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-1,"suw_tv prl_tv mest_tv prq_tv")  )
       { l[i]=omo1; Z[1086]++; if(dbg){print "Z1086"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[1087]++; if(dbg){print "Z1087"}; stopp=ret=1; }; #hit_3

       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
            qir(1,bf_n-1,"gl_aux_be gl_in gl_ed gl_mn prq_kred")  )
       { l[i]=omo1; Z[1088]++; if(dbg){print "Z1088"}; stopp=ret=1; continue }; #hit_22
    ...
    if ( vgl(Y["n-_suw_na_lock"],tn-5,tn-1) ) { bb_n=Y["n-_suw_na_lock"];
       ...
       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo2; Z[1089]++; if(dbg){print "Z1089"}; stopp=ret=1; continue }; #hit_3798
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-1,"suw_tv prl_tv mest_tv prq_tv")  )
       { l[i]=omo2; Z[1090]++; if(dbg){print "Z1090"}; stopp=ret=1; continue }; #hit_27
    ...
    if ( vgl(Y["n-_gl_na_castle"],tn-7,tn-1) ) { bb_n=Y["n-_gl_na_castle"]; BB_n=Y["w-_gl_na_castle"];
       ...
       { l[i]=omo2; Z[1091]++; if(dbg){print "Z1091"}; stopp=ret=1; continue }; #hit_1
       cst="накинуть";
       cst2="крючок петлю приспособу цепочку";
       if ( veq(bb_n,tn-1) && bw(tn-1,cst) && s(tn-1) &&
              w(1,cst2) && s(0) )
       { l[i]=omo2; Z[1092]++; if(dbg){print "Z1092"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bb_n,tn-5,tn-2) && bw(bb_n,cst) && s(bb_n,tn-1) &&
              w(tn-1,cst2) &&
            qir(bb_n+1,tn-2,"prl_vi mest_vi mest_3e qik_vi") )
       { l[i]=omo2; Z[1093]++; if(dbg){print "Z1093"}; stopp=ret=1; continue }; #hit_1
       cst="опуститься";
       cst2="голова рука";
       if ( bw(tn-2,cst2) &&
            bw(tn-1,cst) && s(tn-2,tn-1) )
       { l[i]=omo2; Z[1094]++; if(dbg){print "Z1094"}; stopp=ret=1; continue }; #hit_2
       cst="отправить";
       cst2="последовательность";
       if ( bw(tn-1,cst) &&
            wf(1,3,cst2) && s(tn-2,wfn-1) &&
           qir(1,wfn-1,"prl_vi mest_3e") )
       { l[i]=omo2; Z[1095]++; if(dbg){print "Z1095"}; stopp=ret=1; continue }; #hit_1
       cst="наступать наступить";
       cst2="нога";
       if ( bw(tn-1,cst) &&
            bb(tn-5,tn-2,cst2) && s(bbn,tn-1) &&
           qir(bbn+1,tn-2,"nar_any") )
       { l[i]=omo2; Z[1096]++; if(dbg){print "Z1096"}; stopp=ret=1; continue }; #hit_1
       # pointesque

       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-1,"qast_any nar_any prl_kred_sr") )
       { l[i]=omo1; Z[1097]++; if(dbg){print "Z1097"}; stopp=ret=1; continue }; #hit_852
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-2,"prl_vi prq_vi mest_vi mest_3e qast_any") &&
              q(tn-1,"suw_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[1098]++; if(dbg){print "Z1098"}; stopp=ret=1; continue }; #hit_30
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-2,"prl_im prq_im mest_im mest_3e") &&
              q(tn-1,"suw_im mest_im mest_3e") )
       { l[i]=omo1; Z[1099]++; if(dbg){print "Z1099"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
            qir(bb_n+1,tn-2,"prl_tv prq_tv mest_tv mest_3e") &&
              q(tn-1,"suw_tv mest_tv mest_3e") )
       { l[i]=omo1; Z[1100]++; if(dbg){print "Z1100"}; stopp=ret=1; continue }; #hit_14

       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,tn-1) &&
             wf(bb_n+1,tn-2,"с со") &&
            qir(bb_n+1,wfn-1,"qast_any nar_any prl_kred_sr") &&
            qir(wfn+1,tn-1,"mest_tv suw_tv isname suw_ro") )
       { l[i]=omo1; Z[1101]++; if(dbg){print "Z1101"}; stopp=ret=1; continue }; #hit_11
    ...
    if ( bfa(1,4,"_gl_na_castle") ) { bf_n=bfn;
      ...
      { l[i]=omo2; Z[1102]++; if(dbg){print "Z1102"}; stopp=ret=1; continue }; #hit_1
      if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
            Xw(bf_n,"X_gl_na_castle") && Qf(1,bf_n-1,"sz_iili pre_any") &&
          !bfa(1,bf_n-1,"_suw_na_lock") )
      { l[i]=omo1; Z[1103]++; if(dbg){print "Z1103"}; stopp=ret=1; continue }; #hit_377
  };
    # _gl_na_castle:end
    ...
    if ( qb(tn-7,tn-1,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_na_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1104]++; if(dbg){print "Z1104"}; stopp=ret=1; continue }; #hit_98
       ...
       { l[i]=omo2; Z[1105]++; if(dbg){print "Z1105"}; stopp=ret=1; continue }; #hit_9
    ...
    if ( qb(tn-7,tn-1,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_na_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1106]++; if(dbg){print "Z1106"}; stopp=ret=1; continue }; #hit_152
    ...
    if ( qb(tn-7,tn-1,"pre_tv") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_na_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1107]++; if(dbg){print "Z1107"}; stopp=ret=1; continue }; #hit_24
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(-1) &&
            qir(bb_n+1,qb_n-1,"suw_vi mest_vi prq_vi mest_3e prl_kred_sr nar_any qast_any") &&
       ...
       { l[i]=omo2; Z[1108]++; if(dbg){print "Z1108"}; stopp=ret=1; continue }; #hit_9
    ...
    if ( qb(tn-7,tn-1,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_na_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1109]++; if(dbg){print "Z1109"}; stopp=ret=1; continue }; #hit_57
    ...
    if ( qb(tn-7,tn-1,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_na_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1110]++; if(dbg){print "Z1110"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( vgl(Y["n-_gl_na_lock"],tn-7,tn-1) ) { bb_n=Y["n-_gl_na_lock"];
       ...
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=""; Z[1111]++; if(dbg){print "Z1111"}; }; #hit_2

       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo2; Z[1112]++; if(dbg){print "Z1112"}; stopp=ret=1; continue }; #hit_6708
       ...
       { l[i]=omo2; Z[1113]++; if(dbg){print "Z1113"}; stopp=ret=1; continue }; #hit_2217
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
            qir(bb_n+1,tn-1,"suw_im isname prl_im prq_im mest_im mest_3e qast nar_any prl_kred_sr") )
       { l[i]=omo2; Z[1114]++; if(dbg){print "Z1114"}; stopp=ret=1; continue }; #hit_77
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
    ...
    if ( bfa(1,5,"_gl_na_lock") ) { bf_n=bfn;
       ...
       { l[i]=omo1; Z[1118]++; if(dbg){print "Z1118"}; stopp=ret=1; continue }; #hit_1

       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo2; Z[1119]++; if(dbg){print "Z1119"}; stopp=ret=1; continue }; #hit_398
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
            qir(1,bf_n-1,"nar_any prl_kred_sr") )
       { l[i]=omo2; Z[1120]++; if(dbg){print "Z1120"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
            qir(1,bf_n-1,"nar_any prl_kred_sr qast_any") )
       { l[i]=omo2; Z[1121]++; if(dbg){print "Z1121"}; stopp=ret=1; continue }; #hit_1
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
         pre_ro(1) &&
            qiz(2,bf_n-1,"nar_any prl_kred_sr prl_ro mest_ro mest_3e","qast_any suw_ro") )
       { l[i]=omo2; Z[1122]++; if(dbg){print "Z1122"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
         pre_pr(1) &&
            qiz(2,bf_n-1,"nar_any prl_kred_sr prl_pr mest_pr mest_3e","qast_any suw_pr") )
       { l[i]=omo2; Z[1123]++; if(dbg){print "Z1123"}; stopp=ret=1; continue }; #hit_3
       if ( vgl(bf_n,1,5) && s(-1,bf_n-1) &&
            qia(1,bf_n-1,"sz_i","nar_any prl_kred_sr prl_pr mest_pr mest_3e qast_any suw_pr") )
       { l[i]=omo2; Z[1124]++; if(dbg){print "Z1124"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( bw(tn-1,cst) && s(tn-1) ) {
      ...
      { l[i]=omo1; Z[1125]++; if(dbg){print "Z1125"}; stopp=ret=1; continue }; #hit_85
      if ( qxs(1,"комбинаций","песчинок") )
      { l[i]=omo1; Z[1126]++; if(dbg){print "Z1126"}; stopp=ret=1; continue }; #hit_1

      if ( bba(tn-4,tn-2,"_castlelike_things") && zs(bbn) &&
           qir(bbn+1,tn-2,"qast_ne prl_srav") && s(bbn+1,tn-2) )
      { l[i]=omo1; Z[1127]++; if(dbg){print "Z1127"}; stopp=ret=1; continue }; #hit_280
      if ( narph_spos(tn-2) &&
                  bba(xwn-3,xwn-1,"_castlelike_things") && zs(bbn) && s(bbn+1,xwn-1) &&
                  qir(bbn+1,xwn-1,"otsz_any sz_qto") && s(tn-2) )
      { l[i]=omo1; Z[1128]++; if(dbg){print "Z1128"}; stopp=ret=1; continue }; #hit_1
    };
    # похожий I
    ...
    { l[i]=omo1; Z[1129]++; if(dbg){print "Z1129"}; stopp=ret=1; continue }; #hit_8
    if ( qy(tn-1,"sz_iili") &&
         wb(tn-5-qyn,tn-2-qyn,"на") && s(wbn,tn-2-qyn) && zs(tn-1-qyn) &&
         ba(wbn+1,"_castle_environs") &&
        qir(wbn+2,tn-1-qyn,"prl_ro mest_ro mest_3e prq_ro isname") )
    { l[i]=omo1; Z[1130]++; if(dbg){print "Z1130"}; stopp=ret=1; continue }; #hit_3
...
function TN_za_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_za_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_za"]) ) { tn=Y["n_pre_za"];
   ...
   if ( bw(-5,cst1) && z(-5) &&
        ...
        bw(-2,cst2) &&
         w(-1,"за") && s(-4,-1) )
    { l[i]=omo1; Z[1131]++; if(dbg){print "Z1131"}; stopp=ret=1; continue }; #hit_1
    cst="держаться";
    if ( bw(-2,cst) &&
          w(-1,"за") && s(-2,-1) &&
       (qxs(1,"до","последнего")||
        qxs(1,"нет","смысла") ) )
    { l[i]=omo1; Z[1132]++; if(dbg){print "Z1132"}; stopp=ret=1; continue }; #hit_1

    cst="взяться держаться цепляться";
    if ( bw(-3,cst) &&
        qxs(-1,"за","мой твой наш ваш их свой этот тот ее её их его") )
    { l[i]=omo1; Z[1133]++; if(dbg){print "Z1133"}; stopp=ret=1; continue }; #hit_8
    ...
    if ( qxs(-3,"а","уж","потом") &&
          ...
          bw(-2,cst1) &&
           w(-1,"за") && s(-2,-1) )
    { l[i]=omo1; Z[1134]++; if(dbg){print "Z1134"}; stopp=ret=1; continue }; #hit_1
    if ( bw(-3,cst) &&
          w(-2,"за") &&
     isname(-1) && s(-3,-1) )
    { l[i]=omo1; Z[1135]++; if(dbg){print "Z1135"}; stopp=ret=1; continue }; #hit_1
    if ( bw(-2,cst) &&
          w(-1,"за") &&
    sz_iili(1) &&
         bw(2,"спалить сжечь сровнять снести") && s(-2,1) )
    { l[i]=omo1; Z[1136]++; if(dbg){print "Z1136"}; stopp=ret=1; continue }; #hit_1
    if ( wb(-5,-3,"нужно пора") &&
          w(-2,cst) &&
          w(-1,"за") && s(wbn,-1) )
    { l[i]=omo1; Z[1137]++; if(dbg){print "Z1137"}; stopp=ret=1; continue }; #hit_1
    if ( bw(-3,"хотеть") &&
          w(-2,cst) &&
          w(-1,"за") && s(-3,-1) )
    { l[i]=omo1; Z[1138]++; if(dbg){print "Z1138"}; stopp=ret=1; continue }; #hit_1
    cst1="когда крепко наконец основательно плотно";
    if ( w(-3,cst1) &&
        bw(-2,cst) &&
         w(-1,"за") && s(-3,-1) )
    { l[i]=omo1; Z[1139]++; if(dbg){print "Z1139"}; stopp=ret=1; continue }; #hit_6
    ...
    if ( bba(tn-7,tn-1,"_gl_za_lock") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-1) && isname(0) )
       { bb_n=""; Z[1140]++; if(dbg){print "Z1140"}; }; #hit_2

       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo2; Z[1141]++; if(dbg){print "Z1141"}; stopp=ret=1; continue }; #hit_194
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
       ...
       { l[i]=omo2; Z[1142]++; if(dbg){print "Z1142"}; stopp=ret=1; continue }; #hit_21
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
            qir(bb_n+1,tn-2,"prl_tv mest_tv mest_3e prl_kred_sr nar_any") &&
              q(tn-1,"suw_tv mest_tv") )
       { l[i]=omo2; Z[1143]++; if(dbg){print "Z1143"}; stopp=ret=1; continue }; #hit_9
    ...
    if ( bba(tn-7,tn-1,"_gl_za_castle") ) { bb_n=bbn;
       ...
       if ( veq(bb_n,tn-1) && s(tn-1) )
       { l[i]=omo1; Z[1144]++; if(dbg){print "Z1144"}; stopp=ret=1; continue }; #hit_22
       if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
            qir(bb_n+1,tn-2,"prl_vi mest_vi mest_3e prq_vi prl_kred_sr nar_any qast_any isname") &&
              q(tn-1,"suw_vi mest_vi mest_3e prl_kred_sr nar_any qast isname") )
       { l[i]=omo1; Z[1145]++; if(dbg){print "Z1145"}; stopp=ret=1; continue }; #hit_5
    ...
    if ( qb(tn-7,tn-1,"pre_vi") ) { qb_n=qbn; if ( bba(qb_n-5,qb_n-1,"_gl_za_castle") ) { bb_n=bbn;
        ...
        { l[i]=omo1; Z[1146]++; if(dbg){print "Z1146"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[1147]++; if(dbg){print "Z1147"}; stopp=ret=1; continue }; #hit_6
    # _gl_za_lock --->>>:end

    # _gl_za_castle --->>>:start
    if ( bfa(1,5,"_gl_za_castle") && s(0,bfn-1) && Qf(1,bfn-1,"sz_iili pre_any") )
    { l[i]=omo1; Z[1148]++; if(dbg){print "Z1148"}; stopp=ret=1; continue }; #hit_12
    # _gl_za_castle --->>>:end

    # _suw_za_castle:start
    if ( ba(tn-1,"_suw_za_castle") && s(tn-1) )
    { l[i]=omo1; Z[1149]++; if(dbg){print "Z1149"}; stopp=ret=1; continue }; #hit_96
    # _suw_za_castle:end
   Y["tn_ok_za"]="";
   }; # tn_ok
...
function TN_pod_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_pod_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_pod"]) ) { tn=Y["n_pre_pod"];
    ...
    { l[i]=omo1; Z[1152]++; if(dbg){print "Z1152"}; stopp=ret=1; continue }; #hit_1
    if ( bw(tn-2,cst2) &&
         bw(tn-1,cst1) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[1153]++; if(dbg){print "Z1153"}; stopp=ret=1; continue }; #hit_5
    if ( bw(tn-2,cst1) &&
         bw(tn-1,cst2) && s(tn-2,tn-1) )
    { l[i]=omo1; Z[1154]++; if(dbg){print "Z1154"}; stopp=ret=1; continue }; #hit_1
    if ( bw(tn-1,cst1) &&
         bf(1,3,cst2) && s(tn-1,bfn-1) )
    { l[i]=omo1; Z[1155]++; if(dbg){print "Z1155"}; stopp=ret=1; continue }; #hit_3

    cst3="выбор поиск";
    if ( bw(tn-3,cst3) &&
          q(tn-2,"prl_ro") &&
         bw(tn-1,cst1) && s(tn-3,tn-1) )
    { l[i]=omo1; Z[1156]++; if(dbg){print "Z1156"}; stopp=ret=1; continue }; #hit_1

    cst3="уверенный";
    if ( bw(tn-3,cst3) &&
          w(tn-2,"насчет о") &&
         bw(tn-1,cst1) && s(tn-3,tn-1) )
    { l[i]=omo1; Z[1157]++; if(dbg){print "Z1157"}; stopp=ret=1; continue }; #hit_1

    cst4="отправить отправиться отправлять отправляться сложить";
    if ( bw(tn-3,cst4) &&
          w(tn-2,"на") &&
         bw(tn-1,cst1) && s(tn-3,tn-1) )
    { l[i]=omo2; Z[1158]++; if(dbg){print "Z1158"}; stopp=ret=1; continue }; #hit_1
    if ( bw(tn-1,cst4) &&
          w(1,"в") &&
          q(2,"prl_vi prq_vi mest_vi mest_3e") &&
         bw(3,cst1) && s(tn-1,2) )
    { l[i]=omo2; Z[1159]++; if(dbg){print "Z1159"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[1160]++; if(dbg){print "Z1160"}; stopp=ret=1; continue }; #hit_1

    cst="вести вестись";
    cst1="подкоп";
    if ( bw(1,cst) && w(2,cst1) && s(0,1) )
    { l[i]=omo1; Z[1151]++; if(dbg){print "Z1151"}; stopp=ret=1; continue }; #hit_4

    # escape для место:end
    cst="земля подкоп территория участок";
    if ( bw(tn-1,cst) && s(tn-1) )
    { l[i]=omo1; Z[1150]++; if(dbg){print "Z1150"}; stopp=ret=1; continue }; #hit_14

    # _gl_i_pod_lock:start
    if ( ba(tn-2,"_gl_i_pod_lock") && zs(tn-2) && s(tn-1) && w(tn-1,"и") )
    { l[i]=omo2; Z[1161]++; if(dbg){print "Z1161"}; stopp=ret=1; continue }; #hit_11
    if ( ba(tn-3,"_gl_i_pod_lock") && s(tn-3,tn-1) &&
          q(tn-2,"mest_vi mest_3e suw_vi isname") &&
          w(tn-1,"и") )
    { l[i]=omo2; Z[1162]++; if(dbg){print "Z1162"}; stopp=ret=1; continue }; #hit_2
    # _gl_i_pod_lock:start
    ...
    { l[i]=omo2; Z[1167]++; if(dbg){print "Z1167"}; stopp=ret=1; continue }; #hit_1
    if ( qxs(-1,"под","самый") )
    { l[i]=omo1; Z[1168]++; if(dbg){print "Z1168"}; stopp=ret=1; continue }; #hit_5
    ...
    if ( vex(Y["n-_gl_pod_lock"]) ) { bb_n=Y["n-_gl_pod_lock"];
        ...
        { l[i]=omo2; Z[1169]++; if(dbg){print "Z1169"}; stopp=ret=1; continue }; #hit_3384
        if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
        ...
        { l[i]=omo2; Z[1170]++; if(dbg){print "Z1170"}; stopp=ret=1; continue }; #hit_2
        if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
             ...
             qir(bb_n+2,tn-2,"suw_ro suw_edne prl_ro prq_ro mest_ro mest_3e qik_ro qis_ro qast prl_kred_sr isname nar_any") &&
               q(tn-1,"isname suw_ro") )
        { l[i]=omo2; Z[1171]++; if(dbg){print "Z1171"}; stopp=ret=1; continue }; #hit_1
        if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
             qir(bb_n+1,tn-1,"suw_tv suw_edne prl_tv prq_tv mest_tv mest_3e qik_tv qis_tv qast prl_kred_sr isname nar_any") )
        { l[i]=omo2; Z[1172]++; if(dbg){print "Z1172"}; stopp=ret=1; continue }; #hit_7
        if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
        ...
        { l[i]=omo2; Z[1173]++; if(dbg){print "Z1173"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( qb(tn-5,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1174]++; if(dbg){print "Z1174"}; stopp=ret=1; continue }; #hit_116
       if ( vgl(qb_n,tn-5,tn-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,tn-2) && zs(tn-1) &&
       ...
       { l[i]=omo2; Z[1175]++; if(dbg){print "Z1175"}; stopp=ret=1; continue }; #hit_20
       if ( vgl(qb_n,tn-5,tn-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,tn-2) && zs(tn-1) &&
            ...
            qiz(qb_n+1,tn-1,"suw_vi prl_vi mest_vi mest_3e qik_vi qast","suw_ro suw_edne isname") )
       { l[i]=omo2; Z[1176]++; if(dbg){print "Z1176"}; stopp=ret=1; continue }; #hit_2
    ...
    if ( qb(tn-5,tn-2,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1177]++; if(dbg){print "Z1177"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( qb(tn-5,tn-2,"pre_tv") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1178]++; if(dbg){print "Z1178"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( qb(tn-5,tn-2,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1179]++; if(dbg){print "Z1179"}; stopp=ret=1; continue }; #hit_16

    };};

    if ( bfa(1,5,"_gl_pod_lock") && s(0,bfn-1) &&
          Qf(1,bfn-1,"pre_any sz_iili") )
    { l[i]=omo2; Z[1180]++; if(dbg){print "Z1180"}; stopp=ret=1; continue }; #hit_370
    ...
    if ( qf(1,3,"pre_ro") ) { qf_n=qfn;
      if ( bfa(qf_n+1,qf_n+5,"_gl_pod_lock") ) { bf_n=bfn;
       ...
       { l[i]=omo2; Z[1181]++; if(dbg){print "Z1181"}; stopp=ret=1; continue }; #hit_2

    };};
    if ( q(1,"narph_any") &&
       bfa(xwn+1,xwn+3,"_gl_pod_lock") && s(0,bfn-1) &&
       qir(xwn+1,bfn-1,"qast_any") )
    { l[i]=omo2; Z[1182]++; if(dbg){print "Z1182"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( vex(Y["n-_gl_pod_castle"]) ) { bb_n=Y["n-_gl_pod_castle"];
        ...
        if ( vgl(bb_n,tn-7,tn-1) && s(bb_n,tn-2) && zs(tn-1) &&
             qir(bb_n+1,tn-1,"suw_vi suw_edne prl_vi prq_vi mest_vi mest_impe mest_3e qik_vi qis_vi qast prl_kred_sr isname nar_any") )
        { l[i]=omo1; Z[1183]++; if(dbg){print "Z1183"}; stopp=ret=1; continue }; #hit_70
    ...
    if ( qb(tn-5,tn-1,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_castle") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,tn-5,tn-1) && vgl(bb_n,qbn-5,qbn-1) && s(bb_n,tn-2) && zs(tn-1) &&
            qiz(bb_n+1,qb_n-1,"prl_vi prl_ro mest_vi mest_3e qast prl_kred_sr","suw_vi suw_edne isname") &&
            qiz(qb_n+1,tn-1,"prl_vi mest_vi prq_vi mest_3e qast","suw_vi suw_edne isname") )
       { l[i]=omo1; Z[1184]++; if(dbg){print "Z1184"}; stopp=ret=1; continue }; #hit_2
...
#           qiz(bb_n+1,qb_n-1,"prl_vi prl_ro mest_vi mest_3e qast prl_kred_sr","suw_vi suw_edne isname") &&
#           qiz(qb_n+1,tn-1,"prl_ro mest_ro mest_3e qast","suw_ro suw_edne isname") )
#      { l[i]=omo1; Z[1185]++; if(dbg){print "Z1185"}; stopp=ret=1; continue }; #mute
       ...
       { l[i]=omo1; Z[1186]++; if(dbg){print "Z1186"}; stopp=ret=1; continue }; #hit_4
    ...
    if ( qb(tn-5,tn-1,"pre_tv") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_castle") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,tn-5,tn-1) && vgl(bb_n,qbn-5,qbn-1) && s(bb_n,tn-2) && zs(tn-1) &&
            qiz(bb_n+1,qb_n-1,"prl_vi prl_ro mest_vi mest_3e qast prl_kred_sr","suw_vi suw_edne isname") &&
            qiz(qb_n+1,tn-1,"prl_tv mest_tv mest_3e qast","suw_tv suw_edne isname") )
       { l[i]=omo1; Z[1187]++; if(dbg){print "Z1187"}; stopp=ret=1; continue }; #hit_2

    };};

    if ( bfa(1,5,"_gl_pod_castle") && s(0,bfn-1) &&
          Qf(1,bfn-1,"pre_any sz_iili") )
    { l[i]=omo1; Z[1188]++; if(dbg){print "Z1188"}; stopp=ret=1; continue }; #hit_4
    # _gl_pod_castle:end
    ...
    if ( bba(tn-4,tn-1,"_suw_pod_lock") ) { bb_n=bbn;
       if ( vgl(bb_n,tn-4,tn-1) && s(bb_n,-1) &&
            qir(bb_n+1,tn-1,"suw_vi prl_vi mest_vi mest_3e qast") )
       { l[i]=omo2; Z[1189]++; if(dbg){print "Z1189"}; stopp=ret=1; continue }; #hit_38
    };
    if ( bfa(1,3,"_suw_pod_lock") && s(0,bfn-1) )
    { l[i]=omo2; Z[1190]++; if(dbg){print "Z1190"}; stopp=ret=1; continue }; #hit_2
    # _suw_pod_lock:end
    ...
    if ( bba(tn-4,tn-1,"_v_suw-vi_pod_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1193]++; if(dbg){print "Z1193"}; stopp=ret=1; continue }; #hit_7
    };
    if ( w(1,"в во") && bfa(2,4,"_v_suw-vi_pod_lock") && s(0,bfn-1) &&
       qir(2,bfn-1,"prl_vi mest_vi mest_3e") )
    { l[i]=omo2; Z[1194]++; if(dbg){print "Z1194"}; stopp=ret=1; continue }; #hit_3
    # _suw_pod_lock:end

    if ( qxs(1,"на","хранение") )
    { l[i]=omo2; Z[1195]++; if(dbg){print "Z1195"}; stopp=ret=1; continue }; #hit_1

    if ( w(tn-2,"в во") &&
         q(tn-1,"suw_vi") && s(tn-2,tn-1) )
    { l[i]=omo2; Z[1196]++; if(dbg){print "Z1196"}; stopp=ret=1; continue }; #hit_22
    if ( w(tn-3,"в во") &&
         q(tn-2,"suw_vi") &&
         w(tn-1,"не") && s(tn-3,tn-1) )
    { l[i]=omo2; Z[1197]++; if(dbg){print "Z1197"}; stopp=ret=1; continue }; #hit_1
    if ( w(1,"в во") && s(tn-1) &&
         q(2,"suw_vi") && s(0,1) )
    { l[i]=omo2; Z[1198]++; if(dbg){print "Z1198"}; stopp=ret=1; continue }; #hit_5

    if ( q(1,"prl_vi suw_vi mest_vi mest_3e") && p(tn-1) &&
         q(2,"mest_vi suw_vi") && s(1) && p(2) )
    { l[i]=omo2; Z[1199]++; if(dbg){print "Z1199"}; stopp=ret=1; continue }; #hit_4
    if ( q(tn-2,"mest_vi prl_vi mest_3e") && p(tn-3) &&
         q(tn-1,"suw_vi") && s(tn-2,tn-1) && p(0) )
    { l[i]=omo2; Z[1200]++; if(dbg){print "Z1200"}; stopp=ret=1; continue }; #hit_1
    if ( q(tn-1,"mest_vi suw_vi") && p(tn-2) && s(tn-1) && p(0) )
    { l[i]=omo2; Z[1201]++; if(dbg){print "Z1201"}; stopp=ret=1; continue }; #hit_29
    if ( pre_vi(tn-4) && p(tn-5) &&
              q(tn-3,"suw_vi") &&
              q(tn-2,"mest_vi mest_3e") &&
        sz_iili(tn-1) && s(tn-4,tn-1) )
    { l[i]=omo2; Z[1202]++; if(dbg){print "Z1202"}; stopp=ret=1; continue }; #hit_3

    if ( sc(tn-1,"—") && (p(0)||q_w(1,"sz_iili pre_any")) && Qf(tn+1,-1,"qip_vi") )
    { l[i]=omo2; Z[1203]++; if(dbg){print "Z1203"}; stopp=ret=1; continue }; #hit_75
    if ( q(1,"mest_vi mest_3e suw_odvi qis_vi isname") && s(0) && (p(1)||q_w(2,"sz_iili pre_any")) &&
      !veq(Y["n+_castle_pe"],2)  )
    { l[i]=omo2; Z[1204]++; if(dbg){print "Z1204"}; stopp=ret=1; continue }; #hit_29
    if ( q(tn-1,"mest_vi mest_3e suw_odvi qis_vi sz nar_vrem qast_any") &&
         s(tn-1) && p(tn-2) && (p(0)||q_w(1,"sz_iili pre_any")) )
    { l[i]=omo2; Z[1205]++; if(dbg){print "Z1205"}; stopp=ret=1; continue }; #hit_41
    if ( q(tn-2,"nar_vrem") &&
         q(tn-1,"mest_vi mest_3e suw_odvi qis_vi sz") &&
         s(tn-2,tn-1) && p(tn-3) && (p(0)||q_w(1,"sz_iili pre_any")) )
    { l[i]=omo2; Z[1206]++; if(dbg){print "Z1206"}; stopp=ret=1; continue }; #hit_2
    if ( pre_vi(tn-2) && s(tn-2) &&
              q(tn-1,"suw_vi isname") && z(tn-1) && (p(0)||q_w(1,"sz_iili pre_any")) )
    { l[i]=omo2; Z[1207]++; if(dbg){print "Z1207"}; stopp=ret=1; continue }; #hit_31
    if ( pre_vi(tn-3) && s(tn-3,tn-2) &&
              q(tn-2,"prl_vi mest_vi mest_3e qast") &&
              q(tn-1,"suw_vi") && z(tn-1) && (p(0)||q_w(1,"sz_iili pre_any")) )
    { l[i]=omo2; Z[1208]++; if(dbg){print "Z1208"}; stopp=ret=1; continue }; #hit_5
    if ( pre_vi(tn-3) &&
              q(tn-2,"suw_vi") &&
        sz_iili(tn-1) && s(tn-3,tn-1) )
    { l[i]=omo2; Z[1209]++; if(dbg){print "Z1209"}; stopp=ret=1; continue }; #hit_8
   Y["tn_ok_pod"]="";
   } # tn_ok <<== end
...
function TN_pod_zamkom_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_pod_zamkom_f"]);
  ...
  if ( vex(Y["tn_ok_pod_tv"]) ) { tn=Y["n_pre_pod_tv"];
      ...
      { l[i]=omo1; Z[1210]++; if(dbg){print "Z1210"}; stopp=ret=1; continue }; #hit_11
      ...
      if ( bba(tn-5,tn-1,"_gl_pod_lock-tv") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[1211]++; if(dbg){print "Z1211"}; stopp=ret=1; continue }; #hit_1797
...
function TN_o_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_o_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_o"]) ) { tn=Y["n_pre_o"];
    ...
    { l[i]=omo1; Z[1212]++; if(dbg){print "Z1212"}; stopp=ret=1; continue }; #hit_3
    cst="разбиваться удариться";
    if ( (qxs(tn-2,"ветер")||
          qxs(tn-2,"луч","света") ) &&
           bw(tn-1,cst) && s(tn-1) )
    { l[i]=omo1; Z[1213]++; if(dbg){print "Z1213"}; stopp=ret=1; continue }; #hit_2
    if ( qxs(tn-1,"что","это","бьется") )
    { l[i]=omo1; Z[1214]++; if(dbg){print "Z1214"}; stopp=ret=1; continue }; #hit_1

    if ( bba(tn-5,tn-1,"_gl_o_lock") ) { bb_n=bbn;

       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_vi mest_vi mest_3e","suw_vi suw_edne") )
       { l[i]=omo2; Z[1215]++; if(dbg){print "Z1215"}; stopp=ret=1; continue }; #hit_18
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_tv mest_tv mest_3e","suw_tv suw_edne") )
       { l[i]=omo2; Z[1216]++; if(dbg){print "Z1216"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( bba(tn-5,tn-1,"_suw_o_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1217]++; if(dbg){print "Z1217"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( bfa(1,5,"_gl_o_lock") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo2; Z[1218]++; if(dbg){print "Z1218"}; stopp=ret=1; continue }; #hit_1
   ...
   sz_iili(2) &&
    ...
    { l[i]=omo2; Z[1219]++; if(dbg){print "Z1219"}; stopp=ret=1; continue }; #hit_1
   Y["tn_ok_o"]="";
   }; # tn_ok <<== end
...
function TN_po_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_po_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_po"]) ) { tn=Y["n_pre_po"];
    ...
    if ( bba(tn-5,tn-1,"_gl_po_lock") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1220]++; if(dbg){print "Z1220"}; stopp=ret=1; continue }; #hit_1
...
function TN_qerez_zamok_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_qerez_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_qerez"]) ) { tn=Y["n_pre_qerez"];
    ...
    if ( bba(tn-5,tn-1,"_suw_qerez_castle") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1221]++; if(dbg){print "Z1221"}; stopp=ret=1; continue }; #hit_15
    ...
    if ( bba(tn-5,tn-1,"_gl_qerez_castle") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_vi mest_vi mest_3e","suw_vi suw_edne") )
       { l[i]=omo1; Z[1222]++; if(dbg){print "Z1222"}; stopp=ret=1; continue }; #hit_4
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_tv mest_tv mest_3e","suw_tv suw_edne") )
       { l[i]=omo1; Z[1223]++; if(dbg){print "Z1223"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( bfa(1,5,"_gl_qerez_castle") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1224]++; if(dbg){print "Z1224"}; stopp=ret=1; continue }; #hit_1

    };
    if ( q(1,"narph_any") &&
       bfa(xwn+1,xwn+3,"_gl_qerez_castle") && s(0,bfn-1) &&
       qir(xwn+1,bfn-1,"qast_any nar_any") )
    { l[i]=omo1; Z[1225]++; if(dbg){print "Z1225"}; stopp=ret=1; continue }; #hit_1
   Y["tn_ok_qerez"]="";
   }; # tn_ok <<== end
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1226]++; if(dbg){print "Z1226", "-- tn_ok ON", el, "замка"};}; #hit_39462
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1232]++; if(dbg){print "Z1232", "-- tn_ok ON", el, "замка"};}; #hit_3
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1233]++; if(dbg){print "Z1233", "-- tn_ok ON", el, "замка"};}; #hit_8
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1234]++; if(dbg){print "Z1234", "-- tn_ok ON", el, "замка"};}; #hit_167
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1235]++; if(dbg){print "Z1235", "-- tn_ok ON", el, "замка"};}; #hit_20
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1236]++; if(dbg){print "Z1236", "-- tn_ok ON", el, "замка"};}; #hit_4
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1237]++; if(dbg){print "Z1237", "-- tn_ok MOV", el, "замка"};}; #hit_193
}
...
function TN_do_zamka_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_do_zamka_f"]);
  ...
  if ( vex(Y["tn_ok_do"]) ) { tn=Y["n_pre_do"];
    ...
    if ( bba(tn-5,tn-1,"_suw_do_castle") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1240]++; if(dbg){print "Z1240"}; stopp=ret=1; continue }; #hit_374
    ...
    if ( bba(tn-5,tn-1,"_gl_do_castle-ro") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1241]++; if(dbg){print "Z1241"}; stopp=ret=1; continue }; #hit_2219
    ...
    if ( bfa(1,5,"_gl_do_castle-ro") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1243]++; if(dbg){print "Z1243"}; stopp=ret=1; continue }; #hit_385
...
function TN_iz_zamka_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_iz_zamka_f"]);
  ...
  if ( vex(Y["tn_ok_iz"]) ) { tn=Y["n_pre_iz"];
    ...
    if ( bba(tn-5,tn-1,"_suw_iz_castle-ro") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1244]++; if(dbg){print "Z1244"}; stopp=ret=1; continue }; #hit_414
    ...
    if ( bba(tn-5,tn-1,"_suw_iz_lock-ro") ) { bb_n=bbn;
       ...
       { l[i]=omo2; Z[1245]++; if(dbg){print "Z1245"}; stopp=ret=1; continue }; #hit_659
    ...
    if ( bba(tn-5,tn-1,"_gl_iz_castle-ro") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1246]++; if(dbg){print "Z1246"}; stopp=ret=1; continue }; #hit_4210
#      if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
#           qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
#      { l[i]=omo1; Z[1247]++; if(dbg){print "Z1247"}; stopp=ret=1; continue }; #mute
    ...
    if ( bfa(1,5,"_gl_iz_castle-ro") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1248]++; if(dbg){print "Z1248"}; stopp=ret=1; continue }; #hit_712
...
function TN_ot_zamka_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_ot_zamka_f"]);
  ...
  if ( vex(Y["tn_ok_ot"]) ) { tn=Y["n_pre_ot"];
    ...
    if ( bba(tn-5,tn-1,"_suw_ot_castle-ro") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1249]++; if(dbg){print "Z1249"}; stopp=ret=1; continue }; #hit_197

    };
    if ( bba(tn-5,tn-1,"_suw_ot_lock-ro") ) { bb_n=bbn;

       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne qast_any") )
       { l[i]=omo2; Z[1250]++; if(dbg){print "Z1250"}; stopp=ret=1; continue }; #hit_1131
    ...
    if ( bba(tn-5,tn-1,"_gl_ot_castle-ro") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
            qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
       { l[i]=omo1; Z[1251]++; if(dbg){print "Z1251"}; stopp=ret=1; continue }; #hit_142
#      if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
#           qiz(bb_n+1,tn-1,"prl_ro mest_ro mest_3e","suw_ro suw_edne") )
#      { l[i]=omo1; Z[1252]++; if(dbg){print "Z1252"}; stopp=ret=1; continue }; #mute
    ...
    if ( bfa(1,5,"_gl_ot_castle-ro") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1253]++; if(dbg){print "Z1253"}; stopp=ret=1; continue }; #hit_54
...
function TN_v_zamke_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_v_zamke_f"]);
  ...
  if ( vex(Y["tn_ok_v_pr"]) ) { tn=Y["n_pre_v_pr"];
    ...
    if ( bba(tn-5,tn-1,"_gl_v_castle-pr") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1254]++; if(dbg){print "Z1254"}; stopp=ret=1; continue }; #hit_3983
#      if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
#           qiz(bb_n+1,tn-1,"prl_pr mest_pr mest_3e","suw_ro suw_edne") )
#      { l[i]=omo1; Z[1255]++; if(dbg){print "Z1255"}; stopp=ret=1; continue }; #mute
    ...
    if ( bfa(1,5,"_gl_v_castle-pr") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1256]++; if(dbg){print "Z1256"}; stopp=ret=1; continue }; #hit_1144
    ...
    #
    if ( ba(tn-1,"_suw_v_castle-vp") && s(tn-1) )
    { l[i]=omo1; Z[1257]++; if(dbg){print "Z1257"}; stopp=ret=1; continue }; #hit_1173
    if ( ba(tn-2,"_suw_v_castle-vp") && s(tn-2,tn-1) &&
          q(tn-1,"prl_kred_sr nar_any mest_vi mest_3e prl_vi") )
    { l[i]=omo1; Z[1258]++; if(dbg){print "Z1258"}; stopp=ret=1; continue }; #hit_28
    if ( bba(tn-6,tn-2,"_suw_v_castle-vp") &&
         qir(bbn+1,tn-2,"prl_ro prq_ro mcop_ro suw_ro") &&
           q(tn-1,"suw_ro mest_ro mest_3e qast nar_napr isname") && s(bbn,tn-1) )
    { l[i]=omo1; Z[1259]++; if(dbg){print "Z1259"}; stopp=ret=1; continue }; #hit_152
    # _suw_v_castle-vp:end
    # в замке
...
function TN_k_zamku_f(    ret, tn, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["TN_k_zamku_f"]);
  ...
  if ( vex(Y["tn_ok_k_tv"]) ) { tn=Y["n_pre_k_tv"];
    ...
    if ( bba(tn-5,tn-1,"_gl_k_castle-da") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[1260]++; if(dbg){print "Z1260"}; stopp=ret=1; continue }; #hit_742
#      if ( vgl(bb_n,tn-5,tn-1) && s(bb_n,-1) &&
#           qiz(bb_n+1,tn-1,"prl_da mest_da mest_3e","suw_ro suw_edne") )
#      { l[i]=omo1; Z[1261]++; if(dbg){print "Z1261"}; stopp=ret=1; continue }; #mute
    ...
    if ( bfa(1,5,"_gl_k_castle-da") ) { bf_n=bfn;
       ...
       if ( vgl(bf_n,1,5) && s(1,bf_n-1) &&
             Qf(1,bf_n-1,"pre_any sz_iili") )
       { l[i]=omo1; Z[1262]++; if(dbg){print "Z1262"}; stopp=ret=1; continue }; #hit_14
    ...
    #
#   if ( ba(tn-1,"_suw_v_castle-vp") && s(tn-1) )
#   { l[i]=omo1; Z[1263]++; if(dbg){print "Z1263"}; stopp=ret=1; continue }; #mute
#   if ( ba(tn-2,"_suw_v_castle-vp") && s(tn-2,tn-1) &&
#         q(tn-1,"prl_kred_sr nar_any mest_vi mest_3e prl_vi") )
#   { l[i]=omo1; Z[1264]++; if(dbg){print "Z1264"}; stopp=ret=1; continue }; #mute
#   if ( bba(tn-6,tn-2,"_suw_v_castle-vp") &&
#        qir(bbn+1,tn-2,"prl_ro prq_ro mcop_ro suw_ro") &&
#          q(tn-1,"suw_ro mest_ro mest_3e qast nar_napr isname") && s(bbn,tn-1) )
#   { l[i]=omo1; Z[1265]++; if(dbg){print "Z1265"}; stopp=ret=1; continue }; #mute
    # _suw_v_castle-vp:end
    # в замке
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1266]++; if(dbg){print "Z1266"}; stopp=ret=1; continue }; #mute
    if ( iwrd == "замок") {
       ...
       { l[i]=omo1; Z[1267]++; if(dbg){print "Z1267"}; stopp=ret=1; continue }; #hit_11943
       if ( q(-2,"prl_edmuim prl_mnim prl_kred_mu") && ba(-2,str_castle) &&
            q(-1,"prl_any prq_any mest_any qast") && !(bam(-1,"_lock_prl _lock_pe")) && s(-2,-1) )
       { l[i]=omo1; Z[1268]++; if(dbg){print "Z1268"}; stopp=ret=1; continue }; #hit_360
       ...
       { l[i]=omo1; Z[1269]++; if(dbg){print "Z1269"}; stopp=ret=1; continue }; #hit_40
       ...
       { l[i]=omo1; Z[1270]++; if(dbg){print "Z1270"}; stopp=ret=1; continue }; #hit_16

       if ( q(1,"prl_edmuim prl_kred_mu") && w(0,"замок") && ba(1,str_castle) && s(0) &&
          Q_w(2,"suw_vi suw_im suw_edne") )
       { l[i]=omo1; Z[1271]++; if(dbg){print "Z1271"}; stopp=ret=1; continue }; #hit_454
       if ( q(1,"nar_any") &&
            q(2,"prl_edmuim prl_kred_mu") && w(0,"замок") && ba(2,str_castle) && s(0,-1) &&
          Q_w(3,"suw_vi suw_im suw_edne") )
       { l[i]=omo1; Z[1272]++; if(dbg){print "Z1272"}; stopp=ret=1; continue }; #hit_132

       cst="массивный огромный";
       if ( w(-2,cst) && zs(-2) &&
            w(-1,"как") && s(-1) )
       { l[i]=omo1; Z[1273]++; if(dbg){print "Z1273"}; stopp=ret=1; continue }; #hit_6
          ...
          if ( bf(1,4,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
                if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                     ...
                     vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1274]++; if(dbg){print "Z1274"}; }; #hit_145
                if ( s(bf_n,bf_n+1) && sz_iili(bf_n+1) && ba(bf_n+2,"_lock_prl") )
                { wf_n=bf_n=""; Z[1275]++; if(dbg){print "Z1275"}; }; #hit_1

                cst2="ключ отмычка";
                if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+5) && s(0,bf_n-1) && z(bf_n) &&
                      bf(bf_n+1,bf_n+5,cst2) )
                { l[i]=omo2; Z[1276]++; if(dbg){print "Z1276"}; stopp=ret=1; continue }; #hit_1
                ...
                { l[i]=omo1; Z[1277]++; if(dbg){print "Z1277"}; stopp=ret=1; continue }; #hit_640
                if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
                       q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
                { l[i]=omo1; Z[1278]++; if(dbg){print "Z1278"}; stopp=ret=1; continue }; #hit_5
                ...
                if ( vgl(wf_n,1,4) && vgl(bf_n,wf_n+1,wf_n+5) &&
                     ...
                     qir(wf_n+1,bf_n-1,"nar_any prq_kred_mu qast_any prl_edmuim prl_mnim prl_kred_sr predik prl_edmutv mest_im mest_tv mest_3e mest_da isname suw_da") )
                { l[i]=omo1; Z[1279]++; if(dbg){print "Z1279"}; stopp=ret=1; continue }; #hit_7
#               if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
#                      q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
#               { l[i]=omo1; Z[1280]++; if(dbg){print "Z1280"}; stopp=ret=1; continue }; #mute
          };};
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bba(wf_n+1,wf_n+5,str_castle) ) { bb_n=bbn;
                ...
                { l[i]=omo1; Z[1281]++; if(dbg){print "Z1281"}; stopp=ret=1; continue }; #hit_1
          };};
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
               if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                    bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                    vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1282]++; if(dbg){print "Z1282"}; }; #hit_60
                ...
                { l[i]=omo1; Z[1283]++; if(dbg){print "Z1283"}; stopp=ret=1; continue }; #hit_11
#               if ( veq(wf_n,1) && veq(bf_n,3) && s(0) &&
#                      q(2,"prl_edmuim prl_mnim prl_tv prl_kred_mu") && z(2) )
#               { l[i]=omo1; Z[1284]++; if(dbg){print "Z1284"}; stopp=ret=1; continue }; #mute
          };};

       cst2="быть делать казаться сделать стать считаться являться";
       if ( bb(-3,-1,cst2) &&
             q(1,"prl_tv") && ba(1,str_castle) && s(bbn,0) )
       { l[i]=omo1; Z[1285]++; if(dbg){print "Z1285"}; stopp=ret=1; continue }; #hit_20
    };
    if ( iwrd == "замка") {
       if ( q(-1,"prl_edmuro") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1286]++; if(dbg){print "Z1286"}; stopp=ret=1; continue }; #hit_3572
       if ( s(-2,-1) && prl_edmuro(-2) && q(-1,"prl_edmuro") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1287]++; if(dbg){print "Z1287"}; stopp=ret=1; continue }; #hit_93
    };
    if ( iwrd == "замку") {
       if ( q(-1,"prl_edmuda") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1288]++; if(dbg){print "Z1288"}; stopp=ret=1; continue }; #hit_1162
       if ( s(-2,-1) && prl_edmuda(-2) && q(-1,"prl_edmuda") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1289]++; if(dbg){print "Z1289"}; stopp=ret=1; continue }; #hit_31
#      if ( q(1,"prl_edmuda") && (s(0)||se(0," — ")||sc(0,":")) && Q_w(2,"suw_edda") )
#      { l[i]=omo1; Z[1290]++; if(dbg){print "Z1290"}; stopp=ret=1; continue }; #mute
    };
    if ( iwrd == "замком") {
       if ( q(-1,"prl_edmutv") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1291]++; if(dbg){print "Z1291"}; stopp=ret=1; continue }; #hit_1361
       if ( s(-2,-1) && prl_edmutv(-2) && q(-1,"prl_edmutv") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1292]++; if(dbg){print "Z1292"}; stopp=ret=1; continue }; #hit_47
    };
    if ( iwrd == "замке") {
       if ( q(-1,"prl_edmupr") && ba(-1,str_castle) && s(-1) &&
         !(ba(-2,"_lock_pe") && s(-2)) )
       { l[i]=omo1; Z[1293]++; if(dbg){print "Z1293"}; stopp=ret=1; continue }; #hit_6914
       if ( s(-2,-1) && prl_edmupr(-2) && q(-1,"prl_edmupr") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1294]++; if(dbg){print "Z1294"}; stopp=ret=1; continue }; #hit_127
    };
    if ( iwrd == "замки") {
       if ( s(-1) && prl_mnim(-1) && ba(-1,str_castle) )
       { l[i]=omo1; Z[1295]++; if(dbg){print "Z1295"}; stopp=ret=1; continue }; #hit_2236
       if ( s(-2,-1) && prl_mnim(-2) && q(-1,"prl_mnim") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1296]++; if(dbg){print "Z1296"}; stopp=ret=1; continue }; #hit_67
       if ( q(1,"prl_mnim prl_mntv qast_any") && q(2,"prl_mnim prl_mntv") &&
           ba(2,str_castle) && (zs(0)||se(0," — ")) && zs(1) )
       { l[i]=omo1; Z[1297]++; if(dbg){print "Z1297"}; stopp=ret=1; continue }; #hit_18
       ...
       { l[i]=omo1; Z[1298]++; if(dbg){print "Z1298"}; stopp=ret=1; continue }; #hit_60
    };
    if ( iwrd == "замков") {
       if ( s(-1) && q(-1,"prl_mnro") && ba(-1,str_castle) )
       { l[i]=omo1; Z[1299]++; if(dbg){print "Z1299"}; stopp=ret=1; continue }; #hit_1434
       if ( s(-2,-1) && prl_mnro(-2) && q(-1,"prl_mnro") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1300]++; if(dbg){print "Z1300"}; stopp=ret=1; continue }; #hit_38
    };
    if ( iwrd == "замкам") {
       if ( s(-1) && q(-1,"prl_mnda") && ba(-1,str_castle) )
       { l[i]=omo1; Z[1301]++; if(dbg){print "Z1301"}; stopp=ret=1; continue }; #hit_160
       if ( s(-2,-1) && prl_mnda(-2) && q(-1,"prl_mnda") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1302]++; if(dbg){print "Z1302"}; stopp=ret=1; continue }; #hit_3
    };
    if ( iwrd == "замками") {
       if ( s(-1) && q(-1,"prl_mntv") && ba(-1,str_castle) )
       { l[i]=omo1; Z[1303]++; if(dbg){print "Z1303"}; stopp=ret=1; continue }; #hit_369
       if ( s(-2,-1) && prl_mntv(-2) && q(-1,"prl_mntv") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1304]++; if(dbg){print "Z1304"}; stopp=ret=1; continue }; #hit_16
    };
    if ( iwrd == "замках") {
       if ( s(-1) && q(-1,"prl_mnpr") && ba(-1,str_castle) )
       { l[i]=omo1; Z[1305]++; if(dbg){print "Z1305"}; stopp=ret=1; continue }; #hit_763
       if ( s(-2,-1) && prl_mnpr(-2) && q(-1,"prl_mnpr") && ba(-2,str_castle) )
       { l[i]=omo1; Z[1306]++; if(dbg){print "Z1306"}; stopp=ret=1; continue }; #hit_19
    };
    # _castle_prl:end
...
function FN_lock_prl_f(str_lock,    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp_str="FN" str_lock "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1307]++; if(dbg){print "Z1307", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue }; #hit_9484

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1308]++; if(dbg){print "Z1308", "-- TN_v_zamok_f"}; stopp=ret=1; continue }; #hit_4878
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1309]++; if(dbg){print "Z1309", "-- TN_v_zamok_f"}; stopp=ret=1; continue }; #hit_6537
    # _lock_prl:escape
    ...
    # _lock_prl:start
       ...
       if ( iwrd == "замок") {
          ...
          { l[i]=omo2; Z[1310]++; if(dbg){print "Z1310"}; stopp=ret=1; continue }; #hit_12926
          ...
          if ( q(1,"prl_edmuim prl_edmutv") &&
             ...
             (zs(0)||se(0," — ")) && zs(2) )
          { l[i]=omo2; Z[1314]++; if(dbg){print "Z1314"}; stopp=ret=1; continue }; #hit_4
          if ( qm(1,"prl_edmuim",str_lock) && (s(0)||se(0," — ")||sc(0,":")) && Q_w(2,"suw_edvi suw_edim suw_edtv") )
          { l[i]=omo2; Z[1315]++; if(dbg){print "Z1315"}; stopp=ret=1; continue }; #hit_788
          ...
          { l[i]=omo2; Z[1316]++; if(dbg){print "Z1316"}; stopp=ret=1; continue }; #hit_281
          ...
          { l[i]=omo2; Z[1321]++; if(dbg){print "Z1321"}; stopp=ret=1; continue }; #hit_17
          ...
          { l[i]=omo2; Z[1322]++; if(dbg){print "Z1322"}; stopp=ret=1; continue }; #hit_17
          ...
          if ( bf(1,3,cst) ) { wf_n=bfn;
              ...
              { l[i]=omo2; Z[1323]++; if(dbg){print "Z1323"}; stopp=ret=1; continue }; #hit_59
              ...
              { l[i]=omo2; Z[1324]++; if(dbg){print "Z1324"}; stopp=ret=1; continue }; #hit_1
              if ( qf(wf_n+1,wf_n+3,"prl_edmuim prl_kred_mu") && ba(qfn,str_lock) && s(0,qfn-1) && Q_w(qfn+1,"suw_edvi suw_edim") &&
                  qir(1,wf_n-1,"nar_any qast_any") && qir(wf_n+1,qfn-1,"nar_any qast_any") )
              { l[i]=omo2; Z[1325]++; if(dbg){print "Z1325"}; stopp=ret=1; continue }; #hit_12
              ...
              { l[i]=omo2; Z[1326]++; if(dbg){print "Z1326"}; stopp=ret=1; continue }; #hit_1
          };
...
#             { l[i]=omo2; Z[1327]++; if(dbg){print "Z1327"}; stopp=ret=1; continue }; #mute
#         };
       }; #замок
       if ( iwrd == "замка") {
          if ( s(-1) && q(-1,"prl_edmuro") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1328]++; if(dbg){print "Z1328"}; stopp=ret=1; continue }; #hit_2384
          if ( s(-2,-1) && prl_edmuro(-2) && q(-1,"prl_edmuro") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1329]++; if(dbg){print "Z1329"}; stopp=ret=1; continue }; #hit_25
       };
       if ( iwrd == "замку") {
          if ( s(-1) && q(-1,"prl_edmuda") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1330]++; if(dbg){print "Z1330"}; stopp=ret=1; continue }; #hit_757
          if ( s(-2,-1) && prl_edmuda(-2) && q(-1,"prl_edmuda") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1331]++; if(dbg){print "Z1331"}; stopp=ret=1; continue }; #hit_7
          if ( q(1,"prl_edmuda") && (s(0)||se(0," — ")||sc(0,":")) && Q_w(2,"suw_edda") )
          { l[i]=omo2; Z[1332]++; if(dbg){print "Z1332"}; stopp=ret=1; continue }; #hit_39
       };
       if ( iwrd == "замком") {
          if ( s(-1) && q(-1,"prl_edmutv") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1333]++; if(dbg){print "Z1333"}; stopp=ret=1; continue }; #hit_4407
          if ( s(-2,-1) && prl_edmutv(-2) && q(-1,"prl_edmutv") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1334]++; if(dbg){print "Z1334"}; stopp=ret=1; continue }; #hit_58
       };
       if ( iwrd == "замке") {
          if ( s(-1) && q(-1,"prl_edmupr") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1335]++; if(dbg){print "Z1335"}; stopp=ret=1; continue }; #hit_1123
          if ( s(-2,-1) && prl_edmupr(-2) && q(-1,"prl_edmupr") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1336]++; if(dbg){print "Z1336"}; stopp=ret=1; continue }; #hit_13
       };
       if ( iwrd == "замки") {
          if ( s(-1) && q(-1,"prl_mnim prl_krmn") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1337]++; if(dbg){print "Z1337"}; stopp=ret=1; continue }; #hit_2569
          if ( s(-2,-1) && prl_mnim(-2) && q(-1,"prl_mnim") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1338]++; if(dbg){print "Z1338"}; stopp=ret=1; continue }; #hit_41
          if ( q(1,"prl_mnim prl_mntv qast_any") && q(2,"prl_mnim prl_mntv") &&
              ba(2,str_lock) && (zs(0)||se(0," — ")) && zs(1) )
          { l[i]=omo2; Z[1339]++; if(dbg){print "Z1339"}; stopp=ret=1; continue }; #hit_34
          if ( qb(1,5,"prl_mnim prl_mntv prl_krmn") && qir(1,qbn-1,"gl_aux_be qast_any nar_any prl_krmn muk_mnim") &&
               ba(qbn,str_lock) && (zs(0)||se(0," — ")) && s(1,qbn-1) && Q(qbn+1,"suw_mnim suw_mntv") )
          { l[i]=omo2; Z[1340]++; if(dbg){print "Z1340"}; stopp=ret=1; continue }; #hit_178
       };
       if ( iwrd == "замков") {
          if ( s(-1) && q(-1,"prl_mnro") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1341]++; if(dbg){print "Z1341"}; stopp=ret=1; continue }; #hit_909
          if ( s(-2,-1) && prl_mnro(-2) && q(-1,"prl_mnro") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1342]++; if(dbg){print "Z1342"}; stopp=ret=1; continue }; #hit_17
       };
       if ( iwrd == "замкам") {
          if ( s(-1) && q(-1,"prl_mnda") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1343]++; if(dbg){print "Z1343"}; stopp=ret=1; continue }; #hit_102
          if ( s(-2,-1) && prl_mnda(-2) && q(-1,"prl_mnda") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1344]++; if(dbg){print "Z1344"}; stopp=ret=1; continue }; #hit_3
       };
       if ( iwrd == "замками") {
          if ( s(-1) && q(-1,"prl_mntv") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1345]++; if(dbg){print "Z1345"}; stopp=ret=1; continue }; #hit_1334
          if ( s(-2,-1) && prl_mntv(-2) && q(-1,"prl_mntv") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1346]++; if(dbg){print "Z1346"}; stopp=ret=1; continue }; #hit_25
       };
       if ( iwrd == "замках") {
          if ( s(-1) && q(-1,"prl_mnpr") && ba(-1,str_lock) )
          { l[i]=omo2; Z[1347]++; if(dbg){print "Z1347"}; stopp=ret=1; continue }; #hit_163
          if ( s(-2,-1) && prl_mnpr(-2) && q(-1,"prl_mnpr") && ba(-2,str_lock) )
          { l[i]=omo2; Z[1348]++; if(dbg){print "Z1348"}; stopp=ret=1; continue }; #hit_5
       };
    # _lock_prl:end
break}; Y[stopp_str]=1; return ret }

function FN_hw_zamka_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n, fufu) { stopp=v2s(Y["FN_hw_zamka_f"]);
  while ( !stopp ) {
    if ( vex(Y["_hw_castle-ro"]) ) { qb_n = Y["_hw_castle-ro"] };
    if ( vex(Y["_hw_lock-ro"])   ) { qf_n = Y["_hw_lock-ro"]   };
    if ( qb_n && qf_n && vlt(qb_n,qf_n) )
    { fufu = 1; Z[1349]++; if(dbg){print "Z1349","hw_castle < hw_lock"}; }; #hit_1428
    if ( bba(qb_n+1,-1,"_lock_prl_true") )
    { fufu = 1; Z[1350]++; if(dbg){print "Z1350","hw_caste < _lock_prl_true"}; }; #hit_18570
...
#        !(w(1,"на") && wa(2,"_lock_na_suw-pr") && s(0,1)) &&
    ...
    { l[i]=omo1; Z[1351]++; if(dbg){print "Z1351"}; stopp=ret=1; continue }; #hit_99547
    if ( vgl(qf_n,-5,-1) &&
         qir(qf_n+1,-1,"prl_ro prq_ro mest_ro mest_3e nar_any prl_kred_sr") && s(qf_n,-1) )
    { l[i]=omo2; Z[1352]++; if(dbg){print "Z1352"}; stopp=ret=1; continue }; #hit_10856

    cst="сторона часть";
    cst1="восточный западный северный северо-восточный северо-западны центральный юго-восточный юго-западный южный нижний дальний ближний привилегированный исторический";
    if ( bb(-5,-1,cst) && s(bbn-1,-1) &&
         bw(bbn-1,cst1) &&
        qir(bbn+1,-1,"prl_edmuro mest_edmuro mest_3e")  )
    { l[i]=omo1; Z[721]++; if(dbg){print "Z721"}; stopp=ret=1; continue }; #hit_549

    cst="служба";
    cst1="безопасности снабжения связи обеспечения";
    if ( bb(-5,-1,cst) && s(bbn-1,-1) &&
         w(bbn+1,cst1) &&
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1353]++; if(dbg){print "Z1353"}; }; #hit_121
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1354]++; if(dbg){print "Z1354"}; }; #hit_9
    if ( vgl(wf_n,1,5) && vex(Y["tn_ok_na"]) && s(-1,wf_n-1) && TN_na_zamok_f() )
    { Z[1355]++; if(dbg){print "Z1355"}; stopp=ret=1; continue }; #hit_188

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step qip_ro qik_im") )
    { l[i]=omo1; Z[1356]++; if(dbg){print "Z1356"}; stopp=ret=1; continue }; #hit_13106
 };
 # _castle_suw-ro:end
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
       ...
       { l[i]=omo1; Z[1357]++; if(dbg){print "Z1357"}; stopp=ret=1; continue }; #hit_1

       if ( vgl(wf_n,1,5) && s(0,wf_n-1) && !( isname(wf_n)||isname(1) ) &&
            qir(1,wf_n-1,"prl_ro mest_ro prq_ro qik_im qip_ro") )
       { l[i]=omo2; Z[1358]++; if(dbg){print "Z1358"}; stopp=ret=1; continue }; #hit_11121

 };
 # _lock_suw-ro:end
break}; Y["FN_zamok_suwro_f"]=1; return ret }

function FN_zamok_vi_esc_f(    ret, stopp_str,fufu,stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n,BB_n,BF_n,eb_n,EB_n,ef_n,EF_n) { stopp=v2s(Y["FN_zamok_vi_esc_f"]);
  while ( !stopp ) {

 if ( qxw(-3,"часть","ворот двери") && z(0) &&
      qxw(-1,"а","именно") && se(-1," — ") )
 { l[i]=omo2; Z[62]++; if(dbg){print "Z62"}; stopp=ret=1; continue }; #hit_1
  ...
  if ( vex(Y["n-_castle_pe"]) ) { eb_n=Y["n-_castle_pe"]; EB_n=Y["w-_castle_pe"];
    ...
    if ( weq(EB_n,cst) ) {
      ...
      { l[i]=omo2; Z[439]++; if(dbg){print "Z439"}; stopp=ret=1; continue }; #hit_1
    };

    cst="меняться";
    if ( weq(EB_n,cst) ) {
      if ( veq(eb_n,1) && z(1) && s(0) &&
            w(2,"то тогда") &&
            q(3,"qik_vi mest_vi") &&
           veq(Y["n+_lock_parts"],4) && s(2,3) )
      { l[i]=omo2; Z[482]++; if(dbg){print "Z482"}; stopp=ret=1; continue }; #hit_1
    };
 ...
 #===============================================================================================
  if ( vex(Y["n-_lock_pe"]) ) { eb_n=Y["n-_lock_pe"]; EB_n=Y["w-_lock_pe"];
   ...
   if ( weq(EB_n,cst) ) {
       ...
       if ( ( qxs(eb_n-1,"на",cst1)||
              ...
              qxs(eb_n-1,"в",cst5) ) &&
              qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[547]++; if(dbg){print "Z547"}; stopp=ret=1; continue }; #hit_4
       if ( ( qaw(eb_n+1,"на",cst1)||
              qaw(eb_n+1,"по",cst2)||
              qaw(eb_n+1,"до",cst4)||
              qaw(eb_n+1,"в",cst5) ) && s(awn,-1) &&
              qir(awn+1,-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[548]++; if(dbg){print "Z548"}; stopp=ret=1; continue }; #hit_3

       if ( veq(eb_n,-1) &&
            bw(-2,cst3) && s(-2,-1) )
       { l[i]=omo2; Z[541]++; if(dbg){print "Z541"}; stopp=ret=1; continue }; #hit_10
       if ( veq(eb_n,-3) &&
             w(-2,"на") &&
             w(-1,cst1) && s(-3,-1) )
       { l[i]=omo1; Z[543]++; if(dbg){print "Z543"}; stopp=ret=1; continue }; #mute
       if ( veq(eb_n,-1) &&
            wf(1,3,"в") &&
             w(wfn+1,cst5) && s(-1,wfn) )
       { l[i]=omo1; Z[544]++; if(dbg){print "Z544"}; stopp=ret=1; continue }; #hit_1
   ...
   if ( weq(EB_n,cst) ) {
       if ( veq(eb_n,-1) &&
         ...
         isname(1) && s(-2,0) )
       { l[i]=omo1; Z[508]++; if(dbg){print "Z508"}; stopp=ret=1; continue }; #hit_1
    };
   ...
   if ( weq(EB_n,cst) ) {
    ...
    { l[i]=omo1; Z[475]++; if(dbg){print "Z475"}; stopp=ret=1; continue }; #hit_1
  };

   cst="положить";
   if ( weq(EB_n,cst) ) {
     if ( veq(eb_n,-1) && qxs(-1,"на","рот","был","положен") )
     { l[i]=omo2; Z[485]++; if(dbg){print "Z485"}; stopp=ret=1; continue }; #hit_1
     if ( veq(eb_n,-1) && qxs(-1,"по","праву","положен") )
     { l[i]=omo1; Z[485]++; if(dbg){print "Z485"}; stopp=ret=1; continue }; #hit_1
     if ( w(-1,"положен") && s(-1) &&
          q(-2,"mest_da suw_odda isname") )
     { l[i]=omo1; Z[486]++; if(dbg){print "Z486"}; stopp=ret=1; continue }; #hit_2
     if ( w(eb_n+1,"к ко") &&
          q(eb_n+2,"mest_ro mest_3e") &&
          w(eb_n+3,"ногам") && s(0,eb_n+2) )
     { l[i]=omo1; Z[490]++; if(dbg){print "Z490"}; stopp=ret=1; continue }; #hit_1
     if ( veq(eb_n,-1) &&
           w(1,"к ко") &&
           q(2,"mest_ro mest_3e") &&
           w(3,"ногам") && s(0,bfn+2) )
     ...
     if ( veq(eb_n,-1) &&
         qxs(1,"в","руины") && s(-1) )
     { l[i]=omo1; Z[491]++; if(dbg){print "Z491"}; stopp=ret=1; continue }; #hit_1
   };

   cst="потрясти";
   cst2="буря весть взрыв вой вопль грохот гул известие крик рев рычание событие трагедия удар событие пришествие";
   if ( weq(EB_n,cst) ) {
     if ( veq(eb_n,-1) && zs(-2) &&
          bw(-2,cst2) && s(-1) )
     { l[i]=omo1; Z[469]++; if(dbg){print "Z469"}; stopp=ret=1; continue }; #hit_11
     if ( veq(eb_n,-1) &&
          bw(1,cst2) && s(-1,0) )
     { l[i]=omo1; Z[470]++; if(dbg){print "Z470"}; stopp=ret=1; continue }; #hit_3
     if ( veq(eb_n,-1) && zs(-3) &&
           q(-2,"otsz_any") &&
          bw(-3,cst2) && s(-2,-1) )
     { l[i]=omo1; Z[471]++; if(dbg){print "Z471"}; stopp=ret=1; continue }; #hit_1
     if ( veq(eb_n,-1) && s(-1) && vvb(-6,-2) && bw(vvn,cst2) )
     { l[i]=omo1; Z[473]++; if(dbg){print "Z473"}; stopp=ret=1; continue }; #hit_1
     if ( qsb(-5,-2,",") && vgl(eb_n,sbn+1,-1) &&
          qir(sbn+1,eb_n-2,"otsz_any mod_any") && s(sbn+1,-1) && vvb(sbn-5,sbn) &&
         bb(vvn-4,vvn,cst2) && s(bbn,vvn-1) )
     { l[i]=omo1; Z[474]++; if(dbg){print "Z474"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[440]++; if(dbg){print "Z440"}; stopp=ret=1; continue }; #hit_1
   };
  ...
  cst="снять";
    ...
    if ( weq(EB_n,cst) ) {
      ...
      { l[i]=omo1; Z[441]++; if(dbg){print "Z441"}; stopp=ret=1; continue }; #hit_1
      wb_n=Y["n_pre_na_ist"];
      if ( vgl(wb_n,-5,-2) &&
      ...
      { l[i]=omo1; Z[442]++; if(dbg){print "Z442"}; stopp=ret=1; continue }; #hit_1
      if ( w(1,"на") &&
          vgl(eb_n,-3,-1) &&
          wf(2,5,cst1) && s(eb_n,wfn-1) &&
         qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") &&
         qir(2,wfn-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[443]++; if(dbg){print "Z443"}; stopp=ret=1; continue }; #hit_2
      cst1="проведения";
      if ( w(1,"для") &&
          vgl(eb_n,-3,-1) &&
          wf(2,5,cst1) && s(eb_n,wfn-1) &&
         qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") &&
         qir(2,wfn-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[444]++; if(dbg){print "Z444"}; stopp=ret=1; continue }; #hit_1
      if ( vgl(eb_n,-4,-1) && wb(eb_n-5,eb_n-2,"для") &&
          ...
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[445]++; if(dbg){print "Z445"}; stopp=ret=1; continue }; #hit_1
      cst1="затеи";
      if ( vgl(eb_n,-6,-3) &&
            w(eb_n+1,"для") &&
      ...
      { l[i]=omo1; Z[446]++; if(dbg){print "Z446"}; stopp=ret=1; continue }; #hit_1
      cst1="здесь нынче";
      if ( vgl(eb_n,-3,-1) && s(eb_n,0) &&
            w(1,cst1) &&
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[447]++; if(dbg){print "Z447"}; stopp=ret=1; continue }; #hit_1
      if ( veq(eb_n,-2) && s(-2,-1) &&
            w(-1,cst1) )
      { l[i]=omo1; Z[448]++; if(dbg){print "Z448"}; stopp=ret=1; continue }; #hit_1
      ...
      { l[i]=omo1; Z[449]++; if(dbg){print "Z449"}; stopp=ret=1; continue }; #mute
      ...
      { l[i]=omo1; Z[450]++; if(dbg){print "Z450"}; stopp=ret=1; continue }; #hit_1
      ...
      { l[i]=omo1; Z[451]++; if(dbg){print "Z451"}; stopp=ret=1; continue }; #hit_2
      if ( w(1,"у") &&
          vgl(eb_n,-3,-1) &&
           q(2,"isname") && s(eb_n,1) &&
         qir(bbn+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[452]++; if(dbg){print "Z452"}; stopp=ret=1; continue }; #mute
      if ( vgl(eb_n,-3,-1) && s(eb_n,1) &&
            w(1,"в во") &&
            q(2,"isname") &&
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[453]++; if(dbg){print "Z453"}; stopp=ret=1; continue }; #hit_1
      cst1="краях местах";
      if ( vgl(eb_n,-3,-1) && s(eb_n,1) &&
            w(1,"в во") &&
           wf(2,5,cst1) &&
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e isname") &&
          qir(2,wfn-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[454]++; if(dbg){print "Z454"}; stopp=ret=1; continue }; #mute
      if ( vgl(eb_n,-6,-3) && s(eb_n,-1) &&
            w(eb_n+1,"в во") &&
           wf(eb_n+3,-1,cst1) &&
          qir(eb_n+3,wfn-1,"prl_vi mest_vi mest_3e") &&
          qir(wfn+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo1; Z[455]++; if(dbg){print "Z455"}; stopp=ret=1; continue }; #hit_1
      if (  ba(-4,"_gl_dvizh") && s(-4,-3) &&
             w(-3,"в из") &&
             q(-2,"isname suw_ro") && z(-2) &&
            veq(eb_n,-1) && s(-1) )
      { l[i]=omo1; Z[457]++; if(dbg){print "Z457"}; stopp=ret=1; continue }; #hit_1
  ...
  if ( vex(Y["n+_lock_pe"]) ) { ef_n=Y["n+_lock_pe"]; EF_n=Y["w+_lock_pe"];
   ...
   if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo1; Z[545]++; if(dbg){print "Z545"}; stopp=ret=1; continue }; #hit_14

       if ( veq(ef_n,1) &&
            bw(-1,cst3) && s(-1,0) )
       { l[i]=omo2; Z[542]++; if(dbg){print "Z542"}; stopp=ret=1; continue }; #hit_2
       if ( z(ef_n) &&
          qxw(ef_n+1,"для","освобождения очистки","места земли территории") )
       { l[i]=omo1; Z[546]++; if(dbg){print "Z546"}; stopp=ret=1; continue }; #hit_1
    };
   cst="закрыть запереть открыть";
   cst2="держать удерживать унести";
   if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo1; Z[509]++; if(dbg){print "Z509"}; stopp=ret=1; continue }; #hit_1
    };
  ...
  cst="перевесить";
   ...
   if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo1; Z[481]++; if(dbg){print "Z481"}; stopp=ret=1; continue }; }; #hit_1

   cst="положить";
   cst1="конец начало";
   cst2="гранату";
   cst3="столице городу празднику празднествам";
   if ( weq(EF_n,cst) ) {
     if ( w(1,"положен") && s(0) )
     { l[i]=omo1; Z[487]++; if(dbg){print "Z487"}; stopp=ret=1; continue }; #hit_1
     if ( wy(1,"не") &&
           w(1+wyn,"положен положено") && s(0,0+wyn) )
     { l[i]=omo1; Z[488]++; if(dbg){print "Z488"}; stopp=ret=1; continue }; #hit_2
     if ( Qf(1,ef_n-1,"sz_iili pre_any") &&
           w(ef_n+1,cst1) && zs(0) && s(1,ef_n+1) &&
           w(ef_n+2,cst3) )
     { l[i]=omo1; Z[489]++; if(dbg){print "Z489"}; stopp=ret=1; continue }; #hit_4
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo1; Z[484]++; if(dbg){print "Z484"}; stopp=ret=1; continue }; #hit_17
   };
  ...
  cst="потрясти";
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo1; Z[472]++; if(dbg){print "Z472"}; stopp=ret=1; continue }; #hit_8
   };
   ...
   if ( weq(EF_n,cst) ) {
      ...
      { l[i]=omo1; Z[456]++; if(dbg){print "Z456"}; stopp=ret=1; continue }; #hit_1
   };

   cst="закрывать закрыть";
   cst1="телепорта посещения";
   cst2="верности";
   if ( weq(EF_n,cst) ) {
    if ( vgl(ef_n,3,5) && s(0,ef_n-1) &&
         w(1,"для") &&
         q(2,"mest_ro suw_odro") )
    { l[i]=omo1; Z[421]++; if(dbg){print "Z421"}; stopp=ret=1; continue }; #hit_1
    if ( vgl(ef_n,1,3) && s(0,ef_n) &&
        w(ef_n+1,"для") &&
        q(ef_n+2,"mest_ro suw_odro") )
    { l[i]=omo1; Z[422]++; if(dbg){print "Z422"}; stopp=ret=1; continue }; #hit_1
    if ( vgl(ef_n,1,3) &&
        qxw(ef_n+1,"для",cst1) && s(0,ef_n-1) )
    { l[i]=omo1; Z[422]++; if(dbg){print "Z422"}; stopp=ret=1; continue }; #hit_1
    if ( vgl(ef_n,1,3) &&
        qxw(ef_n+1,"для",cst2) && s(0,ef_n-1) )
    { l[i]=omo2; Z[422]++; if(dbg){print "Z422"}; stopp=ret=1; continue }; #hit_1
   };
 ...
 #=================================================================================================================================
  if ( vex(Y["n-_gl_zamok"]) ) { eb_n=Y["n-_gl_zamok"]; EB_n=Y["w-_gl_zamok"];
   ...
   if ( weq(EB_n,cst) ) {
    ...
    if ( weq(EB_n,cst) ) {
      if ( veq(eb_n,-3) &&
       pre_pr(-2) &&
           bw(-1,cst2) && s(-3,-1) )
      { l[i]=omo2; Z[499]++; if(dbg){print "Z499"}; stopp=ret=1; continue }; #hit_3
      cst2="дверь ящик";
      if ( veq(eb_n,-4) &&
       pre_pr(-3) &&
           bw(-2,cst2) &&
            q(-1,"prl_edmuim") && s(-4,-1) )
      { l[i]=omo2; Z[500]++; if(dbg){print "Z500"}; stopp=ret=1; continue }; #hit_1
      if ( veq(eb_n,-1) &&
       pre_pr(1) &&
           bw(2,cst2) && s(-1,1) )
      { l[i]=omo2; Z[501]++; if(dbg){print "Z501"}; stopp=ret=1; continue }; #hit_1
      if ( ba(-5,"_gl_dvizh") &&
            q(-4,"nar_any") && z(-4) &&
            w(-3,"чтобы") &&
            veq(eb_n,-2) &&
            q(-1,"prl_vi mest_vi mest_3e") && s(-3,-1) )
      { l[i]=omo2; Z[502]++; if(dbg){print "Z502"}; stopp=ret=1; continue }; #hit_1
      if ( vgl(Y["n_pre_na_ist"],-5,-1) && s(Y["n_pre_na_ist"],-1) &&
            veq(eb_n,Y["n_pre_na_ist"]-1) &&
           qir(Y["n_pre_na_ist"]+1,-1,"prl_vi mest_vi mest_3e") )
      { l[i]=omo2; Z[503]++; if(dbg){print "Z503"}; stopp=ret=1; continue }; #hit_23
    };
    ...
    if ( weq(EB_n,cst) ) {
       if ( s(eb_n,-1) &&
            ...
            qir(wbn+1,-1,"prl_vi mest_vi mest_3e") )
       { l[i]=omo1; Z[510]++; if(dbg){print "Z510"}; stopp=ret=1; continue }; #hit_1
       if ( s(eb_n,-1) &&
              w(1,"на") &&
             wf(2,5,cst4) && s(0,wfn-1) &&
            qir(eb_n+1,-1,"nar_any prl_kred_sr prl_vi mest_vi mest_3e") &&
            qir(2,wfn-1,"prl_mnvi mest_vi mest_3e") )
       { l[i]=omo1; Z[511]++; if(dbg){print "Z511"}; stopp=ret=1; continue }; #hit_1
       if ( veq(eb_n,-3) &&
              w(-2,"в") &&
              w(-1,cst3) && s(-3,-1) )
       { l[i]=omo2; Z[512]++; if(dbg){print "Z512"}; stopp=ret=1; continue }; #hit_4
    };
   ...
   if ( weq(EB_n,cst) ) {
      ...
      { l[i]=omo1; Z[537]++; if(dbg){print "Z537"}; stopp=ret=1; continue }; #hit_4
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[461]++; if(dbg){print "Z461"}; stopp=ret=1; continue }; #hit_9
     if ( wb(eb_n-4,eb_n-1,"у") && s(wbn,-1) &&
            q(wbn+1,"suw_ro mest_ro mest_3e") &&
          qir(wbn+2,eb_n-1,"mod_any gl_aux_be nar_any") &&
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[462]++; if(dbg){print "Z462"}; stopp=ret=1; continue }; #hit_2
     if (  veq(eb_n,-1) && s(-1) &&
           qxs(-2,"все","равно","что") )
     { l[i]=omo1; Z[463]++; if(dbg){print "Z463"}; stopp=ret=1; continue }; #hit_1
     if (  veq(eb_n,-1) && s(-1) &&
             q(0,"isname") )
     { l[i]=omo1; Z[464]++; if(dbg){print "Z464"}; stopp=ret=1; continue }; #hit_2
     if (  veq(eb_n,-2) && s(-2,-1) &&
            ba(-1,"_castle_prl") )
     { l[i]=omo1; Z[465]++; if(dbg){print "Z465"}; stopp=ret=1; continue }; #mute
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[458]++; if(dbg){print "Z458"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[438]++; if(dbg){print "Z438"}; stopp=ret=1; continue }; #hit_5
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[518]++; if(dbg){print "Z518"}; stopp=ret=1; continue }; #hit_15
   };
   ...
   if ( weq(EB_n,cst) ) {
     if ( vgl(eb_n,-7,-3) &&
       ...
       ( qaw(eb_n+1+qyn,"в","дар подарок наследство собственность распоряжение управление бенефиций")||
         ...
         qir(awn+1,-1,"prl_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[516]++; if(dbg){print "Z516"}; stopp=ret=1; continue }; #hit_47
   };

   cst="собирать собрать";
   if ( weq(EB_n,cst) ) { bb_n=Y["n-_castle_pe"];
       if ( vgl(bb_n,-7,-3) && q(bb_n+1,"sz_iili") && veq(bb_n+2,eb_n) && s(bb_n,-1) &&
            qir(bb_n+3,-1,"prl_vi mest_vi mest_3e prq_vi") )
       { l[i]=omo1; Z[620]++; if(dbg){print "Z620"}; stopp=ret=1; continue }; #hit_1
   };
   cst="ломать";
   cst2="воздушный";
   if ( weq(EB_n,cst) ) {
     if ( s(eb_n,-1) &&
           ...
           wb(eb_n+1,-1,cst2) &&
          qir(eb_n+1,-1,"prl_vi prq_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[642]++; if(dbg){print "Z642"}; stopp=ret=1; continue }; #hit_2
   };
   ...
   if ( weq(EB_n,cst) ) {
    if ( ba(-2,"_gl_dvizh") && sc(-2,"—") && s(-1) &&
         veq(eb_n,-1) )
    { l[i]=omo1; Z[413]++; if(dbg){print "Z413"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EB_n,cst) ) {
      if ( s(eb_n,-1) &&
           ...
           qir(eb_n+1,-1,"prl_vi prq_vi mest_vi mest_3e") &&
           qir(2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
      { l[i]=omo1; Z[643]++; if(dbg){print "Z643"}; stopp=ret=1; continue }; #hit_5
   };
   ...
   if ( weq(EB_n,cst) ) {
      if ( bba(eb_n-5,eb_n-1,"_castle_platz") ) { bb_n=bbn;
       ...
       { l[i]=omo1; Z[400]++; if(dbg){print "Z400"}; stopp=ret=1; continue }; #hit_93
       if ( q(bb_n+1,"pre_any") && z(bb_n) && s(bb_n+1,-1) &&
            q(bb_n+2,"otsz_any") )
       { l[i]=omo1; Z[401]++; if(dbg){print "Z401"}; stopp=ret=1; continue }; #hit_295
      ...
      if ( wba(eb_n-5,eb_n-1,"_lock_na_suw-pr") ) { bb_n=wbn;
       ...
       { l[i]=omo2; Z[402]++; if(dbg){print "Z402"}; stopp=ret=1; continue }; #hit_1
       };
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[257]++; if(dbg){print "Z257"}; stopp=ret=1; continue }; #hit_3
   };
   ...
   if ( weq(EB_n,cst) ) {
     if ( (qxs(-1,"внешним своим","видом")||
           ...
           veq(Y["n-_castlelike_things"],xsn-2) )
     { l[i]=omo1; Z[86]++; if(dbg){print "Z86"}; stopp=ret=1; continue }; #hit_1
     if (  vgl(eb_n,-4,-1) &&
           bba(eb_n-4,eb_n-1,"_castlelike_things") && s(bbn,-1) && zs(eb_n-1) &&
           qir(bbn+1,eb_n-1,"nar_srav qast_any") &&
           qir(eb_n+1,-1,"prl_vi mest_vi mest_3e prq_vi") )
     { l[i]=omo1; Z[87]++; if(dbg){print "Z87"}; stopp=ret=1; continue }; #hit_78
   };
   ...
   if ( weq(EB_n,cst) ) {
     if ( veq(eb_n,-3) &&
           w(-2,"с со") &&
           q(-1,"suw_ro") && s(-3,-1) )
     { l[i]=omo2; Z[185]++; if(dbg){print "Z185"}; stopp=ret=1; continue }; #hit_6
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo2; Z[191]++; if(dbg){print "Z191"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EB_n,cst) ) {
     if (  w(1,"из") &&
           w(2,cst2) && s(eb_n,1) &&
           qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[192]++; if(dbg){print "Z192"}; stopp=ret=1; continue }; #hit_1
   };

   cst="обращать превратить превратиться превращать превращаться";
   if ( weq(EB_n,cst) ) { wf_n=Y["n+_castlelike_things"];
     if ( wf(1,3,"в во") &&
     ...
     { l[i]=omo1; Z[80]++; if(dbg){print "Z80"}; stopp=ret=1; continue }; #hit_95
     if ( veq(Y["n-_castlelike_things"],-3) &&
          veq(eb_n,-2) && w(-1,"в") && s(-3,-1) )
     { l[i]=omo1; Z[82]++; if(dbg){print "Z82"}; stopp=ret=1; continue }; #hit_12
     if ( veq(eb_n,-3) && veq(Y["n-_castlelike_things"],-1) &&
           w(-2,"в") && s(-3,-1) )
     { l[i]=omo1; Z[83]++; if(dbg){print "Z83"}; stopp=ret=1; continue }; #hit_4
   };
   ...
   { l[i]=omo1; Z[1359]++; if(dbg){print "Z1359"}; stopp=ret=1; continue }; #hit_13
   ...
   if ( weq(EB_n,cst) ) {
     if ( vgl(eb_n,-4,-1) && s(eb_n,-1) &&
         (qxs(1,"на","то")||
          ...
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[1361]++; if(dbg){print "Z1361"}; stopp=ret=1; continue }; #hit_1
     if ( weq(EB_n,cst) && s(eb_n,0) &&
         (qxs(1,"на в во")||
          qxs(1,"то","ли","в") ) &&
       isname(xsn+1) && s(xsn) &&
          qir(eb_n+1,-1,"prl_edmuvi mest_edmuvi mest_3e") )
     { l[i]=omo1; Z[1362]++; if(dbg){print "Z1362"}; stopp=ret=1; continue }; #hit_7
     if ( weq(EB_n,cst) && qsb(eb_n-5,eb_n-1,",") && s(sbn+1,-1) &&
            w(sbn-1,"в во на")  &&
       isname(sbn) && s(sbn-1) &&
          qir(eb_n+1,-1,"prl_edmuvi mest_edmuvi mest_3e") )
     { l[i]=omo1; Z[1363]++; if(dbg){print "Z1363"}; stopp=ret=1; continue }; #hit_2
     if ( weq(EB_n,cst) && s(eb_n+1,-1) &&
          qaw(eb_n+1,"под","удар") &&
          qir(awn+1,-1,"prl_edmuvi mest_edmuvi mest_3e") )
     { l[i]=omo1; Z[1364]++; if(dbg){print "Z1364"}; stopp=ret=1; continue }; #hit_1
     qf_n=Y["n+_castle_environs"];
     if ( weq(EB_n,cst) && vgl(qf_n,2,5) && s(eb_n,qf_n-1) &&
            w(1,"против напротив") &&
          qir(eb_n+1,-1,"prl_edmuvi mest_edmuvi mest_3e") &&
          qir(2,qf_n-1,"prl_ro mest_ro mest_3e") )
     { l[i]=omo1; Z[1365]++; if(dbg){print "Z1365"}; stopp=ret=1; continue }; #hit_2
   };
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[1372]++; if(dbg){print "Z1372"}; stopp=ret=1; continue }; #hit_1
   ...
   if ( weq(EB_n,cst) && s(eb_n+1,-1) &&
         w(eb_n+1,cst1) &&
       qir(eb_n+2,-1,"prl_edmuvi mest_edmuvi mest_3e") )
   { l[i]=omo2; Z[1373]++; if(dbg){print "Z1373"}; stopp=ret=1; continue }; #hit_3
...
#  qb_n=Y["n-_lock_pe"];
   ...
   if ( weq(EB_n,cst) && wb(eb_n-3,eb_n-1,cst1) && s(wbn,-1) &&
       (qxs(wbn,"по","прямой","наводке")||
        ...
        qir(eb_n+1,-1,"prl_vi mest_vi mest_3e") )
   { l[i]=omo1; Z[1366]++; if(dbg){print "Z1366"}; stopp=ret=1; continue }; #hit_1
   ...
   if ( weq(EB_n,cst) ) {
     ...
     { l[i]=omo1; Z[30]++; if(dbg){print "Z30"}; stopp=ret=1; continue }; #hit_1
     cst1="армия взвод дивизия имперец отряд полк рота";
     if ( veq(eb_n,-1) &&
          bw(1,cst1) && s(-1,0) )
     { l[i]=omo1; Z[29]++; if(dbg){print "Z29"}; stopp=ret=1; continue }; #hit_2
     if ( veq(eb_n,-1) && bw(-3,cst1) && s(-1) &&
            q(-2,"suw_ro isname") && z(-2) &&
       prq_any(-1) )
     { l[i]=omo1; Z[31]++; if(dbg){print "Z31"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EB_n,cst) && veq(eb_n,-3) && q(-3,"prq_any") &&
        q(-2,"prl_tv prq_tv") &&
        q(-1,"suw_tv") && s(-3,-1) )
   { l[i]=omo1; Z[59]++; if(dbg){print "Z59"}; stopp=ret=1; continue }; #hit_4

   cst="быть вставать выбрать избрать именовать именоваться казаться объявить объявлять оказаться провозгласить провозглашать сделать служить стать считаться \
        являться";
   cst0="дверью клеткой ключом пеплом руинами целью центром";
   if ( weq(EB_n,cst) ) { bf_n=wfn="";
     ...
     if ( vgl(Y["n+_castlelike_things"],1,5) ) { bf_n=Y["n+_castlelike_things"] } else {
      ...
      { l[i]=omo1; Z[104]++; if(dbg){print "Z104"}; stopp=ret=1; continue }; #hit_110
  
   };};

  cst="забросить бросить";
  if ( weq(EB_n,cst) ) {
    ...
    if (  w(1,"в") &&
          ...
          qir(eb_n+1,-1,"prl_vi mest_vi mest_3e prq_vi")  &&
          qir(2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") )
    { l[i]=omo2; Z[114]++; if(dbg){print "Z114"}; stopp=ret=1; continue }; #hit_1
  };
  # брать взять <<<---:start
  cst="брать взять";
  if ( weq(EB_n,cst) ) {
    ...
    { l[i]=omo1; Z[1374]++; if(dbg){print "Z1374"}; stopp=ret=1; continue }; #hit_159
    if ( wf(1,4,cst2) && s(eb_n,wfn-1) &&
         qir(eb_n+1,-1,"qast_any mest_vi prl_vi") &&
         qir(1,wfn-1,"qast_any mest_tv prl_tv") )
    { l[i]=omo1; Z[1375]++; if(dbg){print "Z1375"}; stopp=ret=1; continue }; #hit_170
    ...
    { l[i]=omo1; Z[1376]++; if(dbg){print "Z1376"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo1; Z[1377]++; if(dbg){print "Z1377"}; stopp=ret=1; continue }; #hit_1

    if ( veq(eb_n,-1) && w(-1,"взят") && s(-1) )
    { l[i]=omo1; Z[1378]++; if(dbg){print "Z1378"}; stopp=ret=1; continue }; #hit_12

    if ( qxs(-1,"на","себя") &&
         veq(eb_n,xsn-1) && s(xsn-1) )
    { l[i]=omo1; Z[1379]++; if(dbg){print "Z1379"}; stopp=ret=1; continue }; #hit_2
    if ( veq(eb_n,-1) && s(-1) &&
         qxs(1,"в","крепкие","руки") )
    { l[i]=omo1; Z[1380]++; if(dbg){print "Z1380"}; stopp=ret=1; continue }; #hit_1

    if ( vgl(Y["n+_lock_pe"],1,5) && s(eb_n,-1) && zs(0) && s(1,Y["n+_lock_pe"]-1) &&
         qir(eb_n+1,-1,"qast_any mest_vi prl_vi") &&
         qir(1,Y["n+_lock_pe"]-1,"prl_kred_sr mest_vi mest_3e") )
    { l[i]=omo2; Z[1381]++; if(dbg){print "Z1381"}; stopp=ret=1; continue }; #hit_4

    if ( veq(Y["n+_lock_pe"],2) && s(1) && s(eb_n,-1) && z(0) &&
         q(1,"otsz_vi isname suw_im") &&
         qir(eb_n+1,-1,"qast_any mest_vi prl_vi")  )
    { l[i]=omo2; Z[1382]++; if(dbg){print "Z1382"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[1383]++; if(dbg){print "Z1383"}; stopp=ret=1; continue }; #hit_26
    if ( w(1,"в во") && s(eb_n,-1) &&
          wf(2,4,cst2) &&
         qir(eb_n+1,-1,"mest_vi prl_vi prq_vi") &&
         qir(2,wfn-1,"mest_vi prl_vi prq_vi nar_any") )
    { l[i]=omo2; Z[1384]++; if(dbg){print "Z1384"}; stopp=ret=1; continue }; #hit_15
    ...
    { l[i]=omo2; Z[1385]++; if(dbg){print "Z1385"}; stopp=ret=1; continue }; #hit_2

    if ( veq(eb_n,-1) &&
           q(1,"nar_napr") && s(-1,0) )
    { l[i]=omo2; Z[1386]++; if(dbg){print "Z1386"}; stopp=ret=1; continue }; #hit_3
    ...
    if ( veq(eb_n,-1) && s(-5,-1) &&
          ba(-5,"_lock_parts") &&
           ...
           q(-3,"suw_vi") &&
        sz_i(-2) )
    { l[i]=omo2; Z[1391]++; if(dbg){print "Z1391"}; stopp=ret=1; continue }; #hit_1

    if ( veq(eb_n,-2) && ba(-4,"_gl_dvizh") && s(-4) &&
           q(-3,"nar_napr nar_spos nar_vrem") && z(-3) &&
           q(-1,"prl_vi mest_vi mest_3e") && s(-2,-1) )
    { l[i]=omo2; Z[1392]++; if(dbg){print "Z1392"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( veq(eb_n,-1) &&
        sz_i(1) &&
          bw(2,cst2) &&
           w(3,cst3) && s(-1,2) )
    { l[i]=omo2; Z[1393]++; if(dbg){print "Z1393"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( (qxs(-1,"в",cst2)||
          ...
          qxs(-1,"на","мушку") ) &&
           veq(eb_n,xsn-1) && s(xsn-1) )
    { l[i]=omo2; Z[1394]++; if(dbg){print "Z1394"}; stopp=ret=1; continue }; #hit_1
    if ( veq(eb_n,-1) &&
        (qxs(1,"в",cst2)||
         qxs(1,"в","правую левую свою свои обе",cst2)||
         qxs(1,"на","мушку") ) && s(-1) )
    { l[i]=omo2; Z[1395]++; if(dbg){print "Z1395"}; stopp=ret=1; continue }; #hit_1
    ...
    { l[i]=omo2; Z[1396]++; if(dbg){print "Z1396"}; stopp=ret=1; continue }; #hit_5
    ...
    if ( veq(eb_n,-1) &&
           ...
           w(3,"из") &&
           w(4,cst2) && s(-1,3) )
    { l[i]=omo2; Z[1397]++; if(dbg){print "Z1397"}; stopp=ret=1; continue }; #hit_1
    cst2="пальцами руками рукой";
    if ( veq(eb_n,-1) &&
           q(1,"prl_tv mest_tv mest_3e") && W(1,"голыми") &&
           w(2,cst2) && s(-1,1) )
    { l[i]=omo2; Z[1398]++; if(dbg){print "Z1398"}; stopp=ret=1; continue }; #hit_2

    cst2="опять осторожно снова";
    if ( veq(eb_n,-1) && s(-3,-1) &&
         q(-3,"isname suw_odim mest_im") &&
         w(-2,cst2) )
    { l[i]=omo2; Z[1399]++; if(dbg){print "Z1399"}; stopp=ret=1; continue }; #hit_1
    if ( veq(eb_n,-1) &&
        sz_i(1) &&
          bw(2,"сказать") &&
           q(3,"suw_odda mest_da isname") && s(-1,2) )
    { l[i]=omo2; Z[1400]++; if(dbg){print "Z1400"}; stopp=ret=1; continue }; #hit_1
  ...
  if ( vex(Y["n+_gl_zamok"]) ) { ef_n=Y["n+_gl_zamok"]; EF_n=Y["w+_gl_zamok"];
   ...
   if ( weq(EF_n,cst) && !wa(-1,"_lock_prl_true") ) {fufu="";
     ...
     { l[i]=omo1; Z[492]++; if(dbg){print "Z492"}; stopp=ret=1; continue }; #hit_12
     if ( !fufu && q(1,"prq_any") && veq(ef_n,1) && z(0) &&
          w(2,"в во из") &&
         bf(3,6,cst2) && s(1,bfn-1) &&
         qir(3,bfn-1,"prl_vi mest_vi mest_3e prl_ro mest_ro prq_vi prq_ro") )
     { l[i]=omo1; Z[493]++; if(dbg){print "Z493"}; stopp=ret=1; continue }; #hit_8
   fufu=""};
   ...
   if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo1; Z[476]++; if(dbg){print "Z476"}; stopp=ret=1; continue }; #hit_47
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo1; Z[468]++; if(dbg){print "Z468"}; stopp=ret=1; continue }; #hit_3
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo2; Z[466]++; if(dbg){print "Z466"}; stopp=ret=1; continue }; #hit_1

     if ( vgl(ef_n,1,5) &&
           qf(ef_n+1,ef_n+4,"suw_im mest_im") && s(0,qfn-1) &&
          qir(1,ef_n-1,"nar_any prl_kred_sr gl_aux_be mod_any") &&
          qir(ef_n+1,qfn-1,"prl_vi mest_vi mest_3e") )
     { l[i]=omo1; Z[467]++; if(dbg){print "Z467"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo1; Z[460]++; if(dbg){print "Z460"}; stopp=ret=1; continue }; #hit_9
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo2; Z[459]++; if(dbg){print "Z459"}; stopp=ret=1; continue }; #hit_2
   };

   cst="быть стать явиться являться";
   cst2="отмычек отмычки техника мужчины"
   if ( weq(EF_n,cst) ) {
    if ( vgl(ef_n,1,3) &&
          q(ef_n+1,"suw_tv") &&
          w(ef_n+2,"для к") &&
    ...
    { l[i]=omo2; Z[424]++; if(dbg){print "Z424"}; stopp=ret=1; continue }; #hit_11
    if ( vgl(ef_n,1,3) &&
          q(ef_n+1,"suw_tv") &&
          w(ef_n+2,"для к") && s(0,ef_n+1) )
    { l[i]=omo1; Z[424]++; if(dbg){print "Z424"}; stopp=ret=1; continue }; #hit_11
   };
   ...
   if ( weq(EF_n,cst) ) {
    ...
    { l[i]=omo1; Z[517]++; if(dbg){print "Z517"}; stopp=ret=1; continue }; #hit_42
   };
   ...
   if ( weq(EF_n,cst) ) {
       if ( veq(ef_n,1) && s(0,1) &&
              w(2,"дома") )
       { l[i]=omo2; Z[666]++; if(dbg){print "Z666"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo2; Z[685]++; if(dbg){print "Z685"}; stopp=ret=1; continue }; #hit_1
   };

   cst="собирать собрать";
   if ( weq(EF_n,cst) ) { bb_n=Y["n-_castle_pe"];
     if ( s(0,ef_n) &&
          q(ef_n+1,"mest_mnvi") &&
         Qf(1,ef_n-1,"sz_iili pre_any") )
     { l[i]=omo1; Z[686]++; if(dbg){print "Z686"}; stopp=ret=1; continue }; #hit_1
     cst1="крышей";
     ...
     { l[i]=omo1; Z[1370]++; if(dbg){print "Z1370"}; stopp=ret=1; continue }; #hit_1
   ...
   if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo1; Z[688]++; if(dbg){print "Z688"}; stopp=ret=1; continue }; #hit_2
   };
   ...
   if ( weq(EF_n,cst) ) {
       if ( s(0,ef_n) &&
            ...
            qir(ef_n+1,wfn-1,"prl_vi mest_3e") &&
             Qf(1,ef_n-1,"sz_iili pre_any") )
       { l[i]=omo1; Z[689]++; if(dbg){print "Z689"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EF_n,cst) ) {
       if ( s(0,ef_n) &&
            ...
            qir(ef_n+1,bfn-1,"mest_da suw_da isname prl_vi prl_ro mest_vi mest_ro mest_3e") &&
             Qf(1,ef_n-1,"sz_iili pre_any") )
       { l[i]=omo1; Z[690]++; if(dbg){print "Z690"}; stopp=ret=1; continue }; #hit_1
   };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     if ( weq(EF_n,cst) ) {
      if ( bba(-5,-1,"_castle_platz") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,-5,-1) &&
              ...
              q(bb_n+2,"otsz_any") )
       { l[i]=omo1; Z[403]++; if(dbg){print "Z403"}; stopp=ret=1; continue }; #hit_5
      };
       cst1="высоко";
       if ( vgl(ef_n,1,3) && z(0) && s(1,ef_n) &&
              w(ef_n+1,cst1) &&
            qir(1,ef_n-1,"otsz_any nar_any qast_any") )
       { l[i]=omo1; Z[404]++; if(dbg){print "Z404"}; stopp=ret=1; continue }; #hit_2
    };
     ...
     if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo2; Z[258]++; if(dbg){print "Z258"}; stopp=ret=1; continue }; #hit_1
     };
     ...
     if ( weq(EF_n,cst) ) {
       ...
       { l[i]=omo2; Z[112]++; if(dbg){print "Z112"}; stopp=ret=1; continue }; #hit_1
     };

     cst="представлять";
     if ( weq(EF_n,cst) && w(ef_n+1,"собой") ) {
       if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[88]++; if(dbg){print "Z88", "-- TN_v_zamok_f"}; stopp=ret=1; continue}; #hit_1
 
      qf_n=Y["n+_castlelike_things"];
      cst1="систему";
      cst2="рвов укреплений ходов";
      if ( zs(0) && s(1,qf_n-1) &&
            wf(ef_n+1,ef_n+3,cst1) &&
           ...
           qir(wfn+1,wfN-1,"prl_ro prq_ro mest_ro mest_3e") )
      { l[i]=omo1; Z[90]++; if(dbg){print "Z90"}; stopp=ret=1; continue }; #mute
    ...
    if ( vex(Y["n+_castlelike_things"]) ) { qf_n=Y["n+_castlelike_things"];
      ...
      { l[i]=omo1; Z[89]++; if(dbg){print "Z89"}; stopp=ret=1; continue }; #hit_93
    ...
    if ( vex(Y["n+_locklike_things"]) ) { qf_n=Y["n+_locklike_things"];
      ...
      if ( zs(0) && s(1,qf_n-1) &&
           ...
           qiz(ef_n+2,qf_n-1,"prl_vi prq_vi mest_vi mest_3e","qast") )
      { l[i]=omo2; Z[91]++; if(dbg){print "Z91"}; stopp=ret=1; continue }; #hit_5
    ...
    if ( weq(EF_n,cst) ) {
      ...
      { l[i]=omo2; Z[1360]++; if(dbg){print "Z1360"}; stopp=ret=1; continue }; #hit_1
    ...
    if ( weq(EF_n,cst) ) {
      ...
      { l[i]=omo1; Z[81]++; if(dbg){print "Z81"}; stopp=ret=1; continue }; #hit_114
    };
    cst="напоминать";
    if ( weq(EF_n,cst) ) {
      if ( bfa(ef_n+1,ef_n+4,"_castlelike_things") ) { wf_n=bfn;
         ...
         { l[i]=omo1; Z[85]++; if(dbg){print "Z85"}; stopp=ret=1; continue }; #hit_74
    ...
    if ( weq(EF_n,cst) ) {
      if ( veq(ef_n,3) && s(0,2) &&
           w(1,"с со") &&
           q(2,"suw_ro") )
      { l[i]=omo2; Z[186]++; if(dbg){print "Z186"}; stopp=ret=1; continue }; #hit_4
      if ( veq(ef_n,4) && s(0,3) &&
           w(1,"с со") &&
           q(2,"suw_ro") &&
           q(3,"nar_any prl_kred_sr") )
      { l[i]=omo2; Z[187]++; if(dbg){print "Z187"}; stopp=ret=1; continue }; #hit_3
    };
   ...
   if ( weq(EF_n,cst) ) {
     ...
     { l[i]=omo1; Z[1367]++; if(dbg){print "Z1367"}; stopp=ret=1; continue }; #hit_2
     if ( wf(ef_n+1,ef_n+4,cst2) && s(0,wfn-1) &&
         qir(1,ef_n-1,"isname nar_any suw_ro") &&
         qir(ef_n+1,wfn-1,"prl_tv mest_tv mest_3e prq_tv") )
     { l[i]=omo1; Z[1368]++; if(dbg){print "Z1368"}; stopp=ret=1; continue }; #hit_1
     if ( w(ef_n+1,"от") &&
         wf(ef_n+1,ef_n+4,cst3) && s(0,wfn-1) &&
        qir(1,ef_n-1,"isname nar_any suw_ro") &&
        qir(ef_n+2,wfn-1,"prl_ro mest_ro mest_3e prq_ro") )
     { l[i]=omo1; Z[1369]++; if(dbg){print "Z1369"}; stopp=ret=1; continue }; #hit_1
   };

   cst="выжечь выжигать";
   cst2="полностью";
   if ( weq(EF_n,cst) && s(0,ef_n-1) &&
         w(ef_n-1,cst2) &&
       qir(1,ef_n-2,"prl_vi mest_vi mest_3e mest_im isname suw_im") )
   { l[i]=omo1; Z[1371]++; if(dbg){print "Z1371"}; stopp=ret=1; continue }; #hit_1


   cst="быть вставать выбрать избрать именовать именоваться казаться объявить объявлять оказаться провозгласить провозглашать сделать служить стать считаться \
        являться окружить";
   cst8="быть стать";
   cst0="дверью клеткой ключом пеплом руинами целью центром замком";
   if ( weq(EF_n,cst) ) {
#    if ( weq(EF_n,cst8) ) { if ( s(ef_n,3) && bf(ef_n+1,4,cst) ) ef_n=bfn };
     bbn=wfn=bfn=qb_n="";
     if ( bfa(ef_n+1,ef_n+5,"_castlelike_things")||
           wf(ef_n+1,ef_n+5,cst0)||
        ( bfa(ef_n+1,ef_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) { if (bfn) qb_n=bfn; if (wfn) qb_n=wfn;

         if ( zs(0) && s(1,qb_n-1) &&
              vgl(Y["n-_lock_pe"],-4,-1) && s(Y["n-_lock_pe"],-1) &&
              qir(qb_n+1,-1,"prl_edmuim prl_mnim mest_vi mest_3e qast_any") )
         { qb_n=""; Z[93]++; if(dbg){print "Z93"}; }; #hit_29
         cst="ключом";
         if ( zs(0) && s(1,qb_n-1) &&
                w(qb_n,cst) && !w(qb_n+1,"к") )
         { qb_n=""; Z[94]++; if(dbg){print "Z94"}; }; #hit_26
      ...
      { l[i]=omo1; Z[95]++; if(dbg){print "Z95"}; stopp=ret=1; continue }; #hit_993
      if ( vex(qb_n) && zs(0) && s(1,qb_n-1) &&
             w(ef_n+1,"для") &&
             q(qb_n,"suw_tv prq_tv") &&
      ...
      { l[i]=omo1; Z[96]++; if(dbg){print "Z96"}; stopp=ret=1; continue }; #hit_34
      if ( vex(qb_n) && zs(0) && s(1,qb_n-1) &&
             q(qb_n,"suw_tv") &&
      ...
      { l[i]=omo1; Z[97]++; if(dbg){print "Z97"}; stopp=ret=1; continue }; #hit_2
      cst="быть стать";
   ...
   if ( qf(ef_n+1,ef_n+4,"prl_kred_mu prq_kred_mu") ) { qf_n=qfn;
       ...
       if (  w(qf_n+1,"по") &&
            ...
            qir(qf_n+2,wfn-1,"prl_da prq_da mest_da mest_3e") )
       { l[i]=omo1; Z[98]++; if(dbg){print "Z98"}; stopp=ret=1; continue }; #hit_1
       if ( w(qf_n+1,"от") &&
             wf(qf_n+2,qf_n+5,cst2) && s(0,wfn-1) &&
            qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
            qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       { l[i]=omo2; Z[99]++; if(dbg){print "Z99"}; stopp=ret=1; continue }; #hit_2
       if ( w(qf_n+1,"с со") &&
             wf(qf_n+2,qf_n+5,cst4) && s(0,wfn-1) &&
            qir(ef_n+1,qf_n-1,"mod_any nar_any prl_kred_sr gl_aux_be isname") &&
            qir(qf_n+2,wfn-1,"prl_ro prq_ro mest_ro mest_3e") )
       { l[i]=omo2; Z[100]++; if(dbg){print "Z100"}; stopp=ret=1; continue }; #hit_1
  
    };
 };
    # cst + _locklike_things по той же схеме
      cst="быть вставать выбрать избрать именовать именоваться казаться объявить объявлять оказаться провозгласить провозглашать сделать служить стать считаться являться";
    ...
    if ( weq(EF_n,cst) ) {
       wfn=bfn=""; if ( bfa(ef_n+1,ef_n+5,"_locklike_things")||wf(ef_n+1,ef_n+5,cst2) ) { if (bfn) qf_n=bfn; if (wfn) qf_n=wfn;
   
          if ( vgl(ef_n,1,3) && zs(0) && s(1,qf_n-1) &&
               vex(Y["tn_ok_vi"]) )
          { qf_n=""; Z[105]++; if(dbg){print "Z105"}; }; #hit_38
       ...
       if ( zs(0) && w(qf_n,cst0) && s(1,qf_n) ) {
          ...
          { l[i]=omo1; Z[106]++; if(dbg){print "Z106"}; stopp=ret=1; continue }; #hit_2
          if( qxs(qf_n+1,"на по",cst3,"в к") )
          { l[i]=omo1; Z[107]++; if(dbg){print "Z107"}; stopp=ret=1; continue }; #hit_1
       };
       ...
       { l[i]=omo2; Z[108]++; if(dbg){print "Z108"}; stopp=ret=1; continue }; #hit_2
       ...
       { l[i]=omo2; Z[109]++; if(dbg){print "Z109"}; stopp=ret=1; continue }; #hit_54
       if ( zs(0) && s(1,qf_n-1) &&
              q(qf_n,"suw_ro") &&
            qir(1,ef_n-1,"otsz_any mod_any gl_aux_be qast nar_any suw_ro isname") &&
       ...
       { l[i]=omo2; Z[110]++; if(dbg){print "Z110"}; stopp=ret=1; continue }; #hit_40
    ...
    if ( weq(EF_n,cst) ) {
      if ( wf(ef_n+1,ef_n+4,cst2) ) { qf_n=wfn;
        ...
        { l[i]=omo2; Z[101]++; if(dbg){print "Z101"}; stopp=ret=1; continue }; #hit_13
    ...
    if ( weq(EF_n,cst) ) {
      if ( q(ef_n,"prq_kred_mu") && wf(ef_n+1,ef_n+4,cst2) ) { qf_n=wfn;
        ...
        if ( s(0,qf_n-1) &&
             ...
             qir(ef_n+1,qf_n-1,"prl_tv prq_tv mest_tv mest_3e") )
        { l[i]=omo1; Z[101]++; if(dbg){print "Z101"}; stopp=ret=1; continue }; #hit_13
  ...
  if ( weq(EF_n,cst) ) {
    ...
    if ( vgl(ef_n,1,3) && s(0,ef_n-1) && w(ef_n+1,cst2) &&
         qir(1,ef_n-1,"gl_aux_be mod_any") )
    { l[i]=omo1; Z[1401]++; if(dbg){print "Z1401"}; stopp=ret=1; continue }; #hit_52

    cst2="ногтем пальцами руками рукой";
    if ( veq(ef_n,2) && w(1,cst2) && s(0,1) )
    { l[i]=omo2; Z[1402]++; if(dbg){print "Z1402"}; stopp=ret=1; continue }; #hit_1

    if ( veq(ef_n,1) &&
        sz_i(2) &&
          ba(3,"_lock_bratj_i_gl") && zs(0) && s(1,2) )
    { l[i]=omo2; Z[1403]++; if(dbg){print "Z1403"}; stopp=ret=1; continue };
    if ( veq(ef_n,1) &&
           w(2,"у") &&
           q(3,"suw_ro mest_ro mest_3e") &&
          bw(4,"кровь") && s(0,3) )
    { l[i]=omo2; Z[1404]++; if(dbg){print "Z1404"}; stopp=ret=1; continue };
    ...
    if ( ba(-2,"_lock_parts") &&
          ...
          w(1,"не") &&
        veq(ef_n,2) && s(-2,1) )
    { l[i]=omo2; Z[1406]++; if(dbg){print "Z1406"}; stopp=ret=1; continue };
   }; # брать взять --->>>:end
  };

break}; Y["FN_zamok_vi_esc_f"]=1; return ret }
