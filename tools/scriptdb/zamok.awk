 ...
 if( veq(Y["iwrd_case"],"iv") ) {
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[499]++; if(dbg){print "Z499"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_da") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[511]++; if(dbg){print "Z511"}; continue }; #check
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
               w(1,"на") &&
             wfa(2,5,"_lock_na_suw-vi") && s(0,wfn-1) &&
             qir(2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") )
        { l[i]=omo2; Z[512]++; if(dbg){print "Z512"}; continue }; #check
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[523]++; if(dbg){print "Z523"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[556]++; if(dbg){print "Z556"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"];
       ...
       { bf_n=bf_n+1; Z[581]++; if(dbg){print "Z581"}; };
       ...
       { l[i]=omo2; Z[591]++; if(dbg){print "Z591"}; continue };
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"ediv") &&
          prq_any(bf_n) && !q(bf_n,"prq_edmuim prq_edmutv prq_kred_mu") &&
             !exb(-1,"тот ни другой ") )
         { bf_n=""; Z[592]++; if(dbg){print "Z592"}; };
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
               ba(-1,"_castle_prl_geo") && s(-1) )
         { bf_n=""; Z[593]++; if(dbg){print "Z593"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[605]++; if(dbg){print "Z605", "-- TN_do_zamka_f"}; continue };
 ...
 if ( bfa(1,5,"_lock_parts") ) { bf_n=bfn;
    ...
    { bf_n=""; Z[627]++; if(dbg){print "Z627"}; };
    if ( vgl(bf_n,1,5) && s(1,bf_n-1) && (s(0)||se(0," — ")) && bw(bf_n-1,"визитный") )
    { bf_n=""; Z[628]++; if(dbg){print "Z628"}; };
 ...
 { l[i]=omo2; Z[643]++; if(dbg){print "Z643"}; continue };
 ...
 { Z[651]++; if(dbg){print "Z651", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[652]++; if(dbg){print "Z652", "-- FN_lock_prl_after_f"}; continue };
...
# };
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[660]++; if(dbg){print "Z660"}; continue } else
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[668]++; if(dbg){print "Z668", "-- is_CAP"}; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[675]++; if(dbg){print "Z675", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[676]++; if(dbg){print "Z676", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[677]++; if(dbg){print "Z677", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[678]++; if(dbg){print "Z678", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[679]++; if(dbg){print "Z679", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-3) &&
#           q(-2,"prq_vi") && q(-1,"suw_vi") )
#   { tn_ok=fufu=1; Z[680]++; if(dbg){print "Z680", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-4) &&
#           q(-3,"prq_vi") && q(-2,"suw_vi") && q(-1,"prl_vi mest_vi") )
#   { tn_ok=fufu=1; Z[681]++; if(dbg){print "Z681", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[682]++; if(dbg){print "Z682", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[683]++; if(dbg){print "Z683", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[684]++; if(dbg){print "Z684", "-- tn_ok OFF", el, "замок"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    { l[i]=omo2; Z[712]++; if(dbg){print "Z712"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[713]++; if(dbg){print "Z713"}; stopp=ret=1; continue }; #edit
    ...
    { l[i]=omo1; Z[726]++; if(dbg){print "Z726"}; stopp=ret=1; continue };
    if ( bba(tn-6,tn-2,"_suw_v_castle-vp") &&
         qir(bbn+1,tn-2,"prl_ro prq_ro mcop_ro suw_ro") &&
           q(tn-1,"suw_ro mest_ro mest_3e qast nar_napr isname") && s(bbn,tn-1) )
    { l[i]=omo1; Z[727]++; if(dbg){print "Z727"}; stopp=ret=1; continue };

    cst="без для из с";
    if ( bba(tn-6,tn-3,"_suw_v_castle-vp") &&
           w(bbn+1,cst) &&
         qir(bbn+2,tn-1,"suw_ro prl_ro prq_ro mest_ro mest_3e isname") && s(bbn,tn-1) )
    { l[i]=omo1; Z[728]++; if(dbg){print "Z728"}; stopp=ret=1; continue };
    if ( ba(tn-1,"_suw_v_castle-vp") && se(tn-1," — ") )
    { l[i]=omo1; Z[729]++; if(dbg){print "Z729"}; stopp=ret=1; continue };
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[754]++; if(dbg){print "Z754"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[756]++; if(dbg){print "Z756"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[762]++; if(dbg){print "Z762"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[768]++; if(dbg){print "Z768"}; };
...
!(narph_any(bbn)||w(bbn-1,"в на")) )
    ...
    { l[i]=omo2; Z[802]++; if(dbg){print "Z802"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[803]++; if(dbg){print "Z803"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst3) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[838]++; if(dbg){print "Z838"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[839]++; if(dbg){print "Z839"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[912]++; if(dbg){print "Z912"}; stopp=ret=1; continue };
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[931]++; if(dbg){print "Z931"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[940]++; if(dbg){print "Z940", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
...
#   { tn_ok = 1; Z[1075]++; if(dbg){print "Z1075", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1076]++; if(dbg){print "Z1076", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1077]++; if(dbg){print "Z1077", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1078]++; if(dbg){print "Z1078", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1079]++; if(dbg){print "Z1079", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1080]++; if(dbg){print "Z1080", "-- tn_ok ON", el, "замка"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1081]++; if(dbg){print "Z1081", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1082]++; if(dbg){print "Z1082", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1083]++; if(dbg){print "Z1083", "-- tn_ok OFF", el, "замка"};};
}
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1088]++; if(dbg){print "Z1088"}; stopp=ret=1; continue };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
             if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                  ...
                  vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[1094]++; if(dbg){print "Z1094"}; };
...
function FN_lock_prl_f(str_is_lock_prl,    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_is_lock_prl "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1106]++; if(dbg){print "Z1106", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };
    ...
    { Z[1108]++; if(dbg){print "Z1108", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    ...
    if ( iwrd == "замок" ) {
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1112]++; if(dbg){print "Z1112", "-- TN_v_zamok_f"}; stopp=ret=1; continue};
          ...
          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) ||
               ( bfa(wf_n+1,bf_n-1,"_castle_prl") && Q(bfn,"prl_kred_sr") ) ||
                 vex(Y["tn_ok_vi"]) ||
            ( isname(0) && s(-1) ) )
             { wf_n=bf_n=""; Z[1113]++; if(dbg){print "Z1113"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1116]++; if(dbg){print "Z1116", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,str_is_lock_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) || vex(Y["tn_ok_vi"]) )
            { wf_n=bf_n=""; Z[1117]++; if(dbg){print "Z1117"}; };
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1159]++; if(dbg){print "Z1159"}; }; #check_!под
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1160]++; if(dbg){print "Z1160"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[1161]++; if(dbg){print "Z1161"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
