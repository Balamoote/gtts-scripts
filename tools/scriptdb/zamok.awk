...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 ...
 # собрать слова
#if ( w(-1,"в") && s(-2,-1)  )
#{ getBFx(-3,-1,"_2v.txt"); Z[1]++; if(dbg){print "Z1"}; }; continue

#getBFx(-3,3,"_an.txt"); Z[2]++; if(dbg){print "Z2"}; continue
...
sz_iili(-3) &&
 ...
 if(iwrd=="замок" || iwrd=="замки") {
  ...
  if ( wb(-3,-1,"в во") && s(wbn,-1) ) {
    ...
    cst2="голень голова грудь колено ладонь ладошка лодыжка локоть ляжка нога ножка палец пальчик плечо поясница рука ручка стопа талия туловище шея";
