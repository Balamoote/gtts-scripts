...
#if ( q(1,"isname") && s(0) )
#{ print l[i+1] >> "_isname.txt"; Z[17]++; if(dbg){print "Z17"}; }; continue
 ...
 { Y["tn_ok_vi"]=1; Z[18]++; if(dbg){print "Z18"}; };
 if ( Y["tn_ok_vi"] )
 { Y["tn_ok_any"]=1; Z[19]++; if(dbg){print "Z19"}; };
...
#===============================================================================================================================================================
 ...
 # pointesque
 if ( q(0,"isname") && exf(1,"Копну и Бочку") && se(0,", ") )
...
#{ l[i]=omo1; Z[43]++; if(dbg){print "Z43"}; continue };
 ...
 if ( mest_it(-2) &&
      ...
      mest_it(1) &&
            w(2,"не") &&
          veq(Y["n+_castlelike_things"],3) && s(1,2) )
 ...
 if ( mest_it(-2) &&
      ...
      mest_it(1) &&
          ...
          veq(Y["n+_castlelike_things"],4) && s(1,3) )
 ...
 if ( bf(-3,-1,cst) ) { wf_n=bfn;
   if ( bfa(wf_n-5,wf_n-1,"_castlelike_things") ) { bf_n=bfn;
    ...
    if ( vgl(wf_n,-3,-1) && vgl(bf_n,wf_n-5,wf_n-1) && z(bf_n) &&
 ...
 if ( bf(1,4,cst) ) { bf_n=bfn;
   ...
   if ( bfa(bf_n+1,bf_n+5,"_castlelike_things")||
      ...
      ( bfa(bf_n+1,bf_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) {
       ...
       { bf_n=bb_n=""; Z[76]++; if(dbg){print "Z76"}; };

    if ( vgl(bf_n,1,4) && zs(0) && s(1,bb_n-1) &&
           q(bb_n,"suw_tv prq_tv") &&
         qir(1,bf_n-1,"otsz_any mod_any gl_aux_be qast nar_any suw_ro suw_edne isname suw_vi prl_vi mest_vi") &&
 ...
 if ( bb(-4,-1,cst) ) { bb_n=bbn; bf_n="";
   ...
   if ( vgl(Y["n+_castlelike_things"],1,5) ) { bf_n=Y["n+_castlelike_things"] } else {
    ...
    if ( vgl(bf_n,1,4) && zs(0) && s(1,bf_n-1) &&
           q(bb_n,"suw_tv") &&
           q(1,"narph_any") &&
         qiz(xwn+1,bb_n-1,"prl_tv prq_tv mest_tv mest_3e nar_any prl_kred_sr qast_any suw_da mest_da isname suw_edne","qast") )
 ...
 if (bb(1,3,cst)) { bf_n=bbn;
   ...
   if ( bfa(bf_n+1,bf_n+5,"_locklike_things")||wf(bf_n+1,bf_n+5,cst2) ) { if (bfn) bb_n=bfn; if (wfn) bb_n=wfn;
       ...
       if ( vgl(bf_n,1,3) && zs(0) && s(1,bb_n-1) &&
            vex(Y["tn_ok_vi"]) )
       { bf_n=bb_n=""; Z[81]++; if(dbg){print "Z81"}; };
    ...
    if ( vgl(bf_n,1,3) && zs(0) && w(bb_n,cst0) && s(1,bb_n) ) {
       ...
       if ( w(bb_n+1,"на") &&
           wf(bb_n+2,bb_n+4,cst3) &&
            w(wfn+1,"на") &&
          qir(bb_n+2,wfn-1,"prl_pr mest_pr mest_3e prq_pr") && s(bb_n+1,wfn) )
...
#{ l[i]=omo1; Z[178]++; if(dbg){print "Z178"}; continue };
 ...
 if ( vgl(Y["n_pre_na"],-6,-1) ) { wb_n=Y["n_pre_na"];
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[184]++; if(dbg){print "Z184"}; }; #check_!под

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[185]++; if(dbg){print "Z185"}; continue }; #check_!под
 };

 if ( wf(1,6,"для") ) { wf_n=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
      ...
      { Z[189]++; if(dbg){print "Z189"}; continue};
      if ( vex(Y["tn_ok_za"]) && TN_za_zamok_f() )
      { Z[190]++; if(dbg){print "Z190"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[191]++; if(dbg){print "Z191"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[192]++; if(dbg){print "Z192"}; continue};
      if ( vex(Y["tn_ok_o"]) && TN_o_zamok_f() )
      { Z[193]++; if(dbg){print "Z193"}; continue};
      if ( vex(Y["tn_ok_qerez"]) && TN_qerez_zamok_f() )
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { l[i]=omo2; Z[206]++; if(dbg){print "Z206"}; continue };
       ...
       if ( vgl(wf_n,2,5) && s(0,wf_n-1) && w(wf_n,"дома") )
       { wf_n=""; Z[207]++; if(dbg){print "Z207"}; };
       if ( vgl(wf_n,2,5) && s(0,wf_n-1) &&
             wa(wf_n,"_lock_suw-ro1") && w(wf_n+1,"в") && s(wfn) )
       { wf_n=""; Z[208]++; if(dbg){print "Z208"}; };
       if ( vgl(wf_n,2,5) && s(0,wf_n-1) && w(wf_n+1,"до") && s(wfn) )
       { wf_n=""; Z[209]++; if(dbg){print "Z209"}; };
       if ( vgl(wf_n,2,5) && s(0,wf_n-1) &&
            vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[210]++; if(dbg){print "Z210"}; continue };

    if ( vgl(wf_n,2,5) && s(0,wf_n-1) &&
           w(1,"для у от") &&
         qir(2,wf_n-1,"prl_ro prq_ro mest_ro mest_3e") )
    { l[i]=omo2; Z[211]++; if(dbg){print "Z211"}; continue };
#   if ( vgl(wf_n,2,5) && s(0,wfn-1) &&
#           w(1,"от") &&
#         qir(2,wf_n-1,"prl_ro prq_ro mest_ro mest_3e") )
#   { l[i]=omo2; Z[212]++; if(dbg){print "Z212"}; continue };
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[229]++; if(dbg){print "Z229"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
...
#{ l[i]=omo1; Z[243]++; if(dbg){print "Z243"}; continue };
 ...
 if ( wf(1,3,"с со") ) { wf_n=wfn;
    if ( vgl(wf_n,1,3) &&
         wfa(wf_n+1,wf_n+4,"_lock_s_suw-tv") && s(0,wfn-1) &&
         qir(1,wf_n-1,"prl_vi mest_vi") &&
         qir(wf_n+1,wfn-1,"prl_tv prq_tv mest_tv mest_3e qik_tv") && W_w(wfn+1,"до по") )
...
#    wa(1,"_lock_s_suw-tv") &&
#    bw(2,"который") && s(1) )
#{ l[i]=omo1; Z[246]++; if(dbg){print "Z246"}; continue }; #check!
 # _lock_s_suw-tv:end
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( w(-1,"дисковой") && s(1) )
    { l[i]=omo2; Z[257]++; if(dbg){print "Z257"}; stopp=ret=1; continue };
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[265]++; if(dbg){print "Z265"}; };
    ...
    # _lock_prl:start
    if ( vex(Y["iwrd_form"],"ediv") && !ba(-1,"_castle_prl_geo")) {
       ...
       if ( bw(1,cst1) ) { wf_n=1;
          if ( bfa(2,5,"_lock_prl") ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],2,bf_n-1) || vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[299]++; if(dbg){print "Z299"}; };
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[327]++; if(dbg){print "Z327", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[339]++; if(dbg){print "Z339", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[340]++; if(dbg){print "Z340", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[341]++; if(dbg){print "Z341", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[342]++; if(dbg){print "Z342", "-- TN_qerez_zamok_f"}; continue };
  ...
  # ниже "замок" без собственного предлога
   ...
   cst1="покрывать покрываться покрыть покрыться";
   cst2="налетом росой";
   ...
   { l[i]=omo1; Z[353]++; if(dbg){print "Z353"}; continue };
    ...
    { l[i]=omo2; Z[361]++; if(dbg){print "Z361"}; stopp=ret=1; continue };
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( bf(1,5,cst) && s(0,bfn-1) ) { bf_n=bfn;
       ...
       if ( ( qxw(bfn+1,"на",cst1)||
              qxw(bfn+1,"по",cst2)||
              qxw(bfn+1,"до",cst4)||
              qxw(bfn+1,"в",cst5) ) &&
              qir(1,bf_n-1,"qast_any mest_3e nar_any prl_kred_sr gl_aux_be mod_any isname") && s(bfn) )
    ...
    if ( vgl(Y["n-_lock_pe"],-7,-1) ) { bb_n=Y["n-_lock_pe"];
       ...
       if ( vgl(bb_n,-7,-1) &&
             bw(bb_n,cst) &&
             wb(bb_n-5,bb_n-1,"перед") && s(wbn,-1) &&
            qir(wbn+1,bb_n-1,"isname prl_ro mest_ro suw_ro") &&
            qir(bb_n+1,-1,"prl_vi prq_vi mest_vi mest_3e qast nar_any prl_kred_sr") )
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
            qaw(bf_n+1,"вокруг","себя") &&
            qir(0,bf_n-1,"gl_aux_be mod_any prl_kred_sr prl_kred_mu nar_any isname") && s(0,awn-1) )
       ...
       if ( vgl(bf_n,1,6) &&
            ...
            qir(bf_n+1,wfn-1,"prl_vi prq_vi mest_vi mest_3e qast nar_any prl_kred_sr isname") )
       ...
       cst="вплавить вплавлять врезать вырезать";
       cst1="скала";
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
...
#         w(bfn+1,"в") &&
#        wf(bfn+2,bfn+5,cst1) &&
#       qir(1,bfn-1,"gl_aux_be mod_any nar_any prl_kred_sr prl_kred_mu isname") &&
#       qir(bfn+2,wfn-1,"prl_vi mest_vi mest_3e prq_vi") && s(0,wfn-1) )
#   { l[i]=omo1; Z[463]++; if(dbg){print "Z463"}; continue };
       ...
       { l[i]=omo1; Z[467]++; if(dbg){print "Z467"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[490]++; if(dbg){print "Z490"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[526]++; if(dbg){print "Z526"}; };
      ...
      { bb_n=""; Z[532]++; if(dbg){print "Z532"};};
       ...
       if ( vgl(bb_n,-7,-1) && s(bb_n,-1) &&
           (qaw(-1,"тот","или","иной этот") ||
            ...
            qaw(-1,"еще","хоть","один")||
            qaw(-1,"хотя","бы")||
            qaw(-1,"хоть еще","один такой какой-то какой-нибудь") ) &&
            vlt(awn,0) &&
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"mniv") &&
          prq_any(bf_n) && !q(bf_n,"prq_mnvi prq_mntv prq_krmn") )
...
#   # _castle_lock_after:start
 ...
 if ( w(1,"на") &&
    bfa(2,5,"_lock_na_suw") && s(0,bfn-1) &&
    qir(2,bfn-1,"prl_vi prl_pr mest_vi mest_pr prq_vi prq_pr") )
 { l[i]=omo2; Z[673]++; if(dbg){print "Z673"}; continue }; #check ислючения "в"
 if ( w(1,"у") && Ww_(-1,"в") &&
 ...
 { l[i]=omo1; Z[685]++; if(dbg){print "Z685"}; continue };
  ...
  if ( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( q(1,"nar_mest") &&
         q(2,"prl_edmuim prl_edmutv prl_kred_mu") && ba(2,"_lock_prl_after") && s(0,1) )
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[693]++; if(dbg){print "Z693"}; continue } else
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[702]++; if(dbg){print "Z702", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
    ...
    { tn = xsn; Z[713]++; if(dbg){print "Z713", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
         bam(qfn,"_castle_pe _lock_pe") &&
           q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
    { tn_ok = ""; Z[714]++; if(dbg){print "Z714", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[715]++; if(dbg){print "Z715", "-- tn_ok OFF", el, "замок"};};
}
...
function TN_v_zamok_f(    ret, tn, stopp) { stopp=v2s(Y["TN_v_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_v"]) ) { tn=Y["n_pre_v"];
    ...
    if ( p(-3) && bw(-2,cst) && veq(tn,-1) && (p(0)||sz_iili(1)) )
    ...
    if ( vgl(Y["tn-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["tn-_gl_v_lock-iv"];
       ...
       { l[i]=omo2; Z[799]++; if(dbg){print "Z799"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[805]++; if(dbg){print "Z805"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[811]++; if(dbg){print "Z811"}; };
    ...
    if ( qb(tn-8,tn-2,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
        ...
        if ( vgl(qb_n,tn-8,tn-4) && veq(bb_n,tn-6) &&
    ...
    if ( veq(Y["tn-_gl_v_lock-iv"],tn-3) &&
      pre_vi(tn-2) &&
    ...
    cst="быстрее быстро все назад обратно сейчас скорее сначала сперва срочно теперь и или";
    cst1="прямо сразу";
    if ( w(tn-1,cst) &&
         p(tn-2,",") && p(0) )
    ...
    { l[i]=omo2; Z[852]++; if(dbg){print "Z852"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[853]++; if(dbg){print "Z853"}; stopp=ret=1; continue }; #check
    ...
    if ( vgl(Y["tn-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["tn-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[892]++; if(dbg){print "Z892"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[893]++; if(dbg){print "Z893"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
    ...
    if ( qb(tn-6,tn-2,"pre_da") ) { qb_n=qbn; if ( bba(qb_n-9,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,qb_n-9,qb_n-1) && vgl(qb_n,tn-6,tn-2) &&
    ...
    if ( qb(tn-7,tn-2,"pre_tv") ) { qb_n=qbn; if ( bba(qb_n-9,qb_n-1,"_gl_v_castle-iv") ) { bb_n=bbn;
       ...
       if ( vgl(bb_n,qb_n-6,qb_n-1) && vgl(qb_n,tn-6,tn-2) && hwy(bb_n+1,"ка") &&
    ...
    #
    if ( q(tn-1,"mqast") &&
       veq(Y["tn-_gl_v_castle-iv"],xwn-1) && s(xwn-1) && zs(tn-1) )
   ...
   if ( vex(Y["tn_ok_na"]) ) { tn=Y["n_pre_na"];
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[1001]++; if(dbg){print "Z1001"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[1010]++; if(dbg){print "Z1010", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
...
function TN_pod_zamok_f(    ret, stopp) { stopp=v2s(Y["TN_pod_zamok_f"]);
  ...
  if ( vex(Y["tn_ok_pod"]) ) { tn=Y["n_pre_pod"];
   ...
   if ( bw(-2,cst) &&
    ...
    #
    if ( qb(tn-5,tn-1,"pre_ro") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
    ...
    if ( qb(tn-5,tn-1,"pre_pr") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
    ...
    if ( qb(tn-5,tn-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_pod_lock") ) { bb_n=bbn;
...
#      if ( vgl(qb_n,tn-5,tn-1) && vgl(bb_n,qbn-5,qbn-1) && s(bb_n,tn-2) && zs(tn-1) &&
#           q(bb_n+1,"pre_tv") &&
#           qiz(bb_n+2,qb_n-1,"prl_tv prq_tv mest_tv mest_3e qast prl_kred_sr","suw_tv suw_edne isname") &&
#           qir(qb_n+1,tn-1,"mest_ro suw_tv") )
