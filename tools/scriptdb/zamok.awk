...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 ...
 # собрать слова
#if ( q(1,"suw_odedro suw_odmnro") && Q(1,"prl_ro prl_kred_mu prl_kred_sr prl_kred_sr mest_ro") && s(0)  )
#{ print l[i+1] >> "_sw.txt"; Z[1]++; if(dbg){print "Z1"}; }; continue
#{ getBFx(1,1,"_sw.txt"); Z[2]++; if(dbg){print "Z2"}; }; continue
