...
sz_iili(-2) &&
 ...
 if ( bfa(1,4,"_locklike_things") ) { bf_n=bfn;
    ...
    if ( z(0) && veq(bf_n,1) && !(isname(1) && bw(1,"люк")) ) {
       if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[161]++; if(dbg){print "Z161"}; continue }
    else { l[i]=omo2; Z[162]++; if(dbg){print "Z162"}; continue };};
 };
...
sz_iili(1) &&
 ...
 if ( FN_zamok_suwro_f() )
 { Z[177]++; if(dbg){print "Z177", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( w(1,"из") ) { wf_n=1;
    ...
    { wf_n=""; Z[185]++; if(dbg){print "Z185"}; continue };
 ...
 if ( wf(1,6,"для") ) { wf_n=Y["n+_pre_dlya"]=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
      ...
      { wf_n=bf_n=""; Z[190]++; if(dbg){print "Z190"}; };
      if ( wa(1,"_lock_suw-ro") && s(0) )
      { wf_n=bf_n=""; Z[191]++; if(dbg){print "Z191"}; };
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[192]++; if(dbg){print "Z192"}; };
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[193]++; if(dbg){print "Z193", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[194]++; if(dbg){print "Z194", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[195]++; if(dbg){print "Z195", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wb(-5,-2,"для") ) { wb_n=Y["n-_pre_dlya"]=wbn;
   if ( wba(wb_n+1,-1,"_castle_suw-ro1") ) { bb_n=wbn;
    ...
    { l[i]=omo1; Z[198]++; if(dbg){print "Z198"}; continue }; #checkro2
   ...
   if ( wba(wb_n+1,-1,"_castle_suw-ro2") ) { bb_n=wbn;
    ...
    if ( vgl(wb_n,-5,-1) && vgl(bb_n,wb_n+1,-1) && s(wb_n-1,-1) &&
         ...
         qir(wb_n+1,bb_n-1,"prl_ro mest_ro mest_3e prq_ro") &&
         qir(bb_n+1,-1,"prl_any mest_any") )
    { l[i]=omo1; Z[199]++; if(dbg){print "Z199"}; continue }; #checkro2
...
#
 ...
 if ( vgl(Y["n_pre_na"],-8,-2) ) { wb_n=Y["n_pre_na"];
   if ( wfa(wb_n+1,-1,"_lock_na_suw-vi") ) { bf_n=wfn;
      ...
      { bf_n=""; Z[268]++; if(dbg){print "Z268"}; };
#       cst0="держать";
#       if ( bb(wb_n-4,wb_n-1,cst0) && s(bbn,-1) &&
#           mwba(bf_n,bbn,"M_lock_na_suw-vi_gl_castle_pe") )
#       { wb_n=bf_n=""; Z[269]++; if(dbg){print "Z269"}; };
 ...
 if ( w(1,"на") ) {
   if ( wfa(2,5,"_lock_na_suw-vi") ) { wf_n=wfn;
     ...
     { Z[273]++; if(dbg){print "Z273"}; continue };
   ...
   if ( bba(-5,-1,"_gl_4_lock_na_suw-vi") ) { bb_n=bbn;
     if ( sos(-20,bb_n-1) && bfa(son+1,bb_n-1,"_lock_na_suw") ) { wb_n=bfn;

     if ( eos(2,5) && s(0,eon-1) &&
            q(eon,"prl_vi mest_vi mest_3e") && rodeq_sw_pl(wb_n,eon) )
     { l[i]=omo2; Z[275]++; if(dbg){print "Z275"}; continue };#uphere
   };};
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[288]++; if(dbg){print "Z288", "-- TN_na_zamok_f"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
 ...
 { l[i]=omo1; Z[308]++; if(dbg){print "Z308"}; continue };
  ...
  ######################################################################################################
    ...
    { Z[309]++; if(dbg){print "Z309", "-- FN_castle_prl_f"}; continue };

    # основные прилагательные -- lock
    if ( FN_lock_prl_f("_lock_prl") )
    { Z[310]++; if(dbg){print "Z310", "-- FN_lock_prl_f(\"_lock_prl\")"}; continue };
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[326]++; if(dbg){print "Z326"}; };
    ...
    if ( bw(1,cst) &&
          w(2,"из") && s(0,1) )
    { l[i]=omo1; Z[338]++; if(dbg){print "Z338"}; stopp=ret=1; continue };
    ...
    if ( bw(1,cst) && s(0) &&
       (qxs(2,"вовсе совсем никак отнюдь","не")||
        qxs(2,"не") ) &&
          w(xsn+1,"из") && s(xsn+1) )
    { l[i]=omo1; Z[341]++; if(dbg){print "Z341"}; stopp=ret=1; continue };
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[357]++; if(dbg){print "Z357", "-- TN_v_zamok_f"}; continue };
    ...
    { l[i]=omo1; Z[358]++; if(dbg){print "Z358"}; continue }; # без tn!
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[361]++; if(dbg){print "Z361", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[372]++; if(dbg){print "Z372", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[373]++; if(dbg){print "Z373", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[374]++; if(dbg){print "Z374", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[375]++; if(dbg){print "Z375", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[376]++; if(dbg){print "Z376", "-- TN_qerez_zamok_f"}; continue };
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[525]++; if(dbg){print "Z525"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_da") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
        ...
        { l[i]=omo2; Z[537]++; if(dbg){print "Z537"}; continue }; #check
        if ( vgl(qb_n,-7,-2) && vgl(bb_n,qb_n-5,qb_n-1) && s(bb_n,-1) &&
               w(1,"на") &&
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[547]++; if(dbg){print "Z547"}; };
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
              w(qb_n,"в во") && qir(qb_n+1,-1,"isname") && ba(bb_n,"_gl_v_castle-iv") )
       { bb_n=qb_n=""; Z[548]++; if(dbg){print "Z548"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[581]++; if(dbg){print "Z581"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"];
       ...
       { bf_n=""; Z[605]++; if(dbg){print "Z605"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[606]++; if(dbg){print "Z606"}; };
       ...
       { l[i]=omo2; Z[616]++; if(dbg){print "Z616"}; continue };
         ...
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) && veq(Y["iwrd_form"],"ediv") &&
          prq_any(bf_n) && !q(bf_n,"prq_edmuim prq_edmutv prq_kred_mu") &&
             !exb(-1,"тот ни другой ") )
         { bf_n=""; Z[617]++; if(dbg){print "Z617"}; };
         if ( vgl(bf_n,1,6) && s(0,bf_n-1) &&
               ba(-1,"_castle_prl_geo") && s(-1) )
         { bf_n=""; Z[618]++; if(dbg){print "Z618"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[643]++; if(dbg){print "Z643", "-- TN_do_zamka_f"}; continue };
 ...
 { Z[689]++; if(dbg){print "Z689", "-- FN_castle_prl_after_f"}; continue };

 if ( FN_lock_prl_f("_lock_prl_after") )
 { Z[690]++; if(dbg){print "Z690", "-- FN_lock_prl_f(\"_lock_prl_after\")"}; continue };
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[694]++; if(dbg){print "Z694"}; continue } else
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[702]++; if(dbg){print "Z702", "-- is_CAP"}; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[703]++; if(dbg){print "Z703", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[704]++; if(dbg){print "Z704", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[705]++; if(dbg){print "Z705", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[706]++; if(dbg){print "Z706", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[707]++; if(dbg){print "Z707", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[708]++; if(dbg){print "Z708", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[709]++; if(dbg){print "Z709", "-- tn_ok ON", el, "замок"};};
...
#   { tn_ok=fufu=1; Z[714]++; if(dbg){print "Z714", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-4) &&
#           q(-3,"prq_vi") && q(-2,"suw_vi") && q(-1,"prl_vi mest_vi") )
#   { tn_ok=fufu=1; Z[715]++; if(dbg){print "Z715", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[716]++; if(dbg){print "Z716", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[717]++; if(dbg){print "Z717", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[718]++; if(dbg){print "Z718", "-- tn_ok OFF", el, "замок"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    { l[i]=omo2; Z[746]++; if(dbg){print "Z746"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[747]++; if(dbg){print "Z747"}; stopp=ret=1; continue }; #edit
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[788]++; if(dbg){print "Z788"}; stopp=ret=1; continue }; #check
       ...
       { l[i]=omo2; Z[790]++; if(dbg){print "Z790"}; stopp=ret=1; continue }; #inspect
       if ( vgl(bb_n,tn-6,tn-2) &&
       ...
       { l[i]=omo2; Z[796]++; if(dbg){print "Z796"}; stopp=ret=1; continue }; #inspect
       if ( veq(bb_n,tn-2) &&
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[803]++; if(dbg){print "Z803"}; };
    ...
    cst1="карта ключ ключик код отвертка отмычка скрепка спичка";
    cst2="жалом картой ключиком ключом отверткой отмычкой ремнем скрепкой спичкой";
...
!(narph_any(bbn)||w(bbn-1,"в на")) )
    ...
    { l[i]=omo2; Z[839]++; if(dbg){print "Z839"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[840]++; if(dbg){print "Z840"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst2) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[876]++; if(dbg){print "Z876"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[877]++; if(dbg){print "Z877"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[952]++; if(dbg){print "Z952"}; stopp=ret=1; continue };
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[971]++; if(dbg){print "Z971"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[982]++; if(dbg){print "Z982", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
...
#         qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
#           q(-1,"mest_vip3e suw_vi") )
#   { tn_ok = 1; Z[1116]++; if(dbg){print "Z1116", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_tv") &&
#         qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
#           q(-1,"suw_tv") )
#   { tn_ok = 1; Z[1117]++; if(dbg){print "Z1117", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[1118]++; if(dbg){print "Z1118", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[1119]++; if(dbg){print "Z1119", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1120]++; if(dbg){print "Z1120", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1121]++; if(dbg){print "Z1121", "-- tn_ok ON", el, "замка"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ского$|ного$") )
    { tn_ok = 1; Z[1122]++; if(dbg){print "Z1122", "-- tn_ok ON", el, "замка"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1123]++; if(dbg){print "Z1123", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1124]++; if(dbg){print "Z1124", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1125]++; if(dbg){print "Z1125", "-- tn_ok OFF", el, "замка"};};
}
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1130]++; if(dbg){print "Z1130"}; stopp=ret=1; continue };
    if ( iwrd == "замок") {
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
                if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                     ...
                     vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1137]++; if(dbg){print "Z1137"}; };
                if ( s(bf_n,bf_n+1) && sz_iili(bf_n+1) && ba(bf_n+2,"_lock_prl") )
                { wf_n=bf_n=""; Z[1138]++; if(dbg){print "Z1138"}; };
          ...
          if ( bf(1,3,cst1) ) { wf_n=bfn;
             if ( bfa(wf_n+1,wf_n+5,str_castle) ) { bf_n=bfn;
               if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                    bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                    vex(Y["tn_ok_vi"]) )
                { wf_n=bf_n=""; Z[1143]++; if(dbg){print "Z1143"}; };
...
function FN_lock_prl_f(str_lock,    ret, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_lock "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Z[1168]++; if(dbg){print "Z1168", "-- _castle_prl_geo -- break"}; stopp=1; ret=0; continue };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1169]++; if(dbg){print "Z1169", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1170]++; if(dbg){print "Z1170", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    # _lock_prl:escape
    ...
    # _lock_prl:start
       ...
       if ( iwrd == "замок") {
          ...
          { l[i]=omo2; Z[1172]++; if(dbg){print "Z1172"}; stopp=ret=1; continue }; #uphere
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1209]++; if(dbg){print "Z1209"}; }; #check_!под
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1210]++; if(dbg){print "Z1210"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[1211]++; if(dbg){print "Z1211"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
