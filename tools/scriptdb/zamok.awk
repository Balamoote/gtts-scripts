...
{makebookvars();for(i in wpos){makewposvars();if(tolower(l[i])!=iwrd)continue; # header2
 ...
 if ( base(1,"быть стать являться") &&
        bf(2,5,cst) && s(0,bfn-1) && Qf(2,bfn-1,"pre_any sz_iili")  )
 ...
 cst="бандитов барона баронов богов боса брата вампира владетеля владыки володаря волшебника генерала герцога герцогов госпитальеров господаря графа графини графства грёз демона дона донны дочери дочки дракона дяди епископа заклинателей зла змея императора иоанитов клана князей князя колдуна колдуньи коннетабля королевства королевы королей короля кронпринца лорда людоеда людоедовш магистра магната мадам маркграфа маркиза маркизы матери милорда мужа надежд надежды наместника некроманта отца отчаяния отшельника правителя предков принца принцев принцессы пэра родителей родичей родственников рыцарей рыцаря семейства семьи скэгов сэра сёгуна тамплиеров тролля ужасов феи фей феодала хана хранителей царевны царицы царя чародея шевалье штурмом эрла эрцгерцога ярла";
 if ( wf(1,4,cst) && s(0,wfn-1) && Qf(1,wfn-1,"pre_any sz_iili") )
 { l[i]=omo1; Z[12]++; if(dbg){print "Z12"}; continue };

 cst="автомата аркебузы багажника барака безопасности браслета бюстгальтера винтовки ворот входа гаража гермолюка двери дверцы домофона забрала зажигания застёжки казённика калитки камеры капота квартиры кейса клатча клетки комбинезона контейнера костюма кошелька крышки куртки лифа лифчика люка мушкета наручника наручников орудия пальто пальцев пальчиков панели перчатки платья портфеля предплечья промежности пулемёта пушки ремня ружья рюкзака саквояжа сарая сейфа секретера системы спальника сумки сумочки сундучка украшения хода цепи цепочки чемодана ширинки шкатулки шкафа шкафчика шлема штанов ящика";
 ...
 cst="ворота дверь клетка ключ ключик петля целочка цепь";
 if ( q(1,"sz_iili") &&
   base(2,cst) && s(0,1) )
 { l[i]=omo2; Z[57]++; if(dbg){print "Z57"}; continue };
 if ( q(-1,"sz_iili") &&
   base(-2,cst) && s(-2,-1) )
 ...
 #
    ...
    cst="аллея бегство визит возвращение ворота вход въезд дверь дорога доставка заключение заточение калитка переезд переход поездка поход прибытие приглашение приезд проникновение путь тропа тропинка ход экскурсия";
    if ( base(-2,cst) &&
            w(-1,"в") && s(-2,-1) )
    ...
    cst="бегом верхом домой пешком ползком";
    if ( base(-2,cst) &&
            w(-1,"в") && s(-2,-1) )
   ...
   cst1="когда крепко наконец основательно плотно";
    ...
    cst1="булавка ключ ключик отмычка сверло";
