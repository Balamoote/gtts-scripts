...
#getBFx(-3,3,"_an.txt"); Z[11]++; if(dbg){print "Z11"}; continue
 ...
 if ( qy(-1,"qast_us") && prl_any(-1-qyn) && isname(-1-qyn) && ba(-1-qyn,"_castle_prl_forced") && s(-1-qyn,-1) )
 { l[i]=omo1; Z[17]++; if(dbg){print "Z17"}; continue };
 if ( qy(-1,"qast_us") && prl_any(-1-qyn) && ba(-1-qyn,"_lock_prl_forced") && s(-1-qyn,-1) )
...
#{ Y["tn_ok_vi"]=1; Z[21]++; if(dbg){print "Z21", "-- tn_ok_vi=1"}; };
#if ( Y["tn_ok_do"] )
#{ Y["tn_ok_ro"]=1; Z[22]++; if(dbg){print "Z22", "-- tn_ok_ro=1"}; };
#if ( Y["tn_ok_vi"]||Y["tn_ok_do"] )
#{ Y["tn_ok_any"]=1; Z[23]++; if(dbg){print "Z23", "-- tn_ok_any=1"}; };
...
#if ( vex(Y["tn_ok_v"]) ) {
#   if ( TN_v_zamok_f() )
#   { Z[26]++; if(dbg){print "Z26", "-- TN_v_zamok_f"}; continue };
#};
#if ( vex(Y["tn_ok_za"]) ) {
#   if ( TN_za_zamok_f() )
#   { Z[27]++; if(dbg){print "Z27", "-- TN_za_zamok_f"}; continue };
#};
#if ( vex(Y["tn_ok_na"]) ) {
#   if ( TN_na_zamok_f() )
#   { Z[28]++; if(dbg){print "Z28", "-- TN_na_zamok_f"}; continue };
#};
#if ( vex(Y["tn_ok_pod"]) ) {
#   if ( TN_pod_zamok_f() )
#   { Z[29]++; if(dbg){print "Z29", "-- TN_pod_zamok_f"}; continue };
#};
#if ( vex(Y["tn_ok_o"]) ) {
#   if ( TN_o_zamok_f() )
#   { Z[30]++; if(dbg){print "Z30", "-- TN_o_zamok_f"}; continue };
#};
#if ( vex(Y["tn_ok_qerez"]) ) {
#   if ( TN_qerez_zamok_f() )
#   { Z[31]++; if(dbg){print "Z31", "-- TN_qerez_zamok_f"}; continue };
#};
...
#{ l[i]=omo1; Z[51]++; if(dbg){print "Z51"}; continue };
 ...
 if ( vgl(Y["n+_castlelike_things"],2,5) ) { bf_n=Y["n+_castlelike_things"];
    ...
    { Z[64]++; if(dbg){print "Z64"}; continue };
 ...
 if ( wf(2,6,"собой") && bw(wfn-1,"представлять") ) { wf_n=wfn;
      ...
      { Z[80]++; if(dbg){print "Z80", "-- TN_v_zamok_f"}; continue};
 ...
 if ( bf(1,4,cst) ) { bf_n=bfn;
   ...
   if ( bfa(bf_n+1,bf_n+5,"_castlelike_things")||
      ...
      ( bfa(bf_n+1,bf_n+5,"_prq_castle") && q(bfn,"prq_tv") ) ) {
       ...
       { bf_n=bb_n=""; Z[85]++; if(dbg){print "Z85"}; };
       if ( vgl(bf_n,1,4) && zs(0) && s(1,bb_n-1) &&
              w(bb_n,"ключом") && !w(bb_n+1,"к") )
       { bf_n=bb_n=""; Z[86]++; if(dbg){print "Z86"}; };
 ...
 if (bb(1,3,cst)) { bf_n=bbn;
   if ( bfa(bf_n+1,bf_n+5,"_locklike_things")||wf(bf_n+1,bf_n+5,cst2) ) { if (bfn) bb_n=bfn; if (wfn) bb_n=wfn;
       ...
       if ( vgl(bf_n,1,3) && zs(0) && s(1,bb_n-1) &&
            vex(Y["tn_ok_vi"]) )
       { bf_n=bb_n=""; Z[97]++; if(dbg){print "Z97"}; };
 ...
 if ( bfa(1,6,"_castle_environs") ) { bf_n=Y["n+_castle_environs"]=bfn;
    ...
    { bf_n=""; Z[119]++; if(dbg){print "Z119"}; };
    if ( vgl(bf_n,1,6) &&
         bam(-1,"_lock_pe _lock_prl") && s(-1) )
    { bf_n=""; Z[120]++; if(dbg){print "Z120"}; };
...
sz_iili(-2) &&
 ...
 if ( bfa(1,4,"_locklike_things") ) { bf_n=bfn;
    ...
    if ( qq(-1,1) && z(0) && s(-1) &&
         veq(bf_n,2) && s(1) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[145]++; if(dbg){print "Z145"}; continue } else
    ...
    if ( q(1,"sz_iili") &&
       ...
       veq(bf_n,3) && s(0,2) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[152]++; if(dbg){print "Z152"}; continue } else
    ...
    if ( z(0) && veq(bf_n,1) && !(isname(1) && bw(1,"люк")) ) {
            if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
            { Z[156]++; if(dbg){print "Z156"}; continue } else
...
sz_iili(1) &&
 ...
 if ( wb(-6,-2,"в во") &&
     wfa(wbn+1,-1,"_lock_v_suw-pr") && zs(wfn) && s(wbn,wfn-1) && s(wfn+1,-1) &&
     qir(wbn+1,wfn-1,"prl_pr mest_pr mest_3e prq_pr") &&
     qir(wfn+1,-1,"gl_need gl_vzed gl_aux_be prl_vi mest_vi mest_3e prq_vi qast_any") &&
      sv(-1,"\\(") && Q_w(1,"isname") )
...
#
 ...
 #######################################################################################################################
 if ( FN_zamok_suwro_f() )
 { Z[224]++; if(dbg){print "Z224", "-- FN_zamok_suwro_f"}; continue };
 ...
 if ( wf(1,6,"для") ) { wf_n=Y["n+_pre_dlya"]=wfn;
   if ( wfa(wf_n+1,wf_n+4,"_castle_suw-ro2") ) { bf_n=wfn;
    ...
    if ( vgl(wf_n,1,3) && vgl(bf_n,wf_n+1,wf_n+4) && s(0,bf_n-1) ) {
      ...
      { wf_n=bf_n=""; Z[226]++; if(dbg){print "Z226"}; };
      if ( wa(1,"_lock_suw-ro") && s(0) )
      { wf_n=bf_n=""; Z[227]++; if(dbg){print "Z227"}; };
      if ( qsb(-10,-1,",") && bb(sbn-3,sbn,"дверь люк") && s(sbn-3,sbn-1) )
      { wf_n=bf_n=""; Z[228]++; if(dbg){print "Z228"}; };
      if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
      { Z[229]++; if(dbg){print "Z229", "-- TN_v_zamok_f"}; continue};
      if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
      { Z[230]++; if(dbg){print "Z230", "-- TN_na_zamok_f"}; continue};
      if ( vex(Y["tn_ok_pod"]) && TN_pod_zamok_f() )
      { Z[231]++; if(dbg){print "Z231", "-- TN_pod_zamok_f"}; continue};
 ...
 if ( wb(-5,-2,"для") ) { wb_n=Y["n-_pre_dlya"]=wbn;
   if ( wba(wb_n+1,-1,"_castle_suw-ro1") ) { bb_n=wbn;
    ...
    { l[i]=omo1; Z[234]++; if(dbg){print "Z234"}; continue }; #checkro2
   ...
   if ( wba(wb_n+1,-1,"_castle_suw-ro2") ) { bb_n=wbn;
    ...
    if ( vgl(wb_n,-5,-1) && vgl(bb_n,wb_n+1,-1) && s(wb_n-1,-1) &&
         ...
         qir(wb_n+1,bb_n-1,"prl_ro mest_ro mest_3e prq_ro") &&
         qir(bb_n+1,-1,"prl_any mest_any") )
    { l[i]=omo1; Z[235]++; if(dbg){print "Z235"}; continue }; #checkro2
 ...
 if ( wfa(1,5,"_lock_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[236]++; if(dbg){print "Z236"}; };

       if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
            vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
       { Z[237]++; if(dbg){print "Z237", "-- TN_v_zamok_f"}; continue };
 ...
 if ( w(1,"с со") ) {
     ...
     { Z[250]++; if(dbg){print "Z250", "-- TN_na_zamok_f"}; continue } else {
     # _castle_s_suw-ro:start
     if ( zs(0) &&
 ...
 if( veq(Y["iwrd_case"],"iv") ) {
    ...
    if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_castle_environs") ) { bb_n=bbn;
         ...
         { bb_n=qb_n=""; Z[282]++; if(dbg){print "Z282"}; };
    ...
    if ( bb(-5,-1,cst) ) { wb_n=bbn;
      ...
      if ( bba(wb_n-5,wb_n-1,"_lock_na_suw") ) { bb_n=bbn;
       if ( vgl(wb_n,-5,-1) && vgl(bb_n,wb_n-5,wb_n-1) &&
       ...
       { l[i]=omo2; Z[291]++; if(dbg){print "Z291"}; continue };#urhere
    };};
#if ( wb(-6,-2,"в во") &&
#    wfa(wbn+1,-1,"_lock_v_suw-pr") && zs(wfn) && s(wbn,wfn-1) && s(wfn+1,-1) &&
#    qir(wbn+1,wfn-1,"prl_pr mest_pr mest_3e prq_pr") &&
...
#{ l[i]=omo2; Z[292]++; if(dbg){print "Z292"}; continue };
  ...
  ######################################################################################################
    ...
    { Z[310]++; if(dbg){print "Z310", "-- FN_lock_prl_f"}; continue };
  ...
  if ( vgl(Y["n_pre_v"],-5,-1) ) {
    ...
    if ( TN_v_zamok_f() )
    { Z[311]++; if(dbg){print "Z311", "-- TN_v_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_na"],-5,-1) ) {
    ...
    if ( TN_na_zamok_f() )
    { Z[315]++; if(dbg){print "Z315", "-- TN_na_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_za"],-5,-1) ) {
    ...
    if ( TN_za_zamok_f() )
    { Z[326]++; if(dbg){print "Z326", "-- TN_za_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_pod"],-5,-1) ) {
    ...
    if ( TN_pod_zamok_f() )
    { Z[327]++; if(dbg){print "Z327", "-- TN_pod_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_o"],-5,-1) ) {
    ...
    if ( TN_o_zamok_f() )
    { Z[328]++; if(dbg){print "Z328", "-- TN_o_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_po"],-5,-1) ) {
    ...
    if ( TN_po_zamok_f() )
    { Z[329]++; if(dbg){print "Z329", "-- TN_po_zamok_f"}; continue };
  ...
  if ( vgl(Y["n_pre_qerez"],-5,-1) ) {
    ...
    if ( TN_qerez_zamok_f() )
    { Z[330]++; if(dbg){print "Z330", "-- TN_qerez_zamok_f"}; continue };
  ...
  if ( bf(1,3,cst1) ) { bf_n=bfn;
    ...
    if ( bf(1,3,cst) ) { bf_n=bfn;
      ...
      if ( vgl(bf_n,1,3) &&
           ...
           qir(1,bf_n-1,"mod_any gl_aux_be nat_any") &&
    ...
    if ( vgl(Y["n+_lock_pe"],1,6) ) { bf_n=Y["n+_lock_pe"];
       ...
       { l[i]=omo1; Z[466]++; if(dbg){print "Z466"}; continue }; # check4 mass errors
       if ( vgl(bf_n,1,6) &&
             bw(bf_n,cst) &&
              w(bf_n+1,"перед") &&
    ...
    if ( qb(-7,-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-7,qb_n-1,"_lock_pe") ) { bb_n=bbn;
       ...
       if ( vgl(qb_n,-7,-2) && vgl(bb_n,qbn-7,qbn-1) && s(bb_n,-1) &&
            vex(Y["tn_ok_vi"]) )
       { bb_n=qb_n=""; Z[489]++; if(dbg){print "Z489"}; };
    ...
    if ( vgl(Y["n-_castle_pe"],-7,-1) ) { bb_n=Y["n-_castle_pe"];
      ...
      if ( qb(-5,-1,"sz_iili") ) { qb_n=qbn;
        if ( bba(qb_n-5,qb_n-1,"_castle_pe") ) { bb_n=bbn;
         ...
         if ( vgl(Y["n+_lock_pe"],1,5) && s(0,Y["n+_lock_pe"]-1) )
         { bb_n=qb_n=""; Z[522]++; if(dbg){print "Z522"}; };
...
#      bfa(xwn-3,xwn-1,"_castle_pe") && s(bfn,-1) && Q(bfn,"suw_any") )
    ...
    if ( vgl(Y["n+_castle_pe"],1,6) ) { bf_n=Y["n+_castle_pe"]; bb_n=Y["n-_lock_pe"];
       ...
       { bf_n=""; Z[546]++; if(dbg){print "Z546"}; };
       if ( vgl(bf_n,1,6) && zs(0) && s(1,bf_n-1) &&
              q(bf_n,"mod_any") && q(bf_n+1,"gl_in") && ba(bf_n+1,"_castle_pe") )
       { bf_n=bf_n+1; Z[547]++; if(dbg){print "Z547"}; };
...
#   cst1="быть ведь же стать считаться являться";
 ...
 #
    ...
    if ( TN_do_zamka_f() )
    { Z[570]++; if(dbg){print "Z570", "-- TN_do_zamka_f"}; continue };
 ...
 # _lock_na_suw:start
 if ( wb(-6,-2,"на") ) { wb_n=wbn;
   if ( bfa(wb_n+1,-1,"_lock_na_suw") ) { bf_n=bfn;
        if ( ba(wb_n-1,"_castle_pe") && s(wb_n-1) )
        { wb_n=bf_n=""; Z[608]++; if(dbg){print "Z608"}; };
 ...
 { Z[621]++; if(dbg){print "Z621", "-- FN_castle_prl_after_f"}; continue };
...
#   { l[i]=omo1; Z[623]++; if(dbg){print "Z623"}; continue };
 ...
 if ( q(1,"isname") && s(0) ) {
      ...
      { l[i]=omo1; Z[629]++; if(dbg){print "Z629"}; continue } else
 ...
 if ( isname(0) && s(-1) )
 { l[i]=omo1; Z[637]++; if(dbg){print "Z637", "-- is_CAP"}; continue };
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    { tn_ok = 1; Z[638]++; if(dbg){print "Z638", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) && wy(-1,"же") &&
          qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
    { tn_ok = 1; Z[639]++; if(dbg){print "Z639", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-1,"suw_tv mest_tv") )
    { tn_ok = 1; Z[640]++; if(dbg){print "Z640", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
          qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
            q(-2,"suw_tv qik_tv") &&
            q(-1,cst3) )
    { tn_ok = 1; Z[641]++; if(dbg){print "Z641", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_vi") &&
          qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
            q(-1,"mest_vip3e suw_vi") )
    { tn_ok = 1; Z[642]++; if(dbg){print "Z642", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) &&
            q(tn+1,cst2) &&
            q(tn+2,"pre_tv") &&
          qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
            q(-1,"suw_tv") )
    { tn_ok = 1; Z[643]++; if(dbg){print "Z643", "-- tn_ok ON", el, "замок"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    { tn_ok = 1; Z[644]++; if(dbg){print "Z644", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(-4) && zs(-3) && s(-2,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"nar_any prl_kred_sr") && q(-1,cst2) )
    { tn_ok = 1; Z[645]++; if(dbg){print "Z645", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[646]++; if(dbg){print "Z646", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && zs(-2) && s(-1) && veq(tn,-3) &&
            q(-2,cst2) && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[647]++; if(dbg){print "Z647", "-- tn_ok ON", el, "замок"};};
    if ( !vex(tn_ok) && s(tn,-1) && veq(tn,-4) &&
            q(-3,cst2) && q(-2,"sz_iili") && wc(-1,"ский$|ный$") )
    { tn_ok = 1; Z[648]++; if(dbg){print "Z648", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-3) &&
#           q(-2,"prq_vi") && q(-1,"suw_vi") )
#   { tn_ok=fufu=1; Z[649]++; if(dbg){print "Z649", "-- tn_ok ON", el, "замок"};};
#   if ( !vex(tn_ok) && s(tn-1,-1) && veq(tn,-4) &&
#           q(-3,"prq_vi") && q(-2,"suw_vi") && q(-1,"prl_vi mest_vi") )
#   { tn_ok=fufu=1; Z[650]++; if(dbg){print "Z650", "-- tn_ok ON", el, "замок"};};
    # передвижка tn_ok
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[651]++; if(dbg){print "Z651", "-- tn_ok MOV", el, "замок"};};
    # выключение tn_ok
    ...
    { tn_ok = ""; Z[652]++; if(dbg){print "Z652", "-- tn_ok OFF", el, "замок"};};
    if ( vex(tn_ok) && !vex(fufu) && s(tn,-1) &&
         qxs(-1,"прошлый тот этот","раз") )
    { tn_ok = ""; Z[653]++; if(dbg){print "Z653", "-- tn_ok OFF", el, "замок"};};
}
...
#   if ( bba(tn-9,tn-1,"_gl_v_lock-iv")   ) { Y["n-_gl_v_lock-iv"]   = bbn };
    ...
    { l[i]=omo1; Z[659]++; if(dbg){print "Z659"}; stopp=ret=1; continue };

    cst="совать";
    if ( bb(tn-4,tn-2,cst) &&
        qir(bbn+1,tn-2,"prl_edmuvi mest_edmuvi mest_3e") &&
          w(tn-1,"нос") && s(bbn,tn-1) )
    { l[i]=omo1; Z[660]++; if(dbg){print "Z660"}; stopp=ret=1; continue };
    if ( bw(1,cst) &&
         wf(2,5,"нос") &&
    ...
    { l[i]=omo1; Z[662]++; if(dbg){print "Z662"}; stopp=ret=1; continue };

    if ( qxs(tn-1,"по","возвращению приезду прибытию") )
    { l[i]=omo1; Z[663]++; if(dbg){print "Z663"}; stopp=ret=1; continue };
    ...
    { l[i]=omo2; Z[681]++; if(dbg){print "Z681"}; stopp=ret=1; continue }; #edit
    if ( bb(tn-9,tn-3,cst) &&
          w(bbn+1,cst1) &&
          q(bbn+2,"pre_tv") &&
        qir(tn+3,tn-1,"suw_tv prl_tv mest_tv prq_tv isname qast nar_any") && s(bbn,tn-2) && zs(tn-1) )
    { l[i]=omo2; Z[682]++; if(dbg){print "Z682"}; stopp=ret=1; continue }; #edit
    ...
    if ( vgl(Y["n-_gl_v_lock-iv"],tn-9,tn-1) ) { bb_n=Y["n-_gl_v_lock-iv"];
       ...
       if ( vgl(bb_n,tn-6,tn-3) && s(bb_n,tn-3) &&
            qir(bb_n+1,tn-3,"prl_vi prq_vi mcop_vi qik_vi qis_vi prl_kred_sr nar_any isname qast_any") &&
              ...
              q(tn-1,"sz") )
       { l[i]=omo2; Z[723]++; if(dbg){print "Z723"}; stopp=ret=1; continue }; #check
    ...
    if ( qb(tn-8,tn-2,"pre_vi") ) { qb_n=qbn;
      if ( bba(qb_n-5,qb_n-1,"_gl_v_lock-iv") ) { bb_n=bbn;
       ...
       { qb_n=bb_n=""; l[i]=omo2; Z[737]++; if(dbg){print "Z737"}; };
...
!(narph_any(bbn)||w(bbn-1,"в на")) )
    ...
    { l[i]=omo2; Z[771]++; if(dbg){print "Z771"}; stopp=ret=1; continue }; #check
    if ( bf(1,3,cst1) &&
        qir(1,bfn-1,"prl_vi mcop_vi prq_vi prl_im mcop_im prq_im") &&
         bw(bfn+1,cst) && s(tn,bfn-1) )
    { l[i]=omo2; Z[772]++; if(dbg){print "Z772"}; stopp=ret=1; continue }; #check
    if ( wf(1,5,cst3) &&
    ...
    if ( vgl(Y["n-_gl_v_castle-iv"],tn-8,tn-1) ) { bb_n = Y["n-_gl_v_castle-iv"];
       ...
       { l[i]=omo1; Z[807]++; if(dbg){print "Z807"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-5,tn-2) &&
              q(bb_n+1,"prq_vi suw_vi mest_vi mest_3e") &&
            qir(bb_n+2,tn-1,"suw_vi mest_vi mest_3e suw_mnsq qis_vi isname") && s(bb_n,tn-2) && zs(tn-1) )
       { l[i]=omo1; Z[808]++; if(dbg){print "Z808"}; stopp=ret=1; continue }; #check
       if ( vgl(bb_n,tn-6,tn-2) &&
...
#   { l[i]=omo2; Z[881]++; if(dbg){print "Z881"}; stopp=ret=1; continue };
    ...
    if ( bfa(1,4,"_suw_na_castle") ) { bf_n=bfn;
       if ( vgl(bf_n,1,4) && s(0,bf_n-1) &&
              q(bf_n,"prl_vi") )
       { bf_n=""; Z[900]++; if(dbg){print "Z900"}; stopp=ret=1; };
    ...
    { l[i]=omo1; Z[909]++; if(dbg){print "Z909", bfn}; stopp=ret=1; continue };
    # _gl_na_castle:end
...
#   cst_muvi="prl_edmuvi prl_mnvi mcop_edmuvi mcop_mnvi qis_vi qik_vi prq_edmuvi prq_mnvi qast_any mest_3e nar_kaq nar_step";
    ...
    if ( hwy(-1,"то") && qir(tn+1,-1-hyn,cst1) && s(tn,-1) )
    { tn_ok = 1; Z[1038]++; if(dbg){print "Z1038", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) && wy(-1,"же") &&
#         qir(tn+2,-1-wyn,"mest_da suw_da prq_da nar_any prl_da") )
#   { tn_ok = 1; Z[1039]++; if(dbg){print "Z1039", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-2,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-1,"suw_tv mest_tv") )
#   { tn_ok = 1; Z[1040]++; if(dbg){print "Z1040", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#         qir(tn+2,-3,"mest_tv prq_tv qik_tv nar_any prl_tv") &&
#           q(-2,"suw_tv qik_tv") &&
#           q(-1,cst3) )
#   { tn_ok = 1; Z[1041]++; if(dbg){print "Z1041", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_vi") &&
#         qir(tn+3,-2,"prl_vi mest_vi mest_3e") &&
#           q(-1,"mest_vip3e suw_vi") )
#   { tn_ok = 1; Z[1042]++; if(dbg){print "Z1042", "-- tn_ok ON", el, "замка"};};
#   if ( !vex(tn_ok) && s(tn,-1) &&
#           q(tn+1,cst2) &&
#           q(tn+2,"pre_tv") &&
#         qir(tn+3,-2,"prl_tv mest_tv mest_3e") &&
#           q(-1,"suw_tv") )
#   { tn_ok = 1; Z[1043]++; if(dbg){print "Z1043", "-- tn_ok ON", el, "замка"};};
    cst="именно собственно";
    if ( !vex(tn_ok) && s(tn,-1) && qiw(tn+1,-1,cst1,cst) )
    ...
    if ( vex(tn_ok) && s(tn,-1) &&
       ( qxs(tn-1,"как","и")||
         qxs(tn-1,"да","еще","и")||
         qxs(tn-1,"ведь даже же прямо сразу") ) )
    { tn = xsn; Z[1049]++; if(dbg){print "Z1049", "-- tn_ok MOV", el, "замка"};};
    # выключение tn_ok
#   if ( vex(tn_ok) && s(tn,-1) && qf(tn+1,-1,"prq_edmuvi prq_mnvi") &&
#        bam(qfn,"_castle_pe _lock_pe") &&
#          q(1,"suw_edmuvi suw_mnvi suw_edne") && s(0) )
#   { tn_ok = ""; Z[1050]++; if(dbg){print "Z1050", "-- tn_ok OFF", el, "замка"};};
#   if ( vex(tn_ok) && s(tn,-1) &&
#        qxs(-1,"прошлый тот этот","раз") )
#   { tn_ok = ""; Z[1051]++; if(dbg){print "Z1051", "-- tn_ok OFF", el, "замка"};};
}
...
#   if ( vex(Y["tn_ok_na"]) && TN_na_zamok_f() )
#   { Z[1056]++; if(dbg){print "Z1056"}; stopp=ret=1; continue };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
             if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                  ...
                  vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[1062]++; if(dbg){print "Z1062"}; };
             if ( s(bf_n,bf_n+1) && sz_iili(bf_n+1) && ba(bf_n+2,"_lock_prl") )
             { wf_n=bf_n=""; Z[1063]++; if(dbg){print "Z1063"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
          if ( bfa(wf_n+1,wf_n+5,str_is_castle_prl) ) { bf_n=bfn;
            if ( vgl(Y["n+_lock_pe"],wf_n+1,bf_n-1) ||
                 bfa(wf_n+1,bf_n-1,"_lock_prl") ||
                 vex(Y["tn_ok_vi"]) )
             { wf_n=bf_n=""; Z[1068]++; if(dbg){print "Z1068"}; };
...
function FN_lock_prl_f(str_is_lock_prl,    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp_str="FN" str_is_lock_prl "_f"; stopp=v2s(Y[stopp_str]);
  while ( !stopp ) {
    ...
    { Y["prl_vi_on"]=1; Z[1076]++; if(dbg){print "Z1076", "-- prl_vi_on=1"}; };

    cst="вернуться";
    if ( vex(Y["tn_ok_v"]) && vex(Y["n-_gl_v_castle-iv"]) && bw(Y["n-_gl_v_castle-iv"],cst) && TN_v_zamok_f() )
    { Z[1077]++; if(dbg){print "Z1077", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    if ( vex(Y["tn_ok_v"]) && vgl(Y["n-_suw_v_castle-vp"],Y["n_pre_v_ist"]-3,Y["n_pre_v_ist"]-1) && TN_v_zamok_f() )
    { Z[1078]++; if(dbg){print "Z1078", "-- TN_v_zamok_f"}; stopp=ret=1; continue };
    ...
    if ( vex(Y["prl_vi_on"]) ) {
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1082]++; if(dbg){print "Z1082", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,"_lock_prl") ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) ||
               ( bfa(wf_n+1,bf_n-1,"_castle_prl") && Q(bfn,"prl_kred_sr") ) ||
                 vex(Y["tn_ok_vi"]) ||
            ( isname(0) && s(-1) ) )
             { wf_n=bf_n=""; Z[1083]++; if(dbg){print "Z1083"}; };
       ...
       if ( bf(1,3,cst1) ) { wf_n=bfn;
              if ( vex(Y["tn_ok_v"]) && TN_v_zamok_f() )
              { Z[1086]++; if(dbg){print "Z1086", "-- TN_v_zamok_f"}; stopp=ret=1; continue};

          if ( bfa(wf_n+1,wf_n+5,"_lock_prl") ) { bf_n=bfn;
            if ( vgl(Y["n+_castle_pe"],wf_n+1,bf_n-1) || vex(Y["tn_ok_vi"]) )
            { wf_n=bf_n=""; Z[1087]++; if(dbg){print "Z1087"}; };
...
function FN_zamok_suwro_f(    ret, stopp_str, stopp, bb_n,bf_n,qb_n,qf_n,wb_n,wf_n) { stopp=v2s(Y["FN_zamok_suwro_f"]);
 ...
 if ( wfa(1,5,"_castle_suw-ro") ) { wf_n=wfn;
    ...
    { wf_n=""; Z[1095]++; if(dbg){print "Z1095"}; }; #check_!под
    cst3="ломать щелкнуть";
    if ( vgl(wf_n,1,5) && s(-1,wf_n-1) && bw(-1,cst3) )
    { wf_n=""; Z[1096]++; if(dbg){print "Z1096"}; };

    if ( vgl(wf_n,1,5) && s(0,wf_n-1) &&
         qir(1,wf_n-1,"prl_ro mest_ro prq_ro prl_kred_sr nar_step") )
    { l[i]=omo1; Z[1097]++; if(dbg){print "Z1097"}; stopp=ret=1; continue }; #check_!под
 };
 # _castle_suw-ro:end
